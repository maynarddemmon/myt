...
(a=>{class b extends Error{constructor(a,b,...f){super(...f);this.name="FetchError";this.status=a;this.url=b}}var c=0,d={},e={"":!0},f=document.fonts,g=a=>{a=a.family+" "+a.weight;0<=a.indexOf("\\35 ")&&(a=a.replace(/\\35 /g,"5"));if(!e[a])for(e[a]=!0,a=d[a]||[];a.length;)a.pop().sizeViewToDom()},h=a.myt={version:2.02004161227E7,IMAGE_ROOT:global.MYT_IMAGE_ROOT||"",generateGuid:()=>++c,addEventListener:(a,b,f,c,d)=>{a.addEventListener(b,f,{capture:c||!1,passive:d||!1})},removeEventListener:(a,b,f,
c)=>{a.removeEventListener(b,f,c||!1)},resolveName:(a,b)=>{if(a&&0!==a.length){b=b||global;for(var f=Array.isArray(a)?a:a.split("."),c=0,d=f.length;c<d;++c)if(b=b[f[c]],void 0===b){console.warn("resolveName failed for:",a,"at part:",c,f[c]);return}return b}},resolveClassname:a=>{"string"===typeof a&&(a=h.resolveName(a));return a&&"function"===typeof a.isA&&a.isA(JS.Class)?a:null},getExtension:function(a){return a?a.split(".")[1]:null},fillTextTemplate:function(a,...b){var f;if(null==a)return"";var c=
0;for(f=b.length;f>c;++c){var d=b[c];a=a.split("{"+c+"}").join(null==d?"":d)}return a},generateLink:(a,b,f,c)=>{var d="",e;if(f)for(e in f)d+=" "+e+'="'+f[e]+'"';return h.fillTextTemplate('<a href="#" onclick=\'myt.__handleGeneratedLink(this, "{0}", &apos;{3}&apos;); return false;\'{2}>{1}</a>',b,a,d,JSON.stringify(c))},__handleGeneratedLink:(a,b,f)=>{for(var c,d;a;){if(c=a.model){try{f&&(d=JSON.parse(f))}catch(r){h.dumpStack(r)}c[b].call(c,d);break}a=a.parentNode}},loadScript:function(a,b,f){var c=
this._loadedScripts||(this._loadedScripts={});if(c[a])return console.warn("script already loaded for src",a),null;c[a]=!0;var d=document.createElement("script");d.type="text/javascript";d.async=!1;if(b){var e=!1;d.onload=d.onreadystatechange=function(){e||this.readyState&&"complete"!==this.readyState||(e=!0,d.onload=d.onreadystatechange=null,b())}}d.src=a+(f?"":(-1!==a.indexOf("?")?"&":"?")+"cacheBust="+Date.now());this.getElement("head").appendChild(d);return d},dumpStack:(a,b)=>{if("string"===typeof a){var f=
a;a=null}h.global.error.notify(b||"error",null,f,a)},getRandom:a=>{var b=Math.random();a&&(b=a(b),1<=b?b=.9999999999:0>b&&(b=0));return b},getRandomArbitrary:(a,b,f)=>{if(a>b){var c=a;a=b;b=c}return h.getRandom(f)*(b-a)+a},getRandomInt:(a,b,f)=>{if(a>b){var c=a;a=b;b=c}return Math.floor(h.getRandom(f)*(b-a+1)+a)},areFloatsEqual:(a,b,f)=>{var c=Math.abs(a),d=Math.abs(b);f=f?Math.abs(f):1E-6;return Math.abs(a-b)<=(c>d?d:c)*f},areArraysEqual:(a,b)=>{if(a!==b){if(null==a||null==b)return!1;var f=a.length;
if(f!==b.length)return!1;for(;f;)if(a[--f]!==b[f])return!1}return!0},areObjectsEqual:(a,b)=>{if(a!==b){if(null==a||null==b)return!1;for(var f in a)if(a[f]!==b[f])return!1;for(f in b)if(a[f]!==b[f])return!1}return!0},getElement:(a,b)=>document.getElementsByTagName(a||"body")[0<b?b:0],loadFontFaces:(a,b)=>{var c=[];a.forEach(a=>{a=new FontFace(a.family,"url("+a.url+")",a.options);c.push(a.load())});Promise.all(c).then(a=>{a.forEach(a=>{f.add(a);g(a)});b&&b(a)})},loadFontFace:(a,b,c={},d)=>{a=new FontFace(a,
"url("+b+")",c);a.loaded.then(a=>{f.add(a);g(a);d&&d(a)});a.load()},registerForFontNotification:(a,b)=>{e[b]?a.sizeViewToDom():(d[b]||(d[b]=[])).push(a)},createBaseFontCSSRule:a=>{h.addCSSRule(h.createStylesheet(),"body, input","font-family:"+a,0)},loadCSSFonts:a=>{(a||[]).forEach(a=>{var b=document.createElement("link");b.rel="stylesheet";b.href=a;document.head.appendChild(b)})},createStylesheet:()=>{var a=document.createElement("style");document.head.appendChild(a);return a.sheet},addCSSRule:(a,
...
getSelectableItem:function(a){for(var b=this.getSelectableItems(),c=b.length,d,f=this.itemSelectionId;c;)if(d=b[--c],d[f]===a)return d;return null}})})(myt);
