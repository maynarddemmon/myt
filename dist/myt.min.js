...
(q=>{const {Class:w,Module:u}=JS,r=Math,{View:h,global:d}=q,e=d.focus,l=d.keys,{LIST_KEYS:k,CODE_ESC:b}=l,g=f=>{const c=f.itemConfig??[],n=c.length,{items:v,defaultItemClass:t,fixedWidth:p}=f,A=v.length,I=0<p,z=f.getContentView(),D=z.getLayouts();let J=0;for(var P of D)P.incrementLockedCounter();h.doWhileRemovedFromDom(z,()=>{const R={listView:f};I&&(R.width=p,R.enableEllipsis=!0);for(;n>J;++J){const m=c[J];let x=v[J],C,M;m&&(C=m.klass??t,M=m.attrs??{});x&&!x.isA(C)&&(x.destroy(),x=null);C&&(x??=
v[J]=new C(z,{...R}),x&&(x.callSetters(M),x.__LAYOUT_IDX=J))}});if(I)P=p;else for(P=f.minWidth,J=0;n>J;){const R=v[J++];R&&(R.syncToDom(),P=r.max(P,R.getMinimumWidth()))}for(;A>J;)v[J++].destroy();v.length=n;if(!I)for(J=0;n>J;)f.updateItemWidth(v[J++],P);f.updateContentWidth(z,P);for(const R of D)R.sortSubviews((m,x)=>m.__LAYOUT_IDX-x.__LAYOUT_IDX),R.decrementLockedCounter(),R.update()},a=q.ListViewItemMixin=new u("ListViewItemMixin",{setListView:function(f){this.listView=f},getMinimumWidth:()=>0,
