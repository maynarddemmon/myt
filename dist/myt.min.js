...
expand:function(c){const h=this,k=h.wrapper;c-=h.getCollapsedHeight();h.setExpansionState("expanding");k.stopActiveAnimators();if(k.height!==c){const a=h.tabContainer.duration;1===a?(k.setHeight(c),h.setExpansionState("expanded")):k.animate({attribute:"height",to:c,duration:a}).next(e=>{h.setExpansionState("expanded")})}else h.setExpansionState("expanded")},collapse:function(){const c=this,h=c.wrapper;c.setExpansionState("collapsing");h.stopActiveAnimators();if(0!==h.height){const k=c.tabContainer.duration;
...
m,{initNode:function(c,h){h.labelTextColorChecked??(h.labelTextColorChecked="#fff");h.labelTextColor??(h.labelTextColor="#333");this.callSuper(c,h);this.button.label=new t.Text(this.button,{domClass:"myt-Text mytTextTabSliderLabel",ignorePlacement:!0,text:this.text,align:"center",valign:"middle",textColor:this.__getTextColor()})},setLabelTextColorChecked:function(c){this.labelTextColorChecked=c},setLabelTextColor:function(c){this.labelTextColor=c},setText:function(c){this.text!==c&&(this.text=c,this.button?.label?.setText(c))},
notifyButtonRedraw:function(){this.button.label?.setTextColor(this.__getTextColor())},__getTextColor:function(){return this.selected&&-1!==this.tabContainer.maxSelected?this.labelTextColorChecked:this.labelTextColor}});const d=t.TabSliderContainer=new JS.Module("TabSliderContainer",{include:[t.SelectionManager],extend:{SPACING:1},initNode:function(c,h){const k=this;k._tabSliders=[];h.defaultPlacement="container";h.spacing??(h.spacing=d.SPACING);h.overflow??(h.overflow="autoy");h.itemSelectionId??
(h.itemSelectionId="tabId");h.maxSelected??(h.maxSelected=1);h.duration??(h.duration=500);k.updateLayout=t.debounce(k.updateLayout);k.callSuper(c,h);c=k.container=new w(k,{ignorePlacement:!0,percentOfParentWidth:100},[u,{subnodeAdded:function(a){this.callSuper(a);a instanceof m&&(k._tabSliders.push(a),k.attachTo(a,"updateLayout","selected"))},subnodeRemoved:function(a){if(a instanceof m){const e=k._tabSliders,f=e.indexOf(a);-1<f&&(k.detachFrom(a,"updateLayout","selected"),e.splice(f,1))}this.callSuper(a)}}]);
c.layout=new t.SpacedLayout(c,{axis:"y",spacing:k.spacing,collapseParent:!0});k.attachTo(k,"updateLayout","height")},setSpacing:function(c){this.spacing!==c&&(this.spacing=c,this.layout?.setSpacing(c))},setDuration:function(c){this.duration=c},updateLayout:function(c,h){c=this._tabSliders;for(var k=c.length,a=k,e=0,f=0,g=0;a;){var b=c[--a];b.visible&&(++g,b.selected?(e+=b.getMinimumExpandedHeight(),f+=b.getPreferredExpandedHeight()):(b=b.getCollapsedHeight(),e+=b,f+=b))}a=this.container.layout;g=
a.inset+a.outset+a.spacing*(g-1);f+=g;a=this.height;e=e+g>a;g=f>a;b=this.duration;f-=a;0<h&&this.setDuration(h);for(a=k;a;)if(k=c[--a],k.visible)if(k.selected){let q;if(e)q=k.getMinimumExpandedHeight();else if(g){const n=k.getPreferredExpandedHeight(),p=k.getMinimumExpandedHeight();q=n-f;p>q?(f-=n-p,q=p):f=0}else q=k.getPreferredExpandedHeight();k.expand(q)}else k.collapse();0<h&&this.setDuration(b)}})})(myt);
