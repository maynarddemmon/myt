...
(n=>{class v extends Error{constructor(A,C,...G){super(...G);this.name="FetchError";this.status=A;this.url=C}}let u=0,t,g={},k={};const d=console.warn,l=Math,{abs:m,min:h,max:b,pow:c}=l,a=Array.isArray,e=document,f=e.head,q=e.createElement.bind(e),y={},r={},D=(A,C,G,L,N)=>{if(C){var T=y[C],R=r[C];if(!0===T)G?.(!0);else if(!1===T)G?.(!1);else if(a(T))"function"===typeof G&&T.push(G);else{T=y[C]=[];"function"===typeof G&&T.push(G);let X;switch(A){case "script":R=q("script");R.type="text/javascript";
R.async=!1;X="src";break;case "css":R=q("link"),R.rel="stylesheet",X="href"}N&&(R.integrity=N,R.crossOrigin="anonymous",R.referrerpolicy="no-referrer");const aa=ca=>{R.onload=R.onerror=null;for(const ta of T)ta(ca);y[C]=ca};R.onerror=()=>{aa(!1)};R.onload=()=>{aa(!0)};R[X]=C+(L?"":(C.includes("?")?"&":"?")+"cacheBust="+Date.now());f.appendChild(R);r[C]=R}return R}},w={},E={"":!0},J=e.fonts,P=A=>{const C=A.family.replaceAll('"',"");for(const G of[C,C+" "+A.weight])if(!E[G]){E[G]=!0;const L=w[G]??[];
for(;L.length;)(A=L.pop())&&!A.destroyed&&A.isVisible()&&(A.sizeViewToDom(),A.notifyFontLoaded?.(C))}},B={},F={},x=/\{\{plural:\$(\d+)\|([^|]+)\|([^}]+)\}\}/g,p=/\$(\d+)/g,z=(A,C=JSON.stringify,G=Infinity)=>{const L=new Map;return(...N)=>{var T=C(N);if(L.has(T))return L.get(T);N=A(...N);L.set(T,N);L.size>G&&(T=L.keys().next().value,L.delete(T));return N}},I=(A,C,G=new WeakMap,L=new WeakMap)=>{if(A!==C){if(A instanceof Date&&C instanceof Date)return A.getTime()===C.getTime();if(null===A||null===C||
"object"!==typeof A||"object"!==typeof C)return!1;if(G.has(A))return L.get(C)===G.get(A);G.set(A,C);L.set(C,A);var N=a(A),T=a(C);if(N!==T)return!1;if(N){var R=A.length;if(R!==C.length)return!1;for(N=0;N<R;N++)if(!I(A[N],C[N],G,L))return!1;return!0}N=Object.keys(A);T=Object.keys(C);if(N.length!==T.length)return!1;for(R of N)if(!Object.hasOwn(C,R)||!I(A[R],C[R],G,L))return!1}return!0},Q=/\\([{}])|\{([^{}]+)\}/g,O=/(,|\r?\n|\r|^)(?:"((?:\\.|""|[^\\"])*)"|([^,"\r\n]*))/gi,S=/[\\"](.)/g,W=n.myt={version:2.02507281019E7,
...
ModalPanelMarginBottom:40,ModalPanelMarginRight:40,TabSliderContainerSpacing:1,TabInset:8,TabOutset:8,TabContainerInset:0,TabContainerSpacing:1,BaseTooltipDelay:500,BaseTooltipHideDelay:100,TooltipEdgeColor:"#444",TooltipBgColor:"#444",TooltipTextColor:"#eee",TooltipShadowColor:"#00000033",TooltipEdgeSize:0,TooltipShadowSize:2,TooltipHInset:6,TooltipVInset:3,focusShadow:[0,0,7,"#666"],disabledOpacity:.5,border1s3:[1,"solid","#333"],border1s9:[1,"solid","#999"]},resolveName:(A,C)=>{if(A&&0!==A.length){var G=
C=C??global,L=a(A)?A:A.split("."),N=L.length;for(let T=0;T<N;T++)if(C=C[L[T]],null==C&&T<N-1){d("resolveName failed for",A,"at part",T,L[T],G);return}return C}},resolveClassname:A=>{"string"===typeof A&&(A=W.resolveName(A));return A&&"function"===typeof A.isA&&A.isA(JS.Class)?A:null},setDeepValue:(A,C,G)=>{if(A&&C){C=a(C)?C:String(C??"").split(".");const L=C.length-1;for(let N=0;N<L;N++){const T=C[N];let R,X;(R=A)[X=T]??(R[X]={});A=A[T]}return A[C[L]]=G}},stableStringify:A=>JSON.stringify(A,(C,G)=>
{if(G&&"object"===typeof G&&!a(G)){C={};for(const L of Object.keys(G).sort())C[L]=G[L];return C}return G}),areFloatsEqual:(A,C,G)=>{const L=m(A),N=m(C);return m(A-C)<=(L>N?N:L)*(null==G?1E-6:m(G))},areArraysEqual:(A,C)=>{if(A!==C){if(null===A||null===C||"object"!==typeof A||"object"!==typeof C)return!1;let G=A.length;if(G!==C.length)return!1;for(;G;)if(A[--G]!==C[G])return!1}return!0},shallowEqual:(A,C)=>{if(A!==C){if(null===A||null===C||"object"!==typeof A||"object"!==typeof C)return!1;const G=Object.keys(A),
L=Object.keys(C);if(G.length!==L.length)return!1;for(const N of G)if(A[N]!==C[N])return!1}return!0},deepEqual:I,getRandom:A=>{const C=l.random();return A?b(0,h(A(C),.9999999999)):C},getRandomArbitrary:(A,C,G)=>{const L=h(A,C);return W.getRandom(G)*(b(A,C)-L)+L},getRandomInt:(A,C,G)=>{const L=h(A,C);return l.floor(W.getRandom(G)*(b(A,C)-L+1)+L)},getElement:(A,C)=>e.getElementsByTagName(A||"body")[0<C?C:0],addEventListener:(A,C,G,L,N)=>{A.addEventListener(C,G,{capture:L||!1,passive:N||!1})},removeEventListener:(A,
C,G,L)=>{A.removeEventListener(C,G,L||!1)},loadFontFaces:(A,C)=>{Promise.all(A.map(({family:G,url:L,options:N})=>(new FontFace(G,"url("+L+")",N)).load())).then(G=>{if(J){for(const L of G)J.add(L),P(L);C?.(G)}})},loadFontFace:(A,C,G={},L)=>{A=new FontFace(A,"url("+C+")",G);A.loaded.then(N=>{J&&(J.add(N),P(N),L?.(N))});A.load()},registerForFontNotification:(A,C)=>{E[C]?A&&!A.destroyed&&A.isVisible()&&(A.sizeViewToDom(),A.notifyFontLoaded?.(C)):(w[C]??(w[C]=[])).push(A)},createBaseFontCSSRule:A=>{W.addCSSRule(W.createStylesheet(),
"body, input","font-family:"+A)},loadCSSFonts:A=>{A?.forEach(W.loadCSS)},loadScript:(...A)=>D("script",...A),loadCSS:(...A)=>D("css",...A),createStylesheet:()=>{const A=q("style");f.appendChild(A);return A.sheet},addCSSRule:(A,C,G,L)=>{A.insertRule(C+"{"+G+"}",L??0)},removeCSSRules:A=>{let C=A.cssRules.length;for(;C;)A.deleteRule(--C)},createInputPlaceholderCSSRule:(A,C,G,L=1)=>{let N=A.__sheet;N?W.removeCSSRules(N):N=A.__sheet=W.createStylesheet();A=A.getODE();A=A.id||(A.id="id"+ ++u);const T=[];
C&&T.push("color:"+C);G&&T.push("font-family:"+G);L&&T.push("opacity:"+L);W.addCSSRule(N,"#"+A+"::placeholder",T.join("; "))},isSorted:(A,C)=>{const G=A.length;for(let L=1;L<G;L++)if(0<C(A[L-1],A[L]))return!1;return!0},chainSortFunc:(...A)=>(C,G)=>{for(const L of A){const N=L(C,G);if(0!==N)return N}return 0},getAlphaObjSortFunc:z((A,C,G,L)=>{const N=C?1:-1,T=W.I18N.getLocale(),R={sensitivity:G?"accent":"variant"};return(X,aa)=>{X=X[A];aa=aa[A];L?("string"!==typeof X&&(X=""+X),"string"!==typeof aa&&
(aa=""+aa)):(X=X||"",aa=aa||"");return X.localeCompare(aa,T,R)*N}}),getNumericObjSortFunc:z((A,C)=>{const G=C?1:-1;return(L,N)=>{L=L[A]||0;N=N[A]||0;return(L-N)*G}}),hash:A=>A.split("").reduce((C,G)=>{C=(C<<5)-C+G.charCodeAt(0);return C&C},0),getExtension:A=>A?(A=A.split("."),1<A.length?A.pop():null):null,formatAsPercentage:(A,C=2)=>{switch(typeof A){case "number":return C=h(16,b(0,C)),A=l.round(b(0,h(1,A))*c(10,2+C))/c(10,C),(0===A%1?A:A.toFixed(C))+"%";case "string":return A;default:return d("formatAsPercentage expects a number"),
A}},interpolateString:(A,C={})=>A?A.replace(Q,(G,L,N)=>L?L:W.resolveName(N,C)??G):"",leftPadNumber:(A,C,G="0",L=10)=>{A=A.toString(L);return G.repeat(b(C-A.length,0))+A},removeMarkup:(A,C)=>{if(!A)return"";C?.brToLineFeed&&(A=A.replace(/<br\s*\/?>/gi,"\n"));return A.replace(/<\/?[^>]+(>|$)/g,"")},escapeMarkup:(()=>{const A=RegExp("[&<>\"']","g"),C={"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#039;"},G=L=>C[L];return L=>L.replace(A,G)})(),memoize:z,debounce:(A,C,G=!1)=>{let L;return function(...N){const T=
this,R=G&&!L;clearTimeout(L);L=setTimeout(()=>{L=null;G||A.apply(T,N)},C);R&&A.apply(T,N)}},dataURIToBlob:A=>{var C=A.indexOf(",");const G=A.slice(0,C).split(":")[1].split(";")[0];A=A.slice(C+1);if(G.startsWith("text/"))A=decodeURIComponent(A);else{A=atob(A);C=A.length;const L=new Uint8Array(C);for(;C;)L[--C]=A.charCodeAt(C);A=L}return new Blob([A],{type:G})},runInWorker:(A,C,G="data")=>new Promise((L,N)=>{const T=URL.createObjectURL(new Blob(["self.onmessage=e=>{try{const result=(function("+G+'){"use strict";'+
A+"})(e.data);self.postMessage({result});}catch(e){self.postMessage({error:e.message});}};"],{type:"application/javascript"})),R=new Worker(T);R.onmessage=X=>{const {result:aa,error:ca}=X.data;ca?N(Error(ca)):L(aa);R.terminate();URL.revokeObjectURL(T)};R.onerror=X=>{N(X);R.terminate();URL.revokeObjectURL(T)};R.postMessage(C)}),dumpStack:(A,C)=>{let G;"string"===typeof A&&(G=A,A=null);W.global.error.notify(C||"error",null,G,A)},createFixedThresholdCounter:(A,C,G,L)=>{var N=W.AccessorSupport.generateName;
const T="function"===typeof A||A instanceof JS.Module,R={};L=L||N("counter",G);const X=N(L,"increment");N=N(L,"decrement");if(void 0!==(T?A.instanceMethod(X):A[X]))return d("Increment: Abort clobber",X,A),!1;if(void 0!==(T?A.instanceMethod(N):A[N]))return d("Decrement: Abort clobber",N,A),!1;R[X]=function(){const aa=this[L]+1;this[L]=aa;this.fireEvent(L,aa);aa===C&&this.set(G,!0)};R[N]=function(){const aa=this[L];if(0!==aa){var ca=aa-1;this[L]=ca;this.fireEvent(L,ca);aa===C&&this.set(G,!1)}};A[T?
"include":"extend"](R);return!0},csvStringToArray:(A,C)=>{if(!A)return[];const G=[[]];let L;for(;L=O.exec(A);)L[1].length&&","!==L[1]&&G.push([]),G[G.length-1].push(void 0!==L[2]?L[2].replace(S,"$1"):L[3]);if(C){const N=G.shift();return G.map(T=>{let R=0;return N.reduce((X,aa)=>{X[aa]=T[R++];return X},{})})}return G},prepareCSVDataURI:(A,C)=>{const G=T=>{if(T&&a(T)){const R=[],X=T.length;for(let aa=0,ca;X>aa;aa++)ca=T[aa],ca=null==ca?"":ca.toString(),ca=ca.replace(/"/g,'""'),0<=ca.search(/("|,|\n)/g)&&
(ca='"'+ca+'"'),R.push(ca);return R.join(",")}d("Unexpected row",T);return null},L=[];if(C){var N=G(C);N&&L.push(N)}if(A)if(a(A)){N=A.length;for(let T=0;N>T;T++){const R=G(A[T]);R&&L.push(R)}}else d("Rows were not an array");return W.encodeCSVDataURI(L.join("\r\n"),C)},encodeCSVDataURI:(A,C)=>"data:text/csv;charset=utf-8"+(null==C?"":";header="+(C?"present":"absent"))+","+encodeURIComponent(A),makeURLSearchParams:(A={})=>{const C=new URLSearchParams;for(const G in A){const L=A[G];void 0!==L&&C.set(G,
L)}return C},doFetch:(A,C,G,L,N,T)=>fetch(A,C).then(R=>{if(!R.ok)throw new v(R.status,A,R.statusText);return R}).then(R=>G?R.text():R.json()).then(R=>{if(L)try{if(G)L(R);else{if(!1===R.success)throw new v(200,A,R.message??R.data?.message);L(R.data)}}catch(X){throw R=new v(200,A,X.message),R.stack=X.stack,R;}}).catch(R=>{if(N){if("FetchError"!==R.name){const X=new v(0,A,R.message);X.stack=R.stack;R=X}N(R)}}).finally(()=>{T?.()}),toNameCase:(A,C)=>{if(!A)return"";A=A.trim().toLowerCase().split(/([\s\-'\u2019"\u201c\u201d().,\/])/).reduce((G,
...
I18N:{setLocale:A=>{t=A??(navigator.language||"en").split("-")[0].toLowerCase();g=B[t];k=F[t]},getLocale:()=>t,addDictionary:(A,C)=>{B[C]=Object.assign(B[C]??{},A);C===t&&(g=B[C])},setDictionary:(A,C)=>{B[C]=A??{};C===t&&(g=B[C])},addResourceDictionary:(A,C)=>{F[C]=Object.assign(F[C]??{},A);C===t&&(k=F[C])},setResourceDictionary:(A,C)=>{F[C]=A??{};C===t&&(k=F[C])},get:(A,...C)=>{const G=g[A];return null!=G?0<C.length?G.replaceAll(x,(L,N,T,R)=>T&&R?1==C[parseInt(N)]?T:R:"").replaceAll(p,(L,N)=>C[N]):
G:A},getRaw:A=>g[A]??A,getResource:A=>k[A]}};J&&(J.onloadingdone=A=>{for(const C of A.fontfaces)P(C)})})(global);
...
this.wrap},setUpdateAgain:function(b){this._updateAgain=b},isUpdateAgain:function(){return this._updateAgain},updateFlexboxLayout:function(){const b=this;if(b.isOKToUpdateLayout()){var c=b.getSubviewsInLexicalOrder().filter(J=>!0!==b.ignoreSubviewForFlex(J)&&!0===J.visible);const E=c.length;if(0<E){b.__isUpdatingFlexboxLayout=!0;var a=b.direction;const J=a.startsWith("row"),P=!J;var e=b.getWidthForFlexboxLayout(),f=b.getHeightForFlexboxLayout();const B=J?b.columnGap:b.rowGap,F=J?e:f;var q=J?b.rowGap:
b.columnGap;const x=J?f:e,p=b.isWrapNotAllowed();e=b.justifyContent;f=b.alignItems;var y=p?"stretch":b.alignContent;a.endsWith("Reverse")&&c.reverse();let z=0,I=0,Q=0,O=0,S={items:[],shrinkCount:0,shrinkBasis:0,growCount:0,extraSize:0,crossPos:Q,crossSize:0};a=[S];for(let W=0;W<E;){const A=c[W++],C=J?A.getFlexBasisWidth():A.getFlexBasisHeight(),G=A.getShrink(),L=A.getGrow();!p&&0<S.items.length&&z+(0===O?0:B)+C>F&&(S.extraSize=F-z,Q+=S.crossSize+q,I=u(I,z),O=z=0,S={items:[],shrinkCount:0,shrinkBasis:0,
growCount:0,extraSize:0,crossPos:Q,crossSize:0},a.push(S));0!==O&&(z+=B);0<G&&0<C&&(S.shrinkCount+=G,S.shrinkBasis+=C*G);0<L&&(S.growCount+=L);S.crossSize=u(S.crossSize,P?A.getFlexBasisWidth():A.getFlexBasisHeight());l(A,J,z);d(A,J,C);z+=C;S.items.push(A);O++}S.extraSize=F-z;Q+=S.crossSize;I=u(I,z);if(b.isCompactOnMainAxis(J)&&(c=a.reduce((W,A)=>null==W?A.extraSize:Math.min(W,A.extraSize),null),0!==c))for(var r of a)r.extraSize-=c;b[J?"setTotalBasisWidth":"setTotalBasisHeight"](I);b[J?"setTotalBasisHeight":
...
D/r;D=0;for(var w of a)w.crossSize+=q,w.crossPos+=D,D+=q}for(const W of a){w=W.items;q=W.extraSize;if(0<q)if(0<W.growCount){q/=W.growCount;a=0;for(const G of w)0<a&&m(G,J,a),D=G.getGrow()*q,0<D&&(d(G,J,D,!0),a+=D)}else switch(a=w.length,e){default:case "start":break;case "end":g(w,J,q,!1);break;case "center":g(w,J,q/2,!1);break;case "spaceBetween":1<a&&g(w,J,q/(a-1),!0,1);break;case "spaceAround":q<=B?g(w,J,q/2,!1):(g(w,J,B/2,!1),q-=B,g(w,J,q/(2*a),!0),g(w,J,q/(2*a),!0,1));break;case "spaceEvenly":q<=
2*B?g(w,J,q/2,!1):(g(w,J,B,!1),q-=2*B,g(w,J,q/(a+1),!0))}else if(0>q&&0<W.shrinkCount){a=W.shrinkBasis;D=0;for(const G of w)0>D&&m(G,J,D),y=G.getShrink()*(J?G.getFlexBasisWidth():G.getFlexBasisHeight())/a*u(q,-a),0>y&&(d(G,J,y,!0),D+=y)}q=!1;const {crossSize:A,crossPos:C}=W;for(const G of w)switch(G.getAlignSelf()||f){case "start":l(G,P,C);d(G,P,P?G.getFlexBasisWidth():G.getFlexBasisHeight(),!1);break;case "end":l(G,P,C+A-(P?G.getFlexBasisWidth():G.getFlexBasisHeight()));d(G,P,P?G.getFlexBasisWidth():
G.getFlexBasisHeight(),!1);break;case "center":l(G,P,C+(A-(P?G.getFlexBasisWidth():G.getFlexBasisHeight()))/2);d(G,P,P?G.getFlexBasisWidth():G.getFlexBasisHeight(),!1);break;case "baseline":q=!0;l(G,P,C+(A-(P?G.getFlexBasisWidth():G.getFlexBasisHeight()))/2+G.getFlexBaselineOffset(J));d(G,P,P?G.getFlexBasisWidth():G.getFlexBasisHeight(),!1);break;default:l(G,P,C),d(G,P,A,!1)}q&&b.updateFlexboxLayoutBaselineAdjustment(w,J,C)}b.__isUpdatingFlexboxLayout=!1;b.isUpdateAgain()&&!b.__UPDATE_AGAIN_LOOP_PROTECTION&&
...
__loopCnt:C,to:G}=r;var E=v(0,D/x),J=v(0,z/x);null==r.from&&(r.__tmpFrom=!0,r.from=S?A?"#000":0:w.get(W));J=I(E)-(S?I(J):0);var P=r.from,B=S?w.get(W):P;E=w.set;if(A){P=b(P);const L=b(G);B=S?b(B):P;B.setRed(B.red+(L.red-P.red)*J);B.setGreen(B.green+(L.green-P.green)*J);B.setBlue(B.blue+(L.blue-P.blue)*J);J=B.getHtmlHexString()}else J=B+(G-P)*J;E.call(w,W,J);!F&&C===p||F&&0>C&&0<p?(r.setRunning(!1),Q?.call(r,!0)):0<O?(r.fireEvent("repeat",C),r.__prog=F?x:0,q(r,O)):r.__prog=D}else console.log("No target for animator",
...
(n=>{const {Class:v,Module:u}=JS,t=Math,{View:g,global:k}=n,d=k.focus,l=k.keys,{LIST_KEYS:m,CODE_ESC:h}=l,b=a=>{const e=a.itemConfig??[],f=e.length,{items:q,defaultItemClass:y,fixedWidth:r}=a,D=q.length,w=0<r,E=a.getContentView(),J=E.getLayouts();let P=0;for(var B of J)B.incrementLockedCounter();g.doWhileRemovedFromDom(E,()=>{const F={listView:a};w&&(F.width=r,F.enableEllipsis=!0);for(;f>P;++P){const x=e[P];let p=q[P],z,I;x&&(z=x.klass??y,I=x.attrs??{});p&&!p.isA(z)&&(p.destroy(),p=null);z&&(p??=
q[P]=new z(E,{...F}),p&&(p.disabled&&null==I.disabled&&p.setDisabled(!1),p.callSetters(I),p.__LAYOUT_IDX=P))}});if(w)B=r;else for(B=a.minWidth,P=0;f>P;){const F=q[P++];F&&(F.syncToDom(),B=t.max(B,F.getMinimumWidth()))}for(;D>P;)q[P++].destroy();q.length=f;if(!w)for(P=0;f>P;)a.updateItemWidth(q[P++],B);a.updateContentWidth(E,B);for(const F of J)F.sortSubviews((x,p)=>x.__LAYOUT_IDX-p.__LAYOUT_IDX),F.decrementLockedCounter(),F.update()},c=n.ListViewItemMixin=new u("ListViewItemMixin",{setListView:function(a){this.listView=
...
delete this.__sf[p],this.verifyChangedState(),this.verifyValidState());return z},getSubForm:function(p){return this.__sf[p]},getSubForms:function(){return this.__sf},getInvalidSubformIds:function(p){p&&this.doValidation();const z=[];(function O(Q){if(Q.isA(F))Q.isValid||z.push(Q.getFullId());else{Q=Q.getSubForms();for(const S in Q)O(Q[S])}})(this);return z},getAllErrorMessages:function(){const p=this.__sf;let z=(this.errorMessages??[]).slice();for(const I in p)z.push(...p[I].getAllErrorMessages());
...
!1},rollbackForm:function(){this._lockCascade=!0;const p=this.__sf;for(const z in p)p[z].rollbackForm();this.setIsChanged(!1);this.setErrorMessages([]);this.setIsValid(!0);this._lockCascade=!1},getChangedValue:function(){const p={},z=this.__sf;for(const I in z){const Q=z[I];Q.isChanged&&(p[I]=Q.getChangedValue())}return p}}),F=n.FormElement=new t("FormElement",{include:[B],initNode:function(p,z){this.__vp=[];this.callSuper(p,z)},getValue:function(){return D(this,this.callSuper?this.callSuper():this.value,
...
Y.setYear(ya.getFullYear()));K&&!F.includes(Ca)?fa.setDisabled(!0):(0===Ca?fa.setIsSunday(!0):6===Ca&&fa.setIsSaturday(!0),ea===oa&&(C.select(fa),fa.focus()),ba&&ea===da&&fa.setIsToday(!0),ea=new Date(Y.getTime()),ea.setHours(23),ea.setMinutes(59),ea.setSeconds(59),(null!=x&&x>ea.getTime()||null!=p&&p<Y.getTime())&&fa.setDisabled(!0));fa.updateUI()}}if(!J){K=60*I[0]+I[1];da=G;da.deselectAll();da.destroyAllSubviews();let [na,ka]=z;for(Y=new Date(ja);60*na+ka<K;)ja=new Ma(G,{text:("0"+na).slice(-2)+
":"+("0"+ka).slice(-2)}),na===V&&ka===H&&G.select(ja),Y.setHours(na),Y.setMinutes(ka),da=Y.getTime(),(null!=x&&x>da||null!=p&&p<da)&&ja.setDisabled(!0),ka+=Q,60<=ka&&(ka-=60,na++);G.scrollYTo(U)}},Oa=n.DatePicker=new L("DatePicker",N,{initNode:function(H,K){w=this;const M={current:null,dateOnly:!1,timeOnly:!1,locales:null,locale:"",minuteInterval:30,firstDayOfWeek:0,showTodayButton:!0,minDate:null,maxDate:null,minTime:"00:00",maxTime:"23:59",allowedDays:null,...K.opt};delete K.opt;w.callSuper(H,K);
E=M.locales[M.locale??"en"];J=M.dateOnly;P=M.timeOnly;B=M.firstDayOfWeek;Q=ma(5,la(30,M.minuteInterval));F=M.allowedDays;Array.isArray(F)&&0!==F.length||(F=[0,1,2,3,4,5,6]);x=Date.parse(M.minDate);x=isNaN(x)?null:x;p=Date.parse(M.maxDate);p=isNaN(p)?null:p;z=Aa(M.minTime);I=Aa(M.maxTime);H=new N(w,{visible:!P});M.showTodayButton&&new qa(H,{width:24,text:ra(["home"]),tooltip:E.today},[{doActivated:()=>{sa(new Date)}}]);S=new qa(H,{width:24,x:28,text:ra(["chevron-left"]),tooltip:E.prevMonth},[{doActivated:()=>
...
(n=>{const {Class:v,Module:u}=JS,{min:t,max:g,floor:k,ceil:d}=Math,{NOOP:l,View:m,LocalStorage:{setDatum:h,getDatum:b},global:{focus:c,keys:a},getAlphaObjSortFunc:e}=n,f=B=>{const F=B.selectedRow;F&&(F.setSelected(!1),B.set("selectedRow",null,!0))},q=B=>B.__ssk??(B.__ssk="InfiniteGridMixin.sort."+n.hash(B.columnHeaders.map(F=>F.columnId).sort().join(","))),y=n.InfiniteListRow=new u("InfiniteListRow",{include:[n.Reusable],setInfiniteOwner:function(B){this.infiniteOwner=B},setModel:function(B){this.model=
B},setClassKey:function(B){this.classKey=B},notifyRefreshed:l}),r=n.SelectableInfiniteListRow=new u("SelectableInfiniteListRow",{include:[y,n.Selectable]}),D=n.InfiniteList=new v("InfiniteList",m,{initNode:function(B,F){let x=F.rowClasses;delete F.rowClasses;if("function"===typeof x){const p={};p["default"]=x;x=p}F.modelIDName??(F.modelIDName="id");F.numericSort??(F.numericSort=!0);F.ascendingSort??(F.ascendingSort=!0);F.overflow??(F.overflow="autoy");F.bgColor??(F.bgColor="#ccc");F.rowSpacing??(F.rowSpacing=
1);F.rowInset??(F.rowInset=0);F.rowOutset??(F.rowOutset=0);F.rowHeight??(F.rowHeight=30);F.overscrollBehavior??(F.overscrollBehavior="auto contain");this._rowExtent=this.rowSpacing=this.rowHeight=0;this._startIdx=this._endIdx=-1;this._visibleRowsByIdx=new Map;this.callSuper(B,F);B=this._listView=new m(this);this._itemPool=this.makePool(x,B);this.attachTo(this,"refreshListUI","height");this.attachToDom(this,"refreshListUI","scroll")},makePool:(B,F)=>new n.TrackActivesMultiPool(B,F),setOverscrollBehavior:function(B){this.overscrollBehavior=
B;this.getIDS().overscrollBehavior=B},setCollectionModel:function(B){this.collectionModel=B},setModelIDName:function(B){this.modelIDName=B},setRowSpacing:function(B){this.rowSpacing=B;this._rowExtent=this.rowSpacing+this.rowHeight},setRowHeight:function(B){this.rowHeight=B;this._rowExtent=this.rowSpacing+this.rowHeight},getListData:function(){return this._listData},setWidth:function(B){if(0<B&&(this.callSuper(B),this.inited)){B=this._listView;const F=this.width;B.setWidth(F);for(const x of B.getSubviews())x.setWidth(F)}},
getVisibleRowForModel:function(B){for(const F of this._visibleRowsByIdx.values())if(F.model===B)return F},getListViewHeight:function(){return this._listView.height},isScrolledToEnd:function(){return this.getIDE().scrollTop+this.height===this._listView.height},getSortFunction:function(){const B=this.modelIDName,F=this.ascendingSort;return this.numericSort?n.getNumericObjSortFunc(B,F):e(B,F,!1)},getFilterFunction:l,scrollModelIntoView:function(B,F){var x=this.getIndexOfModelInData(B);let p=!1;if(0<=
x){var z=this._rowExtent;const I=this.getIDE().scrollTop,Q=I+this.height;x=this.rowInset+x*z;z=x+z;x<=I?(this.scrollYTo(x,!0),p=!0):z>=Q&&(this.scrollYTo(z-this.height,!0),p=!0);F&&((F=this.getVisibleRowForModel(B))?F.focus():this._focusToModel=B)}return p},isModelInData:function(B){return 0<=this.getIndexOfModelInData(B)},getNextModel:function(B,F=!0,x=!0){const p=this.getListData(),z=p.length;B=this.getIndexOfModelInData(B);if(0<=B)return B+=1,B>=z?F?p[0]:p[z-1]:p[B];if(x&&0<z)return p[z-1]},getPrevModel:function(B,
F=!0,x=!0){const p=this.getListData(),z=p.length;B=this.getIndexOfModelInData(B);if(0<=B)return--B,0>B?F?p[z-1]:p[0]:p[B];if(x&&0<z)return p[0]},areModelsEqual:function(B,F){const x=this.modelIDName;return B[x]===F[x]},getIndexOfModelInData:function(B){if(B){const F=this.getListData(),x=this.areModelsEqual.bind(this);let p=F.length;for(;p;)if(x(F[--p],B))return p}return-1},getActiveRowForModel:function(B){if(B){const F=this._itemPool.getActives(),x=this.areModelsEqual.bind(this);let p=F.length;for(;p;){const z=
F[--p];if(x(z.model,B))return z}}},refreshListData:function(B,F){this._listData=this.collectionModel.getAsSortedList(this.getSortFunction(),this.getFilterFunction());this.resetListUI(B,F)},resetListUI:function(B,F){const x=this.getListData().length;this._listView.setHeight(x*this._rowExtent-(0<x?this.rowSpacing:0)+this.rowInset+this.rowOutset);this._startIdx=this._endIdx=-1;this.forceFullResetOnNextRefresh=F;B||0===this.getIDE().scrollTop?this.refreshListUI():this.scrollYTo(0,!0)},putRowBackInPool:function(B){if(c.focusedView?.isDescendantOf(B)){const F=
this.getFocusTrap();F?F.focus():c.clear()}B.setVisible(!1);this._itemPool.putInstance(B)},refreshListUI:function(B){B=this._rowExtent;const F=this.rowInset,x=this.forceFullResetOnNextRefresh;var p=this.getIDE().scrollTop;const z=this.getListData()??[];var I=g(0,k((p-F)/B));p=t(z.length,d((p-F+this.height)/B));this.forceFullResetOnNextRefresh&&(this.forceFullResetOnNextRefresh=!1);if(this._startIdx!==I||this._endIdx!==p||x){const Q=this.width,O=this.rowHeight,S=this._visibleRowsByIdx,W=this._focusToModel,
A=this.areModelsEqual.bind(this);this._startIdx=I;this._endIdx=p;for(const [C,G]of S)if(C<I||C>=p)this.putRowBackInPool(G),S.delete(C);for(;I<p;I++){let C=S.get(I);const G=z[I],L=this.getClassKey(G);let N=!1;C&&C.classKey===L||(C&&this.putRowBackInPool(C),S.set(I,C=this._itemPool.getInstance(L)),C.setInfiniteOwner(this),C.setClassKey(L),C.setWidth(Q),C.setHeight(O),C.setY(F+I*B),C.setVisible(!0),N=!0);!x&&C.model&&A(C.model,G)?N&&this.updateRow(C):(C.setModel(G),this.updateRow(C));C.notifyRefreshed();
C.bringToFront();W&&A(W,G)&&(C.focus(),this._focusToModel=null)}}},getClassKey:B=>"default",updateRow:l}),w=n.SimpleSelectableInfiniteListRow=new v("SimpleSelectableInfiniteListRow",n.SimpleButton,{include:[r,n.ArrowKeyActivation],initNode:function(B,F){F.selectedColor??(F.selectedColor="#ccf");F.activeColor??(F.activeColor="#f8f8f8");F.hoverColor??(F.hoverColor="#eee");F.readyColor??(F.readyColor="#fff");F.focusIndicator??(F.focusIndicator=!1);F.activationKeys??(F.activationKeys=a.LIST_KEYS);this.callSuper(B,
F)},destroy:function(){this.selected&&this.infiniteOwner.setSelectedRow();this.callSuper()},setSelected:function(B){this.callSuper(B);this.inited&&this.updateUI()},clean:function(){this.setMouseOver(!1);this.setMouseDown(!1);this.focused&&this.blur();this.callSuper()},updateUI:function(){this.callSuper();this.selected&&this.setBgColor(this.selectedColor)},doActivated:function(){this.callSuper();this.infiniteOwner.setSelectedRow(this)},doActivationKeyDown:function(B,F){B===a.CODE_ESC?this.selected&&
this.infiniteOwner.setSelectedRow():this.callSuper(B,F)},doKeyArrowLeftOrUp:function(B,F){this.infiniteOwner.selectPrevRowForModel(this.model);return!0},doKeyArrowRightOrDown:function(B,F){this.infiniteOwner.selectNextRowForModel(this.model);return!0},doActivationKeyUp:function(B){switch(B){case a.CODE_ENTER:case a.CODE_SPACE:this.doActivated()}}}),E=n.SelectableInfiniteList=new v("SelectableInfiniteList",D,{setSelectedRow:function(B){const F=this.selectedRow;B!==F&&(F?.setSelected(!1),this.setSelectedRowModel(),
this.set("selectedRow",B,!0),B&&(this.setSelectedRowModel(B.model),B.setSelected(!0)))},setSelectedRowModel:function(B){this.set("selectedRowModel",B,!0);this.scrollModelIntoView(this.selectedRowModel,!1)},getActiveSelectedRow:function(){return this.getActiveRowForModel(this.selectedRowModel)},selectRowForModel:function(B,F,x){B&&(f(this),this.setSelectedRowModel(B),this.refreshListUI(),F&&this.getActiveSelectedRow()?.focus())},selectNextRowForModel:function(B,F=!0){this.selectRowForModel(this.getNextModel(B),
F,!0)},selectPrevRowForModel:function(B,F=!0){this.selectRowForModel(this.getPrevModel(B),F,!1)},resetListUI:function(B,F){this.isModelInData(this.selectedRowModel)?f(this):this.setSelectedRow();this.callSuper(B,F)},putRowBackInPool:function(B){B.selected&&f(this);this.callSuper(B)},refreshListUI:function(B){B=c.focusedView;this.callSuper();const F=this.getActiveSelectedRow();F&&(this.set("selectedRow",F,!0),F.setSelected(!0),B&&B!==F||F.focus(!0))}}),J=new u("InfiniteGridRowMixin",{notifyXChange:function(B){this.getRef(B.columnId)?.setX(B.x+
B.cellXAdj)},notifyWidthChange:function(B){this.getRef(B.columnId)?.setWidth(B.width+B.cellWidthAdj)},notifyVisibilityChange:function(B){this.getRef(B.columnId)?.setVisible(B.visible)}}),P=new u("InfiniteGridMixin",{setGridHeader:function(B){(this.gridHeader=B)?.setGrid(this)},makeReady:function(B,F){const x=this.gridHeader;x?(this.forceFullResetOnNextRefresh=F,F=x.persistSortOrder?b(q(x)):void 0,x.setSort(F??B),x.setLocked(!1)):this.refreshListData(!1,F)},getSortFunction:function(){const [B,F]=this.gridHeader.sort??
["",""];return B?e(B,F===n.Grid.SORT_ORDER_ASC,!1):this.callSuper()},updateRow:function(B){const F=this.gridHeader;if(F)for(const x of F.columnHeaders)B.notifyXChange(x),B.notifyWidthChange(x),B.notifyVisibilityChange(x)},notifyXChange:function(B){for(const F of this._visibleRowsByIdx.values())F.notifyXChange(B)},notifyWidthChange:function(B){for(const F of this._visibleRowsByIdx.values())F.notifyWidthChange(B)},notifyVisibilityChange:function(B){for(const F of this._visibleRowsByIdx.values())F.notifyVisibilityChange(B)}});
n.InfiniteGridRow=new u("InfiniteGridRow",{include:[y,J]});n.SelectableInfiniteGridRow=new u("SelectableInfiniteGridRow",{include:[r,J]});n.SimpleSelectableInfiniteGridRow=new v("SimpleSelectableInfiniteGridRow",w,{include:[J]});n.InfiniteGrid=new v("InfiniteGrid",D,{include:[P]});n.SelectableInfiniteGrid=new v("InfiniteGrid",E,{include:[P]});n.InfiniteGridHeader=new v("InfiniteGridHeader",m,{include:[n.GridController],initNode:function(B,F){F.columnSpacing??(F.columnSpacing=1);F.persistSortOrder??
(F.persistSortOrder=!0);F.overflow="hidden";this.callSuper(B,F);new n.SpacedLayout(this,{spacing:this.columnSpacing})},setGrid:function(B){this.grid=B},setColumnSpacing:function(B){this.columnSpacing=B},getColumnSpacingInUse:function(){return 0===this.columnSpacing?0:g(0,this.getVisibleHdrs().length-1)*this.columnSpacing},setPersistSortOrder:function(B){this.persistSortOrder=B},setHeight:function(B){this.callSuper(B);if(this.inited){B=this.height;for(const F of this.columnHeaders)F.setHeight(B)}},
...
w,blue:E.blue/w}}h=new n.Path([l+h,m,l+b,m]);h.rotateAroundOrigin(c,l,m);b=h.vectors;let [J,P,B,F]=b,x=0,p=u/360;for(y=0;a>c;){w=J;E=P;r=B;D=F;c+=f;c>a&&(f+=a-c,p=0,c=a);h.rotateAroundOrigin(f+p,l,m);[J,P,B,F]=b;this.beginPath();this.moveTo(w,E);this.lineTo(r,D);this.lineTo(B,F);this.lineTo(J,P);this.closePath();const {color:z,colorDelta:I}=e[y];this.setFillStyle(q.rgbToHex(z.red+x*I.red,z.green+x*I.green,z.blue+x*I.blue,!0));this.fill();0<p&&(h.rotateAroundOrigin(-p,l,m),[J,P,B,F]=b);x++;c>=e[y+
