...
return L}).then(L=>B?L.text():L.json()).then(L=>{if(D)try{if(B)D(L);else{if(!1===L.success)throw new w(200,p,L.message??L.data?.message);D(L.data)}}catch(P){throw L=new w(200,p,P.message),L.stack=P.stack,L;}}).catch(L=>{if(F){if("FetchError"!==L.name){const P=new w(0,p,L.message);P.stack=L.stack;L=P}F(L)}}).finally(()=>{K?.()}),toNameCase:(p,y)=>{if(!p)return"";p=p.trim().toLowerCase().split(/([\s\-'\u2019"\u201c\u201d().,\/])/).reduce((B,D)=>B+(D[0]??"").toUpperCase()+D.slice(1),"");if(/\bMac[A-Za-z]{2,}[^aciozj]\b/.test(p)||
/\bMc/.test(p))p=p.replace(/\b(Ma?c)([A-Za-z]+)/,(B,D,F)=>D+(F[0]??"").toUpperCase()+F.slice(1)),p=p.replace(/\bMacEvicius\b/,"Macevicius").replace(/\bMacHado\b/,"Machado").replace(/\bMacHar\b/,"Machar").replace(/\bMacHin\b/,"Machin").replace(/\bMacHlin\b/,"Machlin").replace(/\bMacIas\b/,"Macias").replace(/\bMacIulis\b/,"Maciulis").replace(/\bMacKie\b/,"Mackie").replace(/\bMacKle\b/,"Mackle").replace(/\bMacKlin\b/,"Macklin").replace(/\bMacQuarie\b/,"Macquarie").replace(/\bMacOmber\b/,"Macomber").replace(/\bMacIn\b/,
