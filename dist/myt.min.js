...
r,l=x.set;var y=b;var E=B,O=f[B];y=O?.__fns__||"function"!==typeof O?O:new c(y,E,O);l.call(x,B,y)}f.constructor=f.klass=b;for(const B in d)b[B]===d[B]||b.hasOwnProperty(B)||(b[B]=d[B]);b.include(e);b.constructor=b.klass=a};m(c,k);m(n,k);m(a,n);t(g(c));t(g(n));g(a).include(n.__meta__);p.Singleton=new a("Singleton",{initialize:(b,e,f)=>new (new a(b,e,f))})})(global.JS={});
(p=>{class v extends Error{constructor(A,C,...G){super(...G);this.name="FetchError";this.status=A;this.url=C}}let u=0,t,g={},h={},c={};const n=console.warn,m=Math,{abs:k,min:a,max:d,pow:b}=m,e=Array.isArray,f=document,r=f.head,x=f.createElement.bind(f),l={},y={"":!0},E=f.fonts,O=A=>{const C=A.family.split('"').join("");for(const G of[C,C+" "+A.weight])if(!y[G]){y[G]=!0;const H=l[G]??[];for(;H.length;)(A=H.pop())&&!A.destroyed&&A.isVisible()&&(A.sizeViewToDom(),A.notifyFontLoaded?.(C))}},B={},D={},
Q=/\{\{plural:\$(\d+)\|([^|]+)\|([^}]+)\}\}/g,R=/\$(\d+)/g,w=(A,C=JSON.stringify,G=Infinity)=>{const H=new Map;return(...I)=>{var S=C(I);if(H.has(S))return H.get(S);I=A(...I);H.set(S,I);H.size>G&&(S=H.keys().next().value,H.delete(S));return I}},q=(A,C,G=new WeakMap,H=new WeakMap)=>{if(A!==C){if(A instanceof Date&&C instanceof Date)return A.getTime()===C.getTime();if(null===A||null===C||"object"!==typeof A||"object"!==typeof C)return!1;if(G.has(A))return H.get(C)===G.get(A);G.set(A,C);H.set(C,A);var I=
e(A),S=e(C);if(I!==S)return!1;if(I){var P=A.length;if(P!==C.length)return!1;for(I=0;I<P;I++)if(!q(A[I],C[I],G,H))return!1;return!0}I=Object.keys(A);S=Object.keys(C);if(I.length!==S.length)return!1;for(P of I)if(!Object.prototype.hasOwnProperty.call(C,P)||!q(A[P],C[P],G,H))return!1}return!0},z=/\\([{}])|\{([^{}]+)\}/g,K=/(,|\r?\n|\r|^)(?:"((?:\\.|""|[^\\"])*)"|([^,"\r\n]*))/gi,N=/[\\"](.)/g,M=p.myt={version:2.02507281019E7,generateGuid:()=>++u,TRUE_FUNC:()=>!0,FALSE_FUNC:()=>!1,NOOP:()=>{},theme:{TextButtonActive:"#ddd",
...
TabContainerInset:0,TabContainerSpacing:1,BaseTooltipDelay:500,BaseTooltipHideDelay:100,TooltipEdgeColor:"#444",TooltipBgColor:"#444",TooltipTextColor:"#eee",TooltipShadowColor:"#00000033",TooltipEdgeSize:0,TooltipShadowSize:2,TooltipHInset:6,TooltipVInset:3,focusShadow:[0,0,7,"#666"],disabledOpacity:.5,border1s3:[1,"solid","#333"],border1s9:[1,"solid","#999"]},resolveName:(A,C)=>{if(A&&0!==A.length){C=C??global;var G=e(A)?A:A.split("."),H=G.length;for(let I=0;I<H;I++)if(C=C[G[I]],null==C){n("resolveName failed for",
A,"at part",I,G[I]);return}return C}},resolveClassname:A=>{"string"===typeof A&&(A=M.resolveName(A));return A&&"function"===typeof A.isA&&A.isA(JS.Class)?A:null},setDeepValue:(A,C,G)=>{if(A&&C){C=e(C)?C:String(C??"").split(".");const H=C.length-1;for(let I=0;I<H;I++){const S=C[I];let P,V;(P=A)[V=S]??(P[V]={});A=A[S]}return A[C[H]]=G}},stableStringify:A=>JSON.stringify(A,(C,G)=>{if(G&&"object"===typeof G&&!e(G)){C={};for(const H of Object.keys(G).sort())C[H]=G[H];return C}return G}),areFloatsEqual:(A,
C,G)=>{const H=k(A),I=k(C);return k(A-C)<=(H>I?I:H)*(null==G?1E-6:k(G))},areArraysEqual:(A,C)=>{if(A!==C){if(null===A||null===C||"object"!==typeof A||"object"!==typeof C)return!1;let G=A.length;if(G!==C.length)return!1;for(;G;)if(A[--G]!==C[G])return!1}return!0},shallowEqual:(A,C)=>{if(A!==C){if(null===A||null===C||"object"!==typeof A||"object"!==typeof C)return!1;const G=Object.keys(A),H=Object.keys(C);if(G.length!==H.length)return!1;for(const I of G)if(A[I]!==C[I])return!1}return!0},deepEqual:q,
getRandom:A=>{const C=m.random();return A?d(0,a(A(C),.9999999999)):C},getRandomArbitrary:(A,C,G)=>{const H=a(A,C);return M.getRandom(G)*(d(A,C)-H)+H},getRandomInt:(A,C,G)=>{const H=a(A,C);return m.floor(M.getRandom(G)*(d(A,C)-H+1)+H)},getElement:(A,C)=>f.getElementsByTagName(A||"body")[0<C?C:0],addEventListener:(A,C,G,H,I)=>{A.addEventListener(C,G,{capture:H||!1,passive:I||!1})},removeEventListener:(A,C,G,H)=>{A.removeEventListener(C,G,H||!1)},loadFontFaces:(A,C)=>{Promise.all(A.map(({family:G,url:H,
options:I})=>(new FontFace(G,"url("+H+")",I)).load())).then(G=>{if(E){for(const H of G)E.add(H),O(H);C?.(G)}})},loadFontFace:(A,C,G={},H)=>{A=new FontFace(A,"url("+C+")",G);A.loaded.then(I=>{E&&(E.add(I),O(I),H?.(I))});A.load()},registerForFontNotification:(A,C)=>{y[C]?A&&!A.destroyed&&A.isVisible()&&(A.sizeViewToDom(),A.notifyFontLoaded?.(C)):(l[C]??(l[C]=[])).push(A)},createBaseFontCSSRule:A=>{M.addCSSRule(M.createStylesheet(),"body, input","font-family:"+A)},loadCSSFonts:A=>{A?.forEach(M.createStylesheetLink)},
createStylesheetLink:A=>{if(A){let C=c[A];C?console.warn("stylesheet already exists",A):(C=c[A]=x("link"),C.rel="stylesheet",C.href=A,r.appendChild(C));return C}},createStylesheet:()=>{const A=x("style");r.appendChild(A);return A.sheet},addCSSRule:(A,C,G,H)=>{A.insertRule(C+"{"+G+"}",H??0)},removeCSSRules:A=>{let C=A.cssRules.length;for(;C;)A.deleteRule(--C)},createInputPlaceholderCSSRule:(A,C,G,H=1)=>{let I=A.__sheet;I?M.removeCSSRules(I):I=A.__sheet=M.createStylesheet();A=A.getODE();A=A.id||(A.id=
"id"+ ++u);const S=[];C&&S.push("color:"+C);G&&S.push("font-family:"+G);H&&S.push("opacity:"+H);M.addCSSRule(I,"#"+A+"::placeholder",S.join("; "))},isSorted:(A,C)=>{const G=A.length;for(let H=1;H<G;H++)if(0<C(A[H-1],A[H]))return!1;return!0},chainSortFunc:(...A)=>(C,G)=>{for(const H of A){const I=H(C,G);if(0!==I)return I}return 0},getAlphaObjSortFunc:w((A,C,G,H)=>{const I=C?1:-1,S=M.I18N.getLocale(),P={sensitivity:G?"accent":"variant"};return(V,Y)=>{V=V[A];Y=Y[A];H?("string"!==typeof V&&(V=""+V),"string"!==
typeof Y&&(Y=""+Y)):(V=V||"",Y=Y||"");return V.localeCompare(Y,S,P)*I}}),getNumericObjSortFunc:w((A,C)=>{const G=C?1:-1;return(H,I)=>{H=H[A]||0;I=I[A]||0;return(H-I)*G}}),hash:A=>A.split("").reduce((C,G)=>{C=(C<<5)-C+G.charCodeAt(0);return C&C},0),getExtension:A=>A?(A=A.split("."),1<A.length?A.pop():null):null,formatAsPercentage:(A,C=2)=>{switch(typeof A){case "number":return C=a(16,d(0,C)),A=m.round(d(0,a(1,A))*b(10,2+C))/b(10,C),(0===A%1?A:A.toFixed(C))+"%";case "string":return A;default:return n("formatAsPercentage expects a number"),
A}},interpolateString:(A,C={})=>A?A.replace(z,(G,H,I)=>H?H:M.resolveName(I,C)??G):"",leftPadNumber:(A,C,G="0",H=10)=>{A=A.toString(H);return G.repeat(d(C-A.length,0))+A},removeMarkup:(A,C)=>{if(!A)return"";C?.brToLineFeed&&(A=A.replace(/<br\s*\/?>/gi,"\n"));return A.replace(/<\/?[^>]+(>|$)/g,"")},escapeMarkup:(()=>{const A=RegExp("[&<>\"']","g"),C={"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#039;"},G=H=>C[H];return H=>H.replace(A,G)})(),memoize:w,debounce:(A,C,G=!1)=>{let H;return function(...I){const S=
this,P=G&&!H;clearTimeout(H);H=setTimeout(()=>{H=null;G||A.apply(S,I)},C);P&&A.apply(S,I)}},dataURIToBlob:A=>{var C=A.indexOf(",");const G=A.slice(0,C).split(":")[1].split(";")[0];A=A.slice(C+1);if(G.startsWith("text/"))A=decodeURIComponent(A);else{A=atob(A);C=A.length;const H=new Uint8Array(C);for(;C;)H[--C]=A.charCodeAt(C);A=H}return new Blob([A],{type:G})},runInWorker:(A,C,G="data")=>new Promise((H,I)=>{const S=URL.createObjectURL(new Blob(["self.onmessage=e=>{try{const result=(function("+G+'){"use strict";'+
A+"})(e.data);self.postMessage({result});}catch(e){self.postMessage({error:e.message});}};"],{type:"application/javascript"})),P=new Worker(S);P.onmessage=V=>{const {result:Y,error:aa}=V.data;aa?I(Error(aa)):H(Y);P.terminate();URL.revokeObjectURL(S)};P.onerror=V=>{I(V);P.terminate();URL.revokeObjectURL(S)};P.postMessage(C)}),loadScript:function(A,C,G,H){const I=this._loadedScripts??(this._loadedScripts={});let S=I[A];if(!0===S)C?.(!0);else if(!1===S)C?.(!1);else if(e(S))C&&S.push(C);else{S=I[A]=[];
C&&S.push(C);const P=x("script");P.type="text/javascript";P.async=!1;H&&(P.integrity=H,P.crossOrigin="anonymous");const V=Y=>{P.onload=P.onreadystatechange=P.onerror=null;for(const aa of S)aa(Y);I[A]=Y};P.onerror=()=>{V(!1)};P.onload=P.onreadystatechange=()=>{P.readyState&&"complete"!==P.readyState||V(!0)};P.src=A+(G?"":(A.includes("?")?"&":"?")+"cacheBust="+Date.now());r.appendChild(P)}},dumpStack:(A,C)=>{let G;"string"===typeof A&&(G=A,A=null);M.global.error.notify(C||"error",null,G,A)},createFixedThresholdCounter:(A,
C,G,H)=>{var I=M.AccessorSupport.generateName;const S="function"===typeof A||A instanceof JS.Module,P={};H=H||I("counter",G);const V=I(H,"increment");I=I(H,"decrement");if(void 0!==(S?A.instanceMethod(V):A[V]))return n("Increment: Abort clobber",V,A),!1;if(void 0!==(S?A.instanceMethod(I):A[I]))return n("Decrement: Abort clobber",I,A),!1;P[V]=function(){const Y=this[H]+1;this[H]=Y;this.fireEvent(H,Y);Y===C&&this.set(G,!0)};P[I]=function(){const Y=this[H];if(0!==Y){var aa=Y-1;this[H]=aa;this.fireEvent(H,
aa);Y===C&&this.set(G,!1)}};A[S?"include":"extend"](P);return!0},csvStringToArray:(A,C)=>{if(!A)return[];const G=[[]];let H;for(;H=K.exec(A);)H[1].length&&","!==H[1]&&G.push([]),G[G.length-1].push(void 0!==H[2]?H[2].replace(N,"$1"):H[3]);if(C){const I=G.shift();return G.map(S=>{let P=0;return I.reduce((V,Y)=>{V[Y]=S[P++];return V},{})})}return G},prepareCSVDataURI:(A,C)=>{const G=S=>{if(S&&e(S)){const P=[],V=S.length;for(let Y=0,aa;V>Y;Y++)aa=S[Y],aa=null==aa?"":aa.toString(),aa=aa.replace(/"/g,'""'),
0<=aa.search(/("|,|\n)/g)&&(aa='"'+aa+'"'),P.push(aa);return P.join(",")}n("Unexpected row",S);return null},H=[];if(C){var I=G(C);I&&H.push(I)}if(A)if(e(A)){I=A.length;for(let S=0;I>S;S++){const P=G(A[S]);P&&H.push(P)}}else n("Rows were not an array");return M.encodeCSVDataURI(H.join("\r\n"),C)},encodeCSVDataURI:(A,C)=>"data:text/csv;charset=utf-8"+(null==C?"":";header="+(C?"present":"absent"))+","+encodeURIComponent(A),makeURLSearchParams:(A={})=>{const C=new URLSearchParams;for(const G in A){const H=
A[G];void 0!==H&&C.set(G,H)}return C},doFetch:(A,C,G,H,I,S)=>fetch(A,C).then(P=>{if(!P.ok)throw new v(P.status,A,P.statusText);return P}).then(P=>G?P.text():P.json()).then(P=>{if(H)try{if(G)H(P);else{if(!1===P.success)throw new v(200,A,P.message??P.data?.message);H(P.data)}}catch(V){throw P=new v(200,A,V.message),P.stack=V.stack,P;}}).catch(P=>{if(I){if("FetchError"!==P.name){const V=new v(0,A,P.message);V.stack=P.stack;P=V}I(P)}}).finally(()=>{S?.()}),toNameCase:(A,C)=>{if(!A)return"";A=A.trim().toLowerCase().split(/([\s\-'\u2019"\u201c\u201d().,\/])/).reduce((G,
H)=>G+(H[0]??"").toUpperCase()+H.slice(1),"");if(/\bMac[A-Za-z]{2,}[^aciozj]\b/.test(A)||/\bMc/.test(A))A=A.replace(/\b(Ma?c)([A-Za-z]+)/,(G,H,I)=>H+(I[0]??"").toUpperCase()+I.slice(1)),A=A.replace(/\bMacEvicius\b/,"Macevicius").replace(/\bMacHado\b/,"Machado").replace(/\bMacHar\b/,"Machar").replace(/\bMacHin\b/,"Machin").replace(/\bMacHlin\b/,"Machlin").replace(/\bMacIas\b/,"Macias").replace(/\bMacIulis\b/,"Maciulis").replace(/\bMacKie\b/,"Mackie").replace(/\bMacKle\b/,"Mackle").replace(/\bMacKlin\b/,
...
I18N:{setLocale:A=>{t=A??(navigator.language||"en").split("-")[0].toLowerCase();g=B[t];h=D[t]},getLocale:()=>t,addDictionary:(A,C)=>{B[C]=Object.assign(B[C]??{},A);C===t&&(g=B[C])},setDictionary:(A,C)=>{B[C]=A??{};C===t&&(g=B[C])},addResourceDictionary:(A,C)=>{D[C]=Object.assign(D[C]??{},A);C===t&&(h=D[C])},setResourceDictionary:(A,C)=>{D[C]=A??{};C===t&&(h=D[C])},get:(A,...C)=>{const G=g[A];return null!=G?0<C.length?G.replaceAll(Q,(H,I,S,P)=>S&&P?1==C[parseInt(I)]?S:P:"").replaceAll(R,(H,I)=>C[I]):
G:A},getRaw:A=>g[A]??A,getResource:A=>h[A]}};E&&(E.onloadingdone=A=>{for(const C of A.fontfaces)O(C)})})(global);
...
x,l,y,E,O]=(h?t:u).exec(g)??[];c.setSource(n??"");c.setProtocol(m??"");c.setAuthority(k??"");c.setUserInfo(a??"");c.setUser(d??"");c.setPassword(b??"");c.setHost(e??"");c.setPort(f??"");c.setRelative(r??"");c.setPath(x??"");c.setDirectory(l??"");c.setFile(y??"");c.setQuery(E??"");c.setAnchor(O??"");c.queryPairs={};c.query.replace(v,(B,D,Q)=>{D&&(c.queryPairs[D]=Q)})},decodeQueryParam:g=>decodeURIComponent(g).split("+").join(" "),getQuery:function(){var g=this.queryPairs;const h=[];for(const c in g)h.push(c+
...
createDomHandler:function(e,f,r){return this.createStandardDomHandler(e,f,r,b,!0)??this.callSuper(e,f,r)}});p.DomObservable=new v("DomObservable",{attachDomObserver:function(e,f,r,x,l){if(e&&f&&r){const y=this.createDomHandler(e,f,r);if(y){x=!!x;const E=this.__dobsbt??(this.__dobsbt={}),O=E[r];O?O.push(e,f,y,x):E[r]=[e,f,y,x];p.addEventListener(this.getDomElementForDomObservable(r),r,y,x,l);return!0}}return!1},getDomElementForDomObservable:function(e){return this.getIDE()},createDomHandler:(e,f,r)=>
null,createStandardDomHandler:function(e,f,r,x,l){if(x.EVENT_TYPES[r]){const y=this,E=x.EVENT;return O=>{O??=window.event;E.source=y;E.type=O.type;E.value=O;e[f](E)||(O.cancelBubble=!0,O.stopPropagation?.(),l&&O.preventDefault());E.source=void 0}}},detachDomObserver:function(e,f,r,x){if(e&&f&&r){var l=this.__dobsbt;if(l&&(l=l[r])){x=!!x;const y=this.getDomElementForDomObservable(r);let E=!1,O=l.length;for(;4<=O;){const B=l[--O],D=l[--O],Q=l[--O],R=l[--O];e===R&&f===Q&&x===B&&(y&&p.removeEventListener(y,
r,D,x),l.splice(O,4),E=!0)}return E}}return!1},detachAllDomObservers:function(){var e=this.__dobsbt;if(e)for(const [f,r]of Object.entries(e)){for(e=r.length+2;4<=e;){const x=r[e-=3],l=r[--e];p.removeEventListener(this.getDomElementForDomObservable(f),f,l,x)}r.length=0}}});p.DomObserver=new v("DomObserver",{extend:{getSourceViewFromEvent:h},attachToDom:function(e,f,r,x,l){if(e&&f&&r){x=!!x;let y;const E=(y=this.__dobt??(this.__dobt={}))[r]??(y[r]=[]);e.attachDomObserver(this,f,r,x,l)&&E.push(x,f,e)}},
detachFromDom:function(e,f,r,x){if(e&&f&&r){var l=this.__dobt;if(l&&(l=l[r])){x=!!x;let y=!1,E=l.length;for(;3<=E;){const O=l[--E],B=l[--E],D=l[--E];e===O&&f===B&&x===D&&e.detachDomObserver(this,f,r,x)&&(l.splice(E,3),y=!0)}return y}}return!1},detachFromAllDomSources:function(){var e=this.__dobt;if(e)for(const [f,r]of Object.entries(e)){for(e=r.length;3<=e;){const x=r[--e],l=r[--e],y=r[--e];x.detachDomObserver(this,l,f,y)}r.length=0}}})})(myt);
...
this.wrap},setUpdateAgain:function(a){this._updateAgain=a},isUpdateAgain:function(){return this._updateAgain},updateFlexboxLayout:function(){const a=this;if(a.isOKToUpdateLayout()){var d=a.getSubviewsInLexicalOrder().filter(B=>!0!==a.ignoreSubviewForFlex(B)&&!0===B.visible);const O=d.length;if(0<O){a.__isUpdatingFlexboxLayout=!0;var b=a.direction;const B=b.startsWith("row"),D=!B;var e=a.getWidthForFlexboxLayout(),f=a.getHeightForFlexboxLayout();const Q=B?a.columnGap:a.rowGap,R=B?e:f;var r=B?a.rowGap:
a.columnGap;const w=B?f:e,q=a.isWrapNotAllowed();e=a.justifyContent;f=a.alignItems;var x=q?"stretch":a.alignContent;b.endsWith("Reverse")&&d.reverse();let z=0,K=0,N=0,M=0,A={items:[],shrinkCount:0,shrinkBasis:0,growCount:0,extraSize:0,crossPos:N,crossSize:0};b=[A];for(let C=0;C<O;){const G=d[C++],H=B?G.getFlexBasisWidth():G.getFlexBasisHeight(),I=G.getShrink(),S=G.getGrow();!q&&0<A.items.length&&z+(0===M?0:Q)+H>R&&(A.extraSize=R-z,N+=A.crossSize+r,K=u(K,z),M=z=0,A={items:[],shrinkCount:0,shrinkBasis:0,
growCount:0,extraSize:0,crossPos:N,crossSize:0},b.push(A));0!==M&&(z+=Q);0<I&&0<H&&(A.shrinkCount+=I,A.shrinkBasis+=H*I);0<S&&(A.growCount+=S);A.crossSize=u(A.crossSize,D?G.getFlexBasisWidth():G.getFlexBasisHeight());n(G,B,z);c(G,B,H);z+=H;A.items.push(G);M++}A.extraSize=R-z;N+=A.crossSize;K=u(K,z);if(a.isCompactOnMainAxis(B)&&(d=b.reduce((C,G)=>null==C?G.extraSize:Math.min(C,G.extraSize),null),0!==d))for(var l of b)l.extraSize-=d;a[B?"setTotalBasisWidth":"setTotalBasisHeight"](K);a[B?"setTotalBasisHeight":
"setTotalBasisWidth"](N);l=b.length;if(1<l&&"wrapReverse"===a.wrap){for(var y of b)y.crossPos=N-(y.crossPos+y.crossSize);b.reverse()}if(!a.isCompactOnCrossAxis(B))switch(y=w-N,x){default:case "start":break;case "end":h(b,y,!1);break;case "center":h(b,y/2,!1);break;case "spaceBetween":1<l&&h(b,y/(l-1),!0,1);break;case "spaceAround":y<=r?h(b,y/2,!1):(h(b,r/2,!1),y-=r,h(b,y/(2*l),!0),h(b,y/(2*l),!0,1));break;case "spaceEvenly":y<=2*r?h(b,y/2,!1):(h(b,r,!1),h(b,(y-2*r)/(l+1),!0));break;case "stretch":r=
y/l;y=0;for(var E of b)E.crossSize+=r,E.crossPos+=y,y+=r}for(const C of b){E=C.items;r=C.extraSize;if(0<r)if(0<C.growCount){r/=C.growCount;b=0;for(const I of E)0<b&&m(I,B,b),y=I.getGrow()*r,0<y&&(c(I,B,y,!0),b+=y)}else switch(b=E.length,e){default:case "start":break;case "end":g(E,B,r,!1);break;case "center":g(E,B,r/2,!1);break;case "spaceBetween":1<b&&g(E,B,r/(b-1),!0,1);break;case "spaceAround":r<=Q?g(E,B,r/2,!1):(g(E,B,Q/2,!1),r-=Q,g(E,B,r/(2*b),!0),g(E,B,r/(2*b),!0,1));break;case "spaceEvenly":r<=
2*Q?g(E,B,r/2,!1):(g(E,B,Q,!1),r-=2*Q,g(E,B,r/(b+1),!0))}else if(0>r&&0<C.shrinkCount){b=C.shrinkBasis;y=0;for(const I of E)0>y&&m(I,B,y),x=I.getShrink()*(B?I.getFlexBasisWidth():I.getFlexBasisHeight())/b*u(r,-b),0>x&&(c(I,B,x,!0),y+=x)}r=!1;const {crossSize:G,crossPos:H}=C;for(const I of E)switch(I.getAlignSelf()||f){case "start":n(I,D,H);c(I,D,D?I.getFlexBasisWidth():I.getFlexBasisHeight(),!1);break;case "end":n(I,D,H+G-(D?I.getFlexBasisWidth():I.getFlexBasisHeight()));c(I,D,D?I.getFlexBasisWidth():
I.getFlexBasisHeight(),!1);break;case "center":n(I,D,H+(G-(D?I.getFlexBasisWidth():I.getFlexBasisHeight()))/2);c(I,D,D?I.getFlexBasisWidth():I.getFlexBasisHeight(),!1);break;case "baseline":r=!0;n(I,D,H+(G-(D?I.getFlexBasisWidth():I.getFlexBasisHeight()))/2+I.getFlexBaselineOffset(B));c(I,D,D?I.getFlexBasisWidth():I.getFlexBasisHeight(),!1);break;default:n(I,D,H),c(I,D,G,!1)}r&&a.updateFlexboxLayoutBaselineAdjustment(E,B,H)}a.__isUpdatingFlexboxLayout=!1;a.isUpdateAgain()&&!a.__UPDATE_AGAIN_LOOP_PROTECTION&&
...
[])},getFirstLayout:function(){return this.getLayouts()[0]},setIgnoreLayout:function(f){if(this.ignoreLayout!==f){var r=this.inited&&this.parent;let x;if(f){if(r)for(r=this.parent.getLayouts(),x=r.length;x;)r[--x].removeSubview(this);this.ignoreLayout=f}else if(this.ignoreLayout=f,r)for(r=this.parent.getLayouts(),x=r.length;x;)r[--x].addSubview(this)}},setDomClass:function(f){this.domClass!==f&&(this.callSuper(f),this.inited&&this.fireEvent("domClass",f))},setDomId:function(f){this.domId!==f&&(this.callSuper(f),
this.inited&&this.fireEvent("domId",f))},setAlignOffset:function(f){this.alignOffset!==f&&(this.alignOffset=f,this.inited&&this.fireEvent("alignOffset",f),this.parent&&"left"===this.align&&this.setX(f))},setAlign:function(f){this.align!==f&&(this.inited&&a(this),this.align=f,this.inited&&(this.fireEvent("align",f),d(this)))},__doAlignCenter:function(f){this.setX(v((this.parent.width-this.width)/2)+(this.alignOffset||0))},__doAlignRight:function(f){this.setX(this.parent.width-this.width-(this.alignOffset||
0))},setValignOffset:function(f){this.valignOffset!==f&&(this.valignOffset=f,this.inited&&this.fireEvent("valignOffset",f),this.parent&&"top"===this.valign&&this.setY(f))},setValign:function(f){this.valign!==f&&(this.inited&&b(this),this.valign=f,this.inited&&(this.fireEvent("valign",f),e(this)))},__doValignMiddle:function(f){this.setY(v((this.parent.height-this.height)/2)+(this.valignOffset||0))},__doValignBottom:function(f){this.setY(this.parent.height-this.height-(this.valignOffset||0))},setX:function(f){this.x!==
f&&(this.x=f,this.visible&&(this.getODS().left=f+"px"),this.inited&&this.fireEvent("x",f))},setY:function(f){this.y!==f&&(this.y=f,this.visible&&(this.getODS().top=f+"px"),this.inited&&this.fireEvent("y",f))},setWidth:function(f){0>f&&(f=0);this.width!==f&&(this.width=f,this.getODS().width=f+"px",this.inited&&(this.__updateBounds(f,this.height),this.fireEvent("width",f)))},setHeight:function(f){0>f&&(f=0);this.height!==f&&(this.height=f,this.getODS().height=f+"px",this.inited&&(this.__updateBounds(this.width,
f),this.fireEvent("height",f)))},setDomMinWidth:function(f,r){(r?this.getIDS():this.getODS()).minWidth=null!=f?f+"px":null},getDomMinWidth:function(f){f=(f?this.getIDS():this.getODS()).minWidth;return null!=f?Number.parseInt(f):null},setDomMinHeight:function(f,r){(r?this.getIDS():this.getODS()).minHeight=null!=f?f+"px":null},getDomMinHeight:function(f){f=(f?this.getIDS():this.getODS()).minHeight;return null!=f?Number.parseInt(f):null},setTextColor:function(f){this.textColor!==f&&(this.textColor=f,
this.getODS().color=f||"inherit",this.inited&&this.fireEvent("textColor",f))},setBgColor:function(f){this.bgColor!==f&&(this.getODS().backgroundColor=this.bgColor=f,this.inited&&this.fireEvent("bgColor",f))},isColorAttr:f=>"bgColor"===f||"textColor"===f,setOpacity:function(f){this.opacity!==f&&(this.getODS().opacity=this.opacity=f,this.inited&&this.fireEvent("opacity",f))},setOverflow:function(f){const r=this.overflow;if(r!==f){this.overflow=f;const x=this.getIDS();if("autox"===f)x.overflowX="auto",
x.overflowY="hidden";else if("autoy"===f)x.overflowY="auto",x.overflowX="hidden";else{if("autox"===r||"autoy"===r)x.overflowX=x.overflowY=null;x.overflow=f||"visible"}this.inited&&this.fireEvent("overflow",f)}},setVisible:function(f){if(this.visible!==f){this.visible=f;const r=this.getODS();r.visibility=f?"inherit":"hidden";r.left=f?this.x+"px":"-100000px";r.top=f?this.y+"px":"-100000px";this.inited&&this.fireEvent("visible",f)}},setPointerEvents:function(f){this.pointerEvents!==f&&(this.pointerEvents=
f,this.getODS().pointerEvents=f||"auto",this.inited&&this.fireEvent("pointerEvents",f))},setCursor:function(f){this.cursor!==f&&(this.cursor=f,this.getODS().cursor=f||"auto",this.inited&&this.fireEvent("cursor",f))},__updateBounds:function(f,r){this.boundsWidth!==f&&this.fireEvent("boundsWidth",this.boundsWidth=f);this.boundsHeight!==r&&this.fireEvent("boundsHeight",this.boundsHeight=r)},setOutline:function(f){const [r,x,l]=f??[];this.setOutlineWidth(r);this.setOutlineStyle(x);this.setOutlineColor(l)},
setOutlineWidth:function(f){this.getODS().outlineWidth=(this.outlineWidth=f||0)+"px"},setOutlineStyle:function(f){this.getODS().outlineStyle=this.outlineStyle=f||"none"},setOutlineColor:function(f){this.getODS().outlineColor=this.outlineColor=f||"#000"},setBorder:function(f){const [r,x,l]=f??[];this.setBorderWidth(r);this.setBorderStyle(x);this.setBorderColor(l)},setBorderWidth:function(f){this.getODS().borderWidth=(this.borderWidth=f||0)+"px"},setBorderStyle:function(f){this.getODS().borderStyle=
...
sendSubviewBehind:function(f,r,x){x?.moveSubviewBefore(f,r);const l=this;if(f?.parent===l&&r?.parent===l){const y=f.getODE(),E=r.getODE();if(y!==E.previousElementSibling){const O=l.getIDE();h(f,()=>{O.insertBefore(y,E);l.doSubviewsReorderedInDom(f)})}}x?.update()},sendSubviewInFrontOf:function(f,r,x){x?.moveSubviewAfter(f,r);const l=this;if(f?.parent===l&&r?.parent===l){const y=f.getODE(),E=r.getODE().nextElementSibling;if(y!==E){const O=l.getIDE();h(f,()=>{O.insertBefore(y,E);l.doSubviewsReorderedInDom(f)})}}x?.update()},
...
c&&(this.textOverflow=c,this.getIDS().textOverflow=c||"inherit",this.inited&&this.fireEvent("textOverflow",c))},setTextAlign:function(c){this.textAlign!==c&&(this.textAlign=c,this.getIDS().textAlign=c||"inherit",this.inited&&this.fireEvent("textAlign",c))},setWhiteSpace:function(c){u(this,c,"whiteSpace")},setWordWrap:function(c){u(this,c,"wordWrap","normal")},setTextIndent:function(c){u(this,c,"textIndent")},setTextTransform:function(c){u(this,c,"textTransform")},setTextDecoration:function(c){u(this,
c,"textDecoration")},setLineHeight:function(c){u(this,c,"lineHeight")},setLetterSpacing:function(c){u(this,c,"letterSpacing")},setWordSpacing:function(c){u(this,c,"wordSpacing")},setFontFamily:function(c){u(this,c,"fontFamily")},setFontStyle:function(c){u(this,c,"fontStyle")},setFontVariant:function(c){u(this,c,"fontVariant")},setFontWeight:function(c){u(this,c,"fontWeight")},setFontSize:function(c){u(this,c,"fontSize")},setUserUnselectable:function(c){this.userUnselectable!==c&&(this.userUnselectable=
c,this[c?"addDomClass":"removeDomClass"]("mytUnselectable"),"default"!==this.cursor&&"text"!==this.cursor||this.setCursor(c?"default":"text"),this.inited&&this.fireEvent("userUnselectable",c))},enableEllipsis:function(){this.setWhiteSpace("nowrap");this.setOverflow("hidden");this.setTextOverflow("ellipsis")},disableEllipsis:function(){this.setOverflow("visible")},showTextShadow:function(c,n,m,k){this.getIDS().textShadow=(c||0)+"px "+(n||0)+"px "+(null!=m?m:2)+"px "+(k||"#000")},hideTextShadow:function(){this.getIDS().textShadow=
"none"}});const t=(c,n,m)=>{const k="padding"+m;n=Math.max(n,0);c[k]!==n&&(c[k]=n,c.getIDS()[k]=n+"px",c.fireEvent(k,n),"Left"===m||"Right"===m?g(c):h(c))},g=c=>{c.__hasSetWidth&&(c.getODS().width=c.width-(c.paddingLeft||0)-(c.paddingRight||0)+"px",c.inited&&c.sizeViewToDom())},h=c=>{c.__hasSetHeight&&(c.getODS().height=c.height-(c.paddingTop||0)-(c.paddingBottom||0)+"px",c.inited&&c.sizeViewToDom())};p.PaddedTextSupport=new v("PaddedTextSupport",{setWidth:function(c){this.callSuper(c);g(this)},setHeight:function(c){this.callSuper(c);
h(this)},setPadding:function(c){let n,m,k;c||=0;"object"===typeof c?(n=c.top||0,m=c.right||0,k=c.bottom||0,c=c.left||0):n=m=k=c;this.setPaddingTop(n);this.setPaddingRight(m);this.setPaddingBottom(k);this.setPaddingLeft(c)},setPaddingTop:function(c){t(this,c,"Top")},setPaddingRight:function(c){t(this,c,"Right")},setPaddingBottom:function(c){t(this,c,"Bottom")},setPaddingLeft:function(c){t(this,c,"Left")}})})(myt);
...
1)+1)/2,wiggle:l=>{l*=6;return 1>l?d(l-0,1):2>l?d(2-l,1):3>l?d(l-2,.5):4>l?d(4-l,.5):5>l?d(l-4,.25):d(6-l,.25)}},e=b.inOutQuad,f=l=>{l.__tmpFrom=!1;l.__loopCnt=l.reverse?l.repeat-1:0;l.__prog=l.reverse?l.duration:0},r=(l,y)=>{const {running:E,paused:O,reverse:B,duration:D,repeat:Q,__prog:R,easingFunction:w,callback:q}=l;if(E&&!O){B&&(y*=-1);y=R+y;let C=0;y>D?(C=y-D,y=D,++l.__loopCnt===Q&&(C=0)):0>y&&(C=-y,y=0,0>--l.__loopCnt&&0<Q&&(C=0));var z=l.target??l.parent;if(z){const {relative:G,attribute:H,
__isColorAnim:I,__loopCnt:S,to:P}=l;var K=v(0,y/D),N=v(0,R/D);null==l.from&&(l.__tmpFrom=!0,l.from=G?I?"#000":0:z.get(H));N=w(K)-(G?w(N):0);var M=l.from,A=G?z.get(H):M;K=z.set;if(I){M=a(M);const V=a(P);A=G?a(A):M;A.setRed(A.red+(V.red-M.red)*N);A.setGreen(A.green+(V.green-M.green)*N);A.setBlue(A.blue+(V.blue-M.blue)*N);N=A.getHtmlHexString()}else N=A+(P-M)*N;K.call(z,H,N);!B&&S===Q||B&&0>S&&0<Q?(l.setRunning(!1),q?.call(l,!0)):0<C?(l.fireEvent("repeat",S),l.__prog=B?D:0,r(l,C)):l.__prog=y}else console.log("No target for animator",
l),l.setRunning(!1),q?.call(l,!1)}},x=(l,y,E,O)=>{l.stopActiveAnimators(O,!0);const B=l[O];l.animate({attribute:O,from:B,to:B+y,duration:E,easingFunction:"wiggle"}).next(()=>{l.set(O,B)})};p.Animator=new JS.Class("Animator",p.Node,{include:[p.Reusable],extend:{easings:b,bounceView:(l,y=-6,E=900)=>{x(l,y,E,"y")},shakeView:(l,y=10,E=300)=>{x(l,y,E,"x")},wiggleView:x},initNode:function(l,y){this.duration=1E3;this.relative=this.reverse=this.running=this.paused=!1;this.repeat=1;this.easingFunction=e;this.callSuper(l,
...
y){const E=this,O=E.callback;O&&!y?E.setCallback(B=>{O.call(E,B);l.call(E,B)}):E.setCallback(l)},reset:function(l){f(this);this.setRunning(!1);this.setPaused(!1);l&&this.callback?.call(this,!1)},clean:function(){this.to=this.from=this.attribute=this.callback=void 0;this.duration=1E3;this.relative=this.reverse=!1;this.repeat=1;this.easingFunction=e;this.reset(!1)},__updateAnim:function(l){r(this,l.value.delta)}})})(myt);
...
(p=>{const {Class:v,Module:u}=JS,t=Math,{View:g,global:h}=p,c=h.focus,n=h.keys,{LIST_KEYS:m,CODE_ESC:k}=n,a=b=>{const e=b.itemConfig??[],f=e.length,{items:r,defaultItemClass:x,fixedWidth:l}=b,y=r.length,E=0<l,O=b.getContentView(),B=O.getLayouts();let D=0;for(var Q of B)Q.incrementLockedCounter();g.doWhileRemovedFromDom(O,()=>{const R={listView:b};E&&(R.width=l,R.enableEllipsis=!0);for(;f>D;++D){const w=e[D];let q=r[D],z,K;w&&(z=w.klass??x,K=w.attrs??{});q&&!q.isA(z)&&(q.destroy(),q=null);z&&(q??=
r[D]=new z(O,{...R}),q&&(q.callSetters(K),q.__LAYOUT_IDX=D))}});if(E)Q=l;else for(Q=b.minWidth,D=0;f>D;){const R=r[D++];R&&(R.syncToDom(),Q=t.max(Q,R.getMinimumWidth()))}for(;y>D;)r[D++].destroy();r.length=f;if(!E)for(D=0;f>D;)b.updateItemWidth(r[D++],Q);b.updateContentWidth(O,Q);for(const R of B)R.sortSubviews((w,q)=>w.__LAYOUT_IDX-q.__LAYOUT_IDX),R.decrementLockedCounter(),R.update()},d=p.ListViewItemMixin=new u("ListViewItemMixin",{setListView:function(b){this.listView=b},getMinimumWidth:()=>0,
...
(p=>{let v;const {Class:u,Module:t}=JS,g=console.warn,{NOOP:h,KeyObservable:c,UpdateableUI:n,dumpStack:m,global:k}=p,a=k.focus,d=k.keys,b={},e=(q,z)=>{if(q){const K=q.id;q.id?z(K):m("No ID")}else m("No processor")},f=q=>{e(q,z=>{b[z]=q})},r=q=>{const z=p.BAG.getGroup("selected",q.groupId);if(z){const K=z.getNodes();q=q.value;let N=K.length;for(;N;){const M=K[--N];if(M.optionValue===q){z.setTrue(M);break}}}},x=q=>{if(q.groupId){const z=p.BAG.getGroup("selected",q.groupId);z&&q.syncTo(z,"__syncValue",
"trueNode")}},l=(q,z)=>{const K=[],N=q.__v;if(N)for(const M of N)z=M.isFormValid(q,null,K)&&z;q.setErrorMessages(K);q.setIsValid(z);return z},y=(q,z,K)=>{if(q=q.__vp)for(const N of q)N[K]&&(z=N.process(z));return z},E=p.ValueProcessor=new u("ValueProcessor",{initialize:function(q,z,K,N){this.id=q;this.runForDefault=!!z;this.runForRollback=!!K;this.runForCurrent=!!N},process:q=>q}),O=p.ToNumberValueProcessor=new u("ToNumberValueProcessor",E,{process:q=>{if(null==q||""===q||"-"===q)return q;const z=
Number(q);return isNaN(z)?q:z}}),B=p.TrimValueProcessor=new u("TrimValueProcessor",E,{initialize:function(q,z,K,N,M){this.callSuper(q,z,K,N);this.trim=M},process:function(q){q+="";switch(this.trim){case "start":case "left":return q.trimStart();case "end":case "right":return q.trimEnd();default:return q.trim()}}}),D=p.UndefinedValueProcessor=new u("UndefinedValueProcessor",E,{initialize:function(q,z,K,N,M){this.callSuper(q,z,K,N);this.defaultValue=M},process:function(q){return void 0===q?this.defaultValue:
q}}),Q=p.Form=new t("Form",{extend:{ACCELERATOR_SCOPE_ROOT:"root",ACCELERATOR_SCOPE_ELEMENT:"element",ACCELERATOR_SCOPE_NONE:"none"},initNode:function(q,z){this.isChanged=this._lockCascade=!1;this.isValid=!0;this.__sf={};this.__v=[];this.__acc={};this.callSuper(q,z);this.form?.isA(Q)&&this.form.addSubForm(this)},destroy:function(){this.form?.isA(Q)&&this.form.removeSubForm(this.id);this.callSuper()},setErrorMessages:function(q){this.set("errorMessages",q,!0)},getFullId:function(){const q=[this.id];
...
setIsChanged:function(q){if(this.isChanged!==q){this.isChanged=q;this.inited&&this.fireEvent("isChanged",q);const z=this.form;z&&!this._lockCascade&&(q?z.notifySubFormChanged():z.verifyChangedState(this))}},setValidators:function(q){let z=q.length;for(;z;){const K=q[--z];"string"===typeof K&&(q[z]=k.validators.getValidator(K),q[z]||q.splice(z,1))}this.__v=q},getValue:function(){if(this.callSuper)return this.callSuper();if(this.isA(R))return this.value;const q={},z=this.__sf;for(const K in z)q[K]=
z[K].getValue();return q},setValue:function(q){this.callSuper?.(q);if("object"===typeof q&&!this.isA(R))for(const z in q){const K=this.getSubForm(z);K?q[z]=K.setValue(q[z]):g("ID in setValue for missing subform",z)}this.form?.notifyValueChanged(this);return q},getDefaultValue:function(){const q={};let z=this.__sf;for(const K in z)q[K]=z[K].getDefaultValue();return q},setDefaultValue:function(q){if("object"===typeof q)for(const z in q){const K=this.getSubForm(z);K?q[z]=K.setDefaultValue(q[z]):g("ID in setDefaultValue for missing subform",
z)}return q},getRollbackValue:function(){const q={},z=this.__sf;for(const K in z)q[K]=z[K].getRollbackValue();return q},setRollbackValue:function(q){if("object"===typeof q)for(const z in q){const K=this.getSubForm(z);K?q[z]=K.setRollbackValue(q[z]):g("ID in setRollbackValue for missing subform",z)}return q},addAccelerator:function(q,z){this.__acc[q]=z},removeAccelerator:function(q){delete this.__acc[q]},invokeAccelerator:function(q,z){this.__acc[q]?.call(this,z??null)},addValidator:function(q){q&&
this.__v.push(q)},removeValidator:function(q){if(q){const z=this.__v;let K=z.length;for(;K;){const N=z[--K];if(N.id===q)return z.splice(K,1),N}}},getRootForm:function(){return this.form?this.form.getRootForm():this},addSubForm:function(q){const z=q.id;null!=this.getSubForm(z)?g("ID in use for subform, add aborted",z,q):(q.setForm(this),this.__sf[z]=q,q.isChanged&&this.notifySubFormChanged(),q.isValid||this.notifySubFormInvalid())},removeSubForm:function(q){const z=this.getSubForm(q);z&&(z.setForm(null),
delete this.__sf[q],this.verifyChangedState(),this.verifyValidState());return z},getSubForm:function(q){return this.__sf[q]},getSubForms:function(){return this.__sf},getInvalidSubformIds:function(q){q&&this.doValidation();const z=[];(function M(N){if(N.isA(R))N.isValid||z.push(N.getFullId());else{N=N.getSubForms();for(const A in N)M(N[A])}})(this);return z},getAllErrorMessages:function(){const q=this.__sf;let z=(this.errorMessages??[]).slice();for(const K in q)z.push(...q[K].getAllErrorMessages());
return z},notifySubFormInvalid:function(){this.setIsValid(!1)},verifyValidState:function(q){const z=this.__sf;let K=!0;for(const N in z){const M=z[N];M!==q&&(K=M.isValid&&K)}return l(this,K)},doValidation:function(){const q=this.__sf;let z=!0;for(const K in q)z=q[K].doValidation()&&z;this._lockCascade=!0;z=l(this,z);this._lockCascade=!1;return z},notifyValueChanged:function(q){this.form?.notifyValueChanged(q)},notifySubFormChanged:function(){this.setIsChanged(!0)},verifyChangedState:function(q){const z=
this.__sf;let K=!1;for(const N in z){const M=z[N];M!==q&&(K=M.isChanged||K)}this.setIsChanged(K);return K},setup:function(q,z,K){this._lockCascade=!0;this.setIsChanged(!1);this.setErrorMessages([]);this.setIsValid(!0);this._lockCascade=!1;q??={};z??={};K??={};const N=this.__sf;for(const M in N)N[M].setup(q[M],z[M],K[M])},resetForm:function(){this._lockCascade=!0;const q=this.__sf;for(const z in q)q[z].resetForm();this.setIsChanged(!1);this.setErrorMessages([]);this.setIsValid(!0);this._lockCascade=
!1},rollbackForm:function(){this._lockCascade=!0;const q=this.__sf;for(const z in q)q[z].rollbackForm();this.setIsChanged(!1);this.setErrorMessages([]);this.setIsValid(!0);this._lockCascade=!1},getChangedValue:function(){const q={},z=this.__sf;for(const K in z){const N=z[K];N.isChanged&&(q[K]=N.getChangedValue())}return q}}),R=p.FormElement=new t("FormElement",{include:[Q],initNode:function(q,z){this.__vp=[];this.callSuper(q,z)},getValue:function(){return y(this,this.callSuper?this.callSuper():this.value,
...
q&&"blurWithKeyFix"!==q||this.verifyValidState()},updateUI:function(){this.setBorderColor(this.isValid?this.normalColor:this.isChanged?this.errorColor:this.actionRequiredColor)}});p.RootForm=new t("RootForm",{include:[Q],initNode:function(q,z){this.callSuper(q,z);this.addAccelerator("submit",this.doSubmit);this.addAccelerator("cancel",this.doCancel)},doSubmit:function(){this.isChanged&&(this.doValidation()?this.doValidSubmit(this.getValue()):this.doInvalidSubmit())},doValidSubmit:h,doInvalidSubmit:h,
doCancel:function(){this.rollbackForm();this.doValidation()}});p.FormCheckbox=new u("FormCheckbox",p.Checkbox,{include:[R],initNode:function(q,z){this.rollbackValue=this.defaultValue=!1;this.callSuper(q,z)}});p.FormComboBox=new u("FormComboBox",p.ComboBox,{include:[R,n],initNode:function(q,z){this.acceleratorScope="element";this.validateWhen="key";this.errorColor="#f99";this.actionRequiredColor="#966";this.normalColor="#999";this.callSuper(q,z);this.addValueProcessor(v);this.addAccelerator("accept",
...
this.actionRequiredColor)}});p.FormEditableText=new u("FormEditableText",p.EditableText,{include:[w],__hndlKeyDown:function(q){"nowrap"===this.whitespace&&this.callSuper(q)}});p.FormInputText=new u("FormInputText",p.InputText,{include:[w]});p.FormInputTextArea=new u("FormInputTextArea",p.InputTextArea,{include:[w],__hndlKeyDown:h});p.FormInputSelect=new u("FormInputSelect",p.InputSelect,{include:[R],setValue:function(q){if(!this.__abortSetValue)return q=this.callSuper(q),this.__abortSetValue=!0,this.deselectAll(),
this.selectValues(q),this.__abortSetValue=!1,this.verifyValidState(),q}});p.FormRadioGroup=new u("FormRadioGroup",p.Node,{include:[p.ValueComponent,R],initNode:function(q,z){z.groupId??(z.groupId=p.generateGuid());this.callSuper(q,z);void 0!==this.value&&r(this);x(this)},setValue:function(q){q=this.callSuper(q);this.inited&&r(this);return q},setGroupId:function(q){if(this.groupId!==q){if(this.groupId){const z=p.BAG.getGroup("selected",this.groupId);z&&this.detachFrom(z,"__syncValue","trueNode")}this.groupId=
q;this.inited&&x(this)}},__syncValue:function(q){q=q.value;this.setValue(q?q.optionValue:null)}});p.UseOtherFieldIfEmptyValueProcessor=new u("UseOtherFieldIfEmptyValueProcessor",E,{initialize:function(q,z,K,N,M){this.callSuper(q,z,K,N);this.otherField=M},process:function(q){return null==q||""===q?this.otherField.getValue():q}});new JS.Singleton("GlobalValueProcessorRegistry",{initialize:function(){f(v=new D("undefToEmpty",!0,!0,!0,""));f(new O("toNumber",!0,!0,!0));f(new B("trimLeft",!0,!0,!0,"left"));
...
(p=>{let v,u,t,g,h=0,c=0,n=0,m=255,k=[],a,d,b,e,f,r,x,l,y,E,O,B,D,Q,R,w,q,z,K,N,M,A,C,G,H,I;const S=JS.Class,{View:P,Text:V,ModalPanel:Y,SizeToChildren:aa,SpacedLayout:Aa,WrappingLayout:Ba,SelectionManager:Ca,Color:ra,LocalStorage:Da,Button:wa,TextButton:oa,Draggable:Ea,FontAwesome:{makeTag:pa},theme:X}=p,{min:ja,max:ka}=Math,Ia=new S("DialogButton",oa,{initNode:function(F,J){J.height??(J.height=17);J.paddingTop??(J.paddingTop=3);J.paddingLeft??(J.paddingLeft=10);J.paddingRight??(J.paddingRight=10);
J.activeColor??(J.activeColor="#bbb");J.hoverColor??(J.hoverColor="#ddd");J.readyColor??(J.readyColor="#ccc");J.domClass="mytButtonText";this.callSuper(F,J)}}),Ja=pa(["check"]),Ga=F=>{for(let J of F)7===J?.length&&(J+="ff"),Fa[J]=!0},Fa={},Ka=new S("Swatch",P,{include:[wa],initNode:function(F,J){const L=J.width=J.height=16;J.border=X.border1s9;J.domClass="mytCheckerboardPattern";const T=J.bgColor;this.callSuper(F,J);this.colorView=new P(this,{width:L,height:L,bgColor:T});T===g&&(F=ra.makeColorFromHexString(g),
new V(this,{x:2,y:2,text:Ja,fontSize:"12px",textColor:382.5>F.red+F.green+F.blue?"#fff":"#000"}))},setBgColor:function(F){this.colorView?.setBgColor(F);this.setTooltip(F)},doActivated:function(){v.setColor(this.colorView.bgColor)},drawHoverState:function(){this.setBorder(X.border1s3)},drawReadyState:function(){this.setBorder(X.border1s9)}}),La=p.ColorPicker=new S("ColorPicker",P,{initNode:function(F,J){v=this;v._isReady=!1;let L=J.color??"transparent";delete J.color;t=J.alphaChannel??!1;delete J.alphaChannel;
a=J.palette??[];Ga(a);delete J.palette;u="transparent"===L;v.callSuper(F,J);d=new P(v,{width:160,height:170});new Ba(d,{spacing:4,lineSpacing:4});b=new P(v,{x:170,width:139,height:139,border:X.border1s3},[Ea,{requestDragPosition:function(T,U){b.callSuper(b.x,b.y);const Z=this.getPagePosition();c=parseFloat(ka(0,ja(1,(T+this.dragInitX-Z.x)/this.width)));n=parseFloat(1-ka(0,ja(1,(U+this.dragInitY-Z.y)/this.height)));u=!1;v.updateUI()}}]);F=new P(b,{width:139,height:139});J=new P(F,{width:139,height:139});
F.getIDS().backgroundImage="linear-gradient(to right, #fff, rgba(204, 154, 129, 0))";J.getIDS().backgroundImage="linear-gradient(to top, #000, rgba(204, 154, 129, 0))";e=new P(J,{width:6,height:6,bgColor:"#000",border:[1,"solid","#fff"],roundedCorners:4});f=new P(v,{x:315,y:30,width:24,height:109,border:X.border1s3},[Ea,{requestDragPosition:function(T,U){this.callSuper(f.x,f.y);h=parseFloat(ka(0,ja(1,(U+this.dragInitY-this.getPagePosition().y)/this.height)));u=!1;v.updateUI()}}]);f.getIDS().background=
"linear-gradient(to top, #f00 0%, #f0f 17%, #00f 33%, #0ff 50%, #0f0 67%, #ff0 83%, #f00 100%)";r=new P(f,{x:-1,width:24,height:2,bgColor:"#fff",border:[1,"solid","#000"]});new P(v,{x:315,width:24,height:24,border:X.border1s3,tooltip:"Set to transparent.",domClass:"mytCheckerboardPattern"},[wa,{doActivated:()=>{v.setColor("transparent")}}]);F=146;t&&(y=new p.LabelSlider(v,{x:170,y:F,width:171,minValue:0,maxValue:255,flipThreshold:55,labelColor:"#fff"},[{setText:function(T,U){U=T.value;if(null==U||
isNaN(U))U=0;T.value=p.formatAsPercentage(U/255,0);this.callSuper(T,!0);m=U;v._isReady&&(u=!1,v.updateUI())}}]),F+=23);x=new p.InputText(v,{x:236,y:F,width:105,height:25,roundedCorners:3,textColor:X.DialogInputFgColor,border:X.border1s3,maxLength:11,fontFamily:"monospace"});v.attachToDom(x,"_submitInput","blur");v.attachToDom(x,"_handleKeyDown","keydown");x.getIDS().paddingLeft="6px";F=new P(v,{x:170,y:F,width:60,height:23,border:X.border1s3,domClass:"mytCheckerboardPattern"});new P(F,{width:30,height:23,
focusIndicator:!1,bgColor:L},[wa,{doActivated:()=>{v.setColor(L)}}]);l=new P(F,{x:30,width:30,height:23});if(F=Da.getItem("myt.default"))k=F.split(";"),Ga(k);v.setColor(L);v._isReady=!0},_handleKeyDown:F=>{p.KeyObservable.isEnterKeyEvent(F)&&v._submitInput()},_submitInput:()=>{v.setColor(x.value)},addToPalette:F=>{F&&"transparent"!==F&&!Fa[F]&&(k.unshift(F),k.length=ja(k.length,56),Da.setItem("myt.default",k.join(";")))},setColor:F=>{const J=F&&"transparent"!==F;if(t){let L="ff";J&&7<F.length&&(L=
F.slice(7,9),F=F.slice(0,7));y.setValue(parseInt(L,16))}else J&&7<F.length&&(F=F.slice(0,7));J?(F=ra.makeColorFromHexString(F).getHSV(),h=F.h/360,c=F.s,n=F.v,u=!1):u=!0;v.updateUI()},getColor:()=>{if(u)return"transparent";let F="";t&&(F=m.toString(16),2>F.length&&(F="0"+F));return ra.makeColorFromHSV(360*h,100*c,100*n).getHtmlHexString()+F},updateUI:()=>{const F=!u;r.setVisible(F);e.setVisible(F);F&&(e.setX(ka(0,ja(1,c)*(b.width+2))-5),e.setY(ka(0,ja(1,1-n)*(b.height+2))-5),r.setY(h*(f.height-2)-
1));b.setBgColor(ra.makeColorFromHSV(360*h,100,100).getHtmlHexString());g=v.getColor();x.setValue(g);l.setBgColor(g);v.redrawPalette()},redrawPalette:p.debounce(()=>{d.destroyAllSubviews();const F=new Set;a.push(...k);for(let J of a)if(t||7===J.length)7===J.length&&(J+="ff"),F.has(J)||(new Ka(d,{bgColor:J}),F.add(J))},50)}),xa=F=>{F=F.split(":");return[ka(0,ja(23,parseInt(F[0]))),ka(0,ja(60,parseInt(F[1])))]},Ha=new S("SelectableBtn",oa,{include:[p.Selectable],setSelected:function(F){this.callSuper(F);
this.updateUI()},updateUI:function(){this.callSuper();this.isSelected()?(this.setBgColor("#666"),this.setTextColor("#fff")):this.setTextColor()}}),Ma=new S("TimeBtn",Ha,{initNode:function(F,J){J.width=D?175:49;this.callSuper(F,J)},doActivated:function(){this.callSuper();const F=this.text,J=xa(F);I.selectById(F);M.setHours(J[0]);M.setMinutes(J[1])}}),Na=new S("DayBtn",Ha,{initNode:function(F,J){J.width=23;J.border=X.DialogDayBtnBorder;this.callSuper(F,J)},setData:function(F){this.data=F},setIsAnotherMonth:function(F){this.isAnotherMonth=
...
qa(F)}}),qa=F=>{F.setSeconds(0);F.setMilliseconds(0);var J=6>=R.length,L;if(J)for(L=7;L--&&!R.includes(F.getDay());)F.setDate(F.getDate()+1);M=F;const T=I.getODE().scrollTop;var U=new Date,Z=U.getFullYear(),W=U.getMonth(),ba=U.getDate();U.getHours();U.getMinutes();const ea=F.getFullYear(),fa=F.getMonth(),ma=F.getDate();var ha=F.getTime();U=F.getHours();F=F.getMinutes();const sa=(new Date(ea,fa,1)).getDay()-Q,ta=(new Date(ea,fa+1,0)).getDate(),ya=(new Date(ea,fa,0)).getDate(),ua=new Date(ea,fa,0),
va=new Date(ea,fa+2,0);Z=Z==ea&&W==fa;if(!D){W=H;W.deselectAll();W.destroyAllSubviews();L=new Date(ha);const la=7+Q;W=O.days;C.setText(ea+" "+O.sep+" "+O.months[fa]);L.setMinutes(59);L.setHours(23);L.setSeconds(59);L.setDate(0);A.setVisible(null==w||w<L.getTime());L.setMinutes(0);L.setHours(0);L.setSeconds(0);L.setDate(1);L.setMonth(fa+1);G.setVisible(null==q||q>L.getTime());for(L=0;7>L;L++)new V(H,{width:23,height:20,textAlign:"center",text:W[(L+la)%7],textColor:"#999"});L=0;let ia=42;0>sa&&(L=-7,
ia=35);W=new Date(ha);W.setHours(0);W.setMinutes(0);for(W.setSeconds(0);L<ia;L++){var ca=L+1-sa;const za=(L+la)%7,da=new Na(H);sa>L?(da.setText(ya+ca),da.setData(ua.getFullYear()+"/"+(ua.getMonth()+1)+"/"+(ya+ca)),da.setIsAnotherMonth(!0),W.setDate(ya+ca),W.setMonth(ua.getMonth()),W.setYear(ua.getFullYear())):L<sa+ta?(da.setText(ca),da.setData(ea+"/"+(fa+1)+"/"+ca),W.setDate(ca),W.setMonth(fa),W.setYear(ea)):(da.setText(ca-ta),da.setData(va.getFullYear()+"/"+(va.getMonth()+1)+"/"+(ca-ta)),da.setIsAnotherMonth(!0),
W.setDate(ca-ta),W.setMonth(va.getMonth()),W.setYear(va.getFullYear()));J&&!R.includes(za)?da.setDisabled(!0):(0===za?da.setIsSunday(!0):6===za&&da.setIsSaturday(!0),ca===ma&&(H.select(da),da.focus()),Z&&ca===ba&&da.setIsToday(!0),ca=new Date(W.getTime()),ca.setHours(23),ca.setMinutes(59),ca.setSeconds(59),(null!=w&&w>ca.getTime()||null!=q&&q<W.getTime())&&da.setDisabled(!0));da.updateUI()}}if(!B){J=60*K[0]+K[1];ba=I;ba.deselectAll();ba.destroyAllSubviews();let [la,ia]=z;for(W=new Date(ha);60*la+
ia<J;)ha=new Ma(I,{text:("0"+la).slice(-2)+":"+("0"+ia).slice(-2)}),la===U&&ia===F&&I.select(ha),W.setHours(la),W.setMinutes(ia),ba=W.getTime(),(null!=w&&w>ba||null!=q&&q<ba)&&ha.setDisabled(!0),ia+=N,60<=ia&&(ia-=60,la++);I.scrollYTo(T)}},Oa=p.DatePicker=new S("DatePicker",P,{initNode:function(F,J){E=this;const L={current:null,dateOnly:!1,timeOnly:!1,locales:null,locale:"",minuteInterval:30,firstDayOfWeek:0,showTodayButton:!0,minDate:null,maxDate:null,minTime:"00:00",maxTime:"23:59",allowedDays:null,
...J.opt};delete J.opt;E.callSuper(F,J);O=L.locales[L.locale??"en"];B=L.dateOnly;D=L.timeOnly;Q=L.firstDayOfWeek;N=ka(5,ja(30,L.minuteInterval));R=L.allowedDays;Array.isArray(R)&&0!==R.length||(R=[0,1,2,3,4,5,6]);w=Date.parse(L.minDate);w=isNaN(w)?null:w;q=Date.parse(L.maxDate);q=isNaN(q)?null:q;z=xa(L.minTime);K=xa(L.maxTime);F=new P(E,{visible:!D});L.showTodayButton&&new oa(F,{width:24,text:pa(["home"]),tooltip:O.today},[{doActivated:()=>{qa(new Date)}}]);A=new oa(F,{width:24,x:28,text:pa(["chevron-left"]),
tooltip:O.prevMonth},[{doActivated:()=>{const T=(new Date(M.getFullYear(),M.getMonth(),0)).getDate();T<M.getDate()&&M.setDate(T);M.setMonth(M.getMonth()-1);qa(M);A.focus()}}]);C=new V(F,{width:100,x:52,textAlign:"center"});G=new oa(F,{width:24,x:152,text:pa(["chevron-right"]),tooltip:O.nextMonth},[{doActivated:()=>{var T=(new Date(M.getFullYear(),M.getMonth()+1,0)).getDate();T<M.getDate()&&M.setDate(T);T=(new Date(M.getFullYear(),M.getMonth()+2,0)).getDate();T<M.getDate()&&M.setDate(T);M.setMonth(M.getMonth()+
1);qa(M);G.focus()}}]);H=new P(E,{y:25,width:175,height:175,visible:!D,maxSelected:1,itemSelectionId:"data"},[Ca]);new Ba(H,{spacing:2,lineInset:2,lineSpacing:3});I=new P(E,{x:D?0:195,y:D?0:25,width:(D?175:49)+p.DomElementProxy.getScrollbarSize(),height:D?200:175,overflow:"autoy",visible:!B,maxSelected:1,itemSelectionId:"text"},[Ca]);new Aa(I,{axis:"y",inset:1,spacing:3});E.setWidth(I.visible?I.x+I.width:H.width);qa(L.current??new Date)},getPickedDate:()=>M}),na=p.Dialog=new S("Dialog",Y,{extend:{BUTTON_CLASS:Ia,
...
color:"#000",locales:{en:{days:"Su Mo Tu We Th Fr Sa".split(" "),months:"Jan Feb Mar Apr May Jun Jul Aug Sep Oct Nov Dec".split(" "),sep:"-",prevMonth:"Previous month",nextMonth:"Next month",today:"Today"}},locale:"en"}},initNode:function(F,J){J.buttonClass??(J.buttonClass=na.BUTTON_CLASS);this.callSuper(F,J);F=this.content;F.setRoundedCorners(X.DialogRadius);F.setBgColor(X.DialogBgColor);F.setBoxShadow(X.DialogShadow);F.setBorder(X.DialogBorder);F.setFocusCage(!0);this.makeCloseButton(F)},setButtonClass:function(F){this.buttonClass=
F},setCallbackFunction:function(F){this.callbackFunction=F},makeButton:function(F,J){const L=this;return new L.buttonClass(F,J,[{doActivated:function(){L.doCallback(this)}}])},makeCloseButton:function(F){return F.closeBtn=this.makeButton(F,{ignoreLayout:!0,y:2,align:"right",alignOffset:2,width:19,height:16,paddingLeft:0,paddingRight:0,roundedCorners:10,activeColor:"#c00",hoverColor:"#f33",readyColor:"#f00",textColor:"#fff",text:pa(["times"]),tooltip:"Close Dialog."})},hide:function(F){this.spinner&&
(this.spinner.setVisible(!1),this.spinner=void 0);this.callSuper(F)},destroyContent:function(){this.spinner&&(this.spinner.setVisible(!1),this.spinner=void 0);const F=this.content,J=F.sizeToChildren;F.destroyAllSubviews(L=>"closeBtn"!==L.name);F.setVisible(!0);this.overlay.setBgColor(X.DimmerColor);this.setCallbackFunction();J.setPaddingY(X.ModalPanelPaddingY);J.setPaddingX(X.ModalPanelPaddingX);F.setRoundedCorners(X.DialogRadius);F.setBgColor(X.DialogBgColor);F.setBoxShadow(X.DialogShadow);F.setBorder(X.DialogBorder)},
doCallback:function(F){const J=this.callbackFunction;J&&J.call(this,F.name)||this.hide()},showBlank:function(F){this.destroyContent();this.content.setVisible(!1);F?.bgColor&&this.overlay.setBgColor(F.bgColor);this.show()},showMessage:function(F,J,L){const T=this.content,U=T.closeBtn;L={...na.WRAP_TEXT_DEFAULTS,...L};this.destroyContent();this.setCallbackFunction(J);F=T.msg=new V(T,{text:F,whiteSpace:L.whiteSpace,wordWrap:L.wordWrap,fontWeight:L.fontWeight,x:null==L.msgX?X.ModalPanelPaddingX:L.msgX,
y:null==L.msgY?X.ModalPanelPaddingY:L.msgY,width:L.width});L.titleText&&(this.setupTitle(T,L.titleText),F.setY(F.y+24));this.show();!1===L.showClose?U.destroyed||U.setVisible(!1):(U.setVisible(!0),U.focus())},showSimple:function(F,J,L,T){L=L??{};const U=this.content,Z=U.closeBtn,W=L.maxContainerHeight;this.destroyContent();L.bgColor&&U.setBgColor(L.bgColor);L.roundedCorners&&U.setRoundedCorners(L.roundedCorners);L.boxShadow&&U.setBoxShadow(L.boxShadow);L.border&&U.setBorder(L.border);U.sizeToChildren.setPaddingX(1);
this.setCallbackFunction(J);J=U.contentContainer=new P(U,{x:1,y:25,overflow:"auto"},[{setHeight:function(ba){ba>W&&(ba=W);this.callSuper(ba)}}]);F.call(this,J);new aa(J,{axis:"both"});this.show();Z.setVisible(!0);Z.focus();L.titleText&&(this.setupTitle(U,L.titleText),J.setY(this.header.height+1));J.initialFocus?.focus();T?.(this)},showConfirm:function(F,J,L){L={...na.CONFIRM_DEFAULTS,...L};this.showMessage(F,J,L);this.setupFooterButtons(this.content.msg,L)},showContentConfirm:function(F,J,L,T){L=
{...na.CONFIRM_DEFAULTS,...L};const U=this.content,Z=U.closeBtn,W=L.maxContainerHeight;this.destroyContent();U.sizeToChildren.setPaddingX(1);this.setCallbackFunction(J);J=U.contentContainer=new P(U,{x:1,y:25,overflow:"auto"},[{setHeight:function(ba){this.callSuper(ba>W?W:ba)}}]);F.call(this,J);new aa(J,{axis:"both"});this.show();!1===L.showClose?Z.destroyed||Z.setVisible(!1):(Z.setVisible(!0),Z.focus());this.setupTitle(U,L.titleText);J.setY(this.header.height+1);this.setupFooterButtons(J,L);J.initialFocus?.focus();
T?.(this)},showSpinner:function(F,J){const L=this.content;J={...na.NO_WRAP_TEXT_DEFAULTS,...J};this.destroyContent();const T=this.spinner=new p.Spinner(L,{align:"center",visible:!0,borderColor:"#ccc",size:50,y:null==J.msgY?X.ModalPanelPaddingY:J.msgY});F&&(this.msgTxt=new V(L,{text:F,whiteSpace:J.whiteSpace,wordWrap:J.wordWrap,fontWeight:J.fontWeight,x:null==J.msgX?X.ModalPanelPaddingX:J.msgX,y:T.y+T.size+X.ModalPanelPaddingY,width:J.width}));this.show();L.closeBtn.setVisible(!1);this.focus()},updateMessage:function(F){this.msgTxt?.setText(F)},
showColorPicker:function(F,J){const L=this,T=L.content,U=T.closeBtn;J={...na.COLOR_PICKER_DEFAULTS,...J};L.destroyContent();L.setCallbackFunction(Z=>{switch(Z){case "closeBtn":case "cancelBtn":F.call(L,Z);break;case "confirmBtn":const W=v.getColor();v.addToPalette(W);F.call(L,Z,W)}v.destroy()});v=new La(T,{x:X.ModalPanelPaddingX,y:X.ModalPanelPaddingY+24,width:337,height:177+(J.alphaChannel?23:0),color:J.color,alphaChannel:J.alphaChannel,palette:"#000000 #111111 #222222 #333333 #444444 #555555 #666666 #777777 #888888 #999999 #aaaaaa #bbbbbb #cccccc #dddddd #eeeeee #ffffff".split(" ")});
L.show();U.setVisible(!0);U.focus();L.setupFooterButtons(v,J);L.setupTitle(T,J.titleText)},showDatePicker:function(F,J){const L=this,T=L.content,U=T.closeBtn;J={...na.DATE_PICKER_DEFAULTS,...J};L.destroyContent();L.setCallbackFunction(Z=>{switch(Z){case "closeBtn":case "cancelBtn":F.call(L,Z);break;case "confirmBtn":F.call(L,Z,E.getPickedDate())}E.destroy()});E=new Oa(T,{x:X.ModalPanelPaddingX,y:X.ModalPanelPaddingY+24,height:195,opt:{current:new Date(J.initialDate??Date.now()),dateOnly:J.dateOnly||
!1,timeOnly:J.timeOnly||!1,locales:J.locales,locale:J.locale}});L.show();U.setVisible(!0);U.focus();L.setupFooterButtons(E,J);L.setupTitle(T,J.timeOnly?J.timeOnlyTitleText:J.titleText)},setupTitle:function(F,J){const L=X.DialogRadius;(this.header=new P(F,{ignoreLayout:!0,width:F.width,height:24,bgColor:"#eee",roundedTopLeftCorner:L,roundedTopRightCorner:L})).sendToBack();F.title=new V(F,{x:L,y:4,text:J,fontWeight:"bold"})},setupFooterButtons:function(F,J){const L=this.content;var T=X.ModalPanelPaddingY,
U=T/2;F=L.btnContainer=new P(L,{y:F.y+F.height+T,align:"center"});const Z=["active","hover","ready","text"];T=J.cancelAttrs??{};var W;(W=T).name??(W.name="cancelBtn");let ba;(ba=T).text??(ba.text=J.cancelTxt);for(let ha of Z)ha+="Color",null!=J[ha]&&(T[ha]=J[ha]);W=this.makeButton(F,T);T=J.confirmAttrs??{};var ea;(ea=T).name??(ea.name="confirmBtn");let fa;(fa=T).text??(fa.text=J.confirmTxt);for(var ma of Z)ma+="Color",ea=ma+"Confirm",null!=J[ea]&&(T[ma]=J[ea]);this.makeButton(F,T);if(ma=J.button)for(const ha of ma)this.makeButton(F,
ha);new aa(F,{axis:"y"});new Aa(F,{spacing:4,collapseParent:!0});L.sizeToChildren.setPaddingY(U);U=F.y-U;(new P(L,{ignoreLayout:!0,y:U,width:L.width,height:L.height-U,bgColor:"#eee"})).sendToBack();!1===J.showClose&&W.focus()}})})(myt);
...
(p=>{let v=0;const {Class:u,Module:t}=JS,{min:g,max:h}=Math,{View:c,SpacedLayout:n,Node:{DEFAULT_PLACEMENT:m}}=p,k=w=>{const q=w.columnHeaders,[z,K]=w.sort??["",""];let N=q.length;for(;N;){const M=q[--N];M.columnId===z?M.sortable&&M.setSortState(K):M.setSortState("none")}w.locked||w.doSort()},a=(w,q,z,K)=>{if(0!==q){const C=0<q,G=[];for(var N=w.length;N;){var M=w[--N];M.resizable&&(z?0<M.flex:0===M.flex)&&G.push({hdr:M,limit:(C?M.maxValue:M.minValue)-M.value,amt:0})}w=G.length;if(0<w){for(N=0;0!==
q;){v>=w&&(N=v=0);M=G[v];var A=M.hdr;M.full?++N:(C?(A=g(z?A.flex:1,q),M.amt+A>M.limit&&(A=M.limit-M.amt,M.full=!0)):(A=h(z?-A.flex:-1,q),M.amt+A<M.limit&&(A=M.limit-M.amt,M.full=!0)),M.amt+=A,q-=A);if(N===w)break;++v}for(const H of G)H.hdr.setValue(H.hdr.value+H.amt);K?.(q)}}},d=w=>(w=w.gridController?.getPrevHdr(w))?w.maxValue-w.value+d(w):0,b=w=>(w=w.gridController?.getNextHdr(w))?w.maxValue-w.value+b(w):0,e=w=>(w=w.gridController?.getPrevHdr(w))?w.minValue-w.value+e(w):0,f=w=>(w=w.gridController?.getNextHdr(w))?
w.minValue-w.value+f(w):0,r=(w,q)=>{w=w.gridController?.getPrevHdr(w);var z=0;if(w){z=w.value+q;w.resizable&&w.setValue(z);const K=z-w.value;z=q-K;0>K&&(z+=r(w,K))}return z},x=(w,q)=>{w=w.gridController?.getPrevHdr(w);var z=0;if(w){z=w.value+q;w.resizable&&w.setValue(z);const K=z-w.value;z=q-K;0<K&&(z+=x(w,K))}return z},l=(w,q)=>{if(w=w.gridController?.getNextHdr(w))q=w.value+q,w.resizable&&w.setValue(q),q-=w.value,0>q&&l(w,q)},y=(w,q)=>{if(w=w.gridController?.getNextHdr(w))q=w.value+q,w.resizable&&
w.setValue(q),q-=w.value,0<q&&y(w,q)},E=w=>{let q="";if(w.sortable)switch(w.sortState){case R.SORT_ORDER_ASC:q="chevron-up";break;case R.SORT_ORDER_DESC:q="chevron-down"}w.sortIcon.setIcon(q)},O=w=>{const q=w.textView;q?.setWidth(w.width-w.outset-q.x)},B=p.GridController=new t("GridController",{initNode:function(w,q){this.columnHeaders=[];this.rows=[];this.maxWidth=this.minWidth=this.gridWidth=0;this.fitToWidth=this.locked=!0;this.callSuper(w,q);this.fitHeadersToWidth();k(this)},setMaxWidth:function(w){this.set("maxWidth",
...
if(0<w)for(;w;){const z=q[--w];if(z.visible)return z}},getNextHdr:function(w){const q=this.columnHeaders,z=q.length;w=this.getHdrIndex(w)+1;if(0<w&&w<z)for(;z>w;){const K=q[w++];if(K.visible)return K}},hasHdr:function(w){return this.columnHeaders.includes(w)},getHdrIndex:function(w){return this.columnHeaders.indexOf(w)},getHdrById:function(w){for(const q of this.columnHeaders)if(q.columnId===w)return q},getVisibleHdrs:function(){return this.columnHeaders.filter(w=>w.visible)},notifyAddHdr:function(w){this.hasHdr(w)||
...
this._notifyHdrChange("Width",w)}},notifyHdrVisibilityChange:function(w){if((!this.__skipInvisibleHeaders||w.visible)&&!this.isLocked()){this.updateRowsForVisibilityChange(w);var q=this.setLastColumn;a:{var z=this.columnHeaders;let K=z.length;for(;K;){const N=z[--K];if(N.visible){z=N;break a}}z=void 0}q.call(this,z);q=w.visible?1:-1;this.setMaxWidth(this.maxWidth+w.maxValue*q);this.setMinWidth(this.minWidth+w.minValue*q);this.fitHeadersToWidth();this.fixupResizerCursors();this._notifyHdrChange("Visibility",
...
1;return this.rows[w<this.rows.length?w:0]},notifyAddRow:function(w,q){if(!this.hasRow(w)&&(this.rows.push(w),!this.locked)){const z=this.width;for(const K of this.columnHeaders)w.setWidth(z),w.notifyHdrXChange(K),w.notifyHdrWidthChange(K),w.notifyHdrVisibilityChange(K);q||this.doSort()}},notifyRemoveRow:function(w){w=this.getRowIndex(w);-1<w&&this.rows.splice(w,1)},fitHeadersToWidth:function(){if(!this.locked&&this.fitToWidth){const w=this.getVisibleHdrs();let q=0;for(const z of w)q=h(q,z.x+z.width);
a(w,this.gridWidth-q,!0,z=>{a(w,z,!1,null)})}},fixupResizerCursors:function(){const w=this.getVisibleHdrs();let q=!0,z=0;w.forEach((A,C)=>{q&&A.isFixed()?A.setResizerCursor("",!0):(q&&(z=C),q=!1,A.restoreResizerCursor())});let K=!0,N=w.length,M=N-1;for(;N>z;){const A=w[--N];K?A.setResizerCursor("",!0):A.restoreResizerCursor();A.isFixed()||(K&&(M=N),K=!1)}z===M&&w[z].setResizerCursor("",!0)}}),D=p.GridColHdr=new t("GridColHdr",{include:[p.BoundedValueComponent],initNode:function(w,q){const z=this,
K=q.resizerCursor??"col-resize";delete q.resizerCursor;q.minValue??(q.minValue=16);q.maxValue??(q.maxValue=1E3);q.value??(q.value=q.minValue);q.resizable??(q.resizable=!0);q.flex??(q.flex=0);q.cellXAdj??(q.cellXAdj=0);q.cellWidthAdj??(q.cellWidthAdj=0);q.sortable??(q.sortable=!0);q.sortState??(q.sortState="none");q.placement??(q.placement=m);z.callSuper(w,q);const N=z.gridController;z.resizer=new c(z,{cursor:K,width:10,zIndex:1,percentOfParentHeight:100,align:"right",alignOffset:-5,draggableAllowBubble:!1},
[p.SizeToParent,p.Draggable,{requestDragPosition:function(M,A){A=M-this.x;if(N.fitToWidth&&(0<A?A=g(A,-f(z),z.maxValue-z.value+d(z)):0>A&&(A=h(A,-b(z),z.minValue-z.value+e(z))),0===A))return;M=z.value+A;z.resizable&&z.setValue(M);M-=z.value;A=M-A;let C=0;0>M?C=r(z,M):0<M&&(C=x(z,M));this.dragInitX+=C;A-=C;N.fitToWidth&&(0>A?l(z,A):0<A&&y(z,A))}}]);N&&(N.notifyAddHdr(z),N.notifyHdrXChange(z),N.notifyHdrVisibilityChange(z));z.setWidth(z.value);z.resizer.setVisible(!(z.last&&z.gridController.fitToWidth))},
destroy:function(w){this.setGridController();this.callSuper(w)},setSortable:function(w){this.set("sortable",w,!0)},setSortState:function(w){this.set("sortState",w,!0)},setResizable:function(w){this.set("resizable",w,!0)},setCellWidthAdj:function(w){this.cellWidthAdj=w},setCellXAdj:function(w){this.cellXAdj=w},setFlex:function(w){this.flex=w},setColumnId:function(w){this.columnId=w},setResizerCursor:function(w,q){const z=this.resizer,K=z.cursor;w!==K&&(q&&(this.__rrc=K),z.setCursor(w))},restoreResizerCursor:function(){const w=
...
setX:function(w){const q=this.x;this.callSuper(w);this.inited&&q!==this.x&&this.gridController?.notifyHdrXChange(this)},setVisible:function(w){const q=this.visible;this.callSuper(w);this.inited&&q!==this.visible&&this.gridController?.notifyHdrVisibilityChange(this)}}),Q=p.GridRow=new t("GridRow",{initNode:function(w,q){q.placement??(q.placement=m);this.callSuper(w,q);this.gridController?.notifyAddRow(this)},destroy:function(w){this.setGridController();this.callSuper(w)},setGridController:function(w){const q=
this.gridController;q!==w&&(q?.notifyRemoveRow(this),this.gridController=w,this.inited&&w?.notifyAddRow(this))},notifyHdrXChange:function(w){this.getRef(w.columnId)?.setX(w.x+w.cellXAdj)},notifyHdrWidthChange:function(w){this.getRef(w.columnId)?.setWidth(w.width+w.cellWidthAdj)},notifyHdrVisibilityChange:function(w){this.getRef(w.columnId)?.setVisible(w.visible)}}),R=p.Grid=new u("Grid",c,{include:[B],extend:{SORT_ORDER_ASC:"ascending",SORT_ORDER_DESC:"descending"},initNode:function(w,q){q.overflow??
...
q){if(w===m){let z;q.isA(Q)?z=this.content:q.isA(D)&&(z=this.header);if(z)return q.gridController!==this&&q.setGridController(this),z}return this.callSuper(w,q)},doSort:function(){const [w,q]=this.sort??["",""],z=this.getSortFunction(w,q);if(z){const K=this.content,N=K.yLayout;this.rows.sort(z);K.sortSubviews(z);N.sortSubviews(z);N.update()}},getSortFunction:(w,q)=>{if(w){const z=q===R.SORT_ORDER_ASC?1:-1;return(K,N)=>{K=K.getRef(w).text;N=N.getRef(w).text;return K>N?z:N>K?-z:0}}}});p.SimpleGridColHdr=
...
N){this.callSuper(K,N);this.getIDS().fontSize="0.7em"}}]);this.textView.enableEllipsis();z&&this.textView.setTextAlign(z);this.setDisabled(!this.sortable);O(this);E(this)},setSortIconColor:function(w){this.sortIconColor=w;this.sortIcon?.setTextColor(w)},setSortable:function(w){this.callSuper(w);this.inited&&(w&&this.setOutset(14),this.setDisabled(!w),E(this))},setSortState:function(w){this.callSuper(w);this.inited&&E(this)},setWidth:function(w){this.callSuper(w);this.inited&&O(this)},doActivated:function(){this.disabled||
...
const D=this.width;B.setWidth(D);for(const Q of B.getSubviews())Q.setWidth(D)}},getVisibleRows:function(){return Object.values(this._visibleRowsByIdx??{})},getVisibleRowForModel:function(B){const D=this.getVisibleRows();let Q=D.length;for(;Q;){const R=D[--Q];if(R.model===B)return R}},getListViewHeight:function(){return this._listView.height},isScrolledToEnd:function(){return this.getIDE().scrollTop+this.height===this._listView.height},getSortFunction:function(){const B=this.modelIDName,D=this.ascendingSort;
return this.numericSort?p.getNumericObjSortFunc(B,D):d(B,D,!1)},getFilterFunction:h,scrollModelIntoView:function(B,D){var Q=this.getIndexOfModelInData(B);let R=!1;if(0<=Q){var w=this._rowExtent;const q=this.getIDE().scrollTop,z=q+this.height;Q=this.rowInset+Q*w;w=Q+w;Q<=q?(this.scrollYTo(Q,!0),R=!0):w>=z&&(this.scrollYTo(w-this.height,!0),R=!0);D&&((D=this.getVisibleRowForModel(B))?D.focus():this._focusToModel=B)}return R},isModelInData:function(B){return 0<=this.getIndexOfModelInData(B)},getNextModel:function(B,
D=!0,Q=!0){const R=this.getListData(),w=R.length;B=this.getIndexOfModelInData(B);if(0<=B)return B+=1,B>=w?D?R[0]:R[w-1]:R[B];if(Q&&0<w)return R[w-1]},getPrevModel:function(B,D=!0,Q=!0){const R=this.getListData(),w=R.length;B=this.getIndexOfModelInData(B);if(0<=B)return--B,0>B?D?R[w-1]:R[0]:R[B];if(Q&&0<w)return R[0]},getIndexOfModelInData:function(B){if(B){const D=this.getListData();let Q=D.length;for(;Q;)if(this.areModelsEqual(D[--Q],B))return Q}return-1},areModelsEqual:function(B,D){const Q=this.modelIDName;
return B[Q]===D[Q]},getActiveRowForModel:function(B){if(B){const D=this._itemPool.getActives();let Q=D.length;for(;Q;){const R=D[--Q];if(this.areModelsEqual(R.model,B))return R}}},refreshListData:function(B,D){this._listData=this.collectionModel.getAsSortedList(this.getSortFunction(),this.getFilterFunction());this.resetListUI(B,D)},resetListUI:function(B,D){const Q=this.getListData().length,R=this._listView,w=this._scrollAnchorView;R.setHeight(Q*this._rowExtent-(0<Q?this.rowSpacing:0)+this.rowInset+
this.rowOutset);w.setY(R.height-w.height);this._startIdx=this._endIdx=-1;this.forceFullResetOnNextRefresh=D;B||0===this.getIDE().scrollTop?this.refreshListUI():this.scrollYTo(0,!0)},putRowBackInPool:function(B){if(k.focusedView?.isDescendantOf(B)){const D=this.getFocusTrap();D?D.focus():k.clear()}B.setVisible(!1);this._itemPool.putInstance(B)},refreshListUI:function(B){B=this._rowExtent;const D=this.rowInset,Q=this.forceFullResetOnNextRefresh;var R=this.getIDE().scrollTop;const w=this.getListData()??
[];var q=g(0,t.floor((R-D)/B));R=t.min(w.length,t.ceil((R-D+this.height)/B));this.forceFullResetOnNextRefresh&&(this.forceFullResetOnNextRefresh=!1);if(this._startIdx!==q||this._endIdx!==R||Q){const K=this.width,N=this.rowHeight,M=this._visibleRowsByIdx,A=this._focusToModel;this._startIdx=q;this._endIdx=R;for(var z in M)if(z<q||z>=R)this.putRowBackInPool(M[z]),delete M[z];for(z=q;z<R;z++){q=M[z];const C=w[z],G=this.getClassKey(C);let H=!1;q&&q.classKey===G||(q&&this.putRowBackInPool(q),M[z]=q=this._itemPool.getInstance(G),
q.setInfiniteOwner(this),q.setClassKey(G),q.setWidth(K),q.setHeight(N),q.setY(D+z*B),q.setVisible(!0),H=!0);!Q&&q.model&&this.areModelsEqual(q.model,C)?H&&this.updateRow(q):(q.setModel(C),this.updateRow(q));q.notifyRefreshed();q.bringToFront();A&&this.areModelsEqual(A,C)&&(q.focus(),this._focusToModel=null)}}},getClassKey:B=>"default",updateRow:h}),l=p.SimpleSelectableInfiniteListRow=new v("SimpleSelectableInfiniteListRow",p.SimpleButton,{include:[r,p.ArrowKeyActivation],initNode:function(B,D){D.selectedColor??
...
(p=>{const {max:v,cos:u,sin:t,PI:g}=Math,h=p.Geometry.degreesToRadians,c=d=>{var b=h(d.startAngle),e=h(d.endAngle);if(b>e){var f=b;b=e;e=f}f=d.__path;var r=d.width/2,x=d.thickness;const l=d.radius,y=l+x;var E=e-b;const O=E+1E-4>=2*g;O&&(b=0,e=g);const B=[r+y*u(b),r+y*t(b)],D=[r+y*u(e),r+y*t(e)];e=[r+l*u(e),r+l*t(e)];b=[r+l*u(b),r+l*t(b)];r=["M"+B.join()];O?r.push("A"+[y,y,0,1,1,D].join(),"A"+[y,y,0,1,1,B].join(),"L"+e.join(),"A"+[l,l,0,1,0,b].join(),"A"+[l,l,0,1,0,e].join()):(E=E%(2*g)>g?1:0,x/=2,
...
b+n);r(d,f-k);0<k&&x(d,f,d+k,f);r(e-a,f);0<a&&x(e,f,e,f-a);r(e,b+m);0<m&&x(e,b,e-m,b);r(d+n,b);0<n&&x(d,b,d,b+n);this.closePath();return this},drawAnnulus:function(n,m,k,a,d,b,e,f=60){const r=p.Color;for(var x of e)"string"===typeof x.color&&(x.color=r.makeColorFromHexString(x.color));1>f&&(v("Invalid segements",f),f=60);f=u/f;x=0;for(var l=e.length-1;x<l;){var y=e[x++];const {angle:z,color:K}=y,{angle:N,color:M}=e[x];var E=Math.round((N-z)/f),O=K.getDiffFrom(M);y.colorDelta={red:O.red/E,green:O.green/
E,blue:O.blue/E}}k=new p.Path([n+k,m,n+a,m]);k.rotateAroundOrigin(d,n,m);a=k.vectors;let [B,D,Q,R]=a,w=0,q=u/360;for(x=0;b>d;){E=B;O=D;l=Q;y=R;d+=f;d>b&&(f+=b-d,q=0,d=b);k.rotateAroundOrigin(f+q,n,m);[B,D,Q,R]=a;this.beginPath();this.moveTo(E,O);this.lineTo(l,y);this.lineTo(Q,R);this.lineTo(B,D);this.closePath();const {color:z,colorDelta:K}=e[x];this.setFillStyle(r.rgbToHex(z.red+w*K.red,z.green+w*K.green,z.blue+w*K.blue,!0));this.fill();0<q&&(k.rotateAroundOrigin(-q,n,m),[B,D,Q,R]=a);w++;d>=e[x+
