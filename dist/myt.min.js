...
P=/\{\{plural:\$(\d+)\|([^|]+)\|([^}]+)\}\}/g,Q=/\$(\d+)/g,w=(A,C=JSON.stringify,G=Infinity)=>{const H=new Map;return(...J)=>{var S=C(J);if(H.has(S))return H.get(S);J=A(...J);H.set(S,J);H.size>G&&(S=H.keys().next().value,H.delete(S));return J}},q=(A,C,G=new WeakMap,H=new WeakMap)=>{if(A!==C){if(A instanceof Date&&C instanceof Date)return A.getTime()===C.getTime();if(null===A||null===C||"object"!==typeof A||"object"!==typeof C)return!1;if(G.has(A))return H.get(C)===G.get(A);G.set(A,C);H.set(C,A);var J=
Array.isArray(A),S=Array.isArray(C);if(J!==S)return!1;if(J){var R=A.length;if(R!==C.length)return!1;for(J=0;J<R;J++)if(!q(A[J],C[J],G,H))return!1;return!0}J=Object.keys(A);S=Object.keys(C);if(J.length!==S.length)return!1;for(R of J)if(!Object.prototype.hasOwnProperty.call(C,R)||!q(A[R],C[R],G,H))return!1}return!0},z=/\\([{}])|\{([^{}]+)\}/g,K=/(,|\r?\n|\r|^)(?:"((?:\\.|""|[^\\"])*)"|([^,"\r\n]*))/gi,N=/[\\"](.)/g,M=p.myt={version:2.02306021558E7,generateGuid:()=>++u,TRUE_FUNC:()=>!0,FALSE_FUNC:()=>
...
TabInset:8,TabOutset:8,TabContainerInset:0,TabContainerSpacing:1,BaseTooltipDelay:500,BaseTooltipHideDelay:100,TooltipEdgeColor:"#444",TooltipBgColor:"#444",TooltipTextColor:"#eee",TooltipShadowColor:"#00000033",TooltipEdgeSize:0,TooltipShadowSize:2,TooltipHInset:6,TooltipVInset:3,focusShadow:[0,0,7,"#666"],disabledOpacity:.5,border1s3:[1,"solid","#333"],border1s9:[1,"solid","#999"]},hash:A=>A.split("").reduce((C,G)=>{C=(C<<5)-C+G.charCodeAt(0);return C&C},0),addEventListener:(A,C,G,H,J)=>{A.addEventListener(C,
G,{capture:H||!1,passive:J||!1})},removeEventListener:(A,C,G,H)=>{A.removeEventListener(C,G,H||!1)},resolveName:(A,C)=>{if(A&&0!==A.length){C=C??global;var G=e(A)?A:A.split("."),H=G.length;for(let J=0;J<H;J++)if(C=C[G[J]],null==C){n("resolveName failed for",A,"at part",J,G[J]);return}return C}},resolveClassname:A=>{"string"===typeof A&&(A=M.resolveName(A));return A&&"function"===typeof A.isA&&A.isA(JS.Class)?A:null},getExtension:A=>A?(A=A.split("."),1<A.length?A.pop():null):null,loadScript:function(A,
C,G,H){const J=this._loadedScripts??(this._loadedScripts={});let S=J[A];if(!0===S)C?.(!0);else if(!1===S)C?.(!1);else if(Array.isArray(S))C&&S.push(C);else{S=J[A]=[];C&&S.push(C);const R=x("script");R.type="text/javascript";R.async=!1;H&&(R.integrity=H,R.crossOrigin="anonymous");const W=Y=>{R.onload=R.onreadystatechange=R.onerror=null;for(const aa of S)aa(Y);J[A]=Y};R.onerror=()=>{W(!1)};R.onload=R.onreadystatechange=()=>{R.readyState&&"complete"!==R.readyState||W(!0)};R.src=A+(G?"":(A.includes("?")?
"&":"?")+"cacheBust="+Date.now());r.appendChild(R)}},dumpStack:(A,C)=>{let G;"string"===typeof A&&(G=A,A=null);M.global.error.notify(C||"error",null,G,A)},getRandom:A=>{const C=m.random();return A?d(0,a(A(C),.9999999999)):C},getRandomArbitrary:(A,C,G)=>{const H=a(A,C);return M.getRandom(G)*(d(A,C)-H)+H},getRandomInt:(A,C,G)=>{const H=a(A,C);return m.floor(M.getRandom(G)*(d(A,C)-H+1)+H)},areFloatsEqual:(A,C,G)=>{const H=k(A),J=k(C);return k(A-C)<=(H>J?J:H)*(null==G?1E-6:k(G))},areArraysEqual:(A,C)=>
{if(A!==C){if(null===A||null===C||"object"!==typeof A||"object"!==typeof C)return!1;let G=A.length;if(G!==C.length)return!1;for(;G;)if(A[--G]!==C[G])return!1}return!0},shallowEqual:(A,C)=>{if(A!==C){if(null===A||null===C||"object"!==typeof A||"object"!==typeof C)return!1;const G=Object.keys(A),H=Object.keys(C);if(G.length!==H.length)return!1;for(const J of G)if(A[J]!==C[J])return!1}return!0},deepEqual:q,stableStringify:A=>JSON.stringify(A,(C,G)=>{if(G&&"object"===typeof G&&!Array.isArray(G)){C={};
for(const H of Object.keys(G).sort())C[H]=G[H];return C}return G}),getElement:(A,C)=>f.getElementsByTagName(A||"body")[0<C?C:0],loadFontFaces:(A,C)=>{Promise.all(A.map(({family:G,url:H,options:J})=>(new FontFace(G,"url("+H+")",J)).load())).then(G=>{if(E){for(const H of G)E.add(H),O(H);C?.(G)}})},loadFontFace:(A,C,G={},H)=>{A=new FontFace(A,"url("+C+")",G);A.loaded.then(J=>{E&&(E.add(J),O(J),H?.(J))});A.load()},registerForFontNotification:(A,C)=>{y[C]?A&&!A.destroyed&&A.isVisible()&&(A.sizeViewToDom(),
A.notifyFontLoaded?.(C)):(l[C]??(l[C]=[])).push(A)},createBaseFontCSSRule:A=>{M.addCSSRule(M.createStylesheet(),"body, input","font-family:"+A)},loadCSSFonts:A=>{A?.forEach(M.createStylesheetLink)},createStylesheetLink:A=>{if(A){let C=c[A];C?console.warn("stylesheet already exists",A):(C=c[A]=x("link"),C.rel="stylesheet",C.href=A,r.appendChild(C));return C}},createStylesheet:()=>{const A=x("style");r.appendChild(A);return A.sheet},addCSSRule:(A,C,G,H)=>{A.insertRule(C+"{"+G+"}",H??0)},removeCSSRules:A=>
{let C=A.cssRules.length;for(;C;)A.deleteRule(--C)},createInputPlaceholderCSSRule:(A,C,G,H=1)=>{let J=A.__sheet;J?M.removeCSSRules(J):J=A.__sheet=M.createStylesheet();A=A.getODE();A=A.id||(A.id="id"+ ++u);const S=[];C&&S.push("color:"+C);G&&S.push("font-family:"+G);H&&S.push("opacity:"+H);M.addCSSRule(J,"#"+A+"::placeholder",S.join("; "))},isSorted:(A,C)=>{const G=A.length;for(let H=1;H<G;H++)if(0<C(A[H-1],A[H]))return!1;return!0},chainSortFunc:(...A)=>(C,G)=>{for(const H of A){const J=H(C,G);if(0!==
J)return J}return 0},getAlphaObjSortFunc:w((A,C,G,H)=>{const J=C?1:-1,S=M.I18N.getLocale(),R={sensitivity:G?"accent":"variant"};return(W,Y)=>{W=W[A];Y=Y[A];H?("string"!==typeof W&&(W=""+W),"string"!==typeof Y&&(Y=""+Y)):(W=W||"",Y=Y||"");return W.localeCompare(Y,S,R)*J}}),getNumericObjSortFunc:w((A,C)=>{const G=C?1:-1;return(H,J)=>{H=H[A]||0;J=J[A]||0;return(H-J)*G}}),csvStringToArray:(A,C)=>{if(!A)return[];const G=[[]];let H;for(;H=K.exec(A);)H[1].length&&","!==H[1]&&G.push([]),G[G.length-1].push(void 0!==
H[2]?H[2].replace(N,"$1"):H[3]);if(C){const J=G.shift();return G.map(S=>{let R=0;return J.reduce((W,Y)=>{W[Y]=S[R++];return W},{})})}return G},prepareCSVDataURI:(A,C)=>{const G=S=>{if(S&&e(S)){const R=[],W=S.length;for(let Y=0,aa;W>Y;Y++)aa=S[Y],aa=null==aa?"":aa.toString(),aa=aa.replace(/"/g,'""'),0<=aa.search(/("|,|\n)/g)&&(aa='"'+aa+'"'),R.push(aa);return R.join(",")}n("Unexpected row",S);return null},H=[];if(C){var J=G(C);J&&H.push(J)}if(A)if(e(A)){J=A.length;for(let S=0;J>S;S++){const R=G(A[S]);
R&&H.push(R)}}else n("Rows were not an array");return M.encodeCSVDataURI(H.join("\r\n"),C)},encodeCSVDataURI:(A,C)=>"data:text/csv;charset=utf-8"+(null==C?"":";header="+(C?"present":"absent"))+","+encodeURIComponent(A),dataURIToBlob:A=>{var C=A.indexOf(",");const G=A.slice(0,C).split(":")[1].split(";")[0];A=A.slice(C+1);if(G.startsWith("text/"))A=decodeURIComponent(A);else{A=atob(A);C=A.length;const H=new Uint8Array(C);for(;C;)H[--C]=A.charCodeAt(C);A=H}return new Blob([A],{type:G})},formatAsPercentage:(A,
C=2)=>{switch(typeof A){case "number":return C=a(16,d(0,C)),A=m.round(d(0,a(1,A))*b(10,2+C))/b(10,C),(0===A%1?A:A.toFixed(C))+"%";case "string":return A;default:return n("formatAsPercentage expects a number"),A}},interpolateString:(A,C={})=>A?A.replace(z,(G,H,J)=>H?H:M.resolveName(J,C)??G):"",leftPadNumber:(A,C,G="0",H=10)=>{A=A.toString(H);return G.repeat(d(C-A.length,0))+A},removeMarkup:(A,C)=>{if(!A)return"";C?.brToLineFeed&&(A=A.replace(/<br\s*\/?>/gi,"\n"));return A.replace(/<\/?[^>]+(>|$)/g,
"")},escapeMarkup:(()=>{const A=RegExp("[&<>\"']","g"),C={"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#039;"},G=H=>C[H];return H=>H.replace(A,G)})(),memoize:w,debounce:(A,C,G=!1)=>{let H;return function(...J){const S=this,R=G&&!H;clearTimeout(H);H=setTimeout(()=>{H=null;G||A.apply(S,J)},C);R&&A.apply(S,J)}},createFixedThresholdCounter:(A,C,G,H)=>{var J=M.AccessorSupport.generateName;const S="function"===typeof A||A instanceof JS.Module,R={};H=H||J("counter",G);const W=J(H,"increment");J=
J(H,"decrement");if(void 0!==(S?A.instanceMethod(W):A[W]))return n("Increment: Abort clobber",W,A),!1;if(void 0!==(S?A.instanceMethod(J):A[J]))return n("Decrement: Abort clobber",J,A),!1;R[W]=function(){const Y=this[H]+1;this[H]=Y;this.fireEvent(H,Y);Y===C&&this.set(G,!0)};R[J]=function(){const Y=this[H];if(0!==Y){var aa=Y-1;this[H]=aa;this.fireEvent(H,aa);Y===C&&this.set(G,!1)}};A[S?"include":"extend"](R);return!0},makeURLSearchParams:(A={})=>{const C=new URLSearchParams;for(const G in A){const H=
A[G];void 0!==H&&C.set(G,H)}return C},doFetch:(A,C,G,H,J,S)=>fetch(A,C).then(R=>{if(!R.ok)throw new v(R.status,A,R.statusText);return R}).then(R=>G?R.text():R.json()).then(R=>{if(H)try{if(G)H(R);else{if(!1===R.success)throw new v(200,A,R.message??R.data?.message);H(R.data)}}catch(W){throw R=new v(200,A,W.message),R.stack=W.stack,R;}}).catch(R=>{if(J){if("FetchError"!==R.name){const W=new v(0,A,R.message);W.stack=R.stack;R=W}J(R)}}).finally(()=>{S?.()}),toNameCase:(A,C)=>{if(!A)return"";A=A.trim().toLowerCase().split(/([\s\-'\u2019"\u201c\u201d().,\/])/).reduce((G,
H)=>G+(H[0]??"").toUpperCase()+H.slice(1),"");if(/\bMac[A-Za-z]{2,}[^aciozj]\b/.test(A)||/\bMc/.test(A))A=A.replace(/\b(Ma?c)([A-Za-z]+)/,(G,H,J)=>H+(J[0]??"").toUpperCase()+J.slice(1)),A=A.replace(/\bMacEvicius\b/,"Macevicius").replace(/\bMacHado\b/,"Machado").replace(/\bMacHar\b/,"Machar").replace(/\bMacHin\b/,"Machin").replace(/\bMacHlin\b/,"Machlin").replace(/\bMacIas\b/,"Macias").replace(/\bMacIulis\b/,"Maciulis").replace(/\bMacKie\b/,"Mackie").replace(/\bMacKle\b/,"Mackle").replace(/\bMacKlin\b/,
...
y/l;y=0;for(var E of b)E.crossSize+=r,E.crossPos+=y,y+=r}for(const C of b){E=C.items;r=C.extraSize;if(0<r)if(0<C.growCount){r/=C.growCount;b=0;for(const J of E)0<b&&m(J,B,b),y=J.getGrow()*r,0<y&&(c(J,B,y,!0),b+=y)}else switch(b=E.length,e){default:case "start":break;case "end":g(E,B,r,!1);break;case "center":g(E,B,r/2,!1);break;case "spaceBetween":1<b&&g(E,B,r/(b-1),!0,1);break;case "spaceAround":r<=P?g(E,B,r/2,!1):(g(E,B,P/2,!1),r-=P,g(E,B,r/(2*b),!0),g(E,B,r/(2*b),!0,1));break;case "spaceEvenly":r<=
2*P?g(E,B,r/2,!1):(g(E,B,P,!1),r-=2*P,g(E,B,r/(b+1),!0))}else if(0>r&&0<C.shrinkCount){b=C.shrinkBasis;y=0;for(const J of E)0>y&&m(J,B,y),x=J.getShrink()*(B?J.getFlexBasisWidth():J.getFlexBasisHeight())/b*u(r,-b),0>x&&(c(J,B,x,!0),y+=x)}r=!1;const {crossSize:G,crossPos:H}=C;for(const J of E)switch(J.getAlignSelf()||f){case "start":n(J,D,H);c(J,D,D?J.getFlexBasisWidth():J.getFlexBasisHeight(),!1);break;case "end":n(J,D,H+G-(D?J.getFlexBasisWidth():J.getFlexBasisHeight()));c(J,D,D?J.getFlexBasisWidth():
J.getFlexBasisHeight(),!1);break;case "center":n(J,D,H+(G-(D?J.getFlexBasisWidth():J.getFlexBasisHeight()))/2);c(J,D,D?J.getFlexBasisWidth():J.getFlexBasisHeight(),!1);break;case "baseline":r=!0;n(J,D,H+(G-(D?J.getFlexBasisWidth():J.getFlexBasisHeight()))/2+J.getFlexBaselineOffset(B));c(J,D,D?J.getFlexBasisWidth():J.getFlexBasisHeight(),!1);break;default:n(J,D,H),c(J,D,G,!1)}r&&a.updateFlexboxLayoutBaselineAdjustment(E,B,H)}a.__isUpdatingFlexboxLayout=!1;a.isUpdateAgain()&&!a.__UPDATE_AGAIN_LOOP_PROTECTION&&
...
1)+1)/2,wiggle:l=>{l*=6;return 1>l?d(l-0,1):2>l?d(2-l,1):3>l?d(l-2,.5):4>l?d(4-l,.5):5>l?d(l-4,.25):d(6-l,.25)}},e=b.inOutQuad,f=l=>{l.__tmpFrom=!1;l.__loopCnt=l.reverse?l.repeat-1:0;l.__prog=l.reverse?l.duration:0},r=(l,y)=>{const {running:E,paused:O,reverse:B,duration:D,repeat:P,__prog:Q,easingFunction:w,callback:q}=l;if(E&&!O){B&&(y*=-1);y=Q+y;let C=0;y>D?(C=y-D,y=D,++l.__loopCnt===P&&(C=0)):0>y&&(C=-y,y=0,0>--l.__loopCnt&&0<P&&(C=0));var z=l.target??l.parent;if(z){const {relative:G,attribute:H,
__isColorAnim:J,__loopCnt:S,to:R}=l;var K=v(0,y/D),N=v(0,Q/D);null==l.from&&(l.__tmpFrom=!0,l.from=G?J?"#000":0:z.get(H));N=w(K)-(G?w(N):0);var M=l.from,A=G?z.get(H):M;K=z.set;if(J){M=a(M);const W=a(R);A=G?a(A):M;A.setRed(A.red+(W.red-M.red)*N);A.setGreen(A.green+(W.green-M.green)*N);A.setBlue(A.blue+(W.blue-M.blue)*N);N=A.getHtmlHexString()}else N=A+(R-M)*N;K.call(z,H,N);!B&&S===P||B&&0>S&&0<P?(l.setRunning(!1),q?.call(l,!0)):0<C?(l.fireEvent("repeat",S),l.__prog=B?D:0,r(l,C)):l.__prog=y}else console.log("No target for animator",
...
(p=>{const {Class:v,Module:u}=JS,t=Math,{View:g,global:h}=p,c=h.focus,n=h.keys,{LIST_KEYS:m,CODE_ESC:k}=n,a=b=>{const e=b.itemConfig??[],f=e.length,{items:r,defaultItemClass:x,fixedWidth:l}=b,y=r.length,E=0<l,O=b.getContentView(),B=O.getLayouts();let D=0;for(var P of B)P.incrementLockedCounter();g.doWhileRemovedFromDom(O,()=>{const Q={listView:b};E&&(Q.width=l,Q.enableEllipsis=!0);for(;f>D;++D){const w=e[D];let q=r[D],z,K;w&&(z=w.klass??x,K=w.attrs??{});q&&!q.isA(z)&&(q.destroy(),q=null);z&&(q??=
r[D]=new z(O,{...Q}),q&&(q.callSetters(K),q.__LAYOUT_IDX=D))}});if(E)P=l;else for(P=b.minWidth,D=0;f>D;){const Q=r[D++];Q&&(Q.syncToDom(),P=t.max(P,Q.getMinimumWidth()))}for(;y>D;)r[D++].destroy();r.length=f;if(!E)for(D=0;f>D;)b.updateItemWidth(r[D++],P);b.updateContentWidth(O,P);for(const Q of B)Q.sortSubviews((w,q)=>w.__LAYOUT_IDX-q.__LAYOUT_IDX),Q.decrementLockedCounter(),Q.update()},d=p.ListViewItemMixin=new u("ListViewItemMixin",{setListView:function(b){this.listView=b},getMinimumWidth:()=>0,
...
q}}),P=p.Form=new t("Form",{extend:{ACCELERATOR_SCOPE_ROOT:"root",ACCELERATOR_SCOPE_ELEMENT:"element",ACCELERATOR_SCOPE_NONE:"none"},initNode:function(q,z){this.isChanged=this._lockCascade=!1;this.isValid=!0;this.__sf={};this.__v=[];this.__acc={};this.callSuper(q,z);this.form?.isA(P)&&this.form.addSubForm(this)},destroy:function(){this.form?.isA(P)&&this.form.removeSubForm(this.id);this.callSuper()},setErrorMessages:function(q){this.set("errorMessages",q,!0)},getFullId:function(){const q=[this.id];
...
setIsChanged:function(q){if(this.isChanged!==q){this.isChanged=q;this.inited&&this.fireEvent("isChanged",q);const z=this.form;z&&!this._lockCascade&&(q?z.notifySubFormChanged():z.verifyChangedState(this))}},setValidators:function(q){let z=q.length;for(;z;){const K=q[--z];"string"===typeof K&&(q[z]=k.validators.getValidator(K),q[z]||q.splice(z,1))}this.__v=q},getValue:function(){if(this.callSuper)return this.callSuper();if(this.isA(Q))return this.value;const q={},z=this.__sf;for(const K in z)q[K]=
z[K].getValue();return q},setValue:function(q){this.callSuper?.(q);if("object"===typeof q&&!this.isA(Q))for(const z in q){const K=this.getSubForm(z);K?q[z]=K.setValue(q[z]):g("ID in setValue for missing subform",z)}this.form?.notifyValueChanged(this);return q},getDefaultValue:function(){const q={};let z=this.__sf;for(const K in z)q[K]=z[K].getDefaultValue();return q},setDefaultValue:function(q){if("object"===typeof q)for(const z in q){const K=this.getSubForm(z);K?q[z]=K.setDefaultValue(q[z]):g("ID in setDefaultValue for missing subform",
...
delete this.__sf[q],this.verifyChangedState(),this.verifyValidState());return z},getSubForm:function(q){return this.__sf[q]},getSubForms:function(){return this.__sf},getInvalidSubformIds:function(q){q&&this.doValidation();const z=[];(function M(N){if(N.isA(Q))N.isValid||z.push(N.getFullId());else{N=N.getSubForms();for(const A in N)M(N[A])}})(this);return z},getAllErrorMessages:function(){const q=this.__sf;let z=(this.errorMessages??[]).slice();for(const K in q)z.push(...q[K].getAllErrorMessages());
...
!1},rollbackForm:function(){this._lockCascade=!0;const q=this.__sf;for(const z in q)q[z].rollbackForm();this.setIsChanged(!1);this.setErrorMessages([]);this.setIsValid(!0);this._lockCascade=!1},getChangedValue:function(){const q={},z=this.__sf;for(const K in z){const N=z[K];N.isChanged&&(q[K]=N.getChangedValue())}return q}}),Q=p.FormElement=new t("FormElement",{include:[P],initNode:function(q,z){this.__vp=[];this.callSuper(q,z)},getValue:function(){return y(this,this.callSuper?this.callSuper():this.value,
...
q&&"blurWithKeyFix"!==q||this.verifyValidState()},updateUI:function(){this.setBorderColor(this.isValid?this.normalColor:this.isChanged?this.errorColor:this.actionRequiredColor)}});p.RootForm=new t("RootForm",{include:[P],initNode:function(q,z){this.callSuper(q,z);this.addAccelerator("submit",this.doSubmit);this.addAccelerator("cancel",this.doCancel)},doSubmit:function(){this.isChanged&&(this.doValidation()?this.doValidSubmit(this.getValue()):this.doInvalidSubmit())},doValidSubmit:h,doInvalidSubmit:h,
...
(p=>{let v,u,t,g,h=0,c=0,n=0,m=255,k=[],a,d,b,e,f,r,x,l,y,E,O,B,D,P,Q,w,q,z,K,N,M,A,C,G,H,J;const S=JS.Class,{View:R,Text:W,ModalPanel:Y,SizeToChildren:aa,SpacedLayout:Aa,WrappingLayout:Ba,SelectionManager:Ca,Color:ra,LocalStorage:Da,Button:wa,TextButton:oa,Draggable:Ea,FontAwesome:{makeTag:pa},theme:X}=p,{min:ja,max:ka}=Math,Ia=new S("DialogButton",oa,{initNode:function(F,I){I.height??(I.height=17);I.paddingTop??(I.paddingTop=3);I.paddingLeft??(I.paddingLeft=10);I.paddingRight??(I.paddingRight=10);
I.activeColor??(I.activeColor="#bbb");I.hoverColor??(I.hoverColor="#ddd");I.readyColor??(I.readyColor="#ccc");I.domClass="mytButtonText";this.callSuper(F,I)}}),Ja=pa(["check"]),Ga=F=>{for(let I of F)7===I?.length&&(I+="ff"),Fa[I]=!0},Fa={},Ka=new S("Swatch",R,{include:[wa],initNode:function(F,I){const L=I.width=I.height=16;I.border=X.border1s9;I.domClass="mytCheckerboardPattern";const T=I.bgColor;this.callSuper(F,I);this.colorView=new R(this,{width:L,height:L,bgColor:T});T===g&&(F=ra.makeColorFromHexString(g),
new W(this,{x:2,y:2,text:Ja,fontSize:"12px",textColor:382.5>F.red+F.green+F.blue?"#fff":"#000"}))},setBgColor:function(F){this.colorView?.setBgColor(F);this.setTooltip(F)},doActivated:function(){v.setColor(this.colorView.bgColor)},drawHoverState:function(){this.setBorder(X.border1s3)},drawReadyState:function(){this.setBorder(X.border1s9)}}),La=p.ColorPicker=new S("ColorPicker",R,{initNode:function(F,I){v=this;v._isReady=!1;let L=I.color??"transparent";delete I.color;t=I.alphaChannel??!1;delete I.alphaChannel;
a=I.palette??[];Ga(a);delete I.palette;u="transparent"===L;v.callSuper(F,I);d=new R(v,{width:160,height:170});new Ba(d,{spacing:4,lineSpacing:4});b=new R(v,{x:170,width:139,height:139,border:X.border1s3},[Ea,{requestDragPosition:function(T,U){b.callSuper(b.x,b.y);const Z=this.getPagePosition();c=parseFloat(ka(0,ja(1,(T+this.dragInitX-Z.x)/this.width)));n=parseFloat(1-ka(0,ja(1,(U+this.dragInitY-Z.y)/this.height)));u=!1;v.updateUI()}}]);F=new R(b,{width:139,height:139});I=new R(F,{width:139,height:139});
F.getIDS().backgroundImage="linear-gradient(to right, #fff, rgba(204, 154, 129, 0))";I.getIDS().backgroundImage="linear-gradient(to top, #000, rgba(204, 154, 129, 0))";e=new R(I,{width:6,height:6,bgColor:"#000",border:[1,"solid","#fff"],roundedCorners:4});f=new R(v,{x:315,y:30,width:24,height:109,border:X.border1s3},[Ea,{requestDragPosition:function(T,U){this.callSuper(f.x,f.y);h=parseFloat(ka(0,ja(1,(U+this.dragInitY-this.getPagePosition().y)/this.height)));u=!1;v.updateUI()}}]);f.getIDS().background=
"linear-gradient(to top, #f00 0%, #f0f 17%, #00f 33%, #0ff 50%, #0f0 67%, #ff0 83%, #f00 100%)";r=new R(f,{x:-1,width:24,height:2,bgColor:"#fff",border:[1,"solid","#000"]});new R(v,{x:315,width:24,height:24,border:X.border1s3,tooltip:"Set to transparent.",domClass:"mytCheckerboardPattern"},[wa,{doActivated:()=>{v.setColor("transparent")}}]);F=146;t&&(y=new p.LabelSlider(v,{x:170,y:F,width:171,minValue:0,maxValue:255,flipThreshold:55,labelColor:"#fff"},[{setText:function(T,U){U=T.value;if(null==U||
isNaN(U))U=0;T.value=p.formatAsPercentage(U/255,0);this.callSuper(T,!0);m=U;v._isReady&&(u=!1,v.updateUI())}}]),F+=23);x=new p.InputText(v,{x:236,y:F,width:105,height:25,roundedCorners:3,textColor:X.DialogInputFgColor,border:X.border1s3,maxLength:11,fontFamily:"monospace"});v.attachToDom(x,"_submitInput","blur");v.attachToDom(x,"_handleKeyDown","keydown");x.getIDS().paddingLeft="6px";F=new R(v,{x:170,y:F,width:60,height:23,border:X.border1s3,domClass:"mytCheckerboardPattern"});new R(F,{width:30,height:23,
focusIndicator:!1,bgColor:L},[wa,{doActivated:()=>{v.setColor(L)}}]);l=new R(F,{x:30,width:30,height:23});if(F=Da.getItem("myt.default"))k=F.split(";"),Ga(k);v.setColor(L);v._isReady=!0},_handleKeyDown:F=>{p.KeyObservable.isEnterKeyEvent(F)&&v._submitInput()},_submitInput:()=>{v.setColor(x.value)},addToPalette:F=>{F&&"transparent"!==F&&!Fa[F]&&(k.unshift(F),k.length=ja(k.length,56),Da.setItem("myt.default",k.join(";")))},setColor:F=>{const I=F&&"transparent"!==F;if(t){let L="ff";I&&7<F.length&&(L=
F.slice(7,9),F=F.slice(0,7));y.setValue(parseInt(L,16))}else I&&7<F.length&&(F=F.slice(0,7));I?(F=ra.makeColorFromHexString(F).getHSV(),h=F.h/360,c=F.s,n=F.v,u=!1):u=!0;v.updateUI()},getColor:()=>{if(u)return"transparent";let F="";t&&(F=m.toString(16),2>F.length&&(F="0"+F));return ra.makeColorFromHSV(360*h,100*c,100*n).getHtmlHexString()+F},updateUI:()=>{const F=!u;r.setVisible(F);e.setVisible(F);F&&(e.setX(ka(0,ja(1,c)*(b.width+2))-5),e.setY(ka(0,ja(1,1-n)*(b.height+2))-5),r.setY(h*(f.height-2)-
1));b.setBgColor(ra.makeColorFromHSV(360*h,100,100).getHtmlHexString());g=v.getColor();x.setValue(g);l.setBgColor(g);v.redrawPalette()},redrawPalette:p.debounce(()=>{d.destroyAllSubviews();const F=new Set;a.push(...k);for(let I of a)if(t||7===I.length)7===I.length&&(I+="ff"),F.has(I)||(new Ka(d,{bgColor:I}),F.add(I))},50)}),xa=F=>{F=F.split(":");return[ka(0,ja(23,parseInt(F[0]))),ka(0,ja(60,parseInt(F[1])))]},Ha=new S("SelectableBtn",oa,{include:[p.Selectable],setSelected:function(F){this.callSuper(F);
this.updateUI()},updateUI:function(){this.callSuper();this.isSelected()?(this.setBgColor("#666"),this.setTextColor("#fff")):this.setTextColor()}}),Ma=new S("TimeBtn",Ha,{initNode:function(F,I){I.width=D?175:49;this.callSuper(F,I)},doActivated:function(){this.callSuper();const F=this.text,I=xa(F);J.selectById(F);M.setHours(I[0]);M.setMinutes(I[1])}}),Na=new S("DayBtn",Ha,{initNode:function(F,I){I.width=23;I.border=X.DialogDayBtnBorder;this.callSuper(F,I)},setData:function(F){this.data=F},setIsAnotherMonth:function(F){this.isAnotherMonth=
...
qa(F)}}),qa=F=>{F.setSeconds(0);F.setMilliseconds(0);var I=6>=Q.length,L;if(I)for(L=7;L--&&!Q.includes(F.getDay());)F.setDate(F.getDate()+1);M=F;const T=J.getODE().scrollTop;var U=new Date,Z=U.getFullYear(),V=U.getMonth(),ba=U.getDate();U.getHours();U.getMinutes();const ea=F.getFullYear(),fa=F.getMonth(),ma=F.getDate();var ha=F.getTime();U=F.getHours();F=F.getMinutes();const sa=(new Date(ea,fa,1)).getDay()-P,ta=(new Date(ea,fa+1,0)).getDate(),ya=(new Date(ea,fa,0)).getDate(),ua=new Date(ea,fa,0),
va=new Date(ea,fa+2,0);Z=Z==ea&&V==fa;if(!D){V=H;V.deselectAll();V.destroyAllSubviews();L=new Date(ha);const la=7+P;V=O.days;C.setText(ea+" "+O.sep+" "+O.months[fa]);L.setMinutes(59);L.setHours(23);L.setSeconds(59);L.setDate(0);A.setVisible(null==w||w<L.getTime());L.setMinutes(0);L.setHours(0);L.setSeconds(0);L.setDate(1);L.setMonth(fa+1);G.setVisible(null==q||q>L.getTime());for(L=0;7>L;L++)new W(H,{width:23,height:20,textAlign:"center",text:V[(L+la)%7],textColor:"#999"});L=0;let ia=42;0>sa&&(L=-7,
ia=35);V=new Date(ha);V.setHours(0);V.setMinutes(0);for(V.setSeconds(0);L<ia;L++){var ca=L+1-sa;const za=(L+la)%7,da=new Na(H);sa>L?(da.setText(ya+ca),da.setData(ua.getFullYear()+"/"+(ua.getMonth()+1)+"/"+(ya+ca)),da.setIsAnotherMonth(!0),V.setDate(ya+ca),V.setMonth(ua.getMonth()),V.setYear(ua.getFullYear())):L<sa+ta?(da.setText(ca),da.setData(ea+"/"+(fa+1)+"/"+ca),V.setDate(ca),V.setMonth(fa),V.setYear(ea)):(da.setText(ca-ta),da.setData(va.getFullYear()+"/"+(va.getMonth()+1)+"/"+(ca-ta)),da.setIsAnotherMonth(!0),
V.setDate(ca-ta),V.setMonth(va.getMonth()),V.setYear(va.getFullYear()));I&&!Q.includes(za)?da.setDisabled(!0):(0===za?da.setIsSunday(!0):6===za&&da.setIsSaturday(!0),ca===ma&&(H.select(da),da.focus()),Z&&ca===ba&&da.setIsToday(!0),ca=new Date(V.getTime()),ca.setHours(23),ca.setMinutes(59),ca.setSeconds(59),(null!=w&&w>ca.getTime()||null!=q&&q<V.getTime())&&da.setDisabled(!0));da.updateUI()}}if(!B){I=60*K[0]+K[1];ba=J;ba.deselectAll();ba.destroyAllSubviews();let [la,ia]=z;for(V=new Date(ha);60*la+
ia<I;)ha=new Ma(J,{text:("0"+la).slice(-2)+":"+("0"+ia).slice(-2)}),la===U&&ia===F&&J.select(ha),V.setHours(la),V.setMinutes(ia),ba=V.getTime(),(null!=w&&w>ba||null!=q&&q<ba)&&ha.setDisabled(!0),ia+=N,60<=ia&&(ia-=60,la++);J.scrollYTo(T)}},Oa=p.DatePicker=new S("DatePicker",R,{initNode:function(F,I){E=this;const L={current:null,dateOnly:!1,timeOnly:!1,locales:null,locale:"",minuteInterval:30,firstDayOfWeek:0,showTodayButton:!0,minDate:null,maxDate:null,minTime:"00:00",maxTime:"23:59",allowedDays:null,
...I.opt};delete I.opt;E.callSuper(F,I);O=L.locales[L.locale??"en"];B=L.dateOnly;D=L.timeOnly;P=L.firstDayOfWeek;N=ka(5,ja(30,L.minuteInterval));Q=L.allowedDays;Array.isArray(Q)&&0!==Q.length||(Q=[0,1,2,3,4,5,6]);w=Date.parse(L.minDate);w=isNaN(w)?null:w;q=Date.parse(L.maxDate);q=isNaN(q)?null:q;z=xa(L.minTime);K=xa(L.maxTime);F=new R(E,{visible:!D});L.showTodayButton&&new oa(F,{width:24,text:pa(["home"]),tooltip:O.today},[{doActivated:()=>{qa(new Date)}}]);A=new oa(F,{width:24,x:28,text:pa(["chevron-left"]),
tooltip:O.prevMonth},[{doActivated:()=>{const T=(new Date(M.getFullYear(),M.getMonth(),0)).getDate();T<M.getDate()&&M.setDate(T);M.setMonth(M.getMonth()-1);qa(M);A.focus()}}]);C=new W(F,{width:100,x:52,textAlign:"center"});G=new oa(F,{width:24,x:152,text:pa(["chevron-right"]),tooltip:O.nextMonth},[{doActivated:()=>{var T=(new Date(M.getFullYear(),M.getMonth()+1,0)).getDate();T<M.getDate()&&M.setDate(T);T=(new Date(M.getFullYear(),M.getMonth()+2,0)).getDate();T<M.getDate()&&M.setDate(T);M.setMonth(M.getMonth()+
1);qa(M);G.focus()}}]);H=new R(E,{y:25,width:175,height:175,visible:!D,maxSelected:1,itemSelectionId:"data"},[Ca]);new Ba(H,{spacing:2,lineInset:2,lineSpacing:3});J=new R(E,{x:D?0:195,y:D?0:25,width:(D?175:49)+p.DomElementProxy.getScrollbarSize(),height:D?200:175,overflow:"autoy",visible:!B,maxSelected:1,itemSelectionId:"text"},[Ca]);new Aa(J,{axis:"y",inset:1,spacing:3});E.setWidth(J.visible?J.x+J.width:H.width);qa(L.current??new Date)},getPickedDate:()=>M}),na=p.Dialog=new S("Dialog",Y,{extend:{BUTTON_CLASS:Ia,
...
color:"#000",locales:{en:{days:"Su Mo Tu We Th Fr Sa".split(" "),months:"Jan Feb Mar Apr May Jun Jul Aug Sep Oct Nov Dec".split(" "),sep:"-",prevMonth:"Previous month",nextMonth:"Next month",today:"Today"}},locale:"en"}},initNode:function(F,I){I.buttonClass??(I.buttonClass=na.BUTTON_CLASS);this.callSuper(F,I);F=this.content;F.setRoundedCorners(X.DialogRadius);F.setBgColor(X.DialogBgColor);F.setBoxShadow(X.DialogShadow);F.setBorder(X.DialogBorder);F.setFocusCage(!0);this.makeCloseButton(F)},setButtonClass:function(F){this.buttonClass=
F},setCallbackFunction:function(F){this.callbackFunction=F},makeButton:function(F,I){const L=this;return new L.buttonClass(F,I,[{doActivated:function(){L.doCallback(this)}}])},makeCloseButton:function(F){return F.closeBtn=this.makeButton(F,{ignoreLayout:!0,y:2,align:"right",alignOffset:2,width:19,height:16,paddingLeft:0,paddingRight:0,roundedCorners:10,activeColor:"#c00",hoverColor:"#f33",readyColor:"#f00",textColor:"#fff",text:pa(["times"]),tooltip:"Close Dialog."})},hide:function(F){this.spinner&&
(this.spinner.setVisible(!1),this.spinner=void 0);this.callSuper(F)},destroyContent:function(){this.spinner&&(this.spinner.setVisible(!1),this.spinner=void 0);const F=this.content,I=F.sizeToChildren;F.destroyAllSubviews(L=>"closeBtn"!==L.name);F.setVisible(!0);this.overlay.setBgColor(X.DimmerColor);this.setCallbackFunction();I.setPaddingY(X.ModalPanelPaddingY);I.setPaddingX(X.ModalPanelPaddingX);F.setRoundedCorners(X.DialogRadius);F.setBgColor(X.DialogBgColor);F.setBoxShadow(X.DialogShadow);F.setBorder(X.DialogBorder)},
doCallback:function(F){const I=this.callbackFunction;I&&I.call(this,F.name)||this.hide()},showBlank:function(F){this.destroyContent();this.content.setVisible(!1);F?.bgColor&&this.overlay.setBgColor(F.bgColor);this.show()},showMessage:function(F,I,L){const T=this.content,U=T.closeBtn;L={...na.WRAP_TEXT_DEFAULTS,...L};this.destroyContent();this.setCallbackFunction(I);F=T.msg=new W(T,{text:F,whiteSpace:L.whiteSpace,wordWrap:L.wordWrap,fontWeight:L.fontWeight,x:null==L.msgX?X.ModalPanelPaddingX:L.msgX,
y:null==L.msgY?X.ModalPanelPaddingY:L.msgY,width:L.width});L.titleText&&(this.setupTitle(T,L.titleText),F.setY(F.y+24));this.show();!1===L.showClose?U.destroyed||U.setVisible(!1):(U.setVisible(!0),U.focus())},showSimple:function(F,I,L,T){L=L??{};const U=this.content,Z=U.closeBtn,V=L.maxContainerHeight;this.destroyContent();L.bgColor&&U.setBgColor(L.bgColor);L.roundedCorners&&U.setRoundedCorners(L.roundedCorners);L.boxShadow&&U.setBoxShadow(L.boxShadow);L.border&&U.setBorder(L.border);U.sizeToChildren.setPaddingX(1);
this.setCallbackFunction(I);I=U.contentContainer=new R(U,{x:1,y:25,overflow:"auto"},[{setHeight:function(ba){ba>V&&(ba=V);this.callSuper(ba)}}]);F.call(this,I);new aa(I,{axis:"both"});this.show();Z.setVisible(!0);Z.focus();L.titleText&&(this.setupTitle(U,L.titleText),I.setY(this.header.height+1));I.initialFocus?.focus();T?.(this)},showConfirm:function(F,I,L){L={...na.CONFIRM_DEFAULTS,...L};this.showMessage(F,I,L);this.setupFooterButtons(this.content.msg,L)},showContentConfirm:function(F,I,L,T){L=
{...na.CONFIRM_DEFAULTS,...L};const U=this.content,Z=U.closeBtn,V=L.maxContainerHeight;this.destroyContent();U.sizeToChildren.setPaddingX(1);this.setCallbackFunction(I);I=U.contentContainer=new R(U,{x:1,y:25,overflow:"auto"},[{setHeight:function(ba){this.callSuper(ba>V?V:ba)}}]);F.call(this,I);new aa(I,{axis:"both"});this.show();!1===L.showClose?Z.destroyed||Z.setVisible(!1):(Z.setVisible(!0),Z.focus());this.setupTitle(U,L.titleText);I.setY(this.header.height+1);this.setupFooterButtons(I,L);I.initialFocus?.focus();
T?.(this)},showSpinner:function(F,I){const L=this.content;I={...na.NO_WRAP_TEXT_DEFAULTS,...I};this.destroyContent();const T=this.spinner=new p.Spinner(L,{align:"center",visible:!0,borderColor:"#ccc",size:50,y:null==I.msgY?X.ModalPanelPaddingY:I.msgY});F&&(this.msgTxt=new W(L,{text:F,whiteSpace:I.whiteSpace,wordWrap:I.wordWrap,fontWeight:I.fontWeight,x:null==I.msgX?X.ModalPanelPaddingX:I.msgX,y:T.y+T.size+X.ModalPanelPaddingY,width:I.width}));this.show();L.closeBtn.setVisible(!1);this.focus()},updateMessage:function(F){this.msgTxt?.setText(F)},
showColorPicker:function(F,I){const L=this,T=L.content,U=T.closeBtn;I={...na.COLOR_PICKER_DEFAULTS,...I};L.destroyContent();L.setCallbackFunction(Z=>{switch(Z){case "closeBtn":case "cancelBtn":F.call(L,Z);break;case "confirmBtn":const V=v.getColor();v.addToPalette(V);F.call(L,Z,V)}v.destroy()});v=new La(T,{x:X.ModalPanelPaddingX,y:X.ModalPanelPaddingY+24,width:337,height:177+(I.alphaChannel?23:0),color:I.color,alphaChannel:I.alphaChannel,palette:"#000000 #111111 #222222 #333333 #444444 #555555 #666666 #777777 #888888 #999999 #aaaaaa #bbbbbb #cccccc #dddddd #eeeeee #ffffff".split(" ")});
L.show();U.setVisible(!0);U.focus();L.setupFooterButtons(v,I);L.setupTitle(T,I.titleText)},showDatePicker:function(F,I){const L=this,T=L.content,U=T.closeBtn;I={...na.DATE_PICKER_DEFAULTS,...I};L.destroyContent();L.setCallbackFunction(Z=>{switch(Z){case "closeBtn":case "cancelBtn":F.call(L,Z);break;case "confirmBtn":F.call(L,Z,E.getPickedDate())}E.destroy()});E=new Oa(T,{x:X.ModalPanelPaddingX,y:X.ModalPanelPaddingY+24,height:195,opt:{current:new Date(I.initialDate??Date.now()),dateOnly:I.dateOnly||
!1,timeOnly:I.timeOnly||!1,locales:I.locales,locale:I.locale}});L.show();U.setVisible(!0);U.focus();L.setupFooterButtons(E,I);L.setupTitle(T,I.timeOnly?I.timeOnlyTitleText:I.titleText)},setupTitle:function(F,I){const L=X.DialogRadius;(this.header=new R(F,{ignoreLayout:!0,width:F.width,height:24,bgColor:"#eee",roundedTopLeftCorner:L,roundedTopRightCorner:L})).sendToBack();F.title=new W(F,{x:L,y:4,text:I,fontWeight:"bold"})},setupFooterButtons:function(F,I){const L=this.content;var T=X.ModalPanelPaddingY,
U=T/2;F=L.btnContainer=new R(L,{y:F.y+F.height+T,align:"center"});const Z=["active","hover","ready","text"];T=I.cancelAttrs??{};var V;(V=T).name??(V.name="cancelBtn");let ba;(ba=T).text??(ba.text=I.cancelTxt);for(let ha of Z)ha+="Color",null!=I[ha]&&(T[ha]=I[ha]);V=this.makeButton(F,T);T=I.confirmAttrs??{};var ea;(ea=T).name??(ea.name="confirmBtn");let fa;(fa=T).text??(fa.text=I.confirmTxt);for(var ma of Z)ma+="Color",ea=ma+"Confirm",null!=I[ea]&&(T[ma]=I[ea]);this.makeButton(F,T);if(ma=I.button)for(const ha of ma)this.makeButton(F,
ha);new aa(F,{axis:"y"});new Aa(F,{spacing:4,collapseParent:!0});L.sizeToChildren.setPaddingY(U);U=F.y-U;(new R(L,{ignoreLayout:!0,y:U,width:L.width,height:L.height-U,bgColor:"#eee"})).sendToBack();!1===I.showClose&&V.focus()}})})(myt);
...
setX:function(w){const q=this.x;this.callSuper(w);this.inited&&q!==this.x&&this.gridController?.notifyHdrXChange(this)},setVisible:function(w){const q=this.visible;this.callSuper(w);this.inited&&q!==this.visible&&this.gridController?.notifyHdrVisibilityChange(this)}}),P=p.GridRow=new t("GridRow",{initNode:function(w,q){q.placement??(q.placement=m);this.callSuper(w,q);this.gridController?.notifyAddRow(this)},destroy:function(w){this.setGridController();this.callSuper(w)},setGridController:function(w){const q=
...
const D=this.width;B.setWidth(D);for(const P of B.getSubviews())P.setWidth(D)}},getVisibleRows:function(){return Object.values(this._visibleRowsByIdx??{})},getVisibleRowForModel:function(B){const D=this.getVisibleRows();let P=D.length;for(;P;){const Q=D[--P];if(Q.model===B)return Q}},getListViewHeight:function(){return this._listView.height},isScrolledToEnd:function(){return this.getIDE().scrollTop+this.height===this._listView.height},getSortFunction:function(){const B=this.modelIDName,D=this.ascendingSort;
return this.numericSort?p.getNumericObjSortFunc(B,D):d(B,D,!1)},getFilterFunction:h,scrollModelIntoView:function(B,D){var P=this.getIndexOfModelInData(B);let Q=!1;if(0<=P){var w=this._rowExtent;const q=this.getIDE().scrollTop,z=q+this.height;P=this.rowInset+P*w;w=P+w;P<=q?(this.scrollYTo(P,!0),Q=!0):w>=z&&(this.scrollYTo(w-this.height,!0),Q=!0);D&&((D=this.getVisibleRowForModel(B))?D.focus():this._focusToModel=B)}return Q},isModelInData:function(B){return 0<=this.getIndexOfModelInData(B)},getNextModel:function(B,
D=!0,P=!0){const Q=this.getListData(),w=Q.length;B=this.getIndexOfModelInData(B);if(0<=B)return B+=1,B>=w?D?Q[0]:Q[w-1]:Q[B];if(P&&0<w)return Q[w-1]},getPrevModel:function(B,D=!0,P=!0){const Q=this.getListData(),w=Q.length;B=this.getIndexOfModelInData(B);if(0<=B)return--B,0>B?D?Q[w-1]:Q[0]:Q[B];if(P&&0<w)return Q[0]},getIndexOfModelInData:function(B){if(B){const D=this.getListData();let P=D.length;for(;P;)if(this.areModelsEqual(D[--P],B))return P}return-1},areModelsEqual:function(B,D){const P=this.modelIDName;
return B[P]===D[P]},getActiveRowForModel:function(B){if(B){const D=this._itemPool.getActives();let P=D.length;for(;P;){const Q=D[--P];if(this.areModelsEqual(Q.model,B))return Q}}},refreshListData:function(B,D){this._listData=this.collectionModel.getAsSortedList(this.getSortFunction(),this.getFilterFunction());this.resetListUI(B,D)},resetListUI:function(B,D){const P=this.getListData().length,Q=this._listView,w=this._scrollAnchorView;Q.setHeight(P*this._rowExtent-(0<P?this.rowSpacing:0)+this.rowInset+
this.rowOutset);w.setY(Q.height-w.height);this._startIdx=this._endIdx=-1;this.forceFullResetOnNextRefresh=D;B||0===this.getIDE().scrollTop?this.refreshListUI():this.scrollYTo(0,!0)},putRowBackInPool:function(B){if(k.focusedView?.isDescendantOf(B)){const D=this.getFocusTrap();D?D.focus():k.clear()}B.setVisible(!1);this._itemPool.putInstance(B)},refreshListUI:function(B){B=this._rowExtent;const D=this.rowInset,P=this.forceFullResetOnNextRefresh;var Q=this.getIDE().scrollTop;const w=this.getListData()??
[];var q=g(0,t.floor((Q-D)/B));Q=t.min(w.length,t.ceil((Q-D+this.height)/B));this.forceFullResetOnNextRefresh&&(this.forceFullResetOnNextRefresh=!1);if(this._startIdx!==q||this._endIdx!==Q||P){const K=this.width,N=this.rowHeight,M=this._visibleRowsByIdx,A=this._focusToModel;this._startIdx=q;this._endIdx=Q;for(var z in M)if(z<q||z>=Q)this.putRowBackInPool(M[z]),delete M[z];for(z=q;z<Q;z++){q=M[z];const C=w[z],G=this.getClassKey(C);let H=!1;q&&q.classKey===G||(q&&this.putRowBackInPool(q),M[z]=q=this._itemPool.getInstance(G),
q.setInfiniteOwner(this),q.setClassKey(G),q.setWidth(K),q.setHeight(N),q.setY(D+z*B),q.setVisible(!0),H=!0);!P&&q.model&&this.areModelsEqual(q.model,C)?H&&this.updateRow(q):(q.setModel(C),this.updateRow(q));q.notifyRefreshed();q.bringToFront();A&&this.areModelsEqual(A,C)&&(q.focus(),this._focusToModel=null)}}},getClassKey:B=>"default",updateRow:h}),l=p.SimpleSelectableInfiniteListRow=new v("SimpleSelectableInfiniteListRow",p.SimpleButton,{include:[r,p.ArrowKeyActivation],initNode:function(B,D){D.selectedColor??
...
D){const P=this.gridHeader;P?(this.forceFullResetOnNextRefresh=D,D=P.persistSortOrder?m(e(P)):void 0,P.setSort(D??B),P.setLocked(!1)):this.refreshListData(!1,D)},getSortFunction:function(){const [B,D]=this.gridHeader.sort??["",""];return B?d(B,D===p.Grid.SORT_ORDER_ASC,!1):this.callSuper()},updateRow:function(B){const D=this.gridHeader;if(D)for(const P of D.columnHeaders)B.notifyXChange(P),B.notifyWidthChange(P),B.notifyVisibilityChange(P)},notifyXChange:function(B){for(const D of this.getVisibleRows())D.notifyXChange(B)},
...
E,blue:O.blue/E}}k=new p.Path([n+k,m,n+a,m]);k.rotateAroundOrigin(d,n,m);a=k.vectors;let [B,D,P,Q]=a,w=0,q=u/360;for(x=0;b>d;){E=B;O=D;l=P;y=Q;d+=f;d>b&&(f+=b-d,q=0,d=b);k.rotateAroundOrigin(f+q,n,m);[B,D,P,Q]=a;this.beginPath();this.moveTo(E,O);this.lineTo(l,y);this.lineTo(P,Q);this.lineTo(B,D);this.closePath();const {color:z,colorDelta:K}=e[x];this.setFillStyle(r.rgbToHex(z.red+w*K.red,z.green+w*K.green,z.blue+w*K.blue,!0));this.fill();0<q&&(k.rotateAroundOrigin(-q,n,m),[B,D,P,Q]=a);w++;d>=e[x+
