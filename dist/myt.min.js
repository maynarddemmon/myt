...
A)return!1;for(const E in v)if(v[E]!==A[E])return!1;for(const E in A)if(v[E]!==A[E])return!1}return!0},getElement:(v,A)=>c.getElementsByTagName(v||"body")[0<A?A:0],loadFontFaces:(v,A)=>{Promise.all(v.map(({family:E,url:D,options:H})=>(new FontFace(E,"url("+D+")",H)).load())).then(E=>{for(const D of E)B.add(D),K(D);A?.(E)})},loadFontFace:(v,A,E={},D)=>{v=new FontFace(v,"url("+A+")",E);v.loaded.then(H=>{B.add(H);K(H);D?.(H)});v.load()},registerForFontNotification:(v,A)=>{n[A]?v&&!v.destroyed&&v.isVisible()&&
(v.sizeViewToDom(),v.notifyFontLoaded?.(A)):(t[A]??(t[A]=[])).push(v)},createBaseFontCSSRule:v=>{x.addCSSRule(x.createStylesheet(),"body, input","font-family:"+v)},loadCSSFonts:v=>{v?.forEach(x.createStylesheetLink)},createStylesheetLink:v=>{if(v){let A=f[v];A?console.warn("stylesheet already exists",v):(A=f[v]=y("link"),A.rel="stylesheet",A.href=v,p.appendChild(A));return A}},createStylesheet:()=>{const v=y("style");p.appendChild(v);return v.sheet},addCSSRule:(v,A,E,D)=>{v.insertRule(A+"{"+E+"}",
D??0)},removeCSSRules:v=>{let A=v.cssRules.length;for(;A;)v.deleteRule(--A)},createInputPlaceholderCSSRule:(v,A,E,D=1)=>{let H=v.__sheet;H?x.removeCSSRules(H):H=v.__sheet=x.createStylesheet();v=v.getODE();v=v.id||(v.id="id"+ ++u);const M=[];A&&M.push("color:"+A);E&&M.push("font-family:"+E);D&&M.push("opacity:"+D);x.addCSSRule(H,"#"+v+"::placeholder",M.join("; "))},isSorted:(v,A)=>{const E=v.length;for(let D=1;D<E;D++)if(0<A(v[D-1],v[D]))return!1;return!0},chainSortFunc:(...v)=>(A,E)=>{for(const D of v){const H=
D(A,E);if(0!==H)return H}return 0},getAlphaObjSortFunc:L((v,A,E,D)=>{const H=A?1:-1,M=x.I18N.getLocale(),N={sensitivity:E?"accent":"variant"};return(O,S)=>{O=O[v];S=S[v];D?("string"!==typeof O&&(O=""+O),"string"!==typeof S&&(S=""+S)):(O=O||"",S=S||"");return O.localeCompare(S,M,N)*H}}),getNumericObjSortFunc:L((v,A)=>{const E=A?1:-1;return(D,H)=>{D=D[v]||0;H=H[v]||0;return(D-H)*E}}),csvStringToArray:(v,A)=>{if(!v)return[];const E=[[]];let D;for(;D=P.exec(v);)D[1].length&&","!==D[1]&&E.push([]),E[E.length-
1].push(void 0!==D[2]?D[2].replace(m,"$1"):D[3]);if(A){const H=E.shift();return E.map(M=>{let N=0;return H.reduce((O,S)=>{O[S]=M[N++];return O},{})})}return E},prepareCSVDataURI:(v,A)=>{const E=M=>{if(M&&g(M)){const N=[],O=M.length;for(let S=0,V;O>S;S++)V=M[S],V=null==V?"":V.toString(),V=V.replace(/"/g,'""'),0<=V.search(/("|,|\n)/g)&&(V='"'+V+'"'),N.push(V);return N.join(",")}d("Unexpected row",M);return null},D=[];if(A){var H=E(A);H&&D.push(H)}if(v)if(g(v)){H=v.length;for(let M=0;H>M;M++){const N=
E(v[M]);N&&D.push(N)}}else d("Rows were not an array");return x.encodeCSVDataURI(D.join("\r\n"),A)},encodeCSVDataURI:(v,A)=>"data:text/csv;charset=utf-8"+(null==A?"":";header="+(A?"present":"absent"))+","+encodeURIComponent(v),dataURIToBlob:v=>{var A=v.indexOf(",");const E=v.slice(0,A).split(":")[1].split(";")[0];v=v.slice(A+1);if(E.startsWith("text/"))v=decodeURIComponent(v);else{v=atob(v);A=v.length;const D=new Uint8Array(A);for(;A;)D[--A]=v.charCodeAt(A);v=D}return new Blob([v],{type:E})},formatAsPercentage:(v,
A=2)=>{switch(typeof v){case "number":return A=a(16,e(0,A)),v=l.round(e(0,a(1,v))*b(10,2+A))/b(10,A),(0===v%1?v:v.toFixed(A))+"%";case "string":return v;default:return d("formatAsPercentage expects a number"),v}},leftPadNumber:(v,A,E="0",D=10)=>{v=v.toString(D);return E.repeat(e(A-v.length,0))+v},removeMarkup:(v,A)=>{if(!v)return"";A?.brToLineFeed&&(v=v.replace(/<br\s*\/?>/gi,"\n"));return v.replace(/<\/?[^>]+(>|$)/g,"")},escapeMarkup:(()=>{const v=RegExp("[&<>\"']","g"),A={"&":"&amp;","<":"&lt;",
">":"&gt;",'"':"&quot;","'":"&#039;"},E=D=>A[D];return D=>D.replace(v,E)})(),memoize:L,debounce:(v,A,E=!1)=>{let D;return function(...H){const M=this,N=E&&!D;clearTimeout(D);D=setTimeout(()=>{D=null;E||v.apply(M,H)},A);N&&v.apply(M,H)}},createFixedThresholdCounter:(v,A,E,D)=>{var H=x.AccessorSupport.generateName;const M="function"===typeof v||v instanceof JS.Module,N={};D=D||H("counter",E);const O=H(D,"increment");H=H(D,"decrement");if(void 0!==(M?v.instanceMethod(O):v[O]))return d("Increment: Abort clobber",
O,v),!1;if(void 0!==(M?v.instanceMethod(H):v[H]))return d("Decrement: Abort clobber",H,v),!1;N[O]=function(){const S=this[D]+1;this[D]=S;this.fireEvent(D,S);S===A&&this.set(E,!0)};N[H]=function(){const S=this[D];if(0!==S){var V=S-1;this[D]=V;this.fireEvent(D,V);S===A&&this.set(E,!1)}};v[M?"include":"extend"](N);return!0},makeURLSearchParams:(v={})=>{const A=new URLSearchParams;for(const E in v){const D=v[E];void 0!==D&&A.set(E,D)}return A},doFetch:(v,A,E,D,H,M)=>fetch(v,A).then(N=>{if(!N.ok)throw new w(N.status,
v,N.statusText);return N}).then(N=>E?N.text():N.json()).then(N=>{if(D)try{if(E)D(N);else{if(!1===N.success)throw new w(200,v,N.message??N.data?.message);D(N.data)}}catch(O){throw N=new w(200,v,O.message),N.stack=O.stack,N;}}).catch(N=>{if(H){if("FetchError"!==N.name){const O=new w(0,v,N.message);O.stack=N.stack;N=O}H(N)}}).finally(()=>{M?.()}),toNameCase:(v,A)=>{if(!v)return"";v=v.trim().toLowerCase().split(/([\s\-'\u2019"\u201c\u201d().,\/])/).reduce((E,D)=>E+(D[0]??"").toUpperCase()+D.slice(1),
"");if(/\bMac[A-Za-z]{2,}[^aciozj]\b/.test(v)||/\bMc/.test(v))v=v.replace(/\b(Ma?c)([A-Za-z]+)/,(E,D,H)=>D+(H[0]??"").toUpperCase()+H.slice(1)),v=v.replace(/\bMacEvicius\b/,"Macevicius").replace(/\bMacHado\b/,"Machado").replace(/\bMacHar\b/,"Machar").replace(/\bMacHin\b/,"Machin").replace(/\bMacHlin\b/,"Machlin").replace(/\bMacIas\b/,"Macias").replace(/\bMacIulis\b/,"Maciulis").replace(/\bMacKie\b/,"Mackie").replace(/\bMacKle\b/,"Mackle").replace(/\bMacKlin\b/,"Macklin").replace(/\bMacQuarie\b/,"Macquarie").replace(/\bMacOmber\b/,
"Macomber").replace(/\bMacIn\b/,"Macin").replace(/\bMacKintosh\b/,"Mackintosh").replace(/\bMacKen\b/,"Macken").replace(/\bMacHen\b/,"Machen").replace(/\bMacHiel\b/,"Machiel").replace(/\bMacIol\b/,"Maciol").replace(/\bMacKell\b/,"Mackell").replace(/\bMacKlem\b/,"Macklem").replace(/\bMacKrell\b/,"Mackrell").replace(/\bMacLin\b/,"Maclin").replace(/\bMacKey\b/,"Mackey").replace(/\bMacKley\b/,"Mackley").replace(/\bMacHell\b/,"Machell").replace(/\bMacHon\b/,"Machon").replace(/\bMacAyla\b/,"Macayla");v=
v.replace(/\bMacmurdo/,"MacMurdo").replace(/\bMacisaac/,"MacIsaac").replace(/\bAl(?=\s+\w)\b/g,"al").replace(/\bAp\b/g,"ap").replace(/\bBen(?=\s+\w)\b/g,"ben").replace(/\bDell([ae])\b/g,"dell$1").replace(/\bD([aeiu])\b/g,"d$1").replace(/\bDe([lr])\b/g,"de$1").replace(/\bEl\b/g,"el").replace(/\bLa\b/g,"la").replace(/\bLe(?=\s+\w)\b/g,"le").replace(/\bLo\b/g,"lo").replace(/\bVan(?=\s+\w)\b/g,"van").replace(/\bVon\b/g,"von").replace(/\bD['\u2019]/g,"d'").replace(/\b(?:\d{4}|(?:[IVX])(?:X{0,3}I{0,3}|X{0,2}VI{0,3}|X{0,2}I?[VX]))$/i,
E=>E.toUpperCase()).replace(/\b[0-9](x)\b/,E=>E.toUpperCase()).replace(/(?:^|\\s)[bcdfghjklmnpqrstvwxzBCDFGHJKLMNPQRSTVWXZ]{2}\s/,E=>E.toUpperCase()).replace(/\bMR\.?\b/,"Mr").replace(/\bMS\.?\b/,"Ms").replace(/\bDR\.?\b/,"Dr").replace(/\bST\.?\b/,"St").replace(/\bJR\.?\b/,"Jr").replace(/\bSR\.?\b/,"Sr").replace(/\bLT\.?\b/,"Lt").replace(/\bThe\b/g,"the").replace(/\bOf\b/g,"of").replace(/\bAnd\b/g,"and").replace(/\bY\s/g,"y").replace(/\s{2,}/g," ");return A?v:(v[0]??"").toUpperCase()+v.slice(1)},
I18N:{setLocale:v=>{r=v??(navigator.language||"en").split("-")[0].toLowerCase();h=z[r]},getLocale:()=>r,addDictionary:(v,A)=>{z[A]=Object.assign(z[A]??{},v);A===r&&(h=z[A])},setDictionary:(v,A)=>{z[A]=v??{};A===r&&(h=z[A])},get:(v,...A)=>{const E=h[v];return null!=E?0<A.length?E.replaceAll(C,(D,H,M,N)=>M&&N?1==A[parseInt(H)]?M:N:"").replaceAll(J,(D,H)=>A[H]):E:v}}};B.onloadingdone=v=>{for(const A of v.fontfaces)K(A)}})(global);
...
new u("SimpleGridColHdr",q.SimpleTextButton,{include:[J],initNode:function(m,x){x.activeColor??(x.activeColor="#999");x.hoverColor??(x.hoverColor="#bbb");x.readyColor??(x.readyColor="#aaa");x.inset??(x.inset=2);x.sortIconColor??(x.sortIconColor="#666");const v=x.textAlign;delete x.textAlign;x.outset??(x.outset="right"===v||(x.sortable??!0)?14:2);this.callSuper(m,x);this.sortIcon=new q.FontAwesome(this,{align:"right",alignOffset:3,valign:"middle",textColor:this.sortIconColor},[{initNode:function(A,
E){this.callSuper(A,E);this.getIDS().fontSize="0.7em"}}]);this.textView.enableEllipsis();v&&this.textView.setTextAlign(v);this.setDisabled(!this.sortable);z(this);K(this)},setSortIconColor:function(m){this.sortIconColor=m;this.sortIcon?.setTextColor(m)},setSortable:function(m){this.callSuper(m);this.inited&&(m&&this.setOutset(14),this.setDisabled(!m),K(this))},setSortState:function(m){this.callSuper(m);this.inited&&K(this)},setWidth:function(m){this.callSuper(m);this.inited&&z(this)},doActivated:function(){this.disabled||
