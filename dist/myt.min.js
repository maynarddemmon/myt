...
switch(BrowserDetect.browser){case "Chrome":case "Safari":h.addCSSRule(d,"#"+c+"::-webkit-input-placeholder",g,0);break;case "Firefox":h.addCSSRule(d,"#"+c+":-moz-placeholder","opacity:1; "+g,0);h.addCSSRule(d,"#"+c+"::-moz-placeholder","opacity:1; "+g,0);break;case "Explorer":h.addCSSRule(d,"#"+c+":-ms-input-placeholder",g,0)}},memoize:a=>function(){const b=JSON.stringify(arguments),f=a.__cache||(a.__cache={});return b in f?f[b]:f[b]=a.apply(this,arguments)},debounce:function(a,b,f){return function(){const c=
this,d=c.__DBTO,g=arguments,e=f&&!d;clearTimeout(d);c.__DBTO=setTimeout(function(){c.__DBTO=null;f||a.apply(c,g)},b);e&&a.apply(c,g)}},createFixedThresholdCounter:(a,b,f,c)=>{var d=h.AccessorSupport.generateName;const g="function"===typeof a||a instanceof JS.Module,e={};let m;c=c||d("counter",f);m=d(c,"increment");d=d(c,"decrement");if(void 0!==(g?a.instanceMethod(m):a[m]))return console.warn("Can't clobber existing property during setup of increment function.",m,a),!1;if(void 0!==(g?a.instanceMethod(d):
a[d]))return console.warn("Can't clobber existing property during setup of decrement function.",d,a),!1;e[m]=function(){const a=this[c]+1;this[c]=a;this.fireEvent(c,a);a===b&&this.set(f,!0)};e[d]=function(){const a=this[c];if(0!==a){var d=a-1;this[c]=d;this.fireEvent(c,d);a===b&&this.set(f,!1)}};a[g?"include":"extend"](e);return!0},makeURLSearchParams:(a={})=>{const b=new URLSearchParams;let f,c;for(f in a)c=a[f],void 0!==c&&b.set(f,c);return b},doFetch:(a,f,c,d,g,e)=>fetch(a,f).then(f=>{if(!f.ok)throw new b(f.status,
a,f.statusText);return f}).then(a=>c?a.text():a.json()).then(f=>{if(d)try{if(c)d(f);else{if(!1===f.success)throw new b(200,a,f.message);d(f.data)}}catch(v){throw f=new b(200,a,v.message),f.stack=v.stack,f;}}).catch(f=>{if(g){if("FetchError"!==f.name){const c=new b(0,a,f.message);c.stack=f.stack;f=c}g(f)}}).finally(a=>{e&&e()})};f.onloadingdone=a=>{a.fontfaces.forEach(a=>{g(a)})}})(global);
...
0)+this.rowInset+this.rowOutset);e.setY(f.height-e.height);this._startIdx=this._endIdx=-1;this.__forceFullResetOnNextRefresh=b;a||0===this.getInnerDomElement().scrollTop?this.refreshListUI():this.scrollYTo(0,!0)},putRowBackInPool:function(a){a.setVisible(!1);this._itemPool.putInstance(a)},refreshListUI:function(a){a=this._rowExtent;const b=this.rowInset,c=this.__forceFullResetOnNextRefresh;var f=this.getInnerDomElement().scrollTop;const g=this.getListData()||[];var h=Math.max(0,Math.floor((f-b)/a));
f=Math.min(g.length,Math.ceil((f-b+this.height)/a));this.__forceFullResetOnNextRefresh&&(this.__forceFullResetOnNextRefresh=!1);if(this._startIdx!==h||this._endIdx!==f||c){const d=this.width,e=this.rowHeight,k=this._visibleRowsByIdx;this._startIdx=h;this._endIdx=f;let n;for(n in k)if(n<h||n>=f)this.putRowBackInPool(k[n]),delete k[n];for(n=h;n<f;n++){h=k[n];const f=g[n],m=this.getClassKey(f);h&&h.classKey===m||(h&&this.putRowBackInPool(h),k[n]=h=this._itemPool.getInstance(m),h.setInfiniteOwner(this),
h.setClassKey(m),h.setWidth(d),h.setHeight(e),h.setY(b+n*a),h.setVisible(!0));h.model&&this.areModelsEqual(h.model,f)&&!c||(h.setModel(f),this.updateRow(h));h.notifyRefreshed();h.bringToFront()}}},getClassKey:a=>"default",updateRow:a=>{}})})(myt);
