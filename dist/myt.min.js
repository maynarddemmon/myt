...
for(const B in q){const D=q[B];void 0!==D&&y.set(B,D)}return y},doFetch:(q,y,B,D,F,K)=>fetch(q,y).then(L=>{if(!L.ok)throw new x(L.status,q,L.statusText);return L}).then(L=>B?L.text():L.json()).then(L=>{if(D)try{if(B)D(L);else{if(!1===L.success)throw new x(200,q,L.message??L.data?.message);D(L.data)}}catch(P){throw L=new x(200,q,P.message),L.stack=P.stack,L;}}).catch(L=>{if(F){if("FetchError"!==L.name){const P=new x(0,q,L.message);P.stack=L.stack;L=P}F(L)}}).finally(()=>{K?.()}),I18N:{setLocale:q=>
{u=q},getLocale:()=>u,addDictionary:(q,y)=>{H[y]=Object.assign(H[y]??{},q)},setDictionary:(q,y)=>{H[y]=q??{}},get:(q,...y)=>{var B=u??=(navigator.language??"en").split("-")[0].toLowerCase();B=(H[B]??H.en??{})[q];return null!=B?0<y.length?B.replaceAll(C,(D,F,K,L)=>K&&L?1==y[parseInt(F)]?K:L:"").replaceAll(J,(D,F)=>y[F]):B:q}}};m.onloadingdone=q=>{for(const y of q.fontfaces)A(y)}})(global);
