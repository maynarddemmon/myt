...
else if(Array.isArray(K))y&&K.push(y);else{K=F[q]=[];y&&K.push(y);const L=p("script");L.type="text/javascript";L.async=!1;E&&(L.integrity=E,L.crossOrigin="anonymous");const P=N=>{L.onload=L.onreadystatechange=L.onerror=null;for(const T of K)T(N);F[q]=N};L.onerror=()=>{P(!1)};L.onload=L.onreadystatechange=()=>{L.readyState&&"complete"!==L.readyState||P(!0)};L.src=q+(B?"":(q.includes("?")?"&":"?")+"cacheBust="+Date.now());c.appendChild(L)}},dumpStack:(q,y)=>{let B;"string"===typeof q&&(B=q,q=null);
...
y(q[E-1],q[E]))return!0},chainSortFunc:(...q)=>(y,B)=>{for(const E of q){const F=E(y,B);if(0!==F)return F}return 0},getAlphaObjSortFunc:z((q,y,B,E)=>{const F=y?1:-1,K=l.I18N.getLocale(),L={sensitivity:B?"accent":"variant"};return(P,N)=>{P=P[q];N=N[q];E?("string"!==typeof P&&(P=""+P),"string"!==typeof N&&(N=""+N)):(P=P||"",N=N||"");return P.localeCompare(N,K,L)*F}}),getNumericObjSortFunc:z((q,y)=>{const B=y?1:-1;return(E,F)=>{E=E[q]||0;F=F[q]||0;return(E-F)*B}}),csvStringToArray:(q,y)=>{if(!q)return[];
const B=[[]];let E;for(;E=M.exec(q);)E[1].length&&","!==E[1]&&B.push([]),B[B.length-1].push(E[2]?E[2].replace(O,"$1"):E[3]);if(y){const F=B.shift();return B.map(K=>{let L=0;return F.reduce((P,N)=>{P[N]=K[L++];return P},{})})}return B},prepareCSVDataURI:(q,y)=>{const B=K=>{if(K&&a(K)){const L=[],P=K.length;for(let N=0,T;P>N;N++)T=K[N],T=null==T?"":T.toString(),T=T.replace(/"/g,'""'),0<=T.search(/("|,|\n)/g)&&(T='"'+T+'"'),L.push(T);return L.join(",")}e("Unexpected row",K);return null},E=[];if(y){var F=
B(y);F&&E.push(F)}if(q)if(a(q)){F=q.length;for(let K=0;F>K;K++){const L=B(q[K]);L&&E.push(L)}}else e("Rows were not an array");return l.encodeCSVDataURI(E.join("\r\n"),y)},encodeCSVDataURI:(q,y)=>"data:text/csv;charset=utf-8"+(null==y?"":";header="+(y?"present":"absent"))+","+encodeURIComponent(q),dataURIToBlob:q=>{var y=q.indexOf(",");const B=q.slice(0,y).split(":")[1].split(";")[0];q=q.slice(y+1);if(B.startsWith("text/"))q=decodeURIComponent(q);else{q=atob(q);y=q.length;const E=new Uint8Array(y);
for(;y;)E[--y]=q.charCodeAt(y);q=E}return new Blob([q],{type:B})},formatAsPercentage:(q,y=2)=>{switch(typeof q){case "number":return y=n(16,b(0,y)),q=g.round(b(0,n(1,q))*d(10,2+y))/d(10,y),(0===q%1?q:q.toFixed(y))+"%";case "string":return q;default:return e("formatAsPercentage: expects a number"),q}},leftPadNumber:(q,y,B="0",E=10)=>{q=q.toString(E);return B.repeat(b(y-q.length,0))+q},removeMarkup:(q,y)=>{if(!q)return"";y?.brToLineFeed&&(q=q.replace(/<br\s*\/?>/gi,"\n"));return q.replace(/<\/?[^>]+(>|$)/g,
"")},memoize:z,debounce:(q,y,B)=>{const E="__DBTO_"+ ++v;return function(){const F=this,K=F[E],L=arguments,P=B&&!K;clearTimeout(K);F[E]=setTimeout(function(){F[E]=null;B||q.apply(F,L)},y);P&&q.apply(F,L)}},createFixedThresholdCounter:(q,y,B,E)=>{var F=l.AccessorSupport.generateName;const K="function"===typeof q||q instanceof JS.Module,L={};E=E||F("counter",B);const P=F(E,"increment");F=F(E,"decrement");if(void 0!==(K?q.instanceMethod(P):q[P]))return e("Increment: Abort clobber",P,q),!1;if(void 0!==
(K?q.instanceMethod(F):q[F]))return e("Decrement: Abort clobber",F,q),!1;L[P]=function(){const N=this[E]+1;this[E]=N;this.fireEvent(E,N);N===y&&this.set(B,!0)};L[F]=function(){const N=this[E];if(0!==N){var T=N-1;this[E]=T;this.fireEvent(E,T);N===y&&this.set(B,!1)}};q[K?"include":"extend"](L);return!0},makeURLSearchParams:(q={})=>{const y=new URLSearchParams;for(const B in q){const E=q[B];void 0!==E&&y.set(B,E)}return y},doFetch:(q,y,B,E,F,K)=>fetch(q,y).then(L=>{if(!L.ok)throw new x(L.status,q,L.statusText);
return L}).then(L=>B?L.text():L.json()).then(L=>{if(E)try{if(B)E(L);else{if(!1===L.success)throw new x(200,q,L.message);E(L.data)}}catch(P){throw L=new x(200,q,P.message),L.stack=P.stack,L;}}).catch(L=>{if(F){if("FetchError"!==L.name){const P=new x(0,q,L.message);P.stack=L.stack;L=P}F(L)}}).finally(()=>{K?.()}),I18N:{setLocale:q=>{u=q},getLocale:()=>u,addDictionary:(q,y)=>{H[y]=Object.assign(H[y]??{},q)},setDictionary:(q,y)=>{H[y]=q??{}},get:(q,...y)=>{var B=u??=(navigator.language??"en").split("-")[0].toLowerCase();
B=(H[B]??H.en??{})[q];return null!=B?0<y.length?B.replaceAll(C,(E,F,K,L)=>K&&L?1==y[parseInt(F)]?K:L:"").replaceAll(J,(E,F)=>y[F]):B:q}}};m.onloadingdone=q=>{for(const y of q.fontfaces)A(y)}})(global);
...
b.columnGap;const O=z?c:f,l=b.isWrapNotAllowed();f=b.justifyContent;c=b.alignItems;var w=l?"stretch":b.alignContent;a.endsWith("Reverse")&&d.reverse();let q=0,y=0,B=0,E=0,F={items:[],shrinkCount:0,shrinkBasis:0,growCount:0,extraSize:0,crossPos:B,crossSize:0};a=[F];for(let K=0;K<H;){const L=d[K++],P=z?L.getFlexBasisWidth():L.getFlexBasisHeight(),N=L.getShrink(),T=L.getGrow();!l&&0<F.items.length&&q+(0===E?0:J)+P>M&&(F.extraSize=M-q,B+=F.crossSize+p,y=v(y,q),E=q=0,F={items:[],shrinkCount:0,shrinkBasis:0,
growCount:0,extraSize:0,crossPos:B,crossSize:0},a.push(F));0!==E&&(q+=J);0<N&&0<P&&(F.shrinkCount+=N,F.shrinkBasis+=P*N);0<T&&(F.growCount+=T);F.crossSize=v(F.crossSize,C?L.getFlexBasisWidth():L.getFlexBasisHeight());g(L,z,q);e(L,z,P);q+=P;F.items.push(L);E++}F.extraSize=M-q;B+=F.crossSize;y=v(y,q);if(b.isCompactOnMainAxis(z)&&(d=a.reduce((K,L)=>null==K?L.extraSize:Math.min(K,L.extraSize),null),0!==d))for(var r of a)r.extraSize-=d;b[z?"setTotalBasisWidth":"setTotalBasisHeight"](y);b[z?"setTotalBasisHeight":
...
__isColorAnim:T,__loopCnt:ea,to:U}=m;var B=x(0,A/J),E=x(0,O/J);null==m.from&&(m.__tmpFrom=!0,m.from=P?T?"#000":0:y.get(N));E=l(B)-(P?l(E):0);var F=m.from,K=P?y.get(N):F;B=y.set;if(T){F=d(F);const fa=d(U);K=P?d(K):F;K.setRed(K.red+(fa.red-F.red)*E);K.setGreen(K.green+(fa.green-F.green)*E);K.setBlue(K.blue+(fa.blue-F.blue)*E);E=K.getHtmlHexString()}else E=K+(U-F)*E;B.call(y,N,E);!C&&ea===M||C&&0>ea&&0<M?(m.setRunning(!1),q?.call(m,!0)):0<L?(m.fireEvent("repeat",ea),m.__prog=C?J:0,w(m,L)):m.__prog=A}else console.log("No target for animator",
...
X===la&&(N.select(Y),Y.focus()),V&&X===W&&Y.setIsToday(!0),X=new Date(S.getTime()),X.setHours(23),X.setMinutes(59),X.setSeconds(59),(null!=l&&l>X.getTime()||null!=q&&q<S.getTime())&&Y.setDisabled(!0));Y.updateUI()}}if(!C){G=60*B[0]+B[1];W=T;W.deselectAll();W.destroyAllSubviews();let [ka,ha]=y;for(S=new Date(da);60*ka+ha<G;)da=new Oa(T,{text:("0"+ka).slice(-2)+":"+("0"+ha).slice(-2)}),ka===R&&ha===D&&T.select(da),S.setHours(ka),S.setMinutes(ha),W=S.getTime(),(null!=l&&l>W||null!=q&&q<W)&&da.setDisabled(!0),
ha+=E,60<=ha&&(ha-=60,ka++);T.scrollYTo(Q)}},Qa=t.DatePicker=new ea("DatePicker",U,{initNode:function(D,G){H=this;const I={current:null,dateOnly:!1,timeOnly:!1,locales:null,locale:"",minuteInterval:30,firstDayOfWeek:0,showTodayButton:!0,minDate:null,maxDate:null,minTime:"00:00",maxTime:"23:59",allowedDays:null,...G.opt};delete G.opt;H.callSuper(D,G);z=I.locales[I.locale??"en"];C=I.dateOnly;J=I.timeOnly;M=I.firstDayOfWeek;E=ja(5,ia(30,I.minuteInterval));O=I.allowedDays;Array.isArray(O)&&0!==O.length||
(O=[0,1,2,3,4,5,6]);l=Date.parse(I.minDate);l=isNaN(l)?null:l;q=Date.parse(I.maxDate);q=isNaN(q)?null:q;y=ya(I.minTime);B=ya(I.maxTime);D=new U(H,{visible:!J});I.showTodayButton&&new na(D,{width:24,text:oa(["home"]),tooltip:z.today},[{doActivated:()=>{pa(new Date)}}]);K=new na(D,{width:24,x:28,text:oa(["chevron-left"]),tooltip:z.prevMonth},[{doActivated:()=>{const Q=(new Date(F.getFullYear(),F.getMonth(),0)).getDate();Q<F.getDate()&&F.setDate(Q);F.setMonth(F.getMonth()-1);pa(F);K.focus()}}]);L=new fa(D,
{width:100,x:52,textAlign:"center"});P=new na(D,{width:24,x:152,text:oa(["chevron-right"]),tooltip:z.nextMonth},[{doActivated:()=>{var Q=(new Date(F.getFullYear(),F.getMonth()+1,0)).getDate();Q<F.getDate()&&F.setDate(Q);Q=(new Date(F.getFullYear(),F.getMonth()+2,0)).getDate();Q<F.getDate()&&F.setDate(Q);F.setMonth(F.getMonth()+1);pa(F);P.focus()}}]);N=new U(H,{y:25,width:175,height:175,visible:!J,maxSelected:1,itemSelectionId:"data"},[Da]);new Ca(N,{spacing:2,lineInset:2,lineSpacing:3});T=new U(H,
...
D?.bgColor&&this.overlay.setBgColor(D.bgColor);this.show()},showMessage:function(D,G,I){const Q=this.content,R=Q.closeBtn;I={...Z.WRAP_TEXT_DEFAULTS,...I};this.destroyContent();this.setCallbackFunction(G);D=Q.msg=new fa(Q,{text:D,whiteSpace:I.whiteSpace,wordWrap:I.wordWrap,fontWeight:I.fontWeight,x:null==I.msgX?ba.PADDING_X:I.msgX,y:null==I.msgY?ba.PADDING_Y:I.msgY,width:I.width});I.titleText&&(this.setupTitle(Q,I.titleText),D.setY(D.y+24));this.show();!1===I.showClose?R.destroyed||R.setVisible(!1):
(R.setVisible(!0),R.focus())},showSimple:function(D,G,I,Q){I=I??{};const R=this.content,V=R.closeBtn,S=I.maxContainerHeight;this.destroyContent();I.bgColor&&R.setBgColor(I.bgColor);I.roundedCorners&&R.setRoundedCorners(I.roundedCorners);I.boxShadow&&R.setBoxShadow(I.boxShadow);I.border&&R.setBorder(I.border);R.sizeToChildren.setPaddingX(1);this.setCallbackFunction(G);G=R.contentContainer=new U(R,{x:1,y:25,overflow:"auto"},[{setHeight:function(W){W>S&&(W=S);this.callSuper(W)}}]);D.call(this,G);new va(G,
{axis:"both"});this.show();V.setVisible(!0);V.focus();I.titleText&&(this.setupTitle(R,I.titleText),G.setY(this.header.height+1));G.initialFocus?.focus();Q?.(this)},showConfirm:function(D,G,I){I={...Z.CONFIRM_DEFAULTS,...I};this.showMessage(D,G,I);this.setupFooterButtons(this.content.msg,I)},showContentConfirm:function(D,G,I,Q){I={...Z.CONFIRM_DEFAULTS,...I};const R=this.content,V=R.closeBtn,S=I.maxContainerHeight;this.destroyContent();R.sizeToChildren.setPaddingX(1);this.setCallbackFunction(G);G=
R.contentContainer=new U(R,{x:1,y:25,overflow:"auto"},[{setHeight:function(W){this.callSuper(W>S?S:W)}}]);D.call(this,G);new va(G,{axis:"both"});this.show();!1===I.showClose?V.destroyed||V.setVisible(!1):(V.setVisible(!0),V.focus());this.setupTitle(R,I.titleText);G.setY(this.header.height+1);this.setupFooterButtons(G,I);G.initialFocus?.focus();Q?.(this)},showSpinner:function(D,G){const I=this.content;G={...Z.NO_WRAP_TEXT_DEFAULTS,...G};this.destroyContent();const Q=this.spinner=new t.Spinner(I,{align:"center",
visible:!0,borderColor:"#ccc",size:50,y:null==G.msgY?ba.PADDING_Y:G.msgY});D&&new fa(I,{text:D,whiteSpace:G.whiteSpace,wordWrap:G.wordWrap,fontWeight:G.fontWeight,x:null==G.msgX?ba.PADDING_X:G.msgX,y:Q.y+Q.size+ba.PADDING_Y,width:G.width});this.show();I.closeBtn.setVisible(!1);this.focus()},showColorPicker:function(D,G){const I=this,Q=I.content,R=Q.closeBtn;G={...Z.COLOR_PICKER_DEFAULTS,...G};I.destroyContent();I.setCallbackFunction(V=>{switch(V){case "closeBtn":case "cancelBtn":D.call(I,V);break;
case "confirmBtn":const S=x.getColor();x.addToPalette(S);D.call(I,V,S)}x.destroy()});x=new Na(Q,{x:ba.PADDING_X,y:ba.PADDING_Y+24,width:337,height:177+(G.alphaChannel?23:0),color:G.color,alphaChannel:G.alphaChannel,palette:"#000000 #111111 #222222 #333333 #444444 #555555 #666666 #777777 #888888 #999999 #aaaaaa #bbbbbb #cccccc #dddddd #eeeeee #ffffff".split(" ")});I.show();R.setVisible(!0);R.focus();I.setupFooterButtons(x,G);I.setupTitle(Q,G.titleText)},showDatePicker:function(D,G){const I=this,Q=
I.content,R=Q.closeBtn;G={...Z.DATE_PICKER_DEFAULTS,...G};I.destroyContent();I.setCallbackFunction(V=>{switch(V){case "closeBtn":case "cancelBtn":D.call(I,V);break;case "confirmBtn":D.call(I,V,H.getPickedDate())}H.destroy()});H=new Qa(Q,{x:ba.PADDING_X,y:ba.PADDING_Y+24,height:195,opt:{current:new Date(G.initialDate??Date.now()),dateOnly:G.dateOnly||!1,timeOnly:G.timeOnly||!1,locales:G.locales,locale:G.locale}});I.show();R.setVisible(!0);R.focus();I.setupFooterButtons(H,G);I.setupTitle(Q,G.timeOnly?
G.timeOnlyTitleText:G.titleText)},setupTitle:function(D,G){const I=Z.RADIUS;(this.header=new U(D,{ignoreLayout:!0,width:D.width,height:24,bgColor:"#eee",roundedTopLeftCorner:I,roundedTopRightCorner:I})).sendToBack();D.title=new fa(D,{x:I,y:4,text:G,fontWeight:"bold"})},setupFooterButtons:function(D,G){const I=this.content;var Q=ba.PADDING_Y,R=Q/2;D=I.btnContainer=new U(I,{y:D.y+D.height+Q,align:"center"});const V=["active","hover","ready","text"];Q=G.cancelAttrs??{};var S;(S=Q).name??(S.name="cancelBtn");
let W;(W=Q).text??(W.text=G.cancelTxt);for(let da of V)da+="Color",null!=G[da]&&(Q[da]=G[da]);S=this.makeButton(D,Q);Q=G.confirmAttrs??{};var aa;(aa=Q).name??(aa.name="confirmBtn");let ca;(ca=Q).text??(ca.text=G.confirmTxt);for(var la of V)la+="Color",aa=la+"Confirm",null!=G[aa]&&(Q[la]=G[aa]);this.makeButton(D,Q);if(la=G.button)for(const da of la)this.makeButton(D,da);new va(D,{axis:"y"});new Ba(D,{spacing:4,collapseParent:!0});I.sizeToChildren.setPaddingY(R);R=D.y-R;(new U(I,{ignoreLayout:!0,y:R,
...
(t=>{let x=0;const {Class:v,Module:u}=JS,{min:h,max:e}=Math,{View:g,SpacedLayout:k,Node:{DEFAULT_PLACEMENT:n}}=t,b=l=>{const q=l.columnHeaders,[y,B]=l.sort??["",""];let E=q.length;for(;E;){const F=q[--E];F.columnId===y?F.sortable&&F.setSortState(B):F.setSortState("none")}l.locked||l.doSort()},d=(l,q,y,B)=>{if(0!==q){const L=0<q,P=[];for(var E=l.length;E;){var F=l[--E];F.resizable&&(y?0<F.flex:0===F.flex)&&P.push({hdr:F,limit:(L?F.maxValue:F.minValue)-F.value,amt:0})}l=P.length;if(0<l){for(E=0;0!==
q;){x>=l&&(E=x=0);F=P[x];var K=F.hdr;F.full?++E:(L?(K=h(y?K.flex:1,q),F.amt+K>F.limit&&(K=F.limit-F.amt,F.full=!0)):(K=e(y?-K.flex:-1,q),F.amt+K<F.limit&&(K=F.limit-F.amt,F.full=!0)),F.amt+=K,q-=K);if(E===l)break;++x}for(const N of P)N.hdr.setValue(N.hdr.value+N.amt);B?.(q)}}},a=l=>(l=l.gridController?.getPrevHdr(l))?l.maxValue-l.value+a(l):0,f=l=>(l=l.gridController?.getNextHdr(l))?l.maxValue-l.value+f(l):0,c=l=>(l=l.gridController?.getPrevHdr(l))?l.minValue-l.value+c(l):0,p=l=>(l=l.gridController?.getNextHdr(l))?
...
new v("SimpleGridColHdr",t.SimpleTextButton,{include:[J],initNode:function(l,q){q.activeColor??(q.activeColor="#999");q.hoverColor??(q.hoverColor="#bbb");q.readyColor??(q.readyColor="#aaa");q.inset??(q.inset=2);q.sortIconColor??(q.sortIconColor="#666");q.outset??(q.outset=q.sortable??!0?14:2);const y=q.textAlign;delete q.textAlign;this.callSuper(l,q);this.sortIcon=new t.FontAwesome(this,{align:"right",alignOffset:3,valign:"middle",textColor:this.sortIconColor},[{initNode:function(B,E){this.callSuper(B,
E);this.getIDS().fontSize="0.7em"}}]);this.textView.enableEllipsis();y&&this.textView.setTextAlign(y);this.setDisabled(!this.sortable);z(this);H(this)},setSortIconColor:function(l){this.sortIconColor=l;this.sortIcon?.setTextColor(l)},setSortable:function(l){this.callSuper(l);this.inited&&(l&&this.setOutset(14),this.setDisabled(!l),H(this))},setSortState:function(l){this.callSuper(l);this.inited&&H(this)},setWidth:function(l){this.callSuper(l);this.inited&&z(this)},doActivated:function(){this.disabled||
