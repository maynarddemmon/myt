...
getPagePosition:(d,c)=>{if(!d)return null;const f="Firefox"===BrowserDetect.browser?2:1;let b=0,a=0;for(;d&&"BODY"!==d.nodeName&&d!==c;)if(b+=d.offsetLeft,a+=d.offsetTop,(d=d.offsetParent)&&"BODY"!==d.nodeName){const c=h.getComputedStyle(d);b+=f*parseInt(c.borderLeftWidth,10)-d.scrollLeft;a+=f*parseInt(c.borderTopWidth,10)-d.scrollTop}return{x:b,y:a}},getTruePagePosition:d=>{if(!d)return null;d=d.getBoundingClientRect();return{x:d.left+h.scrollX,y:d.top+h.scrollY}},simulateDomEvent:(d,c,f)=>{if(d){const b=
...
for(let a=0;l>a;)e.appendChild(c[a++].getOuterDomElement());b.getInnerDomElement().appendChild(e);d&&d.insertBefore(a,f)})},containsPoint:function(a,b,c){const d=this.getOuterDomElement();if(!d)return!1;c=h.getPagePosition(d,c);return k(a,b,c.x,c.y,this.width,this.height)},isPointVisible:function(a,b){const d=this.getTruePagePosition();c(this);return f(this,a-d.x,b-d.y)},getEffectiveScale:function(){return c(this)},getEffectiveScaleX:function(){return c(this).scaleX},getEffectiveScaleY:function(){return c(this).scaleY}})})(myt);
