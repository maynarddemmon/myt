...
for(;y;)E[--y]=q.charCodeAt(y);q=E}return new Blob([q],{type:B})},formatAsPercentage:(q,y=2)=>{switch(typeof q){case "number":return y=n(16,b(0,y)),q=g.round(b(0,n(1,q))*d(10,2+y))/d(10,y),(0===q%1?q:q.toFixed(y))+"%";case "string":return q;default:return e("formatAsPercentage: expects a number"),q}},leftPadNumber:(q,y,B="0",E=10)=>{q=q.toString(E);return B.repeat(b(y-q.length,0))+q},memoize:z,debounce:(q,y,B)=>{const E="__DBTO_"+ ++v;return function(){const F=this,K=F[E],L=arguments,P=B&&!K;clearTimeout(K);
F[E]=setTimeout(function(){F[E]=null;B||q.apply(F,L)},y);P&&q.apply(F,L)}},createFixedThresholdCounter:(q,y,B,E)=>{var F=l.AccessorSupport.generateName;const K="function"===typeof q||q instanceof JS.Module,L={};E=E||F("counter",B);const P=F(E,"increment");F=F(E,"decrement");if(void 0!==(K?q.instanceMethod(P):q[P]))return e("Increment: Abort clobber",P,q),!1;if(void 0!==(K?q.instanceMethod(F):q[F]))return e("Decrement: Abort clobber",F,q),!1;L[P]=function(){const N=this[E]+1;this[E]=N;this.fireEvent(E,
N);N===y&&this.set(B,!0)};L[F]=function(){const N=this[E];if(0!==N){var T=N-1;this[E]=T;this.fireEvent(E,T);N===y&&this.set(B,!1)}};q[K?"include":"extend"](L);return!0},makeURLSearchParams:(q={})=>{const y=new URLSearchParams;for(const B in q){const E=q[B];void 0!==E&&y.set(B,E)}return y},doFetch:(q,y,B,E,F,K)=>fetch(q,y).then(L=>{if(!L.ok)throw new x(L.status,q,L.statusText);return L}).then(L=>B?L.text():L.json()).then(L=>{if(E)try{if(B)E(L);else{if(!1===L.success)throw new x(200,q,L.message);E(L.data)}}catch(P){throw L=
new x(200,q,P.message),L.stack=P.stack,L;}}).catch(L=>{if(F){if("FetchError"!==L.name){const P=new x(0,q,L.message);P.stack=L.stack;L=P}F(L)}}).finally(()=>{K?.()}),I18N:{setLocale:q=>{u=q},getLocale:()=>u,addDictionary:(q,y)=>{H[y]=Object.assign(H[y]??{},q)},setDictionary:(q,y)=>{H[y]=q??{}},get:(q,...y)=>{var B=u??=(navigator.language??"en").split("-")[0].toLowerCase();B=(H[B]??H.en??{})[q];return null!=B?0<y.length?B.replaceAll(C,(E,F,K,L)=>K&&L?1==y[parseInt(F)]?K:L:"").replaceAll(J,(E,F)=>y[F]):
B:q}}};m.onloadingdone=q=>{for(const y of q.fontfaces)A(y)}})(global);
