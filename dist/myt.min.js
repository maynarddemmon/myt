...
this.set("selectedRow",C,!0),C&&(this.setSelectedRowModel(C.model),C.setSelected(!0)))},setSelectedRowModel:function(C){this.set("selectedRowModel",C,!0);this.selectedRowModel&&this.scrollModelIntoView(this.selectedRowModel,!1)},getActiveSelectedRow:function(){return this.getActiveRowForModel(this.selectedRowModel)},selectRowForModel:function(C,F,x){C&&(f(this),this.setSelectedRowModel(C),this.refreshListUI(),F&&this.getActiveSelectedRow()?.focus())},selectNextRowForModel:function(C,F=!0){this.selectRowForModel(this.getNextModel(C),
F,!0)},selectPrevRowForModel:function(C,F=!0){this.selectRowForModel(this.getPrevModel(C),F,!1)},resetListUI:function(C,F){this.isModelInData(this.selectedRowModel)?f(this):this.setSelectedRow();this.callSuper(C,F)},putRowBackInPool:function(C){C.selected&&f(this);this.callSuper(C)},refreshListUI:function(C){C=c.focusedView;this.callSuper();const F=this.getActiveSelectedRow();F&&(this.set("selectedRow",F,!0),F.setSelected(!0),C&&C!==F||F.focus(!0))}}),J=new u("InfiniteGridRowMixin",{notifyXChange:function(C){this.getRef(C.columnId)?.setX(C.x+
C.cellXAdj)},notifyWidthChange:function(C){this.getRef(C.columnId)?.setWidth(C.width+C.cellWidthAdj)},notifyVisibilityChange:function(C){this.getRef(C.columnId)?.setVisible(C.visible)}}),P=new u("InfiniteGridMixin",{setGridHeader:function(C){(this.gridHeader=C)?.setGrid(this)},makeReady:function(C,F){const x=this.gridHeader;x?(this.forceFullResetOnNextRefresh=F,F=x.persistSortOrder?b(q(x)):void 0,x.setSort(F??C),x.setLocked(!1)):this.refreshListData(!1,F)},getSortFunction:function(){const [C,F]=this.gridHeader.sort??
["",""];return C?e(C,F===n.Grid.SORT_ORDER_ASC,!1):this.callSuper()},updateRow:function(C){const F=this.gridHeader;if(F)for(const x of F.columnHeaders)C.notifyXChange(x),C.notifyWidthChange(x),C.notifyVisibilityChange(x)},notifyXChange:function(C){for(const F of this._visibleRowsByIdx.values())F.notifyXChange(C)},notifyWidthChange:function(C){for(const F of this._visibleRowsByIdx.values())F.notifyWidthChange(C)},notifyVisibilityChange:function(C){for(const F of this._visibleRowsByIdx.values())F.notifyVisibilityChange(C)}});
n.InfiniteGridRow=new u("InfiniteGridRow",{include:[y,J]});n.SelectableInfiniteGridRow=new u("SelectableInfiniteGridRow",{include:[r,J]});n.SimpleSelectableInfiniteGridRow=new v("SimpleSelectableInfiniteGridRow",w,{include:[J]});n.InfiniteGrid=new v("InfiniteGrid",D,{include:[P]});n.SelectableInfiniteGrid=new v("InfiniteGrid",E,{include:[P]});n.InfiniteGridHeader=new v("InfiniteGridHeader",m,{include:[n.GridController],initNode:function(C,F){F.columnSpacing??(F.columnSpacing=1);F.persistSortOrder??
(F.persistSortOrder=!0);F.overflow="hidden";this.callSuper(C,F);new n.SpacedLayout(this,{spacing:this.columnSpacing})},setGrid:function(C){this.grid=C},setColumnSpacing:function(C){this.columnSpacing=C},getColumnSpacingInUse:function(){return 0===this.columnSpacing?0:g(0,this.getVisibleHdrs().length-1)*this.columnSpacing},setPersistSortOrder:function(C){this.persistSortOrder=C},setHeight:function(C){this.callSuper(C);if(this.inited){C=this.height;for(const F of this.columnHeaders)F.setHeight(C)}},
setWidth:function(C){this.callSuper(g(this.minWidth,C));this.inited&&(C=this.width,this.setGridWidth(C),this.grid.setWidth(C))},subviewAdded:function(C){this.callSuper(C);C.isA(n.GridColHdr)&&(C.setGridController(this),C.setHeight(this.height))},doSort:function(){this.persistSort();this.grid.refreshListData(!0,this.grid.forceFullResetOnNextRefresh)},persistSort:function(){this.persistSortOrder&&h(q(this),this.sort)},notifyHdrXChange:function(C){this.isLocked()||(this.grid.notifyXChange(C),this._notifyHdrChange("X",
