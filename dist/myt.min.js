...
{const u=f("link");u.rel="stylesheet";A&&(u.href=A);k.appendChild(u);return u},createStylesheet:()=>{const A=f("style");k.appendChild(A);return A.sheet},addCSSRule:(A,u,p,y)=>{"insertRule"in A?A.insertRule(u+"{"+p+"}",y):"addRule"in A&&A.addRule(u,p,y)},removeCSSRules:A=>{let u=A.cssRules.length;for(;u;)u--,"deleteRule"in A?A.deleteRule(u):"removeRule"in A&&A.removeRule(u)},createInputPlaceholderCSSRule:(A,u,p,y=1)=>{var z=A.getODE();z=z.id||(z.id="id"+I.generateGuid());let E=A.__sheet;E?I.removeCSSRules(E):
E=A.__sheet=I.createStylesheet();A=[];u&&A.push("color:"+u);p&&A.push("font-family:"+p);y&&A.push("opacity:"+y);I.addCSSRule(E,"#"+z+"::placeholder",A.join("; "),0)},chainSortFunc:(A,u)=>(p,y)=>{const z=A(p,y);return 0===z?u(p,y):z},getAlphaObjSortFunc:w((A,u,p,y)=>{const z=u?1:-1;return(E,H)=>{E=E[A];H=H[A];y?("string"!==typeof E&&(E=""+E),"string"!==typeof H&&(H=""+H)):(E=E||"",H=H||"");p&&(E=E.toLowerCase(),H=H.toLowerCase());return E.localeCompare(H)*z}}),getNumericObjSortFunc:w((A,u)=>{const p=
u?1:-1;return(y,z)=>{y=y[A]||0;z=z[A]||0;return(y-z)*p}}),dataURIToBlob:A=>{var u=A.indexOf(",");const p=A.slice(0,u).split(":")[1].split(";")[0];A=A.slice(u+1);if(p.startsWith("text/"))A=decodeURIComponent(A);else{A=atob(A);u=A.length;const y=new Uint8Array(u);for(;u;)y[--u]=A.charCodeAt(u);A=y}return new Blob([A],{type:p})},formatAsPercentage:(A,u=2)=>{if("number"===typeof A)return u=a(16,e(0,u)),A=h.round(e(0,a(1,A))*c(10,2+u))/c(10,u),(0===A%1?A:A.toFixed(u))+"%";"string"!==typeof A&&console.warn("formatAsPercentage: expects a number");
return A},memoize:w,debounce:function(A,u,p){const y="__DBTO_"+this.generateGuid();return function(){const z=this,E=z[y],H=arguments,K=p&&!E;clearTimeout(E);z[y]=setTimeout(function(){z[y]=null;p||A.apply(z,H)},u);K&&A.apply(z,H)}},createFixedThresholdCounter:(A,u,p,y)=>{var z=I.AccessorSupport.generateName;const E="function"===typeof A||A instanceof JS.Module,H={};y=y||z("counter",p);const K=z(y,"increment");z=z(y,"decrement");if(void 0!==(E?A.instanceMethod(K):A[K]))return console.warn("Increment: Abort clobber",
K,A),!1;if(void 0!==(E?A.instanceMethod(z):A[z]))return console.warn("Decrement: Abort clobber",z,A),!1;H[K]=function(){const J=this[y]+1;this[y]=J;this.fireEvent(y,J);J===u&&this.set(p,!0)};H[z]=function(){const J=this[y];if(0!==J){var M=J-1;this[y]=M;this.fireEvent(y,M);J===u&&this.set(p,!1)}};A[E?"include":"extend"](H);return!0},makeURLSearchParams:(A={})=>{const u=new URLSearchParams;for(const p in A){const y=A[p];void 0!==y&&u.set(p,y)}return u},doFetch:(A,u,p,y,z,E)=>fetch(A,u).then(H=>{if(!H.ok)throw new v(H.status,
A,H.statusText);return H}).then(H=>p?H.text():H.json()).then(H=>{if(y)try{if(p)y(H);else{if(!1===H.success)throw new v(200,A,H.message);y(H.data)}}catch(K){throw H=new v(200,A,K.message),H.stack=K.stack,H;}}).catch(H=>{if(z){if("FetchError"!==H.name){const K=new v(0,A,H.message);K.stack=H.stack;H=K}z(H)}}).finally(()=>{E?.()}),I18N:{setLocale:A=>{m=A},getLocale:()=>m,addDictionary:(A,u)=>{l[u]=Object.assign(l[u]??{},A)},setDictionary:(A,u)=>{l[u]=A??{}},get:(A,...u)=>{var p=m??=(navigator.language??
"en").split("-")[0].toLowerCase();p=(l[p]??l.en??{})[A];return null!=p?0<u.length?p.replaceAll(C,(y,z,E,H)=>E&&H?1==u[parseInt(z)]?E:H:"").replaceAll(F,(y,z)=>u[z]):p:A}}};x.onloadingdone=A=>{A.fontfaces.forEach(u=>{n(u)})}})(global);
...
n=l?n:x;const A=k.isWrapNotAllowed(),u=k.justifyContent,p=k.alignItems;x=A?"stretch":k.alignContent;q.endsWith("Reverse")&&f.reverse();let y=0,z=0,E=0,H=0,K={items:[],shrinkCount:0,shrinkBasis:0,growCount:0,extraSize:0,crossPos:E,crossSize:0};q=[K];for(let J=0;J<d;){const M=f[J++],O=b(M,l),V=M.getShrink(),X=M.getGrow();!A&&0<K.items.length&&y+(0===H?0:C)+O>F&&(K.extraSize=F-y,E+=K.crossSize+I,z=t(z,y),H=y=0,K={items:[],shrinkCount:0,shrinkBasis:0,growCount:0,extraSize:0,crossPos:E,crossSize:0},q.push(K));
0!==H&&(y+=C);0<V&&0<O&&(K.shrinkCount+=V,K.shrinkBasis+=O*V);0<X&&(K.growCount+=X);K.crossSize=t(K.crossSize,b(M,w));e(M,l,y);a(M,l,O);y+=O;K.items.push(M);H++}K.extraSize=F-y;E+=K.crossSize;z=t(z,y);if(k.isCompactOnMainAxis(l)){let J=q.reduce((M,O)=>null==M?O.extraSize:Math.min(M,O.extraSize),null);0!==J&&q.forEach(M=>{M.extraSize-=J})}k[l?"setTotalBasisWidth":"setTotalBasisHeight"](z);k[l?"setTotalBasisHeight":"setTotalBasisWidth"](E);f=q.length;1<f&&"wrapReverse"===k.wrap&&(q.forEach(J=>{J.crossPos=
E-(J.crossPos+J.crossSize)}),q.reverse());if(!k.isCompactOnCrossAxis(l))switch(d=n-E,x){default:case "start":break;case "end":h(q,d,!1);break;case "center":h(q,d/2,!1);break;case "spaceBetween":1<f&&h(q,d/(f-1),!0,1);break;case "spaceAround":d<=I?h(q,d/2,!1):(h(q,I/2,!1),d-=I,h(q,d/(2*f),!0),h(q,d/(2*f),!0,1));break;case "spaceEvenly":d<=2*I?h(q,d/2,!1):(h(q,I,!1),h(q,(d-2*I)/(f+1),!0));break;case "stretch":const J=d/f;let M=0;q.forEach(O=>{O.crossSize+=J;O.crossPos+=M;M+=J})}q.forEach(J=>{const M=
...
n.reverse?n.repeat-1:0;n.__prog=n.reverse?n.duration:0},x=(n,l)=>{const {running:w,paused:C,reverse:F,duration:I,repeat:A,__prog:u,easingFunction:p,callback:y}=n;if(w&&!C){F&&(l*=-1);l=u+l;let V=0;l>I?(V=l-I,l=I,++n.__loopCnt===A&&(V=0)):0>l&&(V=-l,l=0,0>--n.__loopCnt&&0<A&&(V=0));var z=n.target??n.parent;if(z){const {relative:X,attribute:U,__isColorAnim:Q,__loopCnt:R,to:Y}=n;var E=v(0,l/I),H=v(0,u/I);null==n.from&&(n.__tmpFrom=!0,n.from=X?Q?"#000":0:z.get(U));H=p(E)-(X?p(H):0);var K=n.from,J=X?z.get(U):
K;E=z.set;if(Q){var M=Y,O=X;K=k(K);M=k(M);O=O?k(J):K;O.setRed(O.red+(M.red-K.red)*H);O.setGreen(O.green+(M.green-K.green)*H);O.setBlue(O.blue+(M.blue-K.blue)*H);H=O.getHtmlHexString()}else H=J+(Y-K)*H;E.call(z,U,H);!F&&R===A||F&&0>R&&0<A?(n.setRunning(!1),y?.call(n,!0)):0<V?(n.fireEvent("repeat",R),n.__prog=F?I:0,x(n,V)):n.__prog=l}else console.log("No target for animator",n),n.setRunning(!1),y?.call(n,!1)}};r.Animator=new JS.Class("Animator",r.Node,{include:[r.Reusable],extend:{easings:f},initNode:function(n,
...
(r=>{let v;const t=JS.Class,m=JS.Module,h=r.global,b=h.focus,a=h.keys,e=r.KeyObservable,c={},g=(u,p)=>{if(u){const y=u.id;u.id?p(y):r.dumpStack("No ID")}else r.dumpStack("No processor")},k=u=>{g(u,p=>{c[p]=u})},f=u=>{const p=r.BAG.getGroup("selected",u.groupId);if(p){const y=p.getNodes();u=u.value;let z=y.length;for(;z;){const E=y[--z];if(E.optionValue===u){p.setTrue(E);break}}}},d=u=>{if(u.groupId){const p=r.BAG.getGroup("selected",u.groupId);p&&u.syncTo(p,"__syncValue","trueNode")}},q=(u,p)=>{const y=
...
new t("TrimValueProcessor",n,{initialize:function(u,p,y,z,E){this.callSuper(u,p,y,z);this.trim=E},process:function(u){u+="";switch(this.trim){case "start":case "left":return u.trimStart();case "end":case "right":return u.trimEnd();default:return u.trim()}}}),C=r.UndefinedValueProcessor=new t("UndefinedValueProcessor",n,{initialize:function(u,p,y,z,E){this.callSuper(u,p,y,z);this.defaultValue=E},process:function(u){return void 0===u?this.defaultValue:u}}),F=r.Form=new m("Form",{initNode:function(u,
...
const B=new Date(this.data);B.setHours(J.getHours());B.setMinutes(J.getMinutes());pa(B)}}),pa=B=>{B.setSeconds(0);B.setMilliseconds(0);var D=6>=p.length,G;if(D)for(G=7;G--&&!p.includes(B.getDay());)B.setDate(B.getDate()+1);J=B;const L=U.getODE().scrollTop;var N=new Date,P=N.getFullYear();G=N.getMonth();var T=N.getDate();N.getHours();N.getMinutes();const S=B.getFullYear(),ca=B.getMonth(),ra=B.getDate();var ea=B.getTime();N=B.getHours();B=B.getMinutes();const ka=(new Date(S,ca,1)).getDay()-u,da=(new Date(S,
...
1)+"/"+(aa-da)),ba.setIsAnotherMonth(!0),P.setDate(aa-da),P.setMonth(ta.getMonth()),P.setYear(ta.getFullYear()));D&&!p.includes(ya)?ba.setDisabled(!0):(0===ya?ba.setIsSunday(!0):6===ya&&ba.setIsSaturday(!0),aa===ra&&(X.select(ba),ba.focus()),Oa&&aa===T&&ba.setIsToday(!0),aa=new Date(P.getTime()),aa.setHours(23),aa.setMinutes(59),aa.setSeconds(59),(null!=y&&y>aa.getTime()||null!=z&&z<P.getTime())&&ba.setDisabled(!0));ba.updateUI()}}if(!I){D=60*H[0]+H[1];Ga(U);let [ja,fa]=E;for(P=new Date(ea);60*ja+
fa<D;)T=new Ma(U,{text:("0"+ja).slice(-2)+":"+("0"+fa).slice(-2)}),ja===N&&fa===B&&U.select(T),P.setHours(ja),P.setMinutes(fa),ea=P.getTime(),(null!=y&&y>ea||null!=z&&z<ea)&&T.setDisabled(!0),fa+=K,60<=fa&&(fa-=60,ja++);U.scrollYTo(L)}},Pa=r.DatePicker=new Q("DatePicker",R,{initNode:function(B,D){const G={current:null,dateOnly:!1,timeOnly:!1,locales:null,locale:"",minuteInterval:30,firstDayOfWeek:0,showTodayButton:!0,minDate:null,maxDate:null,minTime:"00:00",maxTime:"23:59",allowedDays:null,...D.opt};
delete D.opt;this.callSuper(B,D);F=G.locales[G.locale??"en"];I=G.dateOnly;A=G.timeOnly;u=G.firstDayOfWeek;K=ia(5,ha(30,G.minuteInterval));p=G.allowedDays;Array.isArray(p)&&0!==p.length||(p=[0,1,2,3,4,5,6]);y=Date.parse(G.minDate);y=isNaN(y)?null:y;z=Date.parse(G.maxDate);z=isNaN(z)?null:z;E=xa(G.minTime);H=xa(G.maxTime);B=new R(this,{visible:!A});G.showTodayButton&&new na(B,{width:24,text:oa(["home"]),tooltip:F.today},[{doActivated:()=>{pa(new Date)}}]);M=new na(B,{width:24,x:28,text:oa(["chevron-left"]),
...
text:oa(["times"]),tooltip:"Close Dialog."})},hide:function(B){this.spinner&&(this.spinner.setVisible(!1),this.spinner=void 0);this.callSuper(B)},destroyContent:function(){this.spinner&&(this.spinner.setVisible(!1),this.spinner=void 0);const B=this.content,D=B.sizeToChildren,G=B.getSubviews();let L=G.length;for(;L;){const N=G[--L];"closeBtn"!==N.name&&N.destroy()}B.setVisible(!0);this.overlay.setBgColor(r.Dimmer.COLOR);this.setCallbackFunction();D.setPaddingY(W.PADDING_Y);D.setPaddingX(W.PADDING_X);
B.setRoundedCorners(Z.RADIUS);B.setBgColor(Z.BGCOLOR);B.setBoxShadow(Z.SHADOW);B.setBorder(Z.BORDER)},doCallback:function(B){const D=this.callbackFunction;D&&D.call(this,B.name)||this.hide()},showBlank:function(B){this.destroyContent();this.content.setVisible(!1);B?.bgColor&&this.overlay.setBgColor(B.bgColor);this.show()},showMessage:function(B,D,G){const L=this.content,N=L.closeBtn;G={...Z.WRAP_TEXT_DEFAULTS,...G};this.destroyContent();this.setCallbackFunction(D);B=L.msg=new Y(L,{text:B,whiteSpace:G.whiteSpace,
wordWrap:G.wordWrap,fontWeight:G.fontWeight,x:null==G.msgX?W.PADDING_X:G.msgX,y:null==G.msgY?W.PADDING_Y:G.msgY,width:G.width});G.titleText&&(this.setupTitle(L,G.titleText),B.setY(B.y+24));this.show();!1===G.showClose?N.setVisible(!1):(N.setVisible(!0),N.focus())},showSimple:function(B,D,G,L){G=G??{};const N=this.content,P=N.closeBtn,T=G.maxContainerHeight;this.destroyContent();G.bgColor&&N.setBgColor(G.bgColor);G.roundedCorners&&N.setRoundedCorners(G.roundedCorners);G.boxShadow&&N.setBoxShadow(G.boxShadow);
G.border&&N.setBorder(G.border);N.sizeToChildren.setPaddingX(1);this.setCallbackFunction(D);D=N.contentContainer=new R(N,{x:1,y:25,overflow:"auto"},[{setHeight:function(S){S>T&&(S=T);this.callSuper(S)}}]);B.call(this,D);new ua(D,{axis:"both"});this.show();P.setVisible(!0);P.focus();G.titleText&&(this.setupTitle(N,G.titleText),D.setY(this.header.height+1));D.initialFocus?.focus();L?.(this)},showConfirm:function(B,D,G){G={...Z.CONFIRM_DEFAULTS,...G};this.showMessage(B,D,G);this.setupFooterButtons(this.content.msg,
G)},showContentConfirm:function(B,D,G,L){G={...Z.CONFIRM_DEFAULTS,...G};const N=this.content,P=N.closeBtn,T=G.maxContainerHeight;this.destroyContent();N.sizeToChildren.setPaddingX(1);this.setCallbackFunction(D);D=N.contentContainer=new R(N,{x:1,y:25,overflow:"auto"},[{setHeight:function(S){this.callSuper(S>T?T:S)}}]);B.call(this,D);new ua(D,{axis:"both"});this.show();P.setVisible(!0);P.focus();this.setupTitle(N,G.titleText);D.setY(this.header.height+1);this.setupFooterButtons(D,G);D.initialFocus?.focus();
L?.(this)},showSpinner:function(B,D){const G=this.content;D={...Z.NO_WRAP_TEXT_DEFAULTS,...D};this.destroyContent();const L=this.spinner=new r.Spinner(G,{align:"center",visible:!0,borderColor:"#ccc",size:50,y:null==D.msgY?W.PADDING_Y:D.msgY});B&&new Y(G,{text:B,whiteSpace:D.whiteSpace,wordWrap:D.wordWrap,fontWeight:D.fontWeight,x:null==D.msgX?W.PADDING_X:D.msgX,y:L.y+L.size+W.PADDING_Y,width:D.width});this.show();G.closeBtn.setVisible(!1);this.focus()},showColorPicker:function(B,D){const G=this,L=
G.content,N=L.closeBtn;D={...Z.COLOR_PICKER_DEFAULTS,...D};G.destroyContent();G.setCallbackFunction(T=>{switch(T){case "closeBtn":case "cancelBtn":B.call(G,T);break;case "confirmBtn":const S=P.getColor();P.addToPalette(S);B.call(G,T,S)}P.destroy()});const P=new La(L,{x:W.PADDING_X,y:W.PADDING_Y+24,width:337,height:177+(D.alphaChannel?23:0),color:D.color,alphaChannel:D.alphaChannel,palette:"#000000 #111111 #222222 #333333 #444444 #555555 #666666 #777777 #888888 #999999 #aaaaaa #bbbbbb #cccccc #dddddd #eeeeee #ffffff".split(" ")});
G.show();N.setVisible(!0);N.focus();G.setupFooterButtons(P,D);G.setupTitle(L,D.titleText)},showDatePicker:function(B,D){const G=this,L=G.content,N=L.closeBtn;D={...Z.DATE_PICKER_DEFAULTS,...D};G.destroyContent();G.setCallbackFunction(T=>{switch(T){case "closeBtn":case "cancelBtn":B.call(G,T);break;case "confirmBtn":B.call(G,T,P.getPickedDate())}P.destroy()});const P=new Pa(L,{x:W.PADDING_X,y:W.PADDING_Y+24,height:195,opt:{current:new Date(D.initialDate??Date.now()),dateOnly:D.dateOnly||!1,timeOnly:D.timeOnly||
!1,locales:D.locales,locale:D.locale}});G.show();N.setVisible(!0);N.focus();G.setupFooterButtons(P,D);G.setupTitle(L,D.timeOnly?D.timeOnlyTitleText:D.titleText)},setupTitle:function(B,D){const G=Z.RADIUS;(this.header=new R(B,{ignoreLayout:!0,width:B.width,height:24,bgColor:"#eee",roundedTopLeftCorner:G,roundedTopRightCorner:G})).sendToBack();B.title=new Y(B,{x:G,y:4,text:D,fontWeight:"bold"})},setupFooterButtons:function(B,D){const G=this,L=G.content,N=W.PADDING_Y;var P=N/2;const T=L.btnContainer=
new R(L,{y:B.y+B.height+N,align:"center"});B=["active","hover","ready","text"];let S=D.cancelAttrs??{};var ca;(ca=S).name??(ca.name="cancelBtn");let ra;(ra=S).text??(ra.text=D.cancelTxt);B.forEach(da=>{da+="Color";null!=D[da]&&(S[da]=D[da])});ca=G.makeButton(T,S);S=D.confirmAttrs??{};var ea;(ea=S).name??(ea.name="confirmBtn");let ka;(ka=S).text??(ka.text=D.confirmTxt);B.forEach(da=>{da+="Color";const ma=da+"Confirm";null!=D[ma]&&(S[da]=D[ma])});G.makeButton(T,S);D.buttons?.forEach(da=>{G.makeButton(T,
da)});new ua(T,{axis:"y"});new za(T,{spacing:4,collapseParent:!0});L.sizeToChildren.setPaddingY(P);ea=Z.RADIUS;P=T.y-P;(new R(L,{ignoreLayout:!0,y:P,width:L.width,height:L.height-P,bgColor:"#eee",roundedBottomLeftCorner:ea,roundedBottomRightCorner:ea})).sendToBack();!1===D.showClose&&ca.focus()}})})(myt);
...
(r=>{let v=0;const t=JS.Class,m=JS.Module,h=Math.min,b=Math.max,a=r.View,e=r.Node.DEFAULT_PLACEMENT,c=p=>{const y=p.columnHeaders,[z,E]=p.sort??["",""];let H=y.length;for(;H;){const K=y[--H];K.columnId===z?K.sortable&&K.setSortState(E):K.setSortState("none")}p.locked||p.doSort()},g=(p,y,z,E)=>{if(0!==y){const M=0<y,O=[];for(var H=p.length;H;){var K=p[--H];K.resizable&&(z?0<K.flex:0===K.flex)&&O.push({hdr:K,limit:(M?K.maxValue:K.minValue)-K.value,amt:0})}p=O.length;if(0<p){for(H=0;0!==y;){v>=p&&(H=
v=0);K=O[v];var J=K.hdr;K.full?++H:(M?(J=h(z?J.flex:1,y),K.amt+J>K.limit&&(J=K.limit-K.amt,K.full=!0)):(J=b(z?-J.flex:-1,y),K.amt+J<K.limit&&(J=K.limit-K.amt,K.full=!0)),K.amt+=J,y-=J);if(H===p)break;++v}O.forEach(V=>{V.hdr.setValue(V.hdr.value+V.amt)});E?.(y)}}},k=p=>(p=p.gridController?p.gridController.getPrevHdr(p):null)?p.maxValue-p.value+k(p):0,f=p=>(p=p.gridController?p.gridController.getNextHdr(p):null)?p.maxValue-p.value+f(p):0,d=p=>(p=p.gridController?p.gridController.getPrevHdr(p):null)?
p.minValue-p.value+d(p):0,q=p=>(p=p.gridController?p.gridController.getNextHdr(p):null)?p.minValue-p.value+q(p):0,x=(p,y)=>{p=p.gridController?p.gridController.getPrevHdr(p):null;var z=0;if(p){z=p.value+y;p.resizable&&p.setValue(z);const E=z-p.value;z=y-E;0>E&&(z+=x(p,E))}return z},n=(p,y)=>{p=p.gridController?p.gridController.getPrevHdr(p):null;var z=0;if(p){z=p.value+y;p.resizable&&p.setValue(z);const E=z-p.value;z=y-E;0<E&&(z+=n(p,E))}return z},l=(p,y)=>{if(p=p.gridController?p.gridController.getNextHdr(p):
...
p:0]},notifyAddRow:function(p,y){if(!this.hasRow(p)&&(this.rows.push(p),!this.locked)){const z=this.width;this.columnHeaders.forEach(E=>{p.setWidth(z);p.notifyHdrXChange(E);p.notifyHdrWidthChange(E);p.notifyHdrVisibilityChange(E)});y||this.doSort()}},notifyRemoveRow:function(p){p=this.getRowIndex(p);-1<p&&this.rows.splice(p,1)},fitHeadersToWidth:function(){if(!this.locked&&this.fitToWidth){const p=this.getVisibleHdrs();let y=0;p.forEach(z=>{y=b(y,z.x+z.width)});g(p,this.gridWidth-y,!0,z=>{g(p,z,!1,
null)})}},fixupResizerCursors:function(){const p=this.getVisibleHdrs();let y=!0,z=0;p.forEach((J,M)=>{y&&J.isFixed()?J.setResizerCursor("",!0):(y&&(z=M),y=!1,J.restoreResizerCursor())});let E=!0,H=p.length,K=H-1;for(;H>z;){const J=p[--H];E?J.setResizerCursor("",!0):J.restoreResizerCursor();J.isFixed()||(E&&(K=H),E=!1)}z===K&&p[z].setResizerCursor("",!0)}}),A=r.GridColHdr=new m("GridColHdr",{include:[r.BoundedValueComponent],initNode:function(p,y){const z=this,E=y.resizerCursor??"col-resize";delete y.resizerCursor;
y.minValue??(y.minValue=16);y.maxValue??(y.maxValue=1E3);y.value??(y.value=y.minValue);y.resizable??(y.resizable=!0);y.flex??(y.flex=0);y.cellXAdj??(y.cellXAdj=0);y.cellWidthAdj??(y.cellWidthAdj=0);y.sortable??(y.sortable=!0);y.sortState??(y.sortState="none");y.placement??(y.placement=e);z.callSuper(p,y);const H=z.gridController;z.resizer=new a(z,{cursor:E,width:10,zIndex:1,percentOfParentHeight:100,align:"right",alignOffset:-5,draggableAllowBubble:!1},[r.SizeToParent,r.Draggable,{requestDragPosition:function(K,
...
setSortable:function(p){this.set("sortable",p,!0)},setSortState:function(p){this.set("sortState",p,!0)},setResizable:function(p){this.set("resizable",p,!0)},setCellWidthAdj:function(p){this.cellWidthAdj=p},setCellXAdj:function(p){this.cellXAdj=p},setFlex:function(p){this.flex=p},setColumnId:function(p){this.columnId=p},setResizerCursor:function(p,y){const z=this.resizer,E=z.cursor;p!==E&&(y&&(this.__rrc=E),z.setCursor(p))},restoreResizerCursor:function(){const p=this.__rrc;p&&(this.setResizerCursor(p),
...
r.Color;d.forEach(E=>{"string"===typeof E.color&&(E.color=x.makeColorFromHexString(E.color))});1>q&&(console.warn("Invalid segements",q),q=60);q=v/q;let n=0;for(var l=d.length-1;n<l;){var w=d[n++];const {angle:E,color:H}=w,{angle:K,color:J}=d[n];var C=Math.round((K-E)/q),F=H.getDiffFrom(J);w.colorDelta={red:F.red/C,green:F.green/C,blue:F.blue/C}}c=new r.Path([a+c,e,a+g,e]);c.rotateAroundOrigin(k,a,e);g=c.vectors;let [I,A,u,p]=g,y=0,z=v/360;for(n=0;f>k;){C=I;F=A;l=u;w=p;k+=q;k>f&&(q+=f-k,z=0,k=f);
c.rotateAroundOrigin(q+z,a,e);[I,A,u,p]=g;this.beginPath();this.moveTo(C,F);this.lineTo(l,w);this.lineTo(u,p);this.lineTo(I,A);this.closePath();const {color:E,colorDelta:H}=d[n];this.setFillStyle(x.rgbToHex(E.red+y*H.red,E.green+y*H.green,E.blue+y*H.blue,!0));this.fill();0<z&&(c.rotateAroundOrigin(-z,a,e),[I,A,u,p]=g);y++;k>=d[n+1].angle&&(y=0,n++)}return this}})})(myt);myt.all=!0;
