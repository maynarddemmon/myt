...
(u=>{class x extends Error{constructor(p,y,...A){super(...A);this.name="FetchError";this.status=p;this.url=y}}let w=0,v,k={};const e=console.warn,c=Math,{abs:h,min:l,max:b,pow:g}=c,d=Array.isArray,f=document,a=f.head,q=f.createElement.bind(f),n={},r={"":!0},t=f.fonts,z=p=>{var y=p.family.split('"').join("");for(const A of[y,y+" "+p.weight])if(!r[A])for(r[A]=!0,y=n[A]??[];y.length;)(p=y.pop())&&!p.destroyed&&p.isVisible()&&p.sizeViewToDom()},D={},G=p=>{const y={};return(...A)=>{const C=JSON.stringify(A);
return C in y?y[C]:y[C]=p(...A)}},L=/\{\{plural:\$(.*?)\|(.*?)\|(.*?)\}\}/g,J=/\$(\d+)/g,N=/(,|\r?\n|\r|^)(?:"((?:\\.|""|[^\\"])*)"|([^,"\r\n]*))/gi,Q=/[\\"](.)/g,m=u.myt={version:2.02306021558E7,generateGuid:()=>++w,hash:p=>p.split("").reduce((y,A)=>{y=(y<<5)-y+A.charCodeAt(0);return y&y},0),addEventListener:(p,y,A,C,E)=>{p.addEventListener(y,A,{capture:C||!1,passive:E||!1})},removeEventListener:(p,y,A,C)=>{p.removeEventListener(y,A,C||!1)},resolveName:(p,y)=>{if(p&&0!==p.length){y=y??global;var A=
d(p)?p:p.split("."),C=A.length;for(let E=0;E<C;E++)if(y=y[A[E]],null==y){e("resolveName failed for",p,"at part",E,A[E]);return}return y}},resolveClassname:p=>{"string"===typeof p&&(p=m.resolveName(p));return p&&"function"===typeof p.isA&&p.isA(JS.Class)?p:null},getExtension:p=>p?(p=p.split("."),1<p.length?p.pop():null):null,loadScript:function(p,y,A,C){const E=this._loadedScripts??(this._loadedScripts={});if(E[p])return e("script already loaded for src",p),null;E[p]=!0;const I=q("script");I.type=
"text/javascript";I.async=!1;C&&(I.integrity=C,I.crossOrigin="anonymous");if(y){let K=!1;I.onerror=()=>{I.onload=I.onreadystatechange=I.onerror=null;y(!1)};I.onload=I.onreadystatechange=()=>{K||I.readyState&&"complete"!==I.readyState||(K=!0,I.onload=I.onreadystatechange=I.onerror=null,y(!0))}}I.src=p+(A?"":(p.includes("?")?"&":"?")+"cacheBust="+Date.now());a.appendChild(I);return I},dumpStack:(p,y)=>{let A;"string"===typeof p&&(A=p,p=null);m.global.error.notify(y||"error",null,A,p)},getRandom:p=>
{const y=c.random();return p?b(0,l(p(y),.9999999999)):y},getRandomArbitrary:(p,y,A)=>{const C=l(p,y);return m.getRandom(A)*(b(p,y)-C)+C},getRandomInt:(p,y,A)=>{const C=l(p,y);return c.floor(m.getRandom(A)*(b(p,y)-C+1)+C)},areFloatsEqual:(p,y,A)=>{const C=h(p),E=h(y);return h(p-y)<=(C>E?E:C)*(null==A?1E-6:h(A))},areArraysEqual:(p,y)=>{if(p!==y){if(null==p||null==y)return!1;let A=p.length;if(A!==y.length)return!1;for(;A;)if(p[--A]!==y[A])return!1}return!0},areObjectsEqual:(p,y)=>{if(p!==y){if(null==
p||null==y)return!1;for(const A in p)if(p[A]!==y[A])return!1;for(const A in y)if(p[A]!==y[A])return!1}return!0},getElement:(p,y)=>f.getElementsByTagName(p||"body")[0<y?y:0],loadFontFaces:(p,y)=>{const A=[];for(const C of p)p=new FontFace(C.family,"url("+C.url+")",C.options),A.push(p.load());Promise.all(A).then(C=>{for(const E of C)t.add(E),z(E);y?.(C)})},loadFontFace:(p,y,A={},C)=>{p=new FontFace(p,"url("+y+")",A);p.loaded.then(E=>{t.add(E);z(E);C?.(E)});p.load()},registerForFontNotification:(p,y)=>
{r[y]?p&&!p.destroyed&&p.isVisible()&&p.sizeViewToDom():(n[y]??(n[y]=[])).push(p)},createBaseFontCSSRule:p=>{m.addCSSRule(m.createStylesheet(),"body, input","font-family:"+p,0)},loadCSSFonts:p=>{p?.forEach(m.createStylesheetLink)},createStylesheetLink:p=>{if(p){let y=k[p];y?console.warn("stylesheet already exists",p):(y=k[p]=q("link"),y.rel="stylesheet",y.href=p,a.appendChild(y));return y}},createStylesheet:()=>{const p=q("style");a.appendChild(p);return p.sheet},addCSSRule:(p,y,A,C)=>{p.insertRule(y+
"{"+A+"}",C)},removeCSSRules:p=>{let y=p.cssRules.length;for(;y;)p.deleteRule(--y)},createInputPlaceholderCSSRule:(p,y,A,C=1)=>{let E=p.__sheet;E?m.removeCSSRules(E):E=p.__sheet=m.createStylesheet();p=p.getODE();p=p.id||(p.id="id"+ ++w);const I=[];y&&I.push("color:"+y);A&&I.push("font-family:"+A);C&&I.push("opacity:"+C);m.addCSSRule(E,"#"+p+"::placeholder",I.join("; "),0)},isNotSorted:(p,y)=>{const A=p.length;for(let C=1;C<A;C++)if(0<y(p[C-1],p[C]))return!0},chainSortFunc:(...p)=>(y,A)=>{for(const C of p){const E=
C(y,A);if(0!==E)return E}return 0},getAlphaObjSortFunc:G((p,y,A,C)=>{const E=y?1:-1;return(I,K)=>{I=I[p];K=K[p];C?("string"!==typeof I&&(I=""+I),"string"!==typeof K&&(K=""+K)):(I=I||"",K=K||"");A&&(I=I.toLowerCase(),K=K.toLowerCase());return I.localeCompare(K)*E}}),getNumericObjSortFunc:G((p,y)=>{const A=y?1:-1;return(C,E)=>{C=C[p]||0;E=E[p]||0;return(C-E)*A}}),csvStringToArray:(p,y)=>{if(!p)return[];const A=[[]];let C;for(;C=N.exec(p);)C[1].length&&","!==C[1]&&A.push([]),A[A.length-1].push(C[2]?
C[2].replace(Q,"$1"):C[3]);if(y){const E=A.shift();return A.map(I=>{let K=0;return E.reduce((R,M)=>{R[M]=I[K++];return R},{})})}return A},prepareCSVDataURI:(p,y)=>{const A=I=>{if(I&&d(I)){const K=[],R=I.length;for(let M=0,T;R>M;M++)T=I[M],T=null==T?"":T.toString(),T=T.replace(/"/g,'""'),0<=T.search(/("|,|\n)/g)&&(T='"'+T+'"'),K.push(T);return K.join(",")}e("Unexpected row",I);return null},C=[];if(y){var E=A(y);E&&C.push(E)}if(p)if(d(p)){E=p.length;for(let I=0;E>I;I++){const K=A(p[I]);K&&C.push(K)}}else e("Rows were not an array");
return m.encodeCSVDataURI(C.join("\r\n"),y)},encodeCSVDataURI:(p,y)=>"data:text/csv;charset=utf-8"+(null==y?"":";header="+(y?"present":"absent"))+","+encodeURIComponent(p),dataURIToBlob:p=>{var y=p.indexOf(",");const A=p.slice(0,y).split(":")[1].split(";")[0];p=p.slice(y+1);if(A.startsWith("text/"))p=decodeURIComponent(p);else{p=atob(p);y=p.length;const C=new Uint8Array(y);for(;y;)C[--y]=p.charCodeAt(y);p=C}return new Blob([p],{type:A})},formatAsPercentage:(p,y=2)=>{switch(typeof p){case "number":return y=
l(16,b(0,y)),p=c.round(b(0,l(1,p))*g(10,2+y))/g(10,y),(0===p%1?p:p.toFixed(y))+"%";case "string":return p;default:return e("formatAsPercentage: expects a number"),p}},memoize:G,debounce:(p,y,A)=>{const C="__DBTO_"+ ++w;return function(){const E=this,I=E[C],K=arguments,R=A&&!I;clearTimeout(I);E[C]=setTimeout(function(){E[C]=null;A||p.apply(E,K)},y);R&&p.apply(E,K)}},createFixedThresholdCounter:(p,y,A,C)=>{var E=m.AccessorSupport.generateName;const I="function"===typeof p||p instanceof JS.Module,K=
{};C=C||E("counter",A);const R=E(C,"increment");E=E(C,"decrement");if(void 0!==(I?p.instanceMethod(R):p[R]))return e("Increment: Abort clobber",R,p),!1;if(void 0!==(I?p.instanceMethod(E):p[E]))return e("Decrement: Abort clobber",E,p),!1;K[R]=function(){const M=this[C]+1;this[C]=M;this.fireEvent(C,M);M===y&&this.set(A,!0)};K[E]=function(){const M=this[C];if(0!==M){var T=M-1;this[C]=T;this.fireEvent(C,T);M===y&&this.set(A,!1)}};p[I?"include":"extend"](K);return!0},makeURLSearchParams:(p={})=>{const y=
