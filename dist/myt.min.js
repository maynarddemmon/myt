...
(r=>{class w extends Error{constructor(p,y,...B){super(...B);this.name="FetchError";this.status=p;this.url=y}}let v=0,t,h={};const d=console.warn,e=Math,{abs:m,min:k,max:a,pow:f}=e,b=Array.isArray,g=document,c=g.head,q=g.createElement.bind(g),x={},u={"":!0},n=g.fonts,A=p=>{const y=p.family.split('"').join("");for(const B of[y,y+" "+p.weight])if(!u[B]){u[B]=!0;const D=x[B]??[];for(;D.length;)(p=D.pop())&&!p.destroyed&&p.isVisible()&&(p.sizeViewToDom(),p.notifyFontLoaded?.(y))}},J={},z=p=>{const y=
{};return(...B)=>{const D=JSON.stringify(B);return D in y?y[D]:y[D]=p(...B)}},C=/\{\{plural:\$(.*?)\|(.*?)\|(.*?)\}\}/g,I=/\$(\d+)/g,M=/(,|\r?\n|\r|^)(?:"((?:\\.|""|[^\\"])*)"|([^,"\r\n]*))/gi,O=/[\\"](.)/g,l=r.myt={version:2.02306021558E7,generateGuid:()=>++v,theme:{TextButtonActive:"#ddd",TextButtonHover:"#eee",TextButtonReady:"#fff",DialogRadius:12,DialogBgColor:"#fff",DialogInputFgColor:"#333",DialogShadow:[0,4,20,"#666"],DialogBorder:[1,"solid","#fff"],DialogDayBtnBorder:[1,"solid","#fff"],DialogDayBtnBorderColor:"#fff",
...
TooltipVInset:3,focusShadow:[0,0,7,"#666"],disabledOpacity:.5,border1s3:[1,"solid","#333"],border1s9:[1,"solid","#999"]},hash:p=>p.split("").reduce((y,B)=>{y=(y<<5)-y+B.charCodeAt(0);return y&y},0),addEventListener:(p,y,B,D,F)=>{p.addEventListener(y,B,{capture:D||!1,passive:F||!1})},removeEventListener:(p,y,B,D)=>{p.removeEventListener(y,B,D||!1)},resolveName:(p,y)=>{if(p&&0!==p.length){y=y??global;var B=b(p)?p:p.split("."),D=B.length;for(let F=0;F<D;F++)if(y=y[B[F]],null==y){d("resolveName failed for",
p,"at part",F,B[F]);return}return y}},resolveClassname:p=>{"string"===typeof p&&(p=l.resolveName(p));return p&&"function"===typeof p.isA&&p.isA(JS.Class)?p:null},getExtension:p=>p?(p=p.split("."),1<p.length?p.pop():null):null,loadScript:function(p,y,B,D){const F=this._loadedScripts??(this._loadedScripts={});let K=F[p];if(!0===K)y?.(!0);else if(!1===K)y?.(!1);else if(Array.isArray(K))y&&K.push(y);else{K=F[p]=[];y&&K.push(y);const L=q("script");L.type="text/javascript";L.async=!1;D&&(L.integrity=D,
L.crossOrigin="anonymous");const P=N=>{L.onload=L.onreadystatechange=L.onerror=null;for(const U of K)U(N);F[p]=N};L.onerror=()=>{P(!1)};L.onload=L.onreadystatechange=()=>{L.readyState&&"complete"!==L.readyState||P(!0)};L.src=p+(B?"":(p.includes("?")?"&":"?")+"cacheBust="+Date.now());c.appendChild(L)}},dumpStack:(p,y)=>{let B;"string"===typeof p&&(B=p,p=null);l.global.error.notify(y||"error",null,B,p)},getRandom:p=>{const y=e.random();return p?a(0,k(p(y),.9999999999)):y},getRandomArbitrary:(p,y,B)=>
{const D=k(p,y);return l.getRandom(B)*(a(p,y)-D)+D},getRandomInt:(p,y,B)=>{const D=k(p,y);return e.floor(l.getRandom(B)*(a(p,y)-D+1)+D)},areFloatsEqual:(p,y,B)=>{const D=m(p),F=m(y);return m(p-y)<=(D>F?F:D)*(null==B?1E-6:m(B))},areArraysEqual:(p,y)=>{if(p!==y){if(null==p||null==y)return!1;let B=p.length;if(B!==y.length)return!1;for(;B;)if(p[--B]!==y[B])return!1}return!0},areObjectsEqual:(p,y)=>{if(p!==y){if(null==p||null==y)return!1;for(const B in p)if(p[B]!==y[B])return!1;for(const B in y)if(p[B]!==
y[B])return!1}return!0},getElement:(p,y)=>g.getElementsByTagName(p||"body")[0<y?y:0],loadFontFaces:(p,y)=>{const B=[];for(const D of p)p=new FontFace(D.family,"url("+D.url+")",D.options),B.push(p.load());Promise.all(B).then(D=>{for(const F of D)n.add(F),A(F);y?.(D)})},loadFontFace:(p,y,B={},D)=>{p=new FontFace(p,"url("+y+")",B);p.loaded.then(F=>{n.add(F);A(F);D?.(F)});p.load()},registerForFontNotification:(p,y)=>{u[y]?p&&!p.destroyed&&p.isVisible()&&(p.sizeViewToDom(),p.notifyFontLoaded?.(y)):(x[y]??
(x[y]=[])).push(p)},createBaseFontCSSRule:p=>{l.addCSSRule(l.createStylesheet(),"body, input","font-family:"+p)},loadCSSFonts:p=>{p?.forEach(l.createStylesheetLink)},createStylesheetLink:p=>{if(p){let y=h[p];y?console.warn("stylesheet already exists",p):(y=h[p]=q("link"),y.rel="stylesheet",y.href=p,c.appendChild(y));return y}},createStylesheet:()=>{const p=q("style");c.appendChild(p);return p.sheet},addCSSRule:(p,y,B,D)=>{p.insertRule(y+"{"+B+"}",D??0)},removeCSSRules:p=>{let y=p.cssRules.length;
for(;y;)p.deleteRule(--y)},createInputPlaceholderCSSRule:(p,y,B,D=1)=>{let F=p.__sheet;F?l.removeCSSRules(F):F=p.__sheet=l.createStylesheet();p=p.getODE();p=p.id||(p.id="id"+ ++v);const K=[];y&&K.push("color:"+y);B&&K.push("font-family:"+B);D&&K.push("opacity:"+D);l.addCSSRule(F,"#"+p+"::placeholder",K.join("; "))},isNotSorted:(p,y)=>{const B=p.length;for(let D=1;D<B;D++)if(0<y(p[D-1],p[D]))return!0},chainSortFunc:(...p)=>(y,B)=>{for(const D of p){const F=D(y,B);if(0!==F)return F}return 0},getAlphaObjSortFunc:z((p,
y,B,D)=>{const F=y?1:-1,K=l.I18N.getLocale(),L={sensitivity:B?"accent":"variant"};return(P,N)=>{P=P[p];N=N[p];D?("string"!==typeof P&&(P=""+P),"string"!==typeof N&&(N=""+N)):(P=P||"",N=N||"");return P.localeCompare(N,K,L)*F}}),getNumericObjSortFunc:z((p,y)=>{const B=y?1:-1;return(D,F)=>{D=D[p]||0;F=F[p]||0;return(D-F)*B}}),csvStringToArray:(p,y)=>{if(!p)return[];const B=[[]];let D;for(;D=M.exec(p);)D[1].length&&","!==D[1]&&B.push([]),B[B.length-1].push(D[2]?D[2].replace(O,"$1"):D[3]);if(y){const F=
B.shift();return B.map(K=>{let L=0;return F.reduce((P,N)=>{P[N]=K[L++];return P},{})})}return B},prepareCSVDataURI:(p,y)=>{const B=K=>{if(K&&b(K)){const L=[],P=K.length;for(let N=0,U;P>N;N++)U=K[N],U=null==U?"":U.toString(),U=U.replace(/"/g,'""'),0<=U.search(/("|,|\n)/g)&&(U='"'+U+'"'),L.push(U);return L.join(",")}d("Unexpected row",K);return null},D=[];if(y){var F=B(y);F&&D.push(F)}if(p)if(b(p)){F=p.length;for(let K=0;F>K;K++){const L=B(p[K]);L&&D.push(L)}}else d("Rows were not an array");return l.encodeCSVDataURI(D.join("\r\n"),
y)},encodeCSVDataURI:(p,y)=>"data:text/csv;charset=utf-8"+(null==y?"":";header="+(y?"present":"absent"))+","+encodeURIComponent(p),dataURIToBlob:p=>{var y=p.indexOf(",");const B=p.slice(0,y).split(":")[1].split(";")[0];p=p.slice(y+1);if(B.startsWith("text/"))p=decodeURIComponent(p);else{p=atob(p);y=p.length;const D=new Uint8Array(y);for(;y;)D[--y]=p.charCodeAt(y);p=D}return new Blob([p],{type:B})},formatAsPercentage:(p,y=2)=>{switch(typeof p){case "number":return y=k(16,a(0,y)),p=e.round(a(0,k(1,
p))*f(10,2+y))/f(10,y),(0===p%1?p:p.toFixed(y))+"%";case "string":return p;default:return d("formatAsPercentage: expects a number"),p}},leftPadNumber:(p,y,B="0",D=10)=>{p=p.toString(D);return B.repeat(a(y-p.length,0))+p},removeMarkup:(p,y)=>{if(!p)return"";y?.brToLineFeed&&(p=p.replace(/<br\s*\/?>/gi,"\n"));return p.replace(/<\/?[^>]+(>|$)/g,"")},escapeMarkup:(()=>{const p=RegExp("[&<>\"']","g"),y={"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#039;"},B=D=>y[D];return D=>D.replace(p,B)})(),
memoize:z,debounce:(p,y,B)=>{const D="__DBTO_"+ ++v;return function(){const F=this,K=F[D],L=arguments,P=B&&!K;clearTimeout(K);F[D]=setTimeout(function(){F[D]=null;B||p.apply(F,L)},y);P&&p.apply(F,L)}},createFixedThresholdCounter:(p,y,B,D)=>{var F=l.AccessorSupport.generateName;const K="function"===typeof p||p instanceof JS.Module,L={};D=D||F("counter",B);const P=F(D,"increment");F=F(D,"decrement");if(void 0!==(K?p.instanceMethod(P):p[P]))return d("Increment: Abort clobber",P,p),!1;if(void 0!==(K?
p.instanceMethod(F):p[F]))return d("Decrement: Abort clobber",F,p),!1;L[P]=function(){const N=this[D]+1;this[D]=N;this.fireEvent(D,N);N===y&&this.set(B,!0)};L[F]=function(){const N=this[D];if(0!==N){var U=N-1;this[D]=U;this.fireEvent(D,U);N===y&&this.set(B,!1)}};p[K?"include":"extend"](L);return!0},makeURLSearchParams:(p={})=>{const y=new URLSearchParams;for(const B in p){const D=p[B];void 0!==D&&y.set(B,D)}return y},doFetch:(p,y,B,D,F,K)=>fetch(p,y).then(L=>{if(!L.ok)throw new w(L.status,p,L.statusText);
return L}).then(L=>B?L.text():L.json()).then(L=>{if(D)try{if(B)D(L);else{if(!1===L.success)throw new w(200,p,L.message??L.data?.message);D(L.data)}}catch(P){throw L=new w(200,p,P.message),L.stack=P.stack,L;}}).catch(L=>{if(F){if("FetchError"!==L.name){const P=new w(0,p,L.message);P.stack=L.stack;L=P}F(L)}}).finally(()=>{K?.()}),toNameCase:(p,y)=>{if(!p)return"";p=p.trim().toLowerCase().split(/([\s\-'\u2019"\u201c\u201d().,\/])/).reduce((B,D)=>B+D.charAt(0).toUpperCase()+D.slice(1),"");if(/\bMac[A-Za-z]{2,}[^aciozj]\b/.test(p)||
/\bMc/.test(p))p=p.replace(/\b(Ma?c)([A-Za-z]+)/,(B,D,F)=>D+F.charAt(0).toUpperCase()+F.slice(1)),p=p.replace(/\bMacEvicius\b/,"Macevicius").replace(/\bMacHado\b/,"Machado").replace(/\bMacHar\b/,"Machar").replace(/\bMacHin\b/,"Machin").replace(/\bMacHlin\b/,"Machlin").replace(/\bMacIas\b/,"Macias").replace(/\bMacIulis\b/,"Maciulis").replace(/\bMacKie\b/,"Mackie").replace(/\bMacKle\b/,"Mackle").replace(/\bMacKlin\b/,"Macklin").replace(/\bMacQuarie\b/,"Macquarie").replace(/\bMacOmber\b/,"Macomber").replace(/\bMacIn\b/,
"Macin").replace(/\bMacKintosh\b/,"Mackintosh").replace(/\bMacKen\b/,"Macken").replace(/\bMacHen\b/,"Machen").replace(/\bMacHiel\b/,"Machiel").replace(/\bMacIol\b/,"Maciol").replace(/\bMacKell\b/,"Mackell").replace(/\bMacKlem\b/,"Macklem").replace(/\bMacKrell\b/,"Mackrell").replace(/\bMacLin\b/,"Maclin").replace(/\bMacKey\b/,"Mackey").replace(/\bMacKley\b/,"Mackley").replace(/\bMacHell\b/,"Machell").replace(/\bMacHon\b/,"Machon").replace(/\bMacAyla\b/,"Macayla");p=p.replace(/\bMacmurdo/,"MacMurdo").replace(/\bMacisaac/,
"MacIsaac").replace(/\bAl(?=\s+\w)\b/g,"al").replace(/\bAp\b/g,"ap").replace(/\bBen(?=\s+\w)\b/g,"ben").replace(/\bDell([ae])\b/g,"dell$1").replace(/\bD([aeiu])\b/g,"d$1").replace(/\bDe([lr])\b/g,"de$1").replace(/\bEl\b/g,"el").replace(/\bLa\b/g,"la").replace(/\bLe(?=\s+\w)\b/g,"le").replace(/\bLo\b/g,"lo").replace(/\bVan(?=\s+\w)\b/g,"van").replace(/\bVon\b/g,"von").replace(/\bD['\u2019]/g,"d'").replace(/\b(?:\d{4}|(?:[IVX])(?:X{0,3}I{0,3}|X{0,2}VI{0,3}|X{0,2}I?[VX]))$/i,B=>B.toUpperCase()).replace(/\b[0-9](x)\b/,
B=>B.toUpperCase()).replace(/(?:^|\\s)[bcdfghjklmnpqrstvwxzBCDFGHJKLMNPQRSTVWXZ]{2}\s/,B=>B.toUpperCase()).replace(/\bMR\.?\b/,"Mr").replace(/\bMS\.?\b/,"Ms").replace(/\bDR\.?\b/,"Dr").replace(/\bST\.?\b/,"St").replace(/\bJR\.?\b/,"Jr").replace(/\bSR\.?\b/,"Sr").replace(/\bLT\.?\b/,"Lt").replace(/\bThe\b/g,"the").replace(/\bOf\b/g,"of").replace(/\bAnd\b/g,"and").replace(/\bY\s/g,"y").replace(/\s{2,}/g," ");return y?p:p.charAt(0).toUpperCase()+p.slice(1)},I18N:{setLocale:p=>{t=p},getLocale:()=>t,addDictionary:(p,
y)=>{J[y]=Object.assign(J[y]??{},p)},setDictionary:(p,y)=>{J[y]=p??{}},get:(p,...y)=>{var B=t??=(navigator.language??"en").split("-")[0].toLowerCase();B=(J[B]??J.en??{})[p];return null!=B?0<y.length?B.replaceAll(C,(D,F,K,L)=>K&&L?1==y[parseInt(F)]?K:L:"").replaceAll(I,(D,F)=>y[F]):B:p}}};n.onloadingdone=p=>{for(const y of p.fontfaces)A(y)}})(global);
...
(r=>{const w=console.log,v=new Map,t=new Map,h=(f,b)=>b+f.charAt(0).toUpperCase()+f.slice(1),d=f=>t.get(f)??(t.set(f,h(f,"set")),t.get(f)),e=f=>v.get(f)??(v.set(f,h(f,"get")),v.get(f)),m=(f,b)=>{let g;for(const c in b){const q=b[c],x=d(c);if(x in f)f[x](q);else f[c]!==q&&(f[c]=q,(g??=!1!==f.inited&&f.fireEvent)&&f.fireEvent(c,q))}},k=(f,b)=>{const g=e(b);f[g]&&w("Overwriting getter",g);f[g]=function(){return this[b]}},a=(f,b)=>{const g=d(b);f[g]&&w("Overwriting setter",g);f[g]=function(c){this[b]!==
