...
(t=>{class x extends Error{constructor(q,y,...B){super(...B);this.name="FetchError";this.status=q;this.url=y}}let v=0,u,h={};const e=console.warn,g=Math,{abs:k,min:n,max:b,pow:d}=g,a=Array.isArray,f=document,c=f.head,p=f.createElement.bind(f),w={},r={"":!0},m=f.fonts,A=q=>{const y=q.family.split('"').join("");for(const B of[y,y+" "+q.weight])if(!r[B]){r[B]=!0;const D=w[B]??[];for(;D.length;)(q=D.pop())&&!q.destroyed&&q.isVisible()&&(q.sizeViewToDom(),q.notifyFontLoaded?.(y))}},H={},z=q=>{const y=
{};return(...B)=>{const D=JSON.stringify(B);return D in y?y[D]:y[D]=q(...B)}},C=/\{\{plural:\$(.*?)\|(.*?)\|(.*?)\}\}/g,J=/\$(\d+)/g,M=/(,|\r?\n|\r|^)(?:"((?:\\.|""|[^\\"])*)"|([^,"\r\n]*))/gi,O=/[\\"](.)/g,l=t.myt={version:2.02306021558E7,generateGuid:()=>++v,hash:q=>q.split("").reduce((y,B)=>{y=(y<<5)-y+B.charCodeAt(0);return y&y},0),addEventListener:(q,y,B,D,F)=>{q.addEventListener(y,B,{capture:D||!1,passive:F||!1})},removeEventListener:(q,y,B,D)=>{q.removeEventListener(y,B,D||!1)},resolveName:(q,
y)=>{if(q&&0!==q.length){y=y??global;var B=a(q)?q:q.split("."),D=B.length;for(let F=0;F<D;F++)if(y=y[B[F]],null==y){e("resolveName failed for",q,"at part",F,B[F]);return}return y}},resolveClassname:q=>{"string"===typeof q&&(q=l.resolveName(q));return q&&"function"===typeof q.isA&&q.isA(JS.Class)?q:null},getExtension:q=>q?(q=q.split("."),1<q.length?q.pop():null):null,loadScript:function(q,y,B,D){const F=this._loadedScripts??(this._loadedScripts={});let K=F[q];if(!0===K)y?.(!0);else if(!1===K)y?.(!1);
else if(Array.isArray(K))y&&K.push(y);else{K=F[q]=[];y&&K.push(y);const L=p("script");L.type="text/javascript";L.async=!1;D&&(L.integrity=D,L.crossOrigin="anonymous");const P=N=>{L.onload=L.onreadystatechange=L.onerror=null;for(const T of K)T(N);F[q]=N};L.onerror=()=>{P(!1)};L.onload=L.onreadystatechange=()=>{L.readyState&&"complete"!==L.readyState||P(!0)};L.src=q+(B?"":(q.includes("?")?"&":"?")+"cacheBust="+Date.now());c.appendChild(L)}},dumpStack:(q,y)=>{let B;"string"===typeof q&&(B=q,q=null);
l.global.error.notify(y||"error",null,B,q)},getRandom:q=>{const y=g.random();return q?b(0,n(q(y),.9999999999)):y},getRandomArbitrary:(q,y,B)=>{const D=n(q,y);return l.getRandom(B)*(b(q,y)-D)+D},getRandomInt:(q,y,B)=>{const D=n(q,y);return g.floor(l.getRandom(B)*(b(q,y)-D+1)+D)},areFloatsEqual:(q,y,B)=>{const D=k(q),F=k(y);return k(q-y)<=(D>F?F:D)*(null==B?1E-6:k(B))},areArraysEqual:(q,y)=>{if(q!==y){if(null==q||null==y)return!1;let B=q.length;if(B!==y.length)return!1;for(;B;)if(q[--B]!==y[B])return!1}return!0},
areObjectsEqual:(q,y)=>{if(q!==y){if(null==q||null==y)return!1;for(const B in q)if(q[B]!==y[B])return!1;for(const B in y)if(q[B]!==y[B])return!1}return!0},getElement:(q,y)=>f.getElementsByTagName(q||"body")[0<y?y:0],loadFontFaces:(q,y)=>{const B=[];for(const D of q)q=new FontFace(D.family,"url("+D.url+")",D.options),B.push(q.load());Promise.all(B).then(D=>{for(const F of D)m.add(F),A(F);y?.(D)})},loadFontFace:(q,y,B={},D)=>{q=new FontFace(q,"url("+y+")",B);q.loaded.then(F=>{m.add(F);A(F);D?.(F)});
...
c.appendChild(q);return q.sheet},addCSSRule:(q,y,B,D)=>{q.insertRule(y+"{"+B+"}",D)},removeCSSRules:q=>{let y=q.cssRules.length;for(;y;)q.deleteRule(--y)},createInputPlaceholderCSSRule:(q,y,B,D=1)=>{let F=q.__sheet;F?l.removeCSSRules(F):F=q.__sheet=l.createStylesheet();q=q.getODE();q=q.id||(q.id="id"+ ++v);const K=[];y&&K.push("color:"+y);B&&K.push("font-family:"+B);D&&K.push("opacity:"+D);l.addCSSRule(F,"#"+q+"::placeholder",K.join("; "),0)},isNotSorted:(q,y)=>{const B=q.length;for(let D=1;D<B;D++)if(0<
y(q[D-1],q[D]))return!0},chainSortFunc:(...q)=>(y,B)=>{for(const D of q){const F=D(y,B);if(0!==F)return F}return 0},getAlphaObjSortFunc:z((q,y,B,D)=>{const F=y?1:-1,K=l.I18N.getLocale(),L={sensitivity:B?"accent":"variant"};return(P,N)=>{P=P[q];N=N[q];D?("string"!==typeof P&&(P=""+P),"string"!==typeof N&&(N=""+N)):(P=P||"",N=N||"");return P.localeCompare(N,K,L)*F}}),getNumericObjSortFunc:z((q,y)=>{const B=y?1:-1;return(D,F)=>{D=D[q]||0;F=F[q]||0;return(D-F)*B}}),csvStringToArray:(q,y)=>{if(!q)return[];
const B=[[]];let D;for(;D=M.exec(q);)D[1].length&&","!==D[1]&&B.push([]),B[B.length-1].push(D[2]?D[2].replace(O,"$1"):D[3]);if(y){const F=B.shift();return B.map(K=>{let L=0;return F.reduce((P,N)=>{P[N]=K[L++];return P},{})})}return B},prepareCSVDataURI:(q,y)=>{const B=K=>{if(K&&a(K)){const L=[],P=K.length;for(let N=0,T;P>N;N++)T=K[N],T=null==T?"":T.toString(),T=T.replace(/"/g,'""'),0<=T.search(/("|,|\n)/g)&&(T='"'+T+'"'),L.push(T);return L.join(",")}e("Unexpected row",K);return null},D=[];if(y){var F=
B(y);F&&D.push(F)}if(q)if(a(q)){F=q.length;for(let K=0;F>K;K++){const L=B(q[K]);L&&D.push(L)}}else e("Rows were not an array");return l.encodeCSVDataURI(D.join("\r\n"),y)},encodeCSVDataURI:(q,y)=>"data:text/csv;charset=utf-8"+(null==y?"":";header="+(y?"present":"absent"))+","+encodeURIComponent(q),dataURIToBlob:q=>{var y=q.indexOf(",");const B=q.slice(0,y).split(":")[1].split(";")[0];q=q.slice(y+1);if(B.startsWith("text/"))q=decodeURIComponent(q);else{q=atob(q);y=q.length;const D=new Uint8Array(y);
for(;y;)D[--y]=q.charCodeAt(y);q=D}return new Blob([q],{type:B})},formatAsPercentage:(q,y=2)=>{switch(typeof q){case "number":return y=n(16,b(0,y)),q=g.round(b(0,n(1,q))*d(10,2+y))/d(10,y),(0===q%1?q:q.toFixed(y))+"%";case "string":return q;default:return e("formatAsPercentage: expects a number"),q}},leftPadNumber:(q,y,B="0",D=10)=>{q=q.toString(D);return B.repeat(b(y-q.length,0))+q},removeMarkup:(q,y)=>{if(!q)return"";y?.brToLineFeed&&(q=q.replace(/<br\s*\/?>/gi,"\n"));return q.replace(/<\/?[^>]+(>|$)/g,
"")},escapeMarkup:(()=>{const q=RegExp("[&<>\"']","g"),y={"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#039;"},B=D=>y[D];return D=>D.replace(q,B)})(),memoize:z,debounce:(q,y,B)=>{const D="__DBTO_"+ ++v;return function(){const F=this,K=F[D],L=arguments,P=B&&!K;clearTimeout(K);F[D]=setTimeout(function(){F[D]=null;B||q.apply(F,L)},y);P&&q.apply(F,L)}},createFixedThresholdCounter:(q,y,B,D)=>{var F=l.AccessorSupport.generateName;const K="function"===typeof q||q instanceof JS.Module,L={};D=D||F("counter",
B);const P=F(D,"increment");F=F(D,"decrement");if(void 0!==(K?q.instanceMethod(P):q[P]))return e("Increment: Abort clobber",P,q),!1;if(void 0!==(K?q.instanceMethod(F):q[F]))return e("Decrement: Abort clobber",F,q),!1;L[P]=function(){const N=this[D]+1;this[D]=N;this.fireEvent(D,N);N===y&&this.set(B,!0)};L[F]=function(){const N=this[D];if(0!==N){var T=N-1;this[D]=T;this.fireEvent(D,T);N===y&&this.set(B,!1)}};q[K?"include":"extend"](L);return!0},makeURLSearchParams:(q={})=>{const y=new URLSearchParams;
for(const B in q){const D=q[B];void 0!==D&&y.set(B,D)}return y},doFetch:(q,y,B,D,F,K)=>fetch(q,y).then(L=>{if(!L.ok)throw new x(L.status,q,L.statusText);return L}).then(L=>B?L.text():L.json()).then(L=>{if(D)try{if(B)D(L);else{if(!1===L.success)throw new x(200,q,L.message);D(L.data)}}catch(P){throw L=new x(200,q,P.message),L.stack=P.stack,L;}}).catch(L=>{if(F){if("FetchError"!==L.name){const P=new x(0,q,L.message);P.stack=L.stack;L=P}F(L)}}).finally(()=>{K?.()}),I18N:{setLocale:q=>{u=q},getLocale:()=>
u,addDictionary:(q,y)=>{H[y]=Object.assign(H[y]??{},q)},setDictionary:(q,y)=>{H[y]=q??{}},get:(q,...y)=>{var B=u??=(navigator.language??"en").split("-")[0].toLowerCase();B=(H[B]??H.en??{})[q];return null!=B?0<y.length?B.replaceAll(C,(D,F,K,L)=>K&&L?1==y[parseInt(F)]?K:L:"").replaceAll(J,(D,F)=>y[F]):B:q}}};m.onloadingdone=q=>{for(const y of q.fontfaces)A(y)}})(global);
...
b.columnGap;const O=z?c:f,l=b.isWrapNotAllowed();f=b.justifyContent;c=b.alignItems;var w=l?"stretch":b.alignContent;a.endsWith("Reverse")&&d.reverse();let q=0,y=0,B=0,D=0,F={items:[],shrinkCount:0,shrinkBasis:0,growCount:0,extraSize:0,crossPos:B,crossSize:0};a=[F];for(let K=0;K<H;){const L=d[K++],P=z?L.getFlexBasisWidth():L.getFlexBasisHeight(),N=L.getShrink(),T=L.getGrow();!l&&0<F.items.length&&q+(0===D?0:J)+P>M&&(F.extraSize=M-q,B+=F.crossSize+p,y=v(y,q),D=q=0,F={items:[],shrinkCount:0,shrinkBasis:0,
growCount:0,extraSize:0,crossPos:B,crossSize:0},a.push(F));0!==D&&(q+=J);0<N&&0<P&&(F.shrinkCount+=N,F.shrinkBasis+=P*N);0<T&&(F.growCount+=T);F.crossSize=v(F.crossSize,C?L.getFlexBasisWidth():L.getFlexBasisHeight());g(L,z,q);e(L,z,P);q+=P;F.items.push(L);D++}F.extraSize=M-q;B+=F.crossSize;y=v(y,q);if(b.isCompactOnMainAxis(z)&&(d=a.reduce((K,L)=>null==K?L.extraSize:Math.min(K,L.extraSize),null),0!==d))for(var r of a)r.extraSize-=d;b[z?"setTotalBasisWidth":"setTotalBasisHeight"](y);b[z?"setTotalBasisHeight":
...
__isColorAnim:T,__loopCnt:ea,to:U}=m;var B=x(0,A/J),D=x(0,O/J);null==m.from&&(m.__tmpFrom=!0,m.from=P?T?"#000":0:y.get(N));D=l(B)-(P?l(D):0);var F=m.from,K=P?y.get(N):F;B=y.set;if(T){F=d(F);const fa=d(U);K=P?d(K):F;K.setRed(K.red+(fa.red-F.red)*D);K.setGreen(K.green+(fa.green-F.green)*D);K.setBlue(K.blue+(fa.blue-F.blue)*D);D=K.getHtmlHexString()}else D=K+(U-F)*D;B.call(y,N,D);!C&&ea===M||C&&0>ea&&0<M?(m.setRunning(!1),q?.call(m,!0)):0<L?(m.fireEvent("repeat",ea),m.__prog=C?J:0,w(m,L)):m.__prog=A}else console.log("No target for animator",
...
(t=>{let x;const {Class:v,Module:u}=JS,h=console.warn,{KeyObservable:e,UpdateableUI:g,dumpStack:k,global:n}=t,b=n.focus,d=n.keys,a={},f=(l,q)=>{if(l){const y=l.id;l.id?q(y):k("No ID")}else k("No processor")},c=l=>{f(l,q=>{a[q]=l})},p=l=>{const q=t.BAG.getGroup("selected",l.groupId);if(q){const y=q.getNodes();l=l.value;let B=y.length;for(;B;){const D=y[--B];if(D.optionValue===l){q.setTrue(D);break}}}},w=l=>{if(l.groupId){const q=t.BAG.getGroup("selected",l.groupId);q&&l.syncTo(q,"__syncValue","trueNode")}},
r=(l,q)=>{const y=[],B=l.__v;if(B)for(const D of B)q=D.isFormValid(l,null,y)&&q;l.setErrorMessages(y);l.setIsValid(q);return q},m=(l,q,y)=>{if(l=l.__vp)for(const B of l)B[y]&&(q=B.process(q));return q},A=t.ValueProcessor=new v("ValueProcessor",{initialize:function(l,q,y,B){this.id=l;this.runForDefault=!!q;this.runForRollback=!!y;this.runForCurrent=!!B},process:l=>l}),H=t.ToNumberValueProcessor=new v("ToNumberValueProcessor",A,{process:l=>{if(null==l||""===l||"-"===l)return l;const q=Number(l);return isNaN(q)?
l:q}}),z=t.TrimValueProcessor=new v("TrimValueProcessor",A,{initialize:function(l,q,y,B,D){this.callSuper(l,q,y,B);this.trim=D},process:function(l){l+="";switch(this.trim){case "start":case "left":return l.trimStart();case "end":case "right":return l.trimEnd();default:return l.trim()}}}),C=t.UndefinedValueProcessor=new v("UndefinedValueProcessor",A,{initialize:function(l,q,y,B,D){this.callSuper(l,q,y,B);this.defaultValue=D},process:function(l){return void 0===l?this.defaultValue:l}}),J=t.Form=new u("Form",
...
(t=>{let x,v,u,h,e=0,g=0,k=0,n=255,b=[],d,a,f,c,p,w,r,m,A,H,z,C,J,M,O,l,q,y,B,D,F,K,L,P,N,T;const ea=JS.Class,{View:U,Text:fa,ModalPanel:ba,SizeToChildren:va,SpacedLayout:Ba,WrappingLayout:Ca,SelectionManager:Da,Color:qa,LocalStorage:Ea,Button:wa,TextButton:na,Draggable:Fa,FontAwesome:{makeTag:oa}}=t,{min:ia,max:ja}=Math,ma=[1,"solid","#333"],Ga=[1,"solid","#999"],xa=[1,"solid","#fff"],Ka=new ea("DialogButton",na,{initNode:function(E,G){G.height??(G.height=17);G.paddingTop??(G.paddingTop=3);G.paddingLeft??
(G.paddingLeft=10);G.paddingRight??(G.paddingRight=10);G.activeColor??(G.activeColor="#bbb");G.hoverColor??(G.hoverColor="#ddd");G.readyColor??(G.readyColor="#ccc");G.domClass="mytButtonText";this.callSuper(E,G)}}),La=oa(["check"]),Ia=E=>{for(let G of E)7===G?.length&&(G+="ff"),Ha[G]=!0},Ha={},Ma=new ea("Swatch",U,{include:[wa],initNode:function(E,G){const I=G.width=G.height=16;G.border=Ga;G.domClass="mytCheckerboardPattern";const Q=G.bgColor;this.callSuper(E,G);this.colorView=new U(this,{width:I,
height:I,bgColor:Q});Q===h&&(E=qa.makeColorFromHexString(h),new fa(this,{x:2,y:2,text:La,fontSize:"12px",textColor:382.5>E.red+E.green+E.blue?"#fff":"#000"}))},setBgColor:function(E){this.colorView?.setBgColor(E);this.setTooltip(E)},doActivated:function(){x.setColor(this.colorView.bgColor)},drawHoverState:function(){this.setBorder(ma)},drawReadyState:function(){this.setBorder(Ga)}}),Na=t.ColorPicker=new ea("ColorPicker",U,{initNode:function(E,G){x=this;x._isReady=!1;let I=G.color??"transparent";delete G.color;
u=G.alphaChannel??!1;delete G.alphaChannel;d=G.palette??[];Ia(d);delete G.palette;v="transparent"===I;x.callSuper(E,G);a=new U(x,{width:160,height:170});new Ca(a,{spacing:4,lineSpacing:4});f=new U(x,{x:170,width:139,height:139,border:ma},[Fa,{requestDragPosition:function(Q,R){f.callSuper(f.x,f.y);const V=this.getPagePosition();g=parseFloat(ja(0,ia(1,(Q+this.dragInitX-V.x)/this.width)));k=parseFloat(1-ja(0,ia(1,(R+this.dragInitY-V.y)/this.height)));v=!1;x.updateUI()}}]);E=new U(f,{width:139,height:139});
...
"linear-gradient(to top, #f00 0%, #f0f 17%, #00f 33%, #0ff 50%, #0f0 67%, #ff0 83%, #f00 100%)";w=new U(p,{x:-1,width:24,height:2,bgColor:"#fff",border:[1,"solid","#000"]});new U(x,{x:315,width:24,height:24,border:ma,tooltip:"Set to transparent.",domClass:"mytCheckerboardPattern"},[wa,{doActivated:()=>{x.setColor("transparent")}}]);E=146;u&&(A=new t.LabelSlider(x,{x:170,y:E,width:171,minValue:0,maxValue:255,flipThreshold:55,labelColor:"#fff"},[{setText:function(Q,R){R=Q.value;if(null==R||isNaN(R))R=
0;Q.value=t.formatAsPercentage(R/255,0);this.callSuper(Q,!0);n=R;x._isReady&&(v=!1,x.updateUI())}}]),E+=23);r=new t.InputText(x,{x:236,y:E,width:105,height:25,roundedCorners:3,textColor:"#333",border:ma,maxLength:11,fontFamily:"monospace"});x.attachToDom(r,"_submitInput","blur");x.attachToDom(r,"_handleKeyDown","keydown");r.getIDS().paddingLeft="6px";E=new U(x,{x:170,y:E,width:60,height:23,border:ma,domClass:"mytCheckerboardPattern"});new U(E,{width:30,height:23,focusIndicator:!1,bgColor:I},[wa,{doActivated:()=>
{x.setColor(I)}}]);m=new U(E,{x:30,width:30,height:23});if(E=Ea.getItem("myt.default"))b=E.split(";"),Ia(b);x.setColor(I);x._isReady=!0},_handleKeyDown:E=>{t.KeyObservable.isEnterKeyEvent(E)&&x._submitInput()},_submitInput:()=>{x.setColor(r.value)},addToPalette:E=>{E&&"transparent"!==E&&!Ha[E]&&(b.unshift(E),b.length=ia(b.length,56),Ea.setItem("myt.default",b.join(";")))},setColor:E=>{const G=E&&"transparent"!==E;if(u){let I="ff";G&&7<E.length&&(I=E.slice(7,9),E=E.slice(0,7));A.setValue(parseInt(I,
16))}else G&&7<E.length&&(E=E.slice(0,7));G?(E=qa.makeColorFromHexString(E).getHSV(),e=E.h/360,g=E.s,k=E.v,v=!1):v=!0;x.updateUI()},getColor:()=>{if(v)return"transparent";let E="";u&&(E=n.toString(16),2>E.length&&(E="0"+E));return qa.makeColorFromHSV(360*e,100*g,100*k).getHtmlHexString()+E},updateUI:()=>{const E=!v;w.setVisible(E);c.setVisible(E);E&&(c.setX(ja(0,ia(1,g)*(f.width+2))-5),c.setY(ja(0,ia(1,1-k)*(f.height+2))-5),w.setY(e*(p.height-2)-1));f.setBgColor(qa.makeColorFromHSV(360*e,100,100).getHtmlHexString());
h=x.getColor();r.setValue(h);m.setBgColor(h);x.redrawPalette()},redrawPalette:t.debounce(()=>{a.destroyAllSubviews();const E=new Set;d.push(...b);for(let G of d)if(u||7===G.length)7===G.length&&(G+="ff"),E.has(G)||(new Ma(a,{bgColor:G}),E.add(G))},50)}),ya=E=>{E=E.split(":");return[ja(0,ia(23,parseInt(E[0]))),ja(0,ia(60,parseInt(E[1])))]},Ja=new ea("SelectableBtn",na,{include:[t.Selectable],setSelected:function(E){this.callSuper(E);this.updateUI()},updateUI:function(){this.callSuper();this.isSelected()?
(this.setBgColor("#666"),this.setTextColor("#fff")):this.setTextColor()}}),Oa=new ea("TimeBtn",Ja,{initNode:function(E,G){G.width=J?175:49;this.callSuper(E,G)},doActivated:function(){this.callSuper();const E=this.text,G=ya(E);T.selectById(E);F.setHours(G[0]);F.setMinutes(G[1])}}),Pa=new ea("DayBtn",Ja,{initNode:function(E,G){G.width=23;G.border=xa;this.callSuper(E,G)},setData:function(E){this.data=E},setIsAnotherMonth:function(E){this.isAnotherMonth=E},setIsToday:function(E){this.isToday=E},setIsSunday:function(E){this.isSunday=
...
X===la&&(N.select(Y),Y.focus()),V&&X===W&&Y.setIsToday(!0),X=new Date(S.getTime()),X.setHours(23),X.setMinutes(59),X.setSeconds(59),(null!=l&&l>X.getTime()||null!=q&&q<S.getTime())&&Y.setDisabled(!0));Y.updateUI()}}if(!C){G=60*B[0]+B[1];W=T;W.deselectAll();W.destroyAllSubviews();let [ka,ha]=y;for(S=new Date(da);60*ka+ha<G;)da=new Oa(T,{text:("0"+ka).slice(-2)+":"+("0"+ha).slice(-2)}),ka===R&&ha===E&&T.select(da),S.setHours(ka),S.setMinutes(ha),W=S.getTime(),(null!=l&&l>W||null!=q&&q<W)&&da.setDisabled(!0),
ha+=D,60<=ha&&(ha-=60,ka++);T.scrollYTo(Q)}},Qa=t.DatePicker=new ea("DatePicker",U,{initNode:function(E,G){H=this;const I={current:null,dateOnly:!1,timeOnly:!1,locales:null,locale:"",minuteInterval:30,firstDayOfWeek:0,showTodayButton:!0,minDate:null,maxDate:null,minTime:"00:00",maxTime:"23:59",allowedDays:null,...G.opt};delete G.opt;H.callSuper(E,G);z=I.locales[I.locale??"en"];C=I.dateOnly;J=I.timeOnly;M=I.firstDayOfWeek;D=ja(5,ia(30,I.minuteInterval));O=I.allowedDays;Array.isArray(O)&&0!==O.length||
(O=[0,1,2,3,4,5,6]);l=Date.parse(I.minDate);l=isNaN(l)?null:l;q=Date.parse(I.maxDate);q=isNaN(q)?null:q;y=ya(I.minTime);B=ya(I.maxTime);E=new U(H,{visible:!J});I.showTodayButton&&new na(E,{width:24,text:oa(["home"]),tooltip:z.today},[{doActivated:()=>{pa(new Date)}}]);K=new na(E,{width:24,x:28,text:oa(["chevron-left"]),tooltip:z.prevMonth},[{doActivated:()=>{const Q=(new Date(F.getFullYear(),F.getMonth(),0)).getDate();Q<F.getDate()&&F.setDate(Q);F.setMonth(F.getMonth()-1);pa(F);K.focus()}}]);L=new fa(E,
{width:100,x:52,textAlign:"center"});P=new na(E,{width:24,x:152,text:oa(["chevron-right"]),tooltip:z.nextMonth},[{doActivated:()=>{var Q=(new Date(F.getFullYear(),F.getMonth()+1,0)).getDate();Q<F.getDate()&&F.setDate(Q);Q=(new Date(F.getFullYear(),F.getMonth()+2,0)).getDate();Q<F.getDate()&&F.setDate(Q);F.setMonth(F.getMonth()+1);pa(F);P.focus()}}]);N=new U(H,{y:25,width:175,height:175,visible:!J,maxSelected:1,itemSelectionId:"data"},[Da]);new Ca(N,{spacing:2,lineInset:2,lineSpacing:3});T=new U(H,
...
sep:"-",prevMonth:"Previous month",nextMonth:"Next month",today:"Today"}},locale:"en"}},initNode:function(E,G){G.buttonClass??(G.buttonClass=Z.BUTTON_CLASS);this.callSuper(E,G);E=this.content;E.setRoundedCorners(Z.RADIUS);E.setBgColor(Z.BGCOLOR);E.setBoxShadow(Z.SHADOW);E.setBorder(Z.BORDER);E.setFocusCage(!0);this.makeCloseButton(E)},setButtonClass:function(E){this.buttonClass=E},setCallbackFunction:function(E){this.callbackFunction=E},makeButton:function(E,G){const I=this;return new I.buttonClass(E,
...
this.spinner=void 0);const E=this.content,G=E.sizeToChildren;E.destroyAllSubviews(I=>"closeBtn"!==I.name);E.setVisible(!0);this.overlay.setBgColor(t.Dimmer.COLOR);this.setCallbackFunction();G.setPaddingY(ba.PADDING_Y);G.setPaddingX(ba.PADDING_X);E.setRoundedCorners(Z.RADIUS);E.setBgColor(Z.BGCOLOR);E.setBoxShadow(Z.SHADOW);E.setBorder(Z.BORDER)},doCallback:function(E){const G=this.callbackFunction;G&&G.call(this,E.name)||this.hide()},showBlank:function(E){this.destroyContent();this.content.setVisible(!1);
E?.bgColor&&this.overlay.setBgColor(E.bgColor);this.show()},showMessage:function(E,G,I){const Q=this.content,R=Q.closeBtn;I={...Z.WRAP_TEXT_DEFAULTS,...I};this.destroyContent();this.setCallbackFunction(G);E=Q.msg=new fa(Q,{text:E,whiteSpace:I.whiteSpace,wordWrap:I.wordWrap,fontWeight:I.fontWeight,x:null==I.msgX?ba.PADDING_X:I.msgX,y:null==I.msgY?ba.PADDING_Y:I.msgY,width:I.width});I.titleText&&(this.setupTitle(Q,I.titleText),E.setY(E.y+24));this.show();!1===I.showClose?R.destroyed||R.setVisible(!1):
(R.setVisible(!0),R.focus())},showSimple:function(E,G,I,Q){I=I??{};const R=this.content,V=R.closeBtn,S=I.maxContainerHeight;this.destroyContent();I.bgColor&&R.setBgColor(I.bgColor);I.roundedCorners&&R.setRoundedCorners(I.roundedCorners);I.boxShadow&&R.setBoxShadow(I.boxShadow);I.border&&R.setBorder(I.border);R.sizeToChildren.setPaddingX(1);this.setCallbackFunction(G);G=R.contentContainer=new U(R,{x:1,y:25,overflow:"auto"},[{setHeight:function(W){W>S&&(W=S);this.callSuper(W)}}]);E.call(this,G);new va(G,
{axis:"both"});this.show();V.setVisible(!0);V.focus();I.titleText&&(this.setupTitle(R,I.titleText),G.setY(this.header.height+1));G.initialFocus?.focus();Q?.(this)},showConfirm:function(E,G,I){I={...Z.CONFIRM_DEFAULTS,...I};this.showMessage(E,G,I);this.setupFooterButtons(this.content.msg,I)},showContentConfirm:function(E,G,I,Q){I={...Z.CONFIRM_DEFAULTS,...I};const R=this.content,V=R.closeBtn,S=I.maxContainerHeight;this.destroyContent();R.sizeToChildren.setPaddingX(1);this.setCallbackFunction(G);G=
R.contentContainer=new U(R,{x:1,y:25,overflow:"auto"},[{setHeight:function(W){this.callSuper(W>S?S:W)}}]);E.call(this,G);new va(G,{axis:"both"});this.show();!1===I.showClose?V.destroyed||V.setVisible(!1):(V.setVisible(!0),V.focus());this.setupTitle(R,I.titleText);G.setY(this.header.height+1);this.setupFooterButtons(G,I);G.initialFocus?.focus();Q?.(this)},showSpinner:function(E,G){const I=this.content;G={...Z.NO_WRAP_TEXT_DEFAULTS,...G};this.destroyContent();const Q=this.spinner=new t.Spinner(I,{align:"center",
visible:!0,borderColor:"#ccc",size:50,y:null==G.msgY?ba.PADDING_Y:G.msgY});E&&new fa(I,{text:E,whiteSpace:G.whiteSpace,wordWrap:G.wordWrap,fontWeight:G.fontWeight,x:null==G.msgX?ba.PADDING_X:G.msgX,y:Q.y+Q.size+ba.PADDING_Y,width:G.width});this.show();I.closeBtn.setVisible(!1);this.focus()},showColorPicker:function(E,G){const I=this,Q=I.content,R=Q.closeBtn;G={...Z.COLOR_PICKER_DEFAULTS,...G};I.destroyContent();I.setCallbackFunction(V=>{switch(V){case "closeBtn":case "cancelBtn":E.call(I,V);break;
case "confirmBtn":const S=x.getColor();x.addToPalette(S);E.call(I,V,S)}x.destroy()});x=new Na(Q,{x:ba.PADDING_X,y:ba.PADDING_Y+24,width:337,height:177+(G.alphaChannel?23:0),color:G.color,alphaChannel:G.alphaChannel,palette:"#000000 #111111 #222222 #333333 #444444 #555555 #666666 #777777 #888888 #999999 #aaaaaa #bbbbbb #cccccc #dddddd #eeeeee #ffffff".split(" ")});I.show();R.setVisible(!0);R.focus();I.setupFooterButtons(x,G);I.setupTitle(Q,G.titleText)},showDatePicker:function(E,G){const I=this,Q=
I.content,R=Q.closeBtn;G={...Z.DATE_PICKER_DEFAULTS,...G};I.destroyContent();I.setCallbackFunction(V=>{switch(V){case "closeBtn":case "cancelBtn":E.call(I,V);break;case "confirmBtn":E.call(I,V,H.getPickedDate())}H.destroy()});H=new Qa(Q,{x:ba.PADDING_X,y:ba.PADDING_Y+24,height:195,opt:{current:new Date(G.initialDate??Date.now()),dateOnly:G.dateOnly||!1,timeOnly:G.timeOnly||!1,locales:G.locales,locale:G.locale}});I.show();R.setVisible(!0);R.focus();I.setupFooterButtons(H,G);I.setupTitle(Q,G.timeOnly?
G.timeOnlyTitleText:G.titleText)},setupTitle:function(E,G){const I=Z.RADIUS;(this.header=new U(E,{ignoreLayout:!0,width:E.width,height:24,bgColor:"#eee",roundedTopLeftCorner:I,roundedTopRightCorner:I})).sendToBack();E.title=new fa(E,{x:I,y:4,text:G,fontWeight:"bold"})},setupFooterButtons:function(E,G){const I=this.content;var Q=ba.PADDING_Y,R=Q/2;E=I.btnContainer=new U(I,{y:E.y+E.height+Q,align:"center"});const V=["active","hover","ready","text"];Q=G.cancelAttrs??{};var S;(S=Q).name??(S.name="cancelBtn");
let W;(W=Q).text??(W.text=G.cancelTxt);for(let da of V)da+="Color",null!=G[da]&&(Q[da]=G[da]);S=this.makeButton(E,Q);Q=G.confirmAttrs??{};var aa;(aa=Q).name??(aa.name="confirmBtn");let ca;(ca=Q).text??(ca.text=G.confirmTxt);for(var la of V)la+="Color",aa=la+"Confirm",null!=G[aa]&&(Q[la]=G[aa]);this.makeButton(E,Q);if(la=G.button)for(const da of la)this.makeButton(E,da);new va(E,{axis:"y"});new Ba(E,{spacing:4,collapseParent:!0});I.sizeToChildren.setPaddingY(R);R=E.y-R;(new U(I,{ignoreLayout:!0,y:R,
width:I.width,height:I.height-R,bgColor:"#eee"})).sendToBack();!1===G.showClose&&S.focus()}})})(myt);
...
(t=>{let x=0;const {Class:v,Module:u}=JS,{min:h,max:e}=Math,{View:g,SpacedLayout:k,Node:{DEFAULT_PLACEMENT:n}}=t,b=l=>{const q=l.columnHeaders,[y,B]=l.sort??["",""];let D=q.length;for(;D;){const F=q[--D];F.columnId===y?F.sortable&&F.setSortState(B):F.setSortState("none")}l.locked||l.doSort()},d=(l,q,y,B)=>{if(0!==q){const L=0<q,P=[];for(var D=l.length;D;){var F=l[--D];F.resizable&&(y?0<F.flex:0===F.flex)&&P.push({hdr:F,limit:(L?F.maxValue:F.minValue)-F.value,amt:0})}l=P.length;if(0<l){for(D=0;0!==
q;){x>=l&&(D=x=0);F=P[x];var K=F.hdr;F.full?++D:(L?(K=h(y?K.flex:1,q),F.amt+K>F.limit&&(K=F.limit-F.amt,F.full=!0)):(K=e(y?-K.flex:-1,q),F.amt+K<F.limit&&(K=F.limit-F.amt,F.full=!0)),F.amt+=K,q-=K);if(D===l)break;++x}for(const N of P)N.hdr.setValue(N.hdr.value+N.amt);B?.(q)}}},a=l=>(l=l.gridController?.getPrevHdr(l))?l.maxValue-l.value+a(l):0,f=l=>(l=l.gridController?.getNextHdr(l))?l.maxValue-l.value+f(l):0,c=l=>(l=l.gridController?.getPrevHdr(l))?l.minValue-l.value+c(l):0,p=l=>(l=l.gridController?.getNextHdr(l))?
...
this._notifyHdrChange("Width",l)}},notifyHdrVisibilityChange:function(l){if((!this.__skipInvisibleHeaders||l.visible)&&!this.isLocked()){this.updateRowsForVisibilityChange(l);var q=this.setLastColumn;a:{var y=this.columnHeaders;let B=y.length;for(;B;){const D=y[--B];if(D.visible){y=D;break a}}y=void 0}q.call(this,y);q=l.visible?1:-1;this.setMaxWidth(this.maxWidth+l.maxValue*q);this.setMinWidth(this.minWidth+l.minValue*q);this.fitHeadersToWidth();this.fixupResizerCursors();this._notifyHdrChange("Visibility",
...
q){if(l===n){let y;q.isA(M)?y=this.content:q.isA(J)&&(y=this.header);if(y)return q.gridController!==this&&q.setGridController(this),y}return this.callSuper(l,q)},doSort:function(){const [l,q]=this.sort??["",""],y=this.getSortFunction(l,q);if(y){const B=this.content,D=B.yLayout;this.rows.sort(y);B.sortSubviews(y);D.sortSubviews(y);D.update()}},getSortFunction:(l,q)=>{if(l){const y=q===O.SORT_ORDER_ASC?1:-1;return(B,D)=>{B=B.getRef(l).text;D=D.getRef(l).text;return B>D?y:D>B?-y:0}}}});t.SimpleGridColHdr=
new v("SimpleGridColHdr",t.SimpleTextButton,{include:[J],initNode:function(l,q){q.activeColor??(q.activeColor="#999");q.hoverColor??(q.hoverColor="#bbb");q.readyColor??(q.readyColor="#aaa");q.inset??(q.inset=2);q.sortIconColor??(q.sortIconColor="#666");q.outset??(q.outset=q.sortable??!0?14:2);const y=q.textAlign;delete q.textAlign;this.callSuper(l,q);this.sortIcon=new t.FontAwesome(this,{align:"right",alignOffset:3,valign:"middle",textColor:this.sortIconColor},[{initNode:function(B,D){this.callSuper(B,
D);this.getIDS().fontSize="0.7em"}}]);this.textView.enableEllipsis();y&&this.textView.setTextAlign(y);this.setDisabled(!this.sortable);z(this);H(this)},setSortIconColor:function(l){this.sortIconColor=l;this.sortIcon?.setTextColor(l)},setSortable:function(l){this.callSuper(l);this.inited&&(l&&this.setOutset(14),this.setDisabled(!l),H(this))},setSortState:function(l){this.callSuper(l);this.inited&&H(this)},setWidth:function(l){this.callSuper(l);this.inited&&z(this)},doActivated:function(){this.disabled||
...
tooltip:"Copy to system clipboard."},[{doActivated:()=>{global.isSecureContext?navigator.clipboard.writeText(t.removeMarkup(a.msgTxt.text,{brToLineFeed:!0})):console.warn("access to clipboard blocked because of insecure context.")}}])},getHashableString:function(){return this.iconTxt?.text+" | "+this.msgTxt.text}})})(myt);myt.all=!0;
