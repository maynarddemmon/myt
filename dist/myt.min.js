...
rotate:function(m){const d=x.cos(m);m=x.sin(m);const c=this.vectors,h=c.length;for(let k=0;h>k;){const a=c[k]*d-c[k+1]*m,e=c[k]*m+c[k+1]*d;c[k++]=a;c[k++]=e}this._boundingBox=null},rotateAroundOrigin:function(m,d,c){this.translate(-d,-c);this.rotate(m);this.translate(d,c)},getBoundingBox:function(){if(this._boundingBox)return this._boundingBox;const m=this.vectors;let d=m.length,c,h,k,a;if(2<=d){k=a=m[--d];for(c=h=m[--d];d;){const e=m[--d],f=m[--d];k=w(e,k);a=u(e,a);c=w(f,c);h=u(f,h)}return this._boundingBox=
{x:c,y:k,width:h-c,height:a-k}}return this._boundingBox=null},getCenter:function(){const m=this.getBoundingBox();return m?{x:m.x+m.width/2,y:m.y+m.height/2}:null},isPointInPath:function(m,d){"object"===typeof m&&(d=m.y,m=m.x);return t.Geometry.isPointInPath(m,d,this.getBoundingBox(),this.vectors)}})})(myt);
