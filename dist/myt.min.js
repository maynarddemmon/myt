...
(q=>{const v=(g,c)=>{var p=g.__mct__.get(c);if(p)return p.slice();const w=[];if(p=g.__anc__){const t=p.length;for(let n=0;n<t;){const C=p[n++].__fns__;C.has(c)&&w.push(C.get(c))}}else{const t=g.__anc__=[],n=C=>{const K=C.__inc__,z=K.length;for(let D=0;D<z;)n(K[D++]);t.includes(C)||(t.push(C),C=C.__fns__,C.has(c)&&w.push(C.get(c)))};n(g)}g.__mct__.set(c,w.slice());return w},u=(g,c)=>{const p=g.callable;return g.__hs?function(...w){const t=this.callSuper,n=v(c,g.name);let C=n.length-1;if(0===C)t&&delete this.callSuper;
else{const z=w.length,D=this.callSuper=(...J)=>{for(let N=J.length;N<z;)J[N]=w[N++];0===--C&&delete this.callSuper;J=n[C].callable.call(this,...J);this.callSuper=D;C++;return J}}const K=p.call(this,...w);t?this.callSuper=t:delete this.callSuper;return K}:p},r=(g,c)=>{c=c??g;if(c===g){g.__anc__=null;g.__mct__=new Map;var p=g.__dep__;for(var w=p.length;0<w;)r(p[--w])}p=c.__tgt__;w=g.__inc__;const t=w.length;for(let n=0;n<t;)r(w[n++],c);for(const [n,C]of g.__fns__)g=C instanceof d?u(C,c):C,p[n]!==g&&
...
w,n=t.set;var C=g;var K=D,z=p[D];C=z?.__fns__||"function"!==typeof z?z:new d(C,K,z);n.call(t,D,C)}p.constructor=p.klass=g;for(const D in b)g[D]===b[D]||g.hasOwnProperty(D)||(g[D]=b[D]);g.include(c);g.constructor=g.klass=e};k(d,a);k(l,a);k(e,l);r(h(d));r(h(l));h(e).include(l.__meta__);q.Singleton=new e("Singleton",{initialize:(g,c,p)=>new (new e(g,c,p))})})(global.JS={});
(q=>{class v extends Error{constructor(y,A,...E){super(...E);this.name="FetchError";this.status=y;this.url=A}}let u=0,r,h={},f={};const d=console.warn,l=Math,{abs:k,min:a,max:e,pow:b}=l,g=Array.isArray,c=document,p=c.head,w=c.createElement.bind(c),t={},n={"":!0},C=c.fonts,K=y=>{const A=y.family.split('"').join("");for(const E of[A,A+" "+y.weight])if(!n[E]){n[E]=!0;const G=t[E]??[];for(;G.length;)(y=G.pop())&&!y.destroyed&&y.isVisible()&&(y.sizeViewToDom(),y.notifyFontLoaded?.(A))}},z={},D=/\{\{plural:\$(\d+)\|([^|]+)\|([^}]+)\}\}/g,
J=/\$(\d+)/g,N=(y,A=JSON.stringify,E=Infinity)=>{const G=new Map;return(...L)=>{var O=A(L);if(G.has(O))return G.get(O);L=y(...L);G.set(O,L);G.size>E&&(O=G.keys().next().value,G.delete(O));return L}},P=/\\([{}])|\{([^{}]+)\}/g,m=/(,|\r?\n|\r|^)(?:"((?:\\.|""|[^\\"])*)"|([^,"\r\n]*))/gi,x=/[\\"](.)/g,B=q.myt={version:2.02306021558E7,generateGuid:()=>++u,theme:{TextButtonActive:"#ddd",TextButtonHover:"#eee",TextButtonReady:"#fff",DialogRadius:12,DialogBgColor:"#fff",DialogInputFgColor:"#333",DialogShadow:[0,
...
TooltipTextColor:"#eee",TooltipShadowColor:"#00000033",TooltipEdgeSize:0,TooltipShadowSize:2,TooltipHInset:6,TooltipVInset:3,focusShadow:[0,0,7,"#666"],disabledOpacity:.5,border1s3:[1,"solid","#333"],border1s9:[1,"solid","#999"]},hash:y=>y.split("").reduce((A,E)=>{A=(A<<5)-A+E.charCodeAt(0);return A&A},0),addEventListener:(y,A,E,G,L)=>{y.addEventListener(A,E,{capture:G||!1,passive:L||!1})},removeEventListener:(y,A,E,G)=>{y.removeEventListener(A,E,G||!1)},resolveName:(y,A)=>{if(y&&0!==y.length){A=
A??global;var E=g(y)?y:y.split("."),G=E.length;for(let L=0;L<G;L++)if(A=A[E[L]],null==A){d("resolveName failed for",y,"at part",L,E[L]);return}return A}},resolveClassname:y=>{"string"===typeof y&&(y=B.resolveName(y));return y&&"function"===typeof y.isA&&y.isA(JS.Class)?y:null},getExtension:y=>y?(y=y.split("."),1<y.length?y.pop():null):null,loadScript:function(y,A,E,G){const L=this._loadedScripts??(this._loadedScripts={});let O=L[y];if(!0===O)A?.(!0);else if(!1===O)A?.(!1);else if(Array.isArray(O))A&&
O.push(A);else{O=L[y]=[];A&&O.push(A);const M=w("script");M.type="text/javascript";M.async=!1;G&&(M.integrity=G,M.crossOrigin="anonymous");const S=V=>{M.onload=M.onreadystatechange=M.onerror=null;for(const T of O)T(V);L[y]=V};M.onerror=()=>{S(!1)};M.onload=M.onreadystatechange=()=>{M.readyState&&"complete"!==M.readyState||S(!0)};M.src=y+(E?"":(y.includes("?")?"&":"?")+"cacheBust="+Date.now());p.appendChild(M)}},dumpStack:(y,A)=>{let E;"string"===typeof y&&(E=y,y=null);B.global.error.notify(A||"error",
null,E,y)},getRandom:y=>{const A=l.random();return y?e(0,a(y(A),.9999999999)):A},getRandomArbitrary:(y,A,E)=>{const G=a(y,A);return B.getRandom(E)*(e(y,A)-G)+G},getRandomInt:(y,A,E)=>{const G=a(y,A);return l.floor(B.getRandom(E)*(e(y,A)-G+1)+G)},areFloatsEqual:(y,A,E)=>{const G=k(y),L=k(A);return k(y-A)<=(G>L?L:G)*(null==E?1E-6:k(E))},areArraysEqual:(y,A)=>{if(y!==A){if(null==y||null==A)return!1;let E=y.length;if(E!==A.length)return!1;for(;E;)if(y[--E]!==A[E])return!1}return!0},areObjectsEqual:(y,
A)=>{if(y!==A){if(null==y||null==A)return!1;for(const E in y)if(y[E]!==A[E])return!1;for(const E in A)if(y[E]!==A[E])return!1}return!0},getElement:(y,A)=>c.getElementsByTagName(y||"body")[0<A?A:0],loadFontFaces:(y,A)=>{Promise.all(y.map(({family:E,url:G,options:L})=>(new FontFace(E,"url("+G+")",L)).load())).then(E=>{for(const G of E)C.add(G),K(G);A?.(E)})},loadFontFace:(y,A,E={},G)=>{y=new FontFace(y,"url("+A+")",E);y.loaded.then(L=>{C.add(L);K(L);G?.(L)});y.load()},registerForFontNotification:(y,
A)=>{n[A]?y&&!y.destroyed&&y.isVisible()&&(y.sizeViewToDom(),y.notifyFontLoaded?.(A)):(t[A]??(t[A]=[])).push(y)},createBaseFontCSSRule:y=>{B.addCSSRule(B.createStylesheet(),"body, input","font-family:"+y)},loadCSSFonts:y=>{y?.forEach(B.createStylesheetLink)},createStylesheetLink:y=>{if(y){let A=f[y];A?console.warn("stylesheet already exists",y):(A=f[y]=w("link"),A.rel="stylesheet",A.href=y,p.appendChild(A));return A}},createStylesheet:()=>{const y=w("style");p.appendChild(y);return y.sheet},addCSSRule:(y,
A,E,G)=>{y.insertRule(A+"{"+E+"}",G??0)},removeCSSRules:y=>{let A=y.cssRules.length;for(;A;)y.deleteRule(--A)},createInputPlaceholderCSSRule:(y,A,E,G=1)=>{let L=y.__sheet;L?B.removeCSSRules(L):L=y.__sheet=B.createStylesheet();y=y.getODE();y=y.id||(y.id="id"+ ++u);const O=[];A&&O.push("color:"+A);E&&O.push("font-family:"+E);G&&O.push("opacity:"+G);B.addCSSRule(L,"#"+y+"::placeholder",O.join("; "))},isSorted:(y,A)=>{const E=y.length;for(let G=1;G<E;G++)if(0<A(y[G-1],y[G]))return!1;return!0},chainSortFunc:(...y)=>
(A,E)=>{for(const G of y){const L=G(A,E);if(0!==L)return L}return 0},getAlphaObjSortFunc:N((y,A,E,G)=>{const L=A?1:-1,O=B.I18N.getLocale(),M={sensitivity:E?"accent":"variant"};return(S,V)=>{S=S[y];V=V[y];G?("string"!==typeof S&&(S=""+S),"string"!==typeof V&&(V=""+V)):(S=S||"",V=V||"");return S.localeCompare(V,O,M)*L}}),getNumericObjSortFunc:N((y,A)=>{const E=A?1:-1;return(G,L)=>{G=G[y]||0;L=L[y]||0;return(G-L)*E}}),csvStringToArray:(y,A)=>{if(!y)return[];const E=[[]];let G;for(;G=m.exec(y);)G[1].length&&
","!==G[1]&&E.push([]),E[E.length-1].push(void 0!==G[2]?G[2].replace(x,"$1"):G[3]);if(A){const L=E.shift();return E.map(O=>{let M=0;return L.reduce((S,V)=>{S[V]=O[M++];return S},{})})}return E},prepareCSVDataURI:(y,A)=>{const E=O=>{if(O&&g(O)){const M=[],S=O.length;for(let V=0,T;S>V;V++)T=O[V],T=null==T?"":T.toString(),T=T.replace(/"/g,'""'),0<=T.search(/("|,|\n)/g)&&(T='"'+T+'"'),M.push(T);return M.join(",")}d("Unexpected row",O);return null},G=[];if(A){var L=E(A);L&&G.push(L)}if(y)if(g(y)){L=y.length;
for(let O=0;L>O;O++){const M=E(y[O]);M&&G.push(M)}}else d("Rows were not an array");return B.encodeCSVDataURI(G.join("\r\n"),A)},encodeCSVDataURI:(y,A)=>"data:text/csv;charset=utf-8"+(null==A?"":";header="+(A?"present":"absent"))+","+encodeURIComponent(y),dataURIToBlob:y=>{var A=y.indexOf(",");const E=y.slice(0,A).split(":")[1].split(";")[0];y=y.slice(A+1);if(E.startsWith("text/"))y=decodeURIComponent(y);else{y=atob(y);A=y.length;const G=new Uint8Array(A);for(;A;)G[--A]=y.charCodeAt(A);y=G}return new Blob([y],
{type:E})},formatAsPercentage:(y,A=2)=>{switch(typeof y){case "number":return A=a(16,e(0,A)),y=l.round(e(0,a(1,y))*b(10,2+A))/b(10,A),(0===y%1?y:y.toFixed(A))+"%";case "string":return y;default:return d("formatAsPercentage expects a number"),y}},interpolateString:(y,A={})=>y?y.replace(P,(E,G,L)=>G?G:B.resolveName(L,A)??E):"",leftPadNumber:(y,A,E="0",G=10)=>{y=y.toString(G);return E.repeat(e(A-y.length,0))+y},removeMarkup:(y,A)=>{if(!y)return"";A?.brToLineFeed&&(y=y.replace(/<br\s*\/?>/gi,"\n"));return y.replace(/<\/?[^>]+(>|$)/g,
"")},escapeMarkup:(()=>{const y=RegExp("[&<>\"']","g"),A={"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#039;"},E=G=>A[G];return G=>G.replace(y,E)})(),memoize:N,debounce:(y,A,E=!1)=>{let G;return function(...L){const O=this,M=E&&!G;clearTimeout(G);G=setTimeout(()=>{G=null;E||y.apply(O,L)},A);M&&y.apply(O,L)}},createFixedThresholdCounter:(y,A,E,G)=>{var L=B.AccessorSupport.generateName;const O="function"===typeof y||y instanceof JS.Module,M={};G=G||L("counter",E);const S=L(G,"increment");L=
L(G,"decrement");if(void 0!==(O?y.instanceMethod(S):y[S]))return d("Increment: Abort clobber",S,y),!1;if(void 0!==(O?y.instanceMethod(L):y[L]))return d("Decrement: Abort clobber",L,y),!1;M[S]=function(){const V=this[G]+1;this[G]=V;this.fireEvent(G,V);V===A&&this.set(E,!0)};M[L]=function(){const V=this[G];if(0!==V){var T=V-1;this[G]=T;this.fireEvent(G,T);V===A&&this.set(E,!1)}};y[O?"include":"extend"](M);return!0},makeURLSearchParams:(y={})=>{const A=new URLSearchParams;for(const E in y){const G=y[E];
void 0!==G&&A.set(E,G)}return A},doFetch:(y,A,E,G,L,O)=>fetch(y,A).then(M=>{if(!M.ok)throw new v(M.status,y,M.statusText);return M}).then(M=>E?M.text():M.json()).then(M=>{if(G)try{if(E)G(M);else{if(!1===M.success)throw new v(200,y,M.message??M.data?.message);G(M.data)}}catch(S){throw M=new v(200,y,S.message),M.stack=S.stack,M;}}).catch(M=>{if(L){if("FetchError"!==M.name){const S=new v(0,y,M.message);S.stack=M.stack;M=S}L(M)}}).finally(()=>{O?.()}),toNameCase:(y,A)=>{if(!y)return"";y=y.trim().toLowerCase().split(/([\s\-'\u2019"\u201c\u201d().,\/])/).reduce((E,
G)=>E+(G[0]??"").toUpperCase()+G.slice(1),"");if(/\bMac[A-Za-z]{2,}[^aciozj]\b/.test(y)||/\bMc/.test(y))y=y.replace(/\b(Ma?c)([A-Za-z]+)/,(E,G,L)=>G+(L[0]??"").toUpperCase()+L.slice(1)),y=y.replace(/\bMacEvicius\b/,"Macevicius").replace(/\bMacHado\b/,"Machado").replace(/\bMacHar\b/,"Machar").replace(/\bMacHin\b/,"Machin").replace(/\bMacHlin\b/,"Machlin").replace(/\bMacIas\b/,"Macias").replace(/\bMacIulis\b/,"Maciulis").replace(/\bMacKie\b/,"Mackie").replace(/\bMacKle\b/,"Mackle").replace(/\bMacKlin\b/,
...
I18N:{setLocale:y=>{r=y??(navigator.language||"en").split("-")[0].toLowerCase();h=z[r]},getLocale:()=>r,addDictionary:(y,A)=>{z[A]=Object.assign(z[A]??{},y);A===r&&(h=z[A])},setDictionary:(y,A)=>{z[A]=y??{};A===r&&(h=z[A])},get:(y,...A)=>{const E=h[y];return null!=E?0<A.length?E.replaceAll(D,(G,L,O,M)=>O&&M?1==A[parseInt(L)]?O:M:"").replaceAll(J,(G,L)=>A[L]):E:y}}};C.onloadingdone=y=>{for(const A of y.fontfaces)K(A)}})(global);
...
t,n,C,K,z]=(f?r:u).exec(h)??[];d.setSource(l??"");d.setProtocol(k??"");d.setAuthority(a??"");d.setUserInfo(e??"");d.setUser(b??"");d.setPassword(g??"");d.setHost(c??"");d.setPort(p??"");d.setRelative(w??"");d.setPath(t??"");d.setDirectory(n??"");d.setFile(C??"");d.setQuery(K??"");d.setAnchor(z??"");d.queryPairs={};d.query.replace(v,(D,J,N)=>{J&&(d.queryPairs[J]=N)})},decodeQueryParam:h=>decodeURIComponent(h).split("+").join(" "),getQuery:function(){var h=this.queryPairs;const f=[];for(const d in h)f.push(d+
...
else{var C=e.EVENT;C.source=t;C.type=n.type;C.value=n;c[p](C)||(n.cancelBubble=!0,n.stopPropagation?.());C.source=void 0}}}return this.callSuper(c,p,w)}}),b=q.InputObservable=new v("InputObservable",{extend:{EVENT_TYPES:{input:!0,select:!0,change:!0,paste:!0},EVENT:h()},createDomHandler:function(c,p,w){return this.createStandardDomHandler(c,p,w,b)??this.callSuper(c,p,w)}}),g=q.DragDropObservable=new v("DragDropObservable",{extend:{EVENT_TYPES:{dragleave:!0,dragenter:!0,dragover:!0,drop:!0},EVENT:h()},
createDomHandler:function(c,p,w){return this.createStandardDomHandler(c,p,w,g,!0)??this.callSuper(c,p,w)}});q.DomObservable=new v("DomObservable",{attachDomObserver:function(c,p,w,t,n){if(c&&p&&w){const C=this.createDomHandler(c,p,w);if(C){t=!!t;const K=this.__dobsbt??(this.__dobsbt={}),z=K[w];z?z.push(c,p,C,t):K[w]=[c,p,C,t];q.addEventListener(this.getDomElementForDomObservable(w),w,C,t,n);return!0}}return!1},getDomElementForDomObservable:function(c){return this.getIDE()},createDomHandler:(c,p,w)=>
null,createStandardDomHandler:function(c,p,w,t,n){if(t.EVENT_TYPES[w]){const C=this,K=t.EVENT;return z=>{z??=window.event;K.source=C;K.type=z.type;K.value=z;c[p](K)||(z.cancelBubble=!0,z.stopPropagation?.(),n&&z.preventDefault());K.source=void 0}}},detachDomObserver:function(c,p,w,t){if(c&&p&&w){var n=this.__dobsbt;if(n&&(n=n[w])){t=!!t;const C=this.getDomElementForDomObservable(w);let K=!1,z=n.length;for(;4<=z;){const D=n[--z],J=n[--z],N=n[--z],P=n[--z];c===P&&p===N&&t===D&&(C&&q.removeEventListener(C,
w,J,t),n.splice(z,4),K=!0)}return K}}return!1},detachAllDomObservers:function(){var c=this.__dobsbt;if(c)for(const [p,w]of Object.entries(c)){for(c=w.length+2;4<=c;){const t=w[c-=3],n=w[--c];q.removeEventListener(this.getDomElementForDomObservable(p),p,n,t)}w.length=0}}});q.DomObserver=new v("DomObserver",{extend:{getSourceViewFromEvent:f},attachToDom:function(c,p,w,t,n){if(c&&p&&w){t=!!t;let C;const K=(C=this.__dobt??(this.__dobt={}))[w]??(C[w]=[]);c.attachDomObserver(this,p,w,t,n)&&K.push(t,p,c)}},
detachFromDom:function(c,p,w,t){if(c&&p&&w){var n=this.__dobt;if(n&&(n=n[w])){t=!!t;let C=!1,K=n.length;for(;3<=K;){const z=n[--K],D=n[--K],J=n[--K];c===z&&p===D&&t===J&&c.detachDomObserver(this,p,w,t)&&(n.splice(K,3),C=!0)}return C}}return!1},detachFromAllDomSources:function(){var c=this.__dobt;if(c)for(const [p,w]of Object.entries(c)){for(c=w.length;3<=c;){const t=w[--c],n=w[--c],C=w[--c];t.detachDomObserver(this,n,p,C)}w.length=0}}})})(myt);
...
this.wrap},setUpdateAgain:function(a){this._updateAgain=a},isUpdateAgain:function(){return this._updateAgain},updateFlexboxLayout:function(){const a=this;if(a.isOKToUpdateLayout()){var e=a.getSubviewsInLexicalOrder().filter(z=>!0!==a.ignoreSubviewForFlex(z)&&!0===z.visible);const K=e.length;if(0<K){a.__isUpdatingFlexboxLayout=!0;var b=a.direction;const z=b.startsWith("row"),D=!z;var g=a.getWidthForFlexboxLayout(),c=a.getHeightForFlexboxLayout();const J=z?a.columnGap:a.rowGap,N=z?g:c;var p=z?a.rowGap:
a.columnGap;const P=z?c:g,m=a.isWrapNotAllowed();g=a.justifyContent;c=a.alignItems;var w=m?"stretch":a.alignContent;b.endsWith("Reverse")&&e.reverse();let x=0,B=0,y=0,A=0,E={items:[],shrinkCount:0,shrinkBasis:0,growCount:0,extraSize:0,crossPos:y,crossSize:0};b=[E];for(let G=0;G<K;){const L=e[G++],O=z?L.getFlexBasisWidth():L.getFlexBasisHeight(),M=L.getShrink(),S=L.getGrow();!m&&0<E.items.length&&x+(0===A?0:J)+O>N&&(E.extraSize=N-x,y+=E.crossSize+p,B=u(B,x),A=x=0,E={items:[],shrinkCount:0,shrinkBasis:0,
growCount:0,extraSize:0,crossPos:y,crossSize:0},b.push(E));0!==A&&(x+=J);0<M&&0<O&&(E.shrinkCount+=M,E.shrinkBasis+=O*M);0<S&&(E.growCount+=S);E.crossSize=u(E.crossSize,D?L.getFlexBasisWidth():L.getFlexBasisHeight());d(L,z,x);f(L,z,O);x+=O;E.items.push(L);A++}E.extraSize=N-x;y+=E.crossSize;B=u(B,x);if(a.isCompactOnMainAxis(z)&&(e=b.reduce((G,L)=>null==G?L.extraSize:Math.min(G,L.extraSize),null),0!==e))for(var t of b)t.extraSize-=e;a[z?"setTotalBasisWidth":"setTotalBasisHeight"](B);a[z?"setTotalBasisHeight":
"setTotalBasisWidth"](y);t=b.length;if(1<t&&"wrapReverse"===a.wrap){for(var n of b)n.crossPos=y-(n.crossPos+n.crossSize);b.reverse()}if(!a.isCompactOnCrossAxis(z))switch(n=P-y,w){default:case "start":break;case "end":h(b,n,!1);break;case "center":h(b,n/2,!1);break;case "spaceBetween":1<t&&h(b,n/(t-1),!0,1);break;case "spaceAround":n<=p?h(b,n/2,!1):(h(b,p/2,!1),n-=p,h(b,n/(2*t),!0),h(b,n/(2*t),!0,1));break;case "spaceEvenly":n<=2*p?h(b,n/2,!1):(h(b,p,!1),h(b,(n-2*p)/(t+1),!0));break;case "stretch":p=
n/t;n=0;for(var C of b)C.crossSize+=p,C.crossPos+=n,n+=p}for(const G of b){C=G.items;p=G.extraSize;if(0<p)if(0<G.growCount){p/=G.growCount;b=0;for(const M of C)0<b&&l(M,z,b),n=M.getGrow()*p,0<n&&(f(M,z,n,!0),b+=n)}else switch(b=C.length,g){default:case "start":break;case "end":r(C,z,p,!1);break;case "center":r(C,z,p/2,!1);break;case "spaceBetween":1<b&&r(C,z,p/(b-1),!0,1);break;case "spaceAround":p<=J?r(C,z,p/2,!1):(r(C,z,J/2,!1),p-=J,r(C,z,p/(2*b),!0),r(C,z,p/(2*b),!0,1));break;case "spaceEvenly":p<=
2*J?r(C,z,p/2,!1):(r(C,z,J,!1),p-=2*J,r(C,z,p/(b+1),!0))}else if(0>p&&0<G.shrinkCount){b=G.shrinkBasis;n=0;for(const M of C)0>n&&l(M,z,n),w=M.getShrink()*(z?M.getFlexBasisWidth():M.getFlexBasisHeight())/b*u(p,-b),0>w&&(f(M,z,w,!0),n+=w)}p=!1;const {crossSize:L,crossPos:O}=G;for(const M of C)switch(M.getAlignSelf()||c){case "start":d(M,D,O);f(M,D,D?M.getFlexBasisWidth():M.getFlexBasisHeight(),!1);break;case "end":d(M,D,O+L-(D?M.getFlexBasisWidth():M.getFlexBasisHeight()));f(M,D,D?M.getFlexBasisWidth():
M.getFlexBasisHeight(),!1);break;case "center":d(M,D,O+(L-(D?M.getFlexBasisWidth():M.getFlexBasisHeight()))/2);f(M,D,D?M.getFlexBasisWidth():M.getFlexBasisHeight(),!1);break;case "baseline":p=!0;d(M,D,O+(L-(D?M.getFlexBasisWidth():M.getFlexBasisHeight()))/2+M.getFlexBaselineOffset(z));f(M,D,D?M.getFlexBasisWidth():M.getFlexBasisHeight(),!1);break;default:d(M,D,O),f(M,D,L,!1)}p&&a.updateFlexboxLayoutBaselineAdjustment(C,z,O)}a.__isUpdatingFlexboxLayout=!1;a.isUpdateAgain()&&!a.__UPDATE_AGAIN_LOOP_PROTECTION&&
...
(q=>{const v=Math.round,u=q.DomElementProxy,r=q.Geometry.rectContainsPoint,h=(c,p)=>{const w=q.global.focus.focusedView,t=c.getIDE();if(w===c||w?.isDescendantOf(c))w._ignoreFocus=!0;const {scrollTop:n,scrollLeft:C}=t;p();w&&(w._ignoreFocus=!1,w.focus(!0));t.scrollTop=n;t.scrollLeft=C},f=(c,p,w)=>{const t=c.getODE(),n=t.parentNode;if(n){const C=()=>{const K=t.nextElementSibling;n.removeChild(t);p();n.insertBefore(t,K)};w?h(c,C):C()}else p()},d=(c,p,w,t)=>{if(p&&"object"===typeof p){if(t){var n=c.getLeastCommonAncestor(p)?.getIDE();
if(n){const C=u.getZIndexRelativeToAncestor,K=w?-1:1;t=K*C(c.getODE(),n);n=K*C(p.getODE(),n);if(t<n)return!0;if(n<t)return!1}}c=c.getODE().compareDocumentPosition(p.getODE());return w?2===c||10===c:4===c||20===c}return!1},l=c=>{c=c.getAncestors();let p=c.length,w=1,t=1;for(;p;){const n=c[--p];w*=n.scaleX||1;t*=n.scaleY||1;n.__effectiveScaleX=w;n.__effectiveScaleY=t}return{scaleX:w,scaleY:t}},k=(c,p,w)=>{const t=c.getODE();if(r(p,w,0,0,t.offsetWidth*c.__effectiveScaleX,t.offsetHeight*c.__effectiveScaleY)){if(c=
...
p,w){w?.moveSubviewBefore(c,p);const t=this;if(c?.parent===t&&p?.parent===t){const n=c.getODE(),C=p.getODE();if(n!==C.previousElementSibling){const K=t.getIDE();h(c,()=>{K.insertBefore(n,C);t.doSubviewsReorderedInDom(c)})}}w?.update()},sendSubviewInFrontOf:function(c,p,w){w?.moveSubviewAfter(c,p);const t=this;if(c?.parent===t&&p?.parent===t){const n=c.getODE(),C=p.getODE().nextElementSibling;if(n!==C){const K=t.getIDE();h(c,()=>{K.insertBefore(n,C);t.doSubviewsReorderedInDom(c)})}}w?.update()},sortSubviews:function(c){const p=
this,w=p.getSubviews();q.isSorted(w,c)||(w.sort(c),f(p,()=>{const t=document.createDocumentFragment(),n=w.length;for(let C=0;n>C;)t.appendChild(w[C++].getODE());p.getIDE().appendChild(t)},!0),p.doSubviewsReorderedInDom(null))},containsPoint:function(c,p,w){const t=this.getODE();if(!t)return!1;const {x:n,y:C}=u.getRelativePosition(t,w);return r(c,p,n,C,this.width,this.height)},isPointVisible:function(c,p){const {x:w,y:t}=this.getPagePosition(!0);l(this);return k(this,c-w,p-t)},intersectsWithSibling:function(c){return c.parent===
...
(q=>{const {max:v,sin:u,cos:r,pow:h,sqrt:f,PI:d}=Math,l=2*d,k=d/2,a=q.global.idle,e=q.Color.makeColorFromHexString,b=(n,C)=>C*(1>(n*=2)?n*n/2:-(--n*(n-2)-1)/2),g={linear:n=>n,inQuad:n=>n*n,outQuad:n=>-n*(n-2),inOutQuad:n=>1>(n*=2)?n*n/2:-(--n*(n-2)-1)/2,inCubic:n=>n*n*n,outCubic:n=>--n*n*n+1,inOutCubic:n=>1>(n*=2)?n*n*n/2:((n-=2)*n*n+2)/2,inQuart:n=>n*n*n*n,outQuart:n=>-(--n*n*n*n-1),inOutQuart:n=>1>(n*=2)?n*n*n*n/2:-((n-=2)*n*n*n-2)/2,inQuint:n=>n*n*n*n*n,outQuint:n=>--n*n*n*n*n+1,inOutQuint:n=>
...
n:1>(n*=2)?-(h(2,10*--n)*u((1*n-.1125)*l/.45))/2:h(2,-10*--n)*u((1*n-.1125)*l/.45)/2+1,inBack:(n,C=1.70158)=>(n/=1)*n*((C+1)*n-C),outBack:(n,C=1.70158)=>(n=n/1-1)*n*((C+1)*n+C)+1,inOutBack:(n,C=1.70158)=>1>(n*=2)?n*n*(((C*=1.525)+1)*n-C)/2:((n-=2)*n*(((C*=1.525)+1)*n+C)+2)/2,inBounce:n=>1-g.outBounce(1-n),outBounce:n=>n<1/2.75?7.5625*n*n:n<2/2.75?7.5625*(n-=1.5/2.75)*n+.75:n<2.5/2.75?7.5625*(n-=2.25/2.75)*n+.9375:7.5625*(n-=2.625/2.75)*n+.984375,inOutBounce:n=>1>(n*=2)?g.inBounce(n)/2:(g.outBounce(n-
1)+1)/2,wiggle:n=>{n*=6;return 1>n?b(n-0,1):2>n?b(2-n,1):3>n?b(n-2,.5):4>n?b(4-n,.5):5>n?b(n-4,.25):b(6-n,.25)}},c=g.inOutQuad,p=n=>{n.__tmpFrom=!1;n.__loopCnt=n.reverse?n.repeat-1:0;n.__prog=n.reverse?n.duration:0},w=(n,C)=>{const {running:K,paused:z,reverse:D,duration:J,repeat:N,__prog:P,easingFunction:m,callback:x}=n;if(K&&!z){D&&(C*=-1);C=P+C;let L=0;C>J?(L=C-J,C=J,++n.__loopCnt===N&&(L=0)):0>C&&(L=-C,C=0,0>--n.__loopCnt&&0<N&&(L=0));var B=n.target??n.parent;if(B){const {relative:O,attribute:M,
__isColorAnim:S,__loopCnt:V,to:T}=n;var y=v(0,C/J),A=v(0,P/J);null==n.from&&(n.__tmpFrom=!0,n.from=O?S?"#000":0:B.get(M));A=m(y)-(O?m(A):0);var E=n.from,G=O?B.get(M):E;y=B.set;if(S){E=e(E);const ea=e(T);G=O?e(G):E;G.setRed(G.red+(ea.red-E.red)*A);G.setGreen(G.green+(ea.green-E.green)*A);G.setBlue(G.blue+(ea.blue-E.blue)*A);A=G.getHtmlHexString()}else A=G+(T-E)*A;y.call(B,M,A);!D&&V===N||D&&0>V&&0<N?(n.setRunning(!1),x?.call(n,!0)):0<L?(n.fireEvent("repeat",V),n.__prog=D?J:0,w(n,L)):n.__prog=C}else console.log("No target for animator",
n),n.setRunning(!1),x?.call(n,!1)}},t=(n,C,K,z)=>{n.stopActiveAnimators(z,!0);const D=n[z];n.animate({attribute:z,from:D,to:D+C,duration:K,easingFunction:"wiggle"}).next(()=>{n.set(z,D)})};q.Animator=new JS.Class("Animator",q.Node,{include:[q.Reusable],extend:{easings:g,bounceView:(n,C=-6,K=900)=>{t(n,C,K,"y")},shakeView:(n,C=10,K=300)=>{t(n,C,K,"x")},wiggleView:t},initNode:function(n,C){this.duration=1E3;this.relative=this.reverse=this.running=this.paused=!1;this.repeat=1;this.easingFunction=c;this.callSuper(n,
C);p(this)},setRunning:function(n){if(this.running!==n&&(this.running=n,this.inited&&this.fireEvent("running",n),!this.paused)){if(n){const C=this.target??this.parent;this.__isColorAnim=C&&"function"===typeof C.isColorAttr?C.isColorAttr(this.attribute):void 0}else this.__tmpFrom&&(this.from=void 0),p(this);this[n?"attachTo":"detachFrom"](a,"__updateAnim","idle")}},setPaused:function(n){if(this.paused!==n&&(this.paused=n,this.inited&&this.fireEvent("paused",n),this.running))this[n?"detachFrom":"attachTo"](a,
...
(q=>{const {Class:v,Module:u}=JS,r=Math,{View:h,global:f}=q,d=f.focus,l=f.keys,{LIST_KEYS:k,CODE_ESC:a}=l,e=g=>{const c=g.itemConfig??[],p=c.length,{items:w,defaultItemClass:t,fixedWidth:n}=g,C=w.length,K=0<n,z=g.getContentView(),D=z.getLayouts();let J=0;for(var N of D)N.incrementLockedCounter();h.doWhileRemovedFromDom(z,()=>{const P={listView:g};K&&(P.width=n,P.enableEllipsis=!0);for(;p>J;++J){var m=c[J];const x=m.klass??t;m=m.attrs??{};let B=w[J];B&&!B.isA(x)&&(B.destroy(),B=null);B??=w[J]=new x(z,
{...P});B&&(B.callSetters(m),B.__LAYOUT_IDX=J)}});if(K)N=n;else for(N=g.minWidth,J=0;p>J;){const P=w[J++];P.syncToDom();N=r.max(N,P.getMinimumWidth())}for(;C>J;)w[J++].destroy();w.length=p;if(!K)for(J=0;p>J;)g.updateItemWidth(w[J++],N);g.updateContentWidth(z,N);for(const P of D)P.sortSubviews((m,x)=>m.__LAYOUT_IDX-x.__LAYOUT_IDX),P.decrementLockedCounter(),P.update()},b=q.ListViewItemMixin=new u("ListViewItemMixin",{setListView:function(g){this.listView=g},getMinimumWidth:()=>0,syncToDom:()=>{}});
...
(q=>{const v=JS.Class,u=Math.min,r=Array.isArray,{SizeToDom:h,View:f,Disableable:d,KeyObservable:l,theme:k,global:{keys:a}}=q,e=(t,n,C)=>{t[C]!==n&&(t[C]=n,t.getIDS()[C]=n+"px",t.inited&&(t.fireEvent(C,n),t.sizeViewToDom()))},b=(t,n,C)=>{t[n]!==C&&(t.getIDE()[n]=t[n]=C,t.inited&&t.fireEvent(n,C))},g=q.NativeInputWrapper=new v("NativeInputWrapper",f,{include:[d,q.InputObservable],initNode:function(t,n){n.tagName??(n.tagName="input");n.focusable??(n.focusable=!0);this.callSuper(t,n);this.addDomClass("mytNativeInput")},
...
(n.floatingAlignOffset=n.borderWidth);n.fullItemConfig??(n.fullItemConfig=[]);n.listViewAttrs={...n.listViewAttrs,maxHeight:n.listViewAttrs?.maxHeight??99};this.callSuper(t,n)},setFullItemConfig:function(t){this.fullItemConfig=t},setFilterItems:function(t){this.filterItems=t},__syncToDom:function(t){const n=this.value;this.callSuper(t);n!==this.value&&this.showFloatingPanel()},showFloatingPanel:function(t){const n=this.getFloatingPanel(t);if(n){let K;if(this.filterItems){K=[];var C=this.value;C=null==
C?"":(""+C).toLowerCase();const z=this.fullItemConfig,D=z.length;for(let J=0;D>J;){const N=z[J++],P=N.attrs.text.toLowerCase(),m=P.indexOf(C);0===m?P!==C&&K.push(N):0<m&&K.push(N)}}else K=this.fullItemConfig;0<K.length?(n.setMinWidth(this.width-2*this.borderWidth),this.setItemConfig(K),this.callSuper(t)):this.hideFloatingPanel(t)}},doItemActivated:function(t){this.setValue(null==t.value?t.text:t.value);this.callSuper(t)},doActivated:function(){this.toggleFloatingPanel()}});q.InputSelect=new v("InputSelect",
...
(q=>{let v;const {Class:u,Module:r}=JS,h=console.warn,{KeyObservable:f,UpdateableUI:d,dumpStack:l,global:k}=q,a=k.focus,e=k.keys,b={},g=(m,x)=>{if(m){const B=m.id;m.id?x(B):l("No ID")}else l("No processor")},c=m=>{g(m,x=>{b[x]=m})},p=m=>{const x=q.BAG.getGroup("selected",m.groupId);if(x){const B=x.getNodes();m=m.value;let y=B.length;for(;y;){const A=B[--y];if(A.optionValue===m){x.setTrue(A);break}}}},w=m=>{if(m.groupId){const x=q.BAG.getGroup("selected",m.groupId);x&&m.syncTo(x,"__syncValue","trueNode")}},
t=(m,x)=>{const B=[],y=m.__v;if(y)for(const A of y)x=A.isFormValid(m,null,B)&&x;m.setErrorMessages(B);m.setIsValid(x);return x},n=(m,x,B)=>{if(m=m.__vp)for(const y of m)y[B]&&(x=y.process(x));return x},C=q.ValueProcessor=new u("ValueProcessor",{initialize:function(m,x,B,y){this.id=m;this.runForDefault=!!x;this.runForRollback=!!B;this.runForCurrent=!!y},process:m=>m}),K=q.ToNumberValueProcessor=new u("ToNumberValueProcessor",C,{process:m=>{if(null==m||""===m||"-"===m)return m;const x=Number(m);return isNaN(x)?
m:x}}),z=q.TrimValueProcessor=new u("TrimValueProcessor",C,{initialize:function(m,x,B,y,A){this.callSuper(m,x,B,y);this.trim=A},process:function(m){m+="";switch(this.trim){case "start":case "left":return m.trimStart();case "end":case "right":return m.trimEnd();default:return m.trim()}}}),D=q.UndefinedValueProcessor=new u("UndefinedValueProcessor",C,{initialize:function(m,x,B,y,A){this.callSuper(m,x,B,y);this.defaultValue=A},process:function(m){return void 0===m?this.defaultValue:m}}),J=q.Form=new r("Form",
...
m){this.isChanged=m;this.inited&&this.fireEvent("isChanged",m);const x=this.form;x&&!this._lockCascade&&(m?x.notifySubFormChanged():x.verifyChangedState(this))}},setValidators:function(m){let x=m.length;for(;x;){const B=m[--x];"string"===typeof B&&(m[x]=k.validators.getValidator(B),m[x]||m.splice(x,1))}this.__v=m},getValue:function(){if(this.callSuper)return this.callSuper();if(this.isA(N))return this.value;const m={},x=this.__sf;for(const B in x)m[B]=x[B].getValue();return m},setValue:function(m){this.callSuper?.(m);
if("object"===typeof m&&!this.isA(N))for(const x in m){const B=this.getSubForm(x);B?m[x]=B.setValue(m[x]):h("ID in setValue for missing subform",x)}this.form?.notifyValueChanged(this);return m},getDefaultValue:function(){const m={};let x=this.__sf;for(const B in x)m[B]=x[B].getDefaultValue();return m},setDefaultValue:function(m){if("object"===typeof m)for(const x in m){const B=this.getSubForm(x);B?m[x]=B.setDefaultValue(m[x]):h("ID in setDefaultValue for missing subform",x)}return m},getRollbackValue:function(){const m=
{},x=this.__sf;for(const B in x)m[B]=x[B].getRollbackValue();return m},setRollbackValue:function(m){if("object"===typeof m)for(const x in m){const B=this.getSubForm(x);B?m[x]=B.setRollbackValue(m[x]):h("ID in setRollbackValue for missing subform",x)}return m},addAccelerator:function(m,x){this.__acc[m]=x},removeAccelerator:function(m){delete this.__acc[m]},invokeAccelerator:function(m,x){this.__acc[m]?.call(this,x??null)},addValidator:function(m){m&&this.__v.push(m)},removeValidator:function(m){if(m){const x=
this.__v;let B=x.length;for(;B;){const y=x[--B];if(y.id===m)return x.splice(B,1),y}}},getRootForm:function(){return this.form?this.form.getRootForm():this},addSubForm:function(m){const x=m.id;null!=this.getSubForm(x)?h("ID in use for subform, add aborted",x,m):(m.setForm(this),this.__sf[x]=m,m.isChanged&&this.notifySubFormChanged(),m.isValid||this.notifySubFormInvalid())},removeSubForm:function(m){const x=this.getSubForm(m);x&&(x.setForm(null),delete this.__sf[m],this.verifyChangedState(),this.verifyValidState());
return x},getSubForm:function(m){return this.__sf[m]},getSubForms:function(){return this.__sf},getInvalidSubformIds:function(m){m&&this.doValidation();const x=[];(function A(y){if(y.isA(N))y.isValid||x.push(y.getFullId());else{y=y.getSubForms();for(const E in y)A(y[E])}})(this);return x},getAllErrorMessages:function(){const m=this.__sf;let x=(this.errorMessages??[]).slice();for(const B in m)x.push(...m[B].getAllErrorMessages());return x},notifySubFormInvalid:function(){this.setIsValid(!1)},verifyValidState:function(m){const x=
this.__sf;let B=!0;for(const y in x){const A=x[y];A!==m&&(B=A.isValid&&B)}return t(this,B)},doValidation:function(){const m=this.__sf;let x=!0;for(const B in m)x=m[B].doValidation()&&x;this._lockCascade=!0;x=t(this,x);this._lockCascade=!1;return x},notifyValueChanged:function(m){this.form?.notifyValueChanged(m)},notifySubFormChanged:function(){this.setIsChanged(!0)},verifyChangedState:function(m){const x=this.__sf;let B=!1;for(const y in x){const A=x[y];A!==m&&(B=A.isChanged||B)}this.setIsChanged(B);
return B},setup:function(m,x,B){this._lockCascade=!0;this.setIsChanged(!1);this.setErrorMessages([]);this.setIsValid(!0);this._lockCascade=!1;m??={};x??={};B??={};const y=this.__sf;for(const A in y)y[A].setup(m[A],x[A],B[A])},resetForm:function(){this._lockCascade=!0;const m=this.__sf;for(const x in m)m[x].resetForm();this.setIsChanged(!1);this.setErrorMessages([]);this.setIsValid(!0);this._lockCascade=!1},rollbackForm:function(){this._lockCascade=!0;const m=this.__sf;for(const x in m)m[x].rollbackForm();
this.setIsChanged(!1);this.setErrorMessages([]);this.setIsValid(!0);this._lockCascade=!1},getChangedValue:function(){const m={},x=this.__sf;for(const B in x){const y=x[B];y.isChanged&&(m[B]=y.getChangedValue())}return m}}),N=q.FormElement=new r("FormElement",{include:[J],initNode:function(m,x){this.__vp=[];this.callSuper(m,x)},getValue:function(){return n(this,this.callSuper?this.callSuper():this.value,"runForCurrent")},setValue:function(m){void 0===m&&(m=this.getRollbackValue());this.value!==m&&
...
m),this.verifyChangedState());return m},setValueProcessors:function(m){let x=m.length;for(;x;){const B=m[--x];"string"===typeof B&&(m[x]=b[B],m[x]||m.splice(x,1))}this.__vp=m},addValueProcessor:function(m){this.__vp.push(m)},removeValueProcessor:function(m){if(m){const x=this.__vp;let B=x.length;for(;B;){const y=x[--B];if(y.id===m)return x.splice(B,1),y}}},addSubForm:m=>{l("addSubForm unsupported on FormElement")},getSubForm:m=>{l("getSubForm unsupported on FormElement")},removeSubForm:m=>{l("removeSubForm unsupported on FormElement")},
...
this.setIsValid(!0);this._lockCascade=!1},rollbackForm:function(){this._lockCascade=!0;this.setValue(this.getRollbackValue());this.setIsChanged(!1);this.setErrorMessages([]);this.setIsValid(!0);this._lockCascade=!1},getChangedValue:function(){return this.isChanged?this.getValue():void 0}}),P=q.FormInputTextMixin=new r("FormInputTextMixin",{include:[N,d],initNode:function(m,x){this.acceleratorScope="element";this.validateWhen="key";this.errorColor="#f99";this.actionRequiredColor="#966";this.normalColor=
...
q.InputText,{include:[P]});q.FormInputTextArea=new u("FormInputTextArea",q.InputTextArea,{include:[P],__hndlKeyDown:m=>{}});q.FormInputSelect=new u("FormInputSelect",q.InputSelect,{include:[N],setValue:function(m){if(!this.__abortSetValue)return m=this.callSuper(m),this.__abortSetValue=!0,this.deselectAll(),this.selectValues(m),this.__abortSetValue=!1,this.verifyValidState(),m}});q.FormRadioGroup=new u("FormRadioGroup",q.Node,{include:[q.ValueComponent,N],initNode:function(m,x){x.groupId??(x.groupId=
q.generateGuid());this.callSuper(m,x);void 0!==this.value&&p(this);w(this)},setValue:function(m){m=this.callSuper(m);this.inited&&p(this);return m},setGroupId:function(m){if(this.groupId!==m){if(this.groupId){const x=q.BAG.getGroup("selected",this.groupId);x&&this.detachFrom(x,"__syncValue","trueNode")}this.groupId=m;this.inited&&w(this)}},__syncValue:function(m){m=m.value;this.setValue(m?m.optionValue:null)}});q.UseOtherFieldIfEmptyValueProcessor=new u("UseOtherFieldIfEmptyValueProcessor",C,{initialize:function(m,
x,B,y,A){this.callSuper(m,x,B,y);this.otherField=A},process:function(m){return null==m||""===m?this.otherField.getValue():m}});new JS.Singleton("GlobalValueProcessorRegistry",{initialize:function(){c(v=new D("undefToEmpty",!0,!0,!0,""));c(new K("toNumber",!0,!0,!0));c(new z("trimLeft",!0,!0,!0,"left"));c(new z("trimRight",!0,!0,!0,"right"));c(new z("trimBoth",!0,!0,!0,"both"));k.register("valueProcessors",this)},getValueProcessor:m=>b[m],register:c,unregister:m=>{g(m,x=>{b[x]&&delete b[x]})}})})(myt);
...
(q=>{let v,u,r,h,f=0,d=0,l=0,k=255,a=[],e,b,g,c,p,w,t,n,C,K,z,D,J,N,P,m,x,B,y,A,E,G,L,O,M,S;const V=JS.Class,{View:T,Text:ea,ModalPanel:Ha,SizeToChildren:ua,SpacedLayout:za,WrappingLayout:Aa,SelectionManager:Ba,Color:pa,LocalStorage:Ca,Button:va,TextButton:ma,Draggable:Da,FontAwesome:{makeTag:na},theme:W}=q,{min:ha,max:ia}=Math,Ia=new V("DialogButton",ma,{initNode:function(F,H){H.height??(H.height=17);H.paddingTop??(H.paddingTop=3);H.paddingLeft??(H.paddingLeft=10);H.paddingRight??(H.paddingRight=
10);H.activeColor??(H.activeColor="#bbb");H.hoverColor??(H.hoverColor="#ddd");H.readyColor??(H.readyColor="#ccc");H.domClass="mytButtonText";this.callSuper(F,H)}}),Ja=na(["check"]),Fa=F=>{for(let H of F)7===H?.length&&(H+="ff"),Ea[H]=!0},Ea={},Ka=new V("Swatch",T,{include:[va],initNode:function(F,H){const I=H.width=H.height=16;H.border=W.border1s9;H.domClass="mytCheckerboardPattern";const Q=H.bgColor;this.callSuper(F,H);this.colorView=new T(this,{width:I,height:I,bgColor:Q});Q===h&&(F=pa.makeColorFromHexString(h),
new ea(this,{x:2,y:2,text:Ja,fontSize:"12px",textColor:382.5>F.red+F.green+F.blue?"#fff":"#000"}))},setBgColor:function(F){this.colorView?.setBgColor(F);this.setTooltip(F)},doActivated:function(){v.setColor(this.colorView.bgColor)},drawHoverState:function(){this.setBorder(W.border1s3)},drawReadyState:function(){this.setBorder(W.border1s9)}}),La=q.ColorPicker=new V("ColorPicker",T,{initNode:function(F,H){v=this;v._isReady=!1;let I=H.color??"transparent";delete H.color;r=H.alphaChannel??!1;delete H.alphaChannel;
e=H.palette??[];Fa(e);delete H.palette;u="transparent"===I;v.callSuper(F,H);b=new T(v,{width:160,height:170});new Aa(b,{spacing:4,lineSpacing:4});g=new T(v,{x:170,width:139,height:139,border:W.border1s3},[Da,{requestDragPosition:function(Q,R){g.callSuper(g.x,g.y);const X=this.getPagePosition();d=parseFloat(ia(0,ha(1,(Q+this.dragInitX-X.x)/this.width)));l=parseFloat(1-ia(0,ha(1,(R+this.dragInitY-X.y)/this.height)));u=!1;v.updateUI()}}]);F=new T(g,{width:139,height:139});H=new T(F,{width:139,height:139});
F.getIDS().backgroundImage="linear-gradient(to right, #fff, rgba(204, 154, 129, 0))";H.getIDS().backgroundImage="linear-gradient(to top, #000, rgba(204, 154, 129, 0))";c=new T(H,{width:6,height:6,bgColor:"#000",border:[1,"solid","#fff"],roundedCorners:4});p=new T(v,{x:315,y:30,width:24,height:109,border:W.border1s3},[Da,{requestDragPosition:function(Q,R){this.callSuper(p.x,p.y);f=parseFloat(ia(0,ha(1,(R+this.dragInitY-this.getPagePosition().y)/this.height)));u=!1;v.updateUI()}}]);p.getIDS().background=
"linear-gradient(to top, #f00 0%, #f0f 17%, #00f 33%, #0ff 50%, #0f0 67%, #ff0 83%, #f00 100%)";w=new T(p,{x:-1,width:24,height:2,bgColor:"#fff",border:[1,"solid","#000"]});new T(v,{x:315,width:24,height:24,border:W.border1s3,tooltip:"Set to transparent.",domClass:"mytCheckerboardPattern"},[va,{doActivated:()=>{v.setColor("transparent")}}]);F=146;r&&(C=new q.LabelSlider(v,{x:170,y:F,width:171,minValue:0,maxValue:255,flipThreshold:55,labelColor:"#fff"},[{setText:function(Q,R){R=Q.value;if(null==R||
isNaN(R))R=0;Q.value=q.formatAsPercentage(R/255,0);this.callSuper(Q,!0);k=R;v._isReady&&(u=!1,v.updateUI())}}]),F+=23);t=new q.InputText(v,{x:236,y:F,width:105,height:25,roundedCorners:3,textColor:W.DialogInputFgColor,border:W.border1s3,maxLength:11,fontFamily:"monospace"});v.attachToDom(t,"_submitInput","blur");v.attachToDom(t,"_handleKeyDown","keydown");t.getIDS().paddingLeft="6px";F=new T(v,{x:170,y:F,width:60,height:23,border:W.border1s3,domClass:"mytCheckerboardPattern"});new T(F,{width:30,height:23,
focusIndicator:!1,bgColor:I},[va,{doActivated:()=>{v.setColor(I)}}]);n=new T(F,{x:30,width:30,height:23});if(F=Ca.getItem("myt.default"))a=F.split(";"),Fa(a);v.setColor(I);v._isReady=!0},_handleKeyDown:F=>{q.KeyObservable.isEnterKeyEvent(F)&&v._submitInput()},_submitInput:()=>{v.setColor(t.value)},addToPalette:F=>{F&&"transparent"!==F&&!Ea[F]&&(a.unshift(F),a.length=ha(a.length,56),Ca.setItem("myt.default",a.join(";")))},setColor:F=>{const H=F&&"transparent"!==F;if(r){let I="ff";H&&7<F.length&&(I=
F.slice(7,9),F=F.slice(0,7));C.setValue(parseInt(I,16))}else H&&7<F.length&&(F=F.slice(0,7));H?(F=pa.makeColorFromHexString(F).getHSV(),f=F.h/360,d=F.s,l=F.v,u=!1):u=!0;v.updateUI()},getColor:()=>{if(u)return"transparent";let F="";r&&(F=k.toString(16),2>F.length&&(F="0"+F));return pa.makeColorFromHSV(360*f,100*d,100*l).getHtmlHexString()+F},updateUI:()=>{const F=!u;w.setVisible(F);c.setVisible(F);F&&(c.setX(ia(0,ha(1,d)*(g.width+2))-5),c.setY(ia(0,ha(1,1-l)*(g.height+2))-5),w.setY(f*(p.height-2)-
1));g.setBgColor(pa.makeColorFromHSV(360*f,100,100).getHtmlHexString());h=v.getColor();t.setValue(h);n.setBgColor(h);v.redrawPalette()},redrawPalette:q.debounce(()=>{b.destroyAllSubviews();const F=new Set;e.push(...a);for(let H of e)if(r||7===H.length)7===H.length&&(H+="ff"),F.has(H)||(new Ka(b,{bgColor:H}),F.add(H))},50)}),wa=F=>{F=F.split(":");return[ia(0,ha(23,parseInt(F[0]))),ia(0,ha(60,parseInt(F[1])))]},Ga=new V("SelectableBtn",ma,{include:[q.Selectable],setSelected:function(F){this.callSuper(F);
this.updateUI()},updateUI:function(){this.callSuper();this.isSelected()?(this.setBgColor("#666"),this.setTextColor("#fff")):this.setTextColor()}}),Ma=new V("TimeBtn",Ga,{initNode:function(F,H){H.width=J?175:49;this.callSuper(F,H)},doActivated:function(){this.callSuper();const F=this.text,H=wa(F);S.selectById(F);E.setHours(H[0]);E.setMinutes(H[1])}}),Na=new V("DayBtn",Ga,{initNode:function(F,H){H.width=23;H.border=W.DialogDayBtnBorder;this.callSuper(F,H)},setData:function(F){this.data=F},setIsAnotherMonth:function(F){this.isAnotherMonth=
F},setIsToday:function(F){this.isToday=F},setIsSunday:function(F){this.isSunday=F},setIsSaturday:function(F){this.isSaturday=F},updateUI:function(){this.destroyed||(this.callSuper(),this.isSelected()||this.setTextColor(this.isAnotherMonth?"#ccc":this.isSunday?"#d40":this.isSaturday?"#04a":null),this.setBorderColor(this.isToday?W.DialogDayBtnBorderColorToday:W.DialogDayBtnBorderColor))},doActivated:function(){this.callSuper();const F=new Date(this.data);F.setHours(E.getHours());F.setMinutes(E.getMinutes());
oa(F)}}),oa=F=>{F.setSeconds(0);F.setMilliseconds(0);var H=6>=P.length,I;if(H)for(I=7;I--&&!P.includes(F.getDay());)F.setDate(F.getDate()+1);E=F;const Q=S.getODE().scrollTop;var R=new Date,X=R.getFullYear(),U=R.getMonth(),Y=R.getDate();R.getHours();R.getMinutes();const ba=F.getFullYear(),ca=F.getMonth(),ka=F.getDate();var da=F.getTime();R=F.getHours();F=F.getMinutes();const qa=(new Date(ba,ca,1)).getDay()-N,ra=(new Date(ba,ca+1,0)).getDate(),xa=(new Date(ba,ca,0)).getDate(),sa=new Date(ba,ca,0),ta=
new Date(ba,ca+2,0);X=X==ba&&U==ca;if(!J){U=M;U.deselectAll();U.destroyAllSubviews();I=new Date(da);const ja=7+N;U=z.days;L.setText(ba+" "+z.sep+" "+z.months[ca]);I.setMinutes(59);I.setHours(23);I.setSeconds(59);I.setDate(0);G.setVisible(null==m||m<I.getTime());I.setMinutes(0);I.setHours(0);I.setSeconds(0);I.setDate(1);I.setMonth(ca+1);O.setVisible(null==x||x>I.getTime());for(I=0;7>I;I++)new ea(M,{width:23,height:20,textAlign:"center",text:U[(I+ja)%7],textColor:"#999"});I=0;let fa=42;0>qa&&(I=-7,
fa=35);U=new Date(da);U.setHours(0);U.setMinutes(0);for(U.setSeconds(0);I<fa;I++){var Z=I+1-qa;const ya=(I+ja)%7,aa=new Na(M);qa>I?(aa.setText(xa+Z),aa.setData(sa.getFullYear()+"/"+(sa.getMonth()+1)+"/"+(xa+Z)),aa.setIsAnotherMonth(!0),U.setDate(xa+Z),U.setMonth(sa.getMonth()),U.setYear(sa.getFullYear())):I<qa+ra?(aa.setText(Z),aa.setData(ba+"/"+(ca+1)+"/"+Z),U.setDate(Z),U.setMonth(ca),U.setYear(ba)):(aa.setText(Z-ra),aa.setData(ta.getFullYear()+"/"+(ta.getMonth()+1)+"/"+(Z-ra)),aa.setIsAnotherMonth(!0),
U.setDate(Z-ra),U.setMonth(ta.getMonth()),U.setYear(ta.getFullYear()));H&&!P.includes(ya)?aa.setDisabled(!0):(0===ya?aa.setIsSunday(!0):6===ya&&aa.setIsSaturday(!0),Z===ka&&(M.select(aa),aa.focus()),X&&Z===Y&&aa.setIsToday(!0),Z=new Date(U.getTime()),Z.setHours(23),Z.setMinutes(59),Z.setSeconds(59),(null!=m&&m>Z.getTime()||null!=x&&x<U.getTime())&&aa.setDisabled(!0));aa.updateUI()}}if(!D){H=60*y[0]+y[1];Y=S;Y.deselectAll();Y.destroyAllSubviews();let [ja,fa]=B;for(U=new Date(da);60*ja+fa<H;)da=new Ma(S,
{text:("0"+ja).slice(-2)+":"+("0"+fa).slice(-2)}),ja===R&&fa===F&&S.select(da),U.setHours(ja),U.setMinutes(fa),Y=U.getTime(),(null!=m&&m>Y||null!=x&&x<Y)&&da.setDisabled(!0),fa+=A,60<=fa&&(fa-=60,ja++);S.scrollYTo(Q)}},Oa=q.DatePicker=new V("DatePicker",T,{initNode:function(F,H){K=this;const I={current:null,dateOnly:!1,timeOnly:!1,locales:null,locale:"",minuteInterval:30,firstDayOfWeek:0,showTodayButton:!0,minDate:null,maxDate:null,minTime:"00:00",maxTime:"23:59",allowedDays:null,...H.opt};delete H.opt;
K.callSuper(F,H);z=I.locales[I.locale??"en"];D=I.dateOnly;J=I.timeOnly;N=I.firstDayOfWeek;A=ia(5,ha(30,I.minuteInterval));P=I.allowedDays;Array.isArray(P)&&0!==P.length||(P=[0,1,2,3,4,5,6]);m=Date.parse(I.minDate);m=isNaN(m)?null:m;x=Date.parse(I.maxDate);x=isNaN(x)?null:x;B=wa(I.minTime);y=wa(I.maxTime);F=new T(K,{visible:!J});I.showTodayButton&&new ma(F,{width:24,text:na(["home"]),tooltip:z.today},[{doActivated:()=>{oa(new Date)}}]);G=new ma(F,{width:24,x:28,text:na(["chevron-left"]),tooltip:z.prevMonth},
[{doActivated:()=>{const Q=(new Date(E.getFullYear(),E.getMonth(),0)).getDate();Q<E.getDate()&&E.setDate(Q);E.setMonth(E.getMonth()-1);oa(E);G.focus()}}]);L=new ea(F,{width:100,x:52,textAlign:"center"});O=new ma(F,{width:24,x:152,text:na(["chevron-right"]),tooltip:z.nextMonth},[{doActivated:()=>{var Q=(new Date(E.getFullYear(),E.getMonth()+1,0)).getDate();Q<E.getDate()&&E.setDate(Q);Q=(new Date(E.getFullYear(),E.getMonth()+2,0)).getDate();Q<E.getDate()&&E.setDate(Q);E.setMonth(E.getMonth()+1);oa(E);
O.focus()}}]);M=new T(K,{y:25,width:175,height:175,visible:!J,maxSelected:1,itemSelectionId:"data"},[Ba]);new Aa(M,{spacing:2,lineInset:2,lineSpacing:3});S=new T(K,{x:J?0:195,y:J?0:25,width:(J?175:49)+q.DomElementProxy.getScrollbarSize(),height:J?200:175,overflow:"autoy",visible:!D,maxSelected:1,itemSelectionId:"text"},[Ba]);new za(S,{axis:"y",inset:1,spacing:3});K.setWidth(S.visible?S.x+S.width:M.width);oa(I.current??new Date)},getPickedDate:()=>E}),la=q.Dialog=new V("Dialog",Ha,{extend:{BUTTON_CLASS:Ia,
...
doCallback:function(F){const H=this.callbackFunction;H&&H.call(this,F.name)||this.hide()},showBlank:function(F){this.destroyContent();this.content.setVisible(!1);F?.bgColor&&this.overlay.setBgColor(F.bgColor);this.show()},showMessage:function(F,H,I){const Q=this.content,R=Q.closeBtn;I={...la.WRAP_TEXT_DEFAULTS,...I};this.destroyContent();this.setCallbackFunction(H);F=Q.msg=new ea(Q,{text:F,whiteSpace:I.whiteSpace,wordWrap:I.wordWrap,fontWeight:I.fontWeight,x:null==I.msgX?W.ModalPanelPaddingX:I.msgX,
y:null==I.msgY?W.ModalPanelPaddingY:I.msgY,width:I.width});I.titleText&&(this.setupTitle(Q,I.titleText),F.setY(F.y+24));this.show();!1===I.showClose?R.destroyed||R.setVisible(!1):(R.setVisible(!0),R.focus())},showSimple:function(F,H,I,Q){I=I??{};const R=this.content,X=R.closeBtn,U=I.maxContainerHeight;this.destroyContent();I.bgColor&&R.setBgColor(I.bgColor);I.roundedCorners&&R.setRoundedCorners(I.roundedCorners);I.boxShadow&&R.setBoxShadow(I.boxShadow);I.border&&R.setBorder(I.border);R.sizeToChildren.setPaddingX(1);
this.setCallbackFunction(H);H=R.contentContainer=new T(R,{x:1,y:25,overflow:"auto"},[{setHeight:function(Y){Y>U&&(Y=U);this.callSuper(Y)}}]);F.call(this,H);new ua(H,{axis:"both"});this.show();X.setVisible(!0);X.focus();I.titleText&&(this.setupTitle(R,I.titleText),H.setY(this.header.height+1));H.initialFocus?.focus();Q?.(this)},showConfirm:function(F,H,I){I={...la.CONFIRM_DEFAULTS,...I};this.showMessage(F,H,I);this.setupFooterButtons(this.content.msg,I)},showContentConfirm:function(F,H,I,Q){I={...la.CONFIRM_DEFAULTS,
...I};const R=this.content,X=R.closeBtn,U=I.maxContainerHeight;this.destroyContent();R.sizeToChildren.setPaddingX(1);this.setCallbackFunction(H);H=R.contentContainer=new T(R,{x:1,y:25,overflow:"auto"},[{setHeight:function(Y){this.callSuper(Y>U?U:Y)}}]);F.call(this,H);new ua(H,{axis:"both"});this.show();!1===I.showClose?X.destroyed||X.setVisible(!1):(X.setVisible(!0),X.focus());this.setupTitle(R,I.titleText);H.setY(this.header.height+1);this.setupFooterButtons(H,I);H.initialFocus?.focus();Q?.(this)},
showSpinner:function(F,H){const I=this.content;H={...la.NO_WRAP_TEXT_DEFAULTS,...H};this.destroyContent();const Q=this.spinner=new q.Spinner(I,{align:"center",visible:!0,borderColor:"#ccc",size:50,y:null==H.msgY?W.ModalPanelPaddingY:H.msgY});F&&(this.msgTxt=new ea(I,{text:F,whiteSpace:H.whiteSpace,wordWrap:H.wordWrap,fontWeight:H.fontWeight,x:null==H.msgX?W.ModalPanelPaddingX:H.msgX,y:Q.y+Q.size+W.ModalPanelPaddingY,width:H.width}));this.show();I.closeBtn.setVisible(!1);this.focus()},updateMessage:function(F){this.msgTxt?.setText(F)},
showColorPicker:function(F,H){const I=this,Q=I.content,R=Q.closeBtn;H={...la.COLOR_PICKER_DEFAULTS,...H};I.destroyContent();I.setCallbackFunction(X=>{switch(X){case "closeBtn":case "cancelBtn":F.call(I,X);break;case "confirmBtn":const U=v.getColor();v.addToPalette(U);F.call(I,X,U)}v.destroy()});v=new La(Q,{x:W.ModalPanelPaddingX,y:W.ModalPanelPaddingY+24,width:337,height:177+(H.alphaChannel?23:0),color:H.color,alphaChannel:H.alphaChannel,palette:"#000000 #111111 #222222 #333333 #444444 #555555 #666666 #777777 #888888 #999999 #aaaaaa #bbbbbb #cccccc #dddddd #eeeeee #ffffff".split(" ")});
I.show();R.setVisible(!0);R.focus();I.setupFooterButtons(v,H);I.setupTitle(Q,H.titleText)},showDatePicker:function(F,H){const I=this,Q=I.content,R=Q.closeBtn;H={...la.DATE_PICKER_DEFAULTS,...H};I.destroyContent();I.setCallbackFunction(X=>{switch(X){case "closeBtn":case "cancelBtn":F.call(I,X);break;case "confirmBtn":F.call(I,X,K.getPickedDate())}K.destroy()});K=new Oa(Q,{x:W.ModalPanelPaddingX,y:W.ModalPanelPaddingY+24,height:195,opt:{current:new Date(H.initialDate??Date.now()),dateOnly:H.dateOnly||
!1,timeOnly:H.timeOnly||!1,locales:H.locales,locale:H.locale}});I.show();R.setVisible(!0);R.focus();I.setupFooterButtons(K,H);I.setupTitle(Q,H.timeOnly?H.timeOnlyTitleText:H.titleText)},setupTitle:function(F,H){const I=W.DialogRadius;(this.header=new T(F,{ignoreLayout:!0,width:F.width,height:24,bgColor:"#eee",roundedTopLeftCorner:I,roundedTopRightCorner:I})).sendToBack();F.title=new ea(F,{x:I,y:4,text:H,fontWeight:"bold"})},setupFooterButtons:function(F,H){const I=this.content;var Q=W.ModalPanelPaddingY,
R=Q/2;F=I.btnContainer=new T(I,{y:F.y+F.height+Q,align:"center"});const X=["active","hover","ready","text"];Q=H.cancelAttrs??{};var U;(U=Q).name??(U.name="cancelBtn");let Y;(Y=Q).text??(Y.text=H.cancelTxt);for(let da of X)da+="Color",null!=H[da]&&(Q[da]=H[da]);U=this.makeButton(F,Q);Q=H.confirmAttrs??{};var ba;(ba=Q).name??(ba.name="confirmBtn");let ca;(ca=Q).text??(ca.text=H.confirmTxt);for(var ka of X)ka+="Color",ba=ka+"Confirm",null!=H[ba]&&(Q[ka]=H[ba]);this.makeButton(F,Q);if(ka=H.button)for(const da of ka)this.makeButton(F,
da);new ua(F,{axis:"y"});new za(F,{spacing:4,collapseParent:!0});I.sizeToChildren.setPaddingY(R);R=F.y-R;(new T(I,{ignoreLayout:!0,y:R,width:I.width,height:I.height-R,bgColor:"#eee"})).sendToBack();!1===H.showClose&&U.focus()}})})(myt);
...
(q=>{let v=0;const {Class:u,Module:r}=JS,{min:h,max:f}=Math,{View:d,SpacedLayout:l,Node:{DEFAULT_PLACEMENT:k}}=q,a=m=>{const x=m.columnHeaders,[B,y]=m.sort??["",""];let A=x.length;for(;A;){const E=x[--A];E.columnId===B?E.sortable&&E.setSortState(y):E.setSortState("none")}m.locked||m.doSort()},e=(m,x,B,y)=>{if(0!==x){const L=0<x,O=[];for(var A=m.length;A;){var E=m[--A];E.resizable&&(B?0<E.flex:0===E.flex)&&O.push({hdr:E,limit:(L?E.maxValue:E.minValue)-E.value,amt:0})}m=O.length;if(0<m){for(A=0;0!==
x;){v>=m&&(A=v=0);E=O[v];var G=E.hdr;E.full?++A:(L?(G=h(B?G.flex:1,x),E.amt+G>E.limit&&(G=E.limit-E.amt,E.full=!0)):(G=f(B?-G.flex:-1,x),E.amt+G<E.limit&&(G=E.limit-E.amt,E.full=!0)),E.amt+=G,x-=G);if(A===m)break;++v}for(const M of O)M.hdr.setValue(M.hdr.value+M.amt);y?.(x)}}},b=m=>(m=m.gridController?.getPrevHdr(m))?m.maxValue-m.value+b(m):0,g=m=>(m=m.gridController?.getNextHdr(m))?m.maxValue-m.value+g(m):0,c=m=>(m=m.gridController?.getPrevHdr(m))?m.minValue-m.value+c(m):0,p=m=>(m=m.gridController?.getNextHdr(m))?
m.minValue-m.value+p(m):0,w=(m,x)=>{m=m.gridController?.getPrevHdr(m);var B=0;if(m){B=m.value+x;m.resizable&&m.setValue(B);const y=B-m.value;B=x-y;0>y&&(B+=w(m,y))}return B},t=(m,x)=>{m=m.gridController?.getPrevHdr(m);var B=0;if(m){B=m.value+x;m.resizable&&m.setValue(B);const y=B-m.value;B=x-y;0<y&&(B+=t(m,y))}return B},n=(m,x)=>{if(m=m.gridController?.getNextHdr(m))x=m.value+x,m.resizable&&m.setValue(x),x-=m.value,0>x&&n(m,x)},C=(m,x)=>{if(m=m.gridController?.getNextHdr(m))x=m.value+x,m.resizable&&
m.setValue(x),x-=m.value,0<x&&C(m,x)},K=m=>{let x="";if(m.sortable)switch(m.sortState){case P.SORT_ORDER_ASC:x="chevron-up";break;case P.SORT_ORDER_DESC:x="chevron-down"}m.sortIcon.setIcon(x)},z=m=>{const x=m.textView;x?.setWidth(m.width-m.outset-x.x)},D=q.GridController=new r("GridController",{initNode:function(m,x){this.columnHeaders=[];this.rows=[];this.maxWidth=this.minWidth=this.gridWidth=0;this.fitToWidth=this.locked=!0;this.callSuper(m,x);this.fitHeadersToWidth();a(this)},setMaxWidth:function(m){this.set("maxWidth",
...
if(0<m)for(;m;){const B=x[--m];if(B.visible)return B}},getNextHdr:function(m){const x=this.columnHeaders,B=x.length;m=this.getHdrIndex(m)+1;if(0<m&&m<B)for(;B>m;){const y=x[m++];if(y.visible)return y}},hasHdr:function(m){return this.columnHeaders.includes(m)},getHdrIndex:function(m){return this.columnHeaders.indexOf(m)},getHdrById:function(m){for(const x of this.columnHeaders)if(x.columnId===m)return x},getVisibleHdrs:function(){return this.columnHeaders.filter(m=>m.visible)},notifyAddHdr:function(m){this.hasHdr(m)||
...
this._notifyHdrChange("Width",m)}},notifyHdrVisibilityChange:function(m){if((!this.__skipInvisibleHeaders||m.visible)&&!this.isLocked()){this.updateRowsForVisibilityChange(m);var x=this.setLastColumn;a:{var B=this.columnHeaders;let y=B.length;for(;y;){const A=B[--y];if(A.visible){B=A;break a}}B=void 0}x.call(this,B);x=m.visible?1:-1;this.setMaxWidth(this.maxWidth+m.maxValue*x);this.setMinWidth(this.minWidth+m.minValue*x);this.fitHeadersToWidth();this.fixupResizerCursors();this._notifyHdrChange("Visibility",
m)}},_notifyHdrChange:function(m,x){const B=this.hdrChangeListener;if(B)B["notifyHdr"+m+"Change"](x)},updateRowsForVisibilityChange:function(m){for(const x of this.rows)x.notifyHdrVisibilityChange(m)},hasRow:function(m){return this.rows.includes(m)},getRowIndex:function(m){return this.rows.indexOf(m)},getRowById:function(m,x=B=>B.model.id===m){return this.rows.find(x)},getPrevRow:function(m){m=this.getRowIndex(m)-1;return this.rows[0>m?this.rows.length-1:m]},getNextRow:function(m){m=this.getRowIndex(m)+
1;return this.rows[m<this.rows.length?m:0]},notifyAddRow:function(m,x){if(!this.hasRow(m)&&(this.rows.push(m),!this.locked)){const B=this.width;for(const y of this.columnHeaders)m.setWidth(B),m.notifyHdrXChange(y),m.notifyHdrWidthChange(y),m.notifyHdrVisibilityChange(y);x||this.doSort()}},notifyRemoveRow:function(m){m=this.getRowIndex(m);-1<m&&this.rows.splice(m,1)},fitHeadersToWidth:function(){if(!this.locked&&this.fitToWidth){const m=this.getVisibleHdrs();let x=0;for(const B of m)x=f(x,B.x+B.width);
e(m,this.gridWidth-x,!0,B=>{e(m,B,!1,null)})}},fixupResizerCursors:function(){const m=this.getVisibleHdrs();let x=!0,B=0;m.forEach((G,L)=>{x&&G.isFixed()?G.setResizerCursor("",!0):(x&&(B=L),x=!1,G.restoreResizerCursor())});let y=!0,A=m.length,E=A-1;for(;A>B;){const G=m[--A];y?G.setResizerCursor("",!0):G.restoreResizerCursor();G.isFixed()||(y&&(E=A),y=!1)}B===E&&m[B].setResizerCursor("",!0)}}),J=q.GridColHdr=new r("GridColHdr",{include:[q.BoundedValueComponent],initNode:function(m,x){const B=this,
y=x.resizerCursor??"col-resize";delete x.resizerCursor;x.minValue??(x.minValue=16);x.maxValue??(x.maxValue=1E3);x.value??(x.value=x.minValue);x.resizable??(x.resizable=!0);x.flex??(x.flex=0);x.cellXAdj??(x.cellXAdj=0);x.cellWidthAdj??(x.cellWidthAdj=0);x.sortable??(x.sortable=!0);x.sortState??(x.sortState="none");x.placement??(x.placement=k);B.callSuper(m,x);const A=B.gridController;B.resizer=new d(B,{cursor:y,width:10,zIndex:1,percentOfParentHeight:100,align:"right",alignOffset:-5,draggableAllowBubble:!1},
[q.SizeToParent,q.Draggable,{requestDragPosition:function(E,G){G=E-this.x;if(A.fitToWidth&&(0<G?G=h(G,-p(B),B.maxValue-B.value+b(B)):0>G&&(G=f(G,-g(B),B.minValue-B.value+c(B))),0===G))return;E=B.value+G;B.resizable&&B.setValue(E);E-=B.value;G=E-G;let L=0;0>E?L=w(B,E):0<E&&(L=t(B,E));this.dragInitX+=L;G-=L;A.fitToWidth&&(0>G?n(B,G):0<G&&C(B,G))}}]);A&&(A.notifyAddHdr(B),A.notifyHdrXChange(B),A.notifyHdrVisibilityChange(B));B.setWidth(B.value);B.resizer.setVisible(!(B.last&&B.gridController.fitToWidth))},
destroy:function(m){this.setGridController();this.callSuper(m)},setSortable:function(m){this.set("sortable",m,!0)},setSortState:function(m){this.set("sortState",m,!0)},setResizable:function(m){this.set("resizable",m,!0)},setCellWidthAdj:function(m){this.cellWidthAdj=m},setCellXAdj:function(m){this.cellXAdj=m},setFlex:function(m){this.flex=m},setColumnId:function(m){this.columnId=m},setResizerCursor:function(m,x){const B=this.resizer,y=B.cursor;m!==y&&(x&&(this.__rrc=y),B.setCursor(m))},restoreResizerCursor:function(){const m=
...
this.inited&&this.setWidth(this.value)},setMinValue:function(m){const x=this.minValue??0,B=this.gridController;this.callSuper(m);this.inited&&x!==this.minValue&&B?.setMinWidth(B.minWidth+this.minValue-x)},setMaxValue:function(m){const x=this.maxValue??0,B=this.gridController;m??=1E3;this.callSuper(m);this.inited&&x!==this.maxValue&&B?.setMaxWidth(B.maxWidth+this.maxValue-x)},setWidth:function(m){const x=this.width;this.callSuper(m);this.inited&&x!==this.width&&this.gridController?.notifyHdrWidthChange(this)},
setX:function(m){const x=this.x;this.callSuper(m);this.inited&&x!==this.x&&this.gridController?.notifyHdrXChange(this)},setVisible:function(m){const x=this.visible;this.callSuper(m);this.inited&&x!==this.visible&&this.gridController?.notifyHdrVisibilityChange(this)}}),N=q.GridRow=new r("GridRow",{initNode:function(m,x){x.placement??(x.placement=k);this.callSuper(m,x);this.gridController?.notifyAddRow(this)},destroy:function(m){this.setGridController();this.callSuper(m)},setGridController:function(m){const x=
this.gridController;x!==m&&(x?.notifyRemoveRow(this),this.gridController=m,this.inited&&m?.notifyAddRow(this))},notifyHdrXChange:function(m){this.getRef(m.columnId)?.setX(m.x+m.cellXAdj)},notifyHdrWidthChange:function(m){this.getRef(m.columnId)?.setWidth(m.width+m.cellWidthAdj)},notifyHdrVisibilityChange:function(m){this.getRef(m.columnId)?.setVisible(m.visible)}}),P=q.Grid=new u("Grid",d,{include:[D],extend:{SORT_ORDER_ASC:"ascending",SORT_ORDER_DESC:"descending"},initNode:function(m,x){x.overflow??
(x.overflow="autox");x.bgColor??(x.bgColor="#ccc");x.rowSpacing??(x.rowSpacing=1);x.columnSpacing??(x.columnSpacing=1);var B=x.isAutoScrolling;delete x.isAutoScrolling;this.callSuper(m,x);m=this.header=new d(this,{overflow:"hidden"});m.xLayout=new l(m,{locked:!0,collapseParent:!0,spacing:this.columnSpacing});m.yLayout=new q.SizeToChildren(m,{locked:!0,axis:"y"});x=this.sizeHeightToRows;B=this.content=new d(this,{overflow:x?"hidden":"autoy"},B?[q.AutoScroller]:[]);B.yLayout=new l(B,{locked:!0,axis:"y",
spacing:this.rowSpacing,collapseParent:x});this.syncTo(this,"setGridWidth","width");this.syncTo(m,"_updateContentWidth","width");this.constrain("_updateContentHeight",[x?B:this,"height",m,"height",m,"y"])},setSizeHeightToRows:function(m){this.sizeHeightToRows=m},setRowSpacing:function(m){this.rowSpacing!==m&&(this.rowSpacing=m,this.inited&&this.content.yLayout.setSpacing(m))},setColumnSpacing:function(m){this.columnSpacing!==m&&(this.columnSpacing=m,this.inited&&this.header.xLayout.setSpacing(m))},
setLocked:function(m){if(this.inited)for(const x of[this.header.xLayout,this.header.yLayout,this.content.yLayout])m?x.incrementLockedCounter():(x.decrementLockedCounter(),x.update());this.callSuper(m)},_updateContentWidth:function(m){const x=this.content;m=m.value;x.setWidth(m);for(const B of x.getSubviews())B.setWidth(m)},_updateContentHeight:function(m){const {header:x,content:B}=this;m=x.y+x.height;B.setY(m);this.sizeHeightToRows?this.setHeight(m+B.height):B.setHeight(this.height-m)},determinePlacement:function(m,
x){if(m===k){let B;x.isA(N)?B=this.content:x.isA(J)&&(B=this.header);if(B)return x.gridController!==this&&x.setGridController(this),B}return this.callSuper(m,x)},doSort:function(){const [m,x]=this.sort??["",""],B=this.getSortFunction(m,x);if(B){const y=this.content,A=y.yLayout;this.rows.sort(B);y.sortSubviews(B);A.sortSubviews(B);A.update()}},getSortFunction:(m,x)=>{if(m){const B=x===P.SORT_ORDER_ASC?1:-1;return(y,A)=>{y=y.getRef(m).text;A=A.getRef(m).text;return y>A?B:A>y?-B:0}}}});q.SimpleGridColHdr=
new u("SimpleGridColHdr",q.SimpleTextButton,{include:[J],initNode:function(m,x){x.activeColor??(x.activeColor="#999");x.hoverColor??(x.hoverColor="#bbb");x.readyColor??(x.readyColor="#aaa");x.inset??(x.inset=2);x.sortIconColor??(x.sortIconColor="#666");const B=x.textAlign;delete x.textAlign;x.outset??(x.outset="right"===B||(x.sortable??!0)?14:2);this.callSuper(m,x);this.sortIcon=new q.FontAwesome(this,{align:"right",alignOffset:3,valign:"middle",textColor:this.sortIconColor},[{initNode:function(y,
A){this.callSuper(y,A);this.getIDS().fontSize="0.7em"}}]);this.textView.enableEllipsis();B&&this.textView.setTextAlign(B);this.setDisabled(!this.sortable);z(this);K(this)},setSortIconColor:function(m){this.sortIconColor=m;this.sortIcon?.setTextColor(m)},setSortable:function(m){this.callSuper(m);this.inited&&(m&&this.setOutset(14),this.setDisabled(!m),K(this))},setSortState:function(m){this.callSuper(m);this.inited&&K(this)},setWidth:function(m){this.callSuper(m);this.inited&&z(this)},doActivated:function(){this.disabled||
...
z},notifyRefreshed:()=>{}}),p=q.SelectableInfiniteListRow=new u("SelectableInfiniteListRow",{include:[c,q.Selectable]}),w=q.InfiniteList=new v("InfiniteList",f,{initNode:function(z,D){let J=D.rowClasses;delete D.rowClasses;if("function"===typeof J){const N={};N["default"]=J;J=N}D.modelIDName??(D.modelIDName="id");D.numericSort??(D.numericSort=!0);D.ascendingSort??(D.ascendingSort=!0);D.overflow??(D.overflow="autoy");D.bgColor??(D.bgColor="#ccc");D.rowSpacing??(D.rowSpacing=1);D.rowInset??(D.rowInset=
...
const D=this.width;z.setWidth(D);for(const J of z.getSubviews())J.setWidth(D)}},getVisibleRows:function(){return Object.values(this._visibleRowsByIdx??{})},getVisibleRowForModel:function(z){const D=this.getVisibleRows();let J=D.length;for(;J;){const N=D[--J];if(N.model===z)return N}},getListViewHeight:function(){return this._listView.height},isScrolledToEnd:function(){return this.getIDE().scrollTop+this.height===this._listView.height},getSortFunction:function(){const z=this.modelIDName,D=this.ascendingSort;
return this.numericSort?q.getNumericObjSortFunc(z,D):e(z,D,!1)},getFilterFunction:()=>{},scrollModelIntoView:function(z,D){var J=this.getIndexOfModelInData(z);let N=!1;if(0<=J){var P=this._rowExtent;const m=this.getIDE().scrollTop,x=m+this.height;J=this.rowInset+J*P;P=J+P;J<=m?(this.scrollYTo(J,!0),N=!0):P>=x&&(this.scrollYTo(P-this.height,!0),N=!0);D&&((D=this.getVisibleRowForModel(z))?D.focus():this._focusToModel=z)}return N},isModelInData:function(z){return 0<=this.getIndexOfModelInData(z)},getNextModel:function(z,
D=!0,J=!0){const N=this.getListData(),P=N.length;z=this.getIndexOfModelInData(z);if(0<=z)return z+=1,z>=P?D?N[0]:N[P-1]:N[z];if(J&&0<P)return N[P-1]},getPrevModel:function(z,D=!0,J=!0){const N=this.getListData(),P=N.length;z=this.getIndexOfModelInData(z);if(0<=z)return--z,0>z?D?N[P-1]:N[0]:N[z];if(J&&0<P)return N[0]},getIndexOfModelInData:function(z){if(z){const D=this.getListData();let J=D.length;for(;J;)if(this.areModelsEqual(D[--J],z))return J}return-1},areModelsEqual:function(z,D){const J=this.modelIDName;
return z[J]===D[J]},getActiveRowForModel:function(z){if(z){const D=this._itemPool.getActives();let J=D.length;for(;J;){const N=D[--J];if(this.areModelsEqual(N.model,z))return N}}},refreshListData:function(z,D){this._listData=this.collectionModel.getAsSortedList(this.getSortFunction(),this.getFilterFunction());this.resetListUI(z,D)},resetListUI:function(z,D){const J=this.getListData().length,N=this._listView,P=this._scrollAnchorView;N.setHeight(J*this._rowExtent-(0<J?this.rowSpacing:0)+this.rowInset+
this.rowOutset);P.setY(N.height-P.height);this._startIdx=this._endIdx=-1;this.forceFullResetOnNextRefresh=D;z||0===this.getIDE().scrollTop?this.refreshListUI():this.scrollYTo(0,!0)},putRowBackInPool:function(z){if(k.focusedView?.isDescendantOf(z)){const D=this.getFocusTrap();D?D.focus():k.clear()}z.setVisible(!1);this._itemPool.putInstance(z)},refreshListUI:function(z){z=this._rowExtent;const D=this.rowInset,J=this.forceFullResetOnNextRefresh;var N=this.getIDE().scrollTop;const P=this.getListData()??
[];var m=h(0,r.floor((N-D)/z));N=r.min(P.length,r.ceil((N-D+this.height)/z));this.forceFullResetOnNextRefresh&&(this.forceFullResetOnNextRefresh=!1);if(this._startIdx!==m||this._endIdx!==N||J){const B=this.width,y=this.rowHeight,A=this._visibleRowsByIdx,E=this._focusToModel;this._startIdx=m;this._endIdx=N;for(var x in A)if(x<m||x>=N)this.putRowBackInPool(A[x]),delete A[x];for(x=m;x<N;x++){m=A[x];const G=P[x],L=this.getClassKey(G);let O=!1;m&&m.classKey===L||(m&&this.putRowBackInPool(m),A[x]=m=this._itemPool.getInstance(L),
m.setInfiniteOwner(this),m.setClassKey(L),m.setWidth(B),m.setHeight(y),m.setY(D+x*z),m.setVisible(!0),O=!0);!J&&m.model&&this.areModelsEqual(m.model,G)?O&&this.updateRow(m):(m.setModel(G),this.updateRow(m));m.notifyRefreshed();m.bringToFront();E&&this.areModelsEqual(E,G)&&(m.focus(),this._focusToModel=null)}}},getClassKey:z=>"default",updateRow:z=>{}}),t=q.SimpleSelectableInfiniteListRow=new v("SimpleSelectableInfiniteListRow",q.SimpleButton,{include:[p,q.ArrowKeyActivation],initNode:function(z,D){D.selectedColor??
...
(q=>{const {max:v,cos:u,sin:r,PI:h}=Math,f=q.Geometry.degreesToRadians,d=b=>{var g=f(b.startAngle),c=f(b.endAngle);if(g>c){var p=g;g=c;c=p}p=b.__path;var w=b.width/2,t=b.thickness;const n=b.radius,C=n+t;var K=c-g;const z=K+1E-4>=2*h;z&&(g=0,c=h);const D=[w+C*u(g),w+C*r(g)],J=[w+C*u(c),w+C*r(c)];c=[w+n*u(c),w+n*r(c)];g=[w+n*u(g),w+n*r(g)];w=["M"+D.join()];z?w.push("A"+[C,C,0,1,1,J].join(),"A"+[C,C,0,1,1,D].join(),"L"+c.join(),"A"+[n,n,0,1,0,g].join(),"A"+[n,n,0,1,0,c].join()):(K=K%(2*h)>h?1:0,t/=2,
w.push("A"+[C,C,0,K,1,J].join(),b.endCapRounding?"A"+[t,t,0,0,1,c].join():"L"+c.join(),"A"+[n,n,0,K,0,g].join(),b.startCapRounding?"A"+[t,t,0,0,1,D].join():""));w.push("z");p.setAttribute("d",w.join(" "));p.setAttribute("fill",b.color)},l=b=>{const g=2*(b.radius+b.thickness),c=b.__svg;b.setWidth(g);b.setHeight(g);c.setAttribute("viewBox","0 0 "+g+" "+g);c.setAttribute("width",g);c.setAttribute("height",g);d(b)},k=(b,g,c)=>{b[g]!==c&&(b[g]=c,b.inited&&(d(b),b.fireEvent(g,c)))},a=(b,g,c)=>{b[g]!==c&&
...
c},setShadowColor:function(c){this.shadowColor=c},setInsetH:function(c){this.insetH=c},setInsetV:function(c){this.insetV=c},setTipBgColor:function(c){this.tipBgColor=c},setTipTextColor:function(c){this.tipTextColor=c},showTip:function(){const c=this.tooltip;var p=c.text;const w=c.parent;var t=this._tipText;const n=this.edgeSize,C=this.shadowSize;t.text!==p&&t.setText(p);t.setWidth("auto");var K=l(t.measureNoWrapWidth(),c.maxTextWidth);p=K+2*t.x;const z=p+2*n;t.setWidth(K);t.sizeViewToDom();K=w.getPagePosition();
t=t.height+2*t.y;const D=t+2*n,J=K.y-D+("below"===c.tipvalign?w.height+D:0);K=K.x;switch(c.tipalign){case "right":K-=z;case "farright":K+=w.width;break;case "farleft":K-=z}this.setX(a(l(k(K,0),h.getWidth()-z)));this.setY(a(l(k(J,0),h.getHeight()-D)));this.setWidth(p);this.setHeight(t);this.setBgColor(this.tipBgColor);this.setBorder([n,"solid",this.edgeColor]);this.setBoxShadow([C,C,C,this.shadowColor]);this.callSuper()}});const g=q.TooltipMixin=new JS.Module("TooltipMixin",{extend:{TIP_CLASS:u,MAX_TEXT_WIDTH:280},
...
g+l);w(b,p-a);0<a&&t(b,p,b+a,p);w(c-e,p);0<e&&t(c,p,c,p-e);w(c,g+k);0<k&&t(c,g,c-k,g);w(b+l,g);0<l&&t(b,g,b,g+l);this.closePath();return this},drawAnnulus:function(l,k,a,e,b,g,c,p=60){const w=q.Color;for(var t of c)"string"===typeof t.color&&(t.color=w.makeColorFromHexString(t.color));1>p&&(v("Invalid segements",p),p=60);p=u/p;t=0;for(var n=c.length-1;t<n;){var C=c[t++];const {angle:B,color:y}=C,{angle:A,color:E}=c[t];var K=Math.round((A-B)/p),z=y.getDiffFrom(E);C.colorDelta={red:z.red/K,green:z.green/
K,blue:z.blue/K}}a=new q.Path([l+a,k,l+e,k]);a.rotateAroundOrigin(b,l,k);e=a.vectors;let [D,J,N,P]=e,m=0,x=u/360;for(t=0;g>b;){K=D;z=J;n=N;C=P;b+=p;b>g&&(p+=g-b,x=0,b=g);a.rotateAroundOrigin(p+x,l,k);[D,J,N,P]=e;this.beginPath();this.moveTo(K,z);this.lineTo(n,C);this.lineTo(N,P);this.lineTo(D,J);this.closePath();const {color:B,colorDelta:y}=c[t];this.setFillStyle(w.rgbToHex(B.red+m*y.red,B.green+m*y.green,B.blue+m*y.blue,!0));this.fill();0<x&&(a.rotateAroundOrigin(-x,l,k),[D,J,N,P]=e);m++;b>=c[t+
