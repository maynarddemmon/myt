...
this.button?.updateUI()},setTabId:function(b){this.tabId=b},setTabContainer:function(b){this.tabContainer=b},setLayoutPaddingY:function(b){this.layoutPaddingY=b;this.inited&&this._wrapperLayout.setPaddingY(b)},setMinContainerHeight:function(b){this.minContainerHeight=b},setButtonClass:function(b){this.buttonClass=b},setFillColorSelected:function(b){this.fillColorSelected=b},setFillColorHover:function(b){this.fillColorHover=b},setFillColorActive:function(b){this.fillColorActive=b},setFillColorReady:function(b){this.fillColorReady=
b},setWrapperExpandedOverflow:function(b){this.wrapperExpandedOverflow=b},setButtonHeight:function(b){this.buttonHeight!==b&&(this.buttonHeight=b,this.button&&(this.button.setHeight(b),this.wrapper.setY(b)))},setExpansionState:function(b){if(this.expansionState!==b){this.expansionState=b;this.inited&&this.fireEvent("expansionState",b);const d=this.wrapper;d&&("expanded"===b?(d.setMaskFocus(!1),d.setOverflow(this.wrapperExpandedOverflow),this.tabContainer?.saveState()):"expanding"===b?d.setVisible(!0):
"collapsed"===b?(d.setVisible(!1),this.tabContainer?.saveState()):"collapsing"===b&&(d.setMaskFocus(!0),d.setOverflow("hidden")))}},doDisabled:function(){this.button?.setDisabled(this.disabled)},notifyButtonRedraw:()=>{},__updateHeight:function(b){this.setHeight(this.wrapper.y+this.wrapper.height)},expand:function(b){const d=this,a=d.wrapper;b-=d.getCollapsedHeight();d.setExpansionState("expanding");a.stopActiveAnimators();if(a.height!==b){const f=d.tabContainer.duration;1===f?(a.setHeight(b),d.setExpansionState("expanded")):
a.animate({attribute:"height",to:b,duration:f}).next(c=>{d.setExpansionState("expanded")})}else d.setExpansionState("expanded")},collapse:function(){const b=this,d=b.wrapper;b.setExpansionState("collapsing");d.stopActiveAnimators();if(0!==d.height){const a=b.tabContainer.duration;1===a?(d.setHeight(0),b.setExpansionState("collapsed")):d.animate({attribute:"height",to:0,duration:a}).next(f=>{b.setExpansionState("collapsed")})}else b.setExpansionState("collapsed")},getCollapsedHeight:function(){return this.buttonHeight},
getMinimumExpandedHeight:function(){return Math.min(this.getPreferredExpandedHeight(),this.buttonHeight+this.minContainerHeight)},getPreferredExpandedHeight:function(){return this.buttonHeight+this.wrapper.container.height}}),k=(b,d,a,f)=>{b[d]!==f&&(b[d]=f,b.button?.label?.set(labelAttrName,f))};t.TextTabSlider=new x("TextTabSlider",g,{initNode:function(b,d){this.labelTextColorChecked="#fff";this.labelTextColor="#333";this.labelAlign="center";this.labelValign="middle";this.labelFontSize="16px";this.labelFontWeight=
"bold";d.labelTextColorChecked??(d.labelTextColorChecked="#fff");d.labelTextColor??(d.labelTextColor="#333");d.labelAlign??(d.labelAlign="center");d.labelValign??(d.labelValign="middle");d.labelFontSize??(d.labelFontSize="16px");d.labelFontWeight??(d.labelFontWeight="bold");this.callSuper(b,d);this.button.label=new t.Text(this.button,{ignorePlacement:!0,fontSize:this.labelFontSize,fontWeight:this.labelFontWeight,text:this.text,align:this.labelAlign,valign:this.labelValign,textColor:this.__getTextColor()})},
...
"text","text",b)},notifyButtonRedraw:function(){this.button?.label?.setTextColor(this.__getTextColor())},__getTextColor:function(){return this.selected&&-1!==this.tabContainer.maxSelected?this.labelTextColorChecked:this.labelTextColor}});const n=t.TabSliderContainer=new JS.Module("TabSliderContainer",{include:[t.SelectionManager],extend:{SPACING:1},initNode:function(b,d){const a=this;a._tabSliders=[];d.defaultPlacement="container";d.spacing??(d.spacing=n.SPACING);d.overflow??(d.overflow="autoy");
d.itemSelectionId??(d.itemSelectionId="tabId");d.maxSelected??(d.maxSelected=1);d.duration??(d.duration=500);a.updateLayout=e(a.updateLayout);a.callSuper(b,d);b=a.container=new v(a,{ignorePlacement:!0,percentOfParentWidth:100},[u,{subnodeAdded:function(f){this.callSuper(f);f instanceof g&&(a._tabSliders.push(f),a.attachTo(f,"updateLayout","selected"))},subnodeRemoved:function(f){if(f instanceof g){const c=a._tabSliders,p=c.indexOf(f);-1<p&&(a.detachFrom(f,"updateLayout","selected"),c.splice(p,1))}this.callSuper(f)}}]);
b.layout=new t.SpacedLayout(b,{axis:"y",spacing:a.spacing,collapseParent:!0});a.attachTo(a,"updateLayout","height")},setPersistenceId:function(b){this.persistenceId=b},getTabSliders:function(){return this._tabSliders},getTabSliderById:function(b){for(const d of this._tabSliders)if(d.tabId===b)return d},setSpacing:function(b){this.spacing!==b&&(this.spacing=b,this.layout?.setSpacing(b))},setDuration:function(b){this.duration=b},updateLayout:function(b,d){b=this._tabSliders;for(var a=b.length,f=a,c=
0,p=0,w=0;f;){var r=b[--f];r.visible&&(++w,r.selected?(c+=r.getMinimumExpandedHeight(),p+=r.getPreferredExpandedHeight()):(r=r.getCollapsedHeight(),c+=r,p+=r))}f=this.container.layout;w=f.inset+f.outset+f.spacing*(w-1);p+=w;f=this.height;c=c+w>f;w=p>f;r=this.duration;p-=f;0<d&&this.setDuration(d);for(f=a;f;)if(a=b[--f],a.visible)if(a.selected){let m;if(c)m=a.getMinimumExpandedHeight();else if(w){const A=a.getPreferredExpandedHeight(),H=a.getMinimumExpandedHeight();m=A-p;H>m?(p-=A-H,m=H):p=0}else m=
a.getPreferredExpandedHeight();a.expand(m)}else a.collapse();0<d&&this.setDuration(r)},readState:b=>h.getDatum(b),writeState:(b,d)=>{h.setDatum(b,d)},saveState:e(function(){const b=[];for(const d of this.getTabSliders())"expanded"===d.expansionState&&b.push(d.tabId);this.writeState(this.persistenceId,b)},1E3),restoreState:function(b){for(const d of this.readState(this.persistenceId)||b)this.select(this.getTabSliderById(d))}})})(myt);
