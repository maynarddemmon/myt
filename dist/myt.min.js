...
{},getMinPixelValueForThumb:function(a){return this.convertValueToPixels(this.minValue)},getMaxPixelValueForThumb:function(a){return this.convertValueToPixels(this.maxValue)}});r.RangeSlider=new w("RangeSlider",k,{include:[r.BoundedRangeComponent],initNode:function(a,f){let b=f.rangeFillSize;const g=f.rangeFillOffset??0,c=f.rangeFillColor??"#666";delete f.rangeFillSize;delete f.rangeFillOffset;delete f.rangeFillColor;this.callSuper(a,f);a=this.rangeFill=new e(this,{bgColor:c});"x"===this.axis?(b??=
this.thumbHeight,a.setY(g+this.thumbOffset),a.setHeight(b)):(b??=this.thumbWidth,a.setX(g+this.thumbOffset),a.setWidth(b));a.setRoundedCorners(b/2);this.thumbLower=new m(this);this.thumbUpper=new m(this);this._syncRangeFillToValue()},setValue:function(a){this.callSuper(a);this.inited&&(this.__lockSync||(a=this.getValue(),this._syncThumbToValue(this.thumbLower,a),this._syncThumbToValue(this.thumbUpper,a)),this._syncRangeFillToValue())},setWidth:function(a){const f=this.width;this.callSuper(a);this.inited&&
"x"===this.axis&&this.width!==f&&(a=this.getValue(),this._syncThumbToValue(this.thumbLower,a),this._syncThumbToValue(this.thumbUpper,a))},setHeight:function(a){const f=this.height;this.callSuper(a);this.inited&&"y"===this.axis&&this.height!==f&&(a=this.getValue(),this._syncThumbToValue(this.thumbLower,a),this._syncThumbToValue(this.thumbUpper,a))},_syncRangeFillToValue:function(){const a=this.rangeFill;var f=this.getValue();const b=this.convertValueToPixels(f.lower);f=this.convertValueToPixels(f.upper)-
b;"x"===this.axis?(a.setX(b),a.setWidth(f)):(a.setY(b),a.setHeight(f))},_syncThumbToValue:function(a,f){this.callSuper(a,a===this.thumbLower?f.lower:f.upper)},_syncValueToThumb:function(a,f){let b=this.getValueCopy();b[a===this.thumbLower?"lower":"upper"]=f;this.setValue(b);b=this.getValue();this.thumbLower&&this._syncThumbToValue(this.thumbLower,b);this.thumbUpper&&this._syncThumbToValue(this.thumbUpper,b)},_nudge:function(a,f){const b=this.getValueCopy();f=this.nudgeAmount*(f?1:-1);a===this.thumbLower?
b.lower=v(b.lower+f,b.upper):b.upper=t(b.upper+f,b.lower);this.setValue(b)},getMinPixelValueForThumb:function(a){return this.convertValueToPixels(a===this.thumbLower?this.minValue:this.getValue().lower)},getMaxPixelValueForThumb:function(a){return this.convertValueToPixels(a===this.thumbLower?this.getValue().upper:this.maxValue)}});r.Slider=new w("Slider",k,{include:[r.BoundedValueComponent],initNode:function(a,f){this.callSuper(a,f);this.thumb=new m(this)},setValue:function(a){this.callSuper(a);
this.inited&&!this.__lockSync&&this._syncThumbToValue(this.thumb,this.getValue())},setWidth:function(a){const f=this.width;this.callSuper(a);this.inited&&"x"===this.axis&&this.width!==f&&this._syncThumbToValue(this.thumb,this.getValue())},setHeight:function(a){const f=this.height;this.callSuper(a);this.inited&&"y"===this.axis&&this.height!==f&&this._syncThumbToValue(this.thumb,this.getValue())},_syncValueToThumb:function(a,f){this.setValue(f);this._syncThumbToValue(a,this.getValue())},_nudge:function(a,
f){this.setValue(this.getValue()+this.nudgeAmount*(f?1:-1))}});r.LabelSlider=new w("LabelSlider",r.Slider,{initNode:function(a,f){const b=this;f.labelX??(f.labelX=8);f.labelY??(f.labelY=2);f.labelFontSize??(f.labelFontSize="12px");f.labelColor??(f.labelColor="#000");f.flipThreshold??(f.flipThreshold=h((f.maxValue-f.minValue)/2)||0);b.quickSet(["labelX","labelY","labelFontSize","labelColor","flipThreshold"],f);b.callSuper(a,f);const g=b.labelX;b.labelTxt=new r.Text(b,{x:g,y:b.labelY,textColor:b.labelColor,
fontSize:b.labelFontSize},[{initNode:function(c,q){this.targetX=g;this.callSuper(c,q)},updateX:function(c){const q=b.getValue()>=b.flipThreshold?g:b.width-this.width-g;this.targetX!==q&&(this.targetX=q,this.stopActiveAnimators("x"),c?this.setX(q):this.animate({attribute:"x",to:q,duration:250}))}}]);b.syncTo(b,"setText","value")},setWidth:function(a){this.callSuper(a);this.labelTxt?.updateX(!0)},setText:function(a,f){const b=this.labelTxt;b&&(b.setText(a.value),b.updateX(f))}})})(myt);
...
(r=>{const {Class:w,Module:v}=JS,{View:t,Text:h,FontAwesome:{makeTag:d}}=r,e=d(["times"]);r.GrowlManager=new w("GrowlManager",t,{include:[r.SizeToWindowHeight],initNode:function(f,b){const g=this;g.growlCount=0;b.overflow??(b.overflow="visible autoy");b.margin??(b.margin=8);b.spacing??(b.spacing=4);b.reverse??(b.reverse=!0);b.width??(b.width=300);b.visible??(b.visible=!1);b.pointerEvents??(b.pointerEvents="none");b.moveDuration??(b.moveDuration=300);b.dedupe??(b.dedupe=!0);b.clearAllThreshold??(b.clearAllThreshold=
0);b.simpleGrowlClass??(b.simpleGrowlClass=r.SimpleGrowl);g.quickSet(["moveDuration","dedupe"],b);g.callSuper(f,b);g.doAlignment();const c=g.moveDuration;new r.SpacedLayout(g,{axis:"y",reverse:g.reverse,spacing:g.spacing},[{updateParent:function(q,x){0<c?(g.stopActiveAnimators("height"),g.animate({attribute:"height",to:x+this.outset-this.spacing,duration:c})):this.callSuper(q,x)},updateSubview:function(q,x,u,n){return 0<c?(x.stopActiveAnimators("y"),x.animate({attribute:"y",to:n,duration:c}),n+x.height+
this.spacing):this.callSuper(q,x,u,n)}}])},setHeight:function(f){this.callSuper(f-2*this.margin)},setVisible:function(f){this.visible!==f&&(this.visible=f,this.getODS().display=f?"block":"none",this.inited&&this.fireEvent("visible",f))},setMargin:function(f){this.set("margin",f,!0);this.getODS().margin=f+"px"},setSpacing:function(f){this.set("spacing",f,!0)},setReverse:function(f){this.set("reverse",f,!0)},doAlignment:function(){const f=this.getODS();f.left=null;f.right="0px"},notifyChanged:function(){this.setVisible(0<
this.getSubviews().length)},subviewAdded:function(f){this.callSuper(f);this.notifyChanged()},subviewRemoved:function(f){this.callSuper(f);this.notifyChanged()},addGrowl:function(f){if(this.dedupe){const b=f.getHash(),g=this.getSubviews();let c=g.length;for(;c;){const q=g[--c];if(q!==f&&q.getHash?.()===b){this.removeGrowl(q);break}}}this.growlCount++;f.setVisible(!0);this.updateClearAll()},removeGrowl:function(f){f.destroy();this.growlCount--;this.updateClearAll()},updateClearAll:function(){const f=
this.clearAllThreshold;0<f&&this.growlCount>=f?(this._clearAllBtn??(this._clearAllBtn=this.makeClearAll()),this._clearAllBtn.setVisible(!0)):this._clearAllBtn?.setVisible(!1)},makeClearAll:function(){const f=this;return new r.TextButton(f,{width:f.width,height:28,text:e+" Dismiss All",fontWeight:"bold",paddingTop:5,zIndex:1,visible:!1,ignoreLayout:!0,pointerEvents:"auto"},[{setVisible:function(b){this.callSuper(b);f.getFirstLayout().setInset(this.visible?this.height+f.spacing:0)},doActivated:function(){const b=
f.getFirstLayout();b.incrementLockedCounter();f.growlCount=0;delete f._clearAllBtn;b.setInset(0);f.destroyAllSubviews();b.decrementLockedCounter()}}])},addSimpleGrowl:function(f,b){this.addGrowl(new this.simpleGrowlClass(this,{text:f,...(b??{})}))}});r.KeepShowingMixin=new v("KeepShowingMixin",{include:[r.MouseOver],initNode:function(f,b){b.initialKeepDuration??(b.initialKeepDuration=-1);b.keepDuration??(b.keepDuration=-1);this.quickSet(["initialKeepDuration","keepDuration"],b);this.callSuper(f,b);
f=this.initialKeepDuration;0<=f&&this.keepShowing(!1,f)},keepShowing:function(f,b){const g=this;b??=g.keepDuration;(g.__keepShowing=f)&&g.__timerIdKS&&clearTimeout(g.__timerIdKS);0<=b&&!f&&(g.__timerIdKS=setTimeout(()=>{("function"===typeof g.isShown?g.isShown():g.visible)&&!g.__keepShowing&&("function"===typeof g.hide?g.hide(!0):g.setVisible(!1))},b))},doMouseOver:function(f){this.keepShowing(!0);this.callSuper(f)},doMouseOut:function(f){this.keepShowing(!1);this.callSuper(f)}});r.KeepShowingChildMixin=
new v("KeepShowingChildMixin",{doMouseOver:function(f){this.callSuper(f);return!0},doMouseOut:function(f){this.callSuper(f);return!0}});r.GrowlMixin=new v("GrowlMixin",{include:[r.SizeToParent,r.KeepShowingMixin],initNode:function(f,b){b.percentOfParentWidth??(b.percentOfParentWidth=100);b.pointerEvents??(b.pointerEvents="auto");b.bgColor??(b.bgColor="#fff");b.roundedCorners??(b.roundedCorners=3);b.boxShadow??(b.boxShadow=[0,0,8,"#00000088"]);b.initialKeepDuration??(b.initialKeepDuration=5E3);b.keepDuration??
(b.keepDuration=1E3);b.showDuration??(b.showDuration=300);b.hideDuration??(b.hideDuration=300);this.quickSet(["showDuration","hideDuration"],b);b.opacity=0;b.visible=!1;this.callSuper(f,b)},setVisible:function(f){const b=this;b.inited?(b.stopActiveAnimators("opacity"),f?(b.animate({attribute:"opacity",to:1,duration:b.showDuration}),b.callSuper(f)):b.animate({attribute:"opacity",to:0,duration:b.hideDuration}).next(g=>{b.parent.removeGrowl(b)})):b.callSuper(f)},getHash:function(){return this.hash??
(this.hash=r.hash(this.getHashableString()))},getHashableString:()=>""+r.generateGuid()});const m=d(["clipboard-check"]),k="maxHeight icon iconSize text padding spacing whiteSpace showCloseButton closeIcon closeOnly showCopyButton copyIcon activeColor hoverColor readyColor".split(" "),a=new w("LocalTxtBtn",r.TextButton,{include:[r.KeepShowingChildMixin],initNode:function(f,b){b.width??(b.width=24);b.height??(b.height=24);b.paddingTop??(b.paddingTop=4);b.activeColor??(b.activeColor="#ccc");b.hoverColor??
(b.hoverColor="#ddd");b.readyColor??(b.readyColor="#eee");this.callSuper(f,b)}});r.SimpleGrowl=new w("SimpleGrowl",t,{include:[r.GrowlMixin],initNode:function(f,b){const g={};b.maxHeight??(b.maxHeight=100);b.spacing??(b.spacing=6);b.padding??(b.padding=12);b.whiteSpace??(b.whiteSpace="normal");b.iconSize??(b.iconSize="24px");b.showCloseButton??(b.showCloseButton=!1);b.showCloseButton&&(b.closeIcon??(b.closeIcon=e),b.closeOnly??(b.closeOnly=!1));b.showCopyButton??(b.showCopyButton=!1);b.showCopyButton&&
(b.copyIcon??(b.copyIcon=m));for(const u of k)g[u]=b[u],delete b[u];g.closeOnly&&(b.initialKeepDuration=b.keepDuration=-1);this.callSuper(f,b);const {maxHeight:c,padding:q,icon:x}=g;x&&(this.iconTxt=new h(this,{y:q,text:x,fontSize:g.iconSize}));f=this.msgTxt=new h(this,{y:q,text:g.text,whiteSpace:g.whiteSpace,layoutHint:1});0<c&&(f.setOverflow("auto"),f.getIDS().maxHeight=c+"px");g.showCopyButton&&(this.copyBtn=this.makeCopyButton(g),f.setUserUnselectable(!1));g.showCloseButton&&(this.closeBtn=this.makeCloseButton(g));
new r.ResizeLayout(this,{inset:q,spacing:g.spacing,outset:q});new r.SizeToChildren(this,{axis:"y",paddingY:q})},makeCloseButton:function(f){const b=this;new a(b,{y:f.padding,text:f.closeIcon,tooltip:"Dismiss this notification.",activeColor:f.activeColor,hoverColor:f.hoverColor,readyColor:f.readyColor},[{doActivated:()=>{b.parent.removeGrowl(b)}}])},makeCopyButton:function(f){const b=this;new a(b,{y:f.padding,text:f.copyIcon,tooltip:"Copy to system clipboard.",activeColor:f.activeColor,hoverColor:f.hoverColor,
readyColor:f.readyColor},[{doActivated:()=>{global.isSecureContext?navigator.clipboard.writeText(r.removeMarkup(b.msgTxt.text,{brToLineFeed:!0})):console.warn("access to clipboard blocked because of insecure context.")}}])},getHashableString:function(){return this.iconTxt?.text+" | "+this.msgTxt.text}})})(myt);myt.all=!0;
