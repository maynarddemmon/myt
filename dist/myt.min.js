...
!1;if(v){let H=!1;D.onload=D.onreadystatechange=()=>{H||D.readyState&&"complete"!==D.readyState||(H=!0,D.onload=D.onreadystatechange=null,v())}}D.src=l+(A?"":(l.includes("?")?"&":"?")+"cacheBust="+Date.now());g.appendChild(D);return D},dumpStack:(l,v)=>{let A;"string"===typeof l&&(A=l,l=null);y.global.error.notify(v||"error",null,A,l)},getRandom:l=>{const v=d.random();return l?k(0,h(l(v),.9999999999)):v},getRandomArbitrary:(l,v,A)=>{const B=h(l,v);return y.getRandom(A)*(k(l,v)-B)+B},getRandomInt:(l,
...
v,A)=>{const B="__DBTO_"+ ++w;return function(){const D=this,H=D[B],K=arguments,Q=A&&!H;clearTimeout(H);D[B]=setTimeout(function(){D[B]=null;A||l.apply(D,K)},v);Q&&l.apply(D,K)}},createFixedThresholdCounter:(l,v,A,B)=>{var D=y.AccessorSupport.generateName;const H="function"===typeof l||l instanceof JS.Module,K={};B=B||D("counter",A);const Q=D(B,"increment");D=D(B,"decrement");if(void 0!==(H?l.instanceMethod(Q):l[Q]))return m("Increment: Abort clobber",Q,l),!1;if(void 0!==(H?l.instanceMethod(D):l[D]))return m("Decrement: Abort clobber",
D,l),!1;K[Q]=function(){const S=this[B]+1;this[B]=S;this.fireEvent(B,S);S===v&&this.set(A,!0)};K[D]=function(){const S=this[B];if(0!==S){var M=S-1;this[B]=M;this.fireEvent(B,M);S===v&&this.set(A,!1)}};l[H?"include":"extend"](K);return!0},makeURLSearchParams:(l={})=>{const v=new URLSearchParams;for(const A in l){const B=l[A];void 0!==B&&v.set(A,B)}return v},doFetch:(l,v,A,B,D,H)=>fetch(l,v).then(K=>{if(!K.ok)throw new x(K.status,l,K.statusText);return K}).then(K=>A?K.text():K.json()).then(K=>{if(B)try{if(A)B(K);
...
A.replaceAll(G,(B,D,H,K)=>H&&K?1==v[parseInt(D)]?H:K:"").replaceAll(L,(B,D)=>v[D]):A:l}}};p.onloadingdone=l=>{for(const v of l.fontfaces)r(v)}})(global);
