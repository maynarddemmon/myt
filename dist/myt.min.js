...
G.initialFocus&&G.initialFocus.focus()},showConfirm:function(D,G,F){F=da({},S.CONFIRM_DEFAULTS,F);this.showMessage(D,G,F);this.setupFooterButtons(this.content.msg,F)},showContentConfirm:function(D,G,F,J){F=da({},S.CONFIRM_DEFAULTS,F);const M=this.content,K=M.closeBtn,O=F.maxContainerHeight;this.destroyContent();M.sizeToChildren.setPaddingX(1);this.setCallbackFunction(G);G=new R(M,{name:"contentContainer",x:1,y:25,overflow:"auto"},[{setHeight:function(N){this.callSuper(N>O?O:N)}}]);D.call(this,G);
new ra(G,{axis:"both"});this.show();K.setVisible(!0);K.focus();this.setupTitle(M,F.titleText);G.setY(this.header.height+1);this.setupFooterButtons(G,F);G.initialFocus&&G.initialFocus.focus();J&&J(this)},showSpinner:function(D,G){const F=this.content;G=da({},S.NO_WRAP_TEXT_DEFAULTS,G);this.destroyContent();const J=this.spinner=new m.Spinner(F,{align:"center",visible:!0,borderColor:"#ccc",size:50,y:null==G.msgY?W.PADDING_Y:G.msgY});D&&new ha(F,{text:D,whiteSpace:G.whiteSpace,wordWrap:G.wordWrap,fontWeight:G.fontWeight,
...
setupFooterButtons:function(D,G){const F=this,J=F.content;var M=W.PADDING_Y,K=M/2;const O=new R(J,{name:"btnContainer",y:D.y+D.height+M,align:"center"});M=["active","hover","ready","text"];let N=G.cancelAttrs||{};X(N,"name","cancelBtn");X(N,"text",G.cancelTxt);M.forEach(Q=>{Q+="Color";null!=G[Q]&&(N[Q]=G[Q])});D=F.makeButton(O,N);N=G.confirmAttrs||{};X(N,"name","confirmBtn");X(N,"text",G.confirmTxt);M.forEach(Q=>{Q+="Color";const ma=Q+"Confirm";null!=G[ma]&&(N[Q]=G[ma])});F.makeButton(O,N);(G.buttons||
[]).forEach(Q=>{F.makeButton(O,Q)});new ra(O,{axis:"y"});new ya(O,{spacing:4,collapseParent:!0});J.sizeToChildren.setPaddingY(K);M=S.RADIUS;K=O.y-K;(new R(J,{ignoreLayout:!0,y:K,width:J.width,height:J.height-K,bgColor:"#eee",roundedBottomLeftCorner:M,roundedBottomRightCorner:M})).sendToBack();!1===G.showClose&&D.focus()}})})(myt);
