...
","!==D[1]&&B.push([]),B[B.length-1].push(D[2]?D[2].replace(O,"$1"):D[3]);if(y){const F=B.shift();return B.map(K=>{let L=0;return F.reduce((P,N)=>{P[N]=K[L++];return P},{})})}return B},prepareCSVDataURI:(p,y)=>{const B=K=>{if(K&&b(K)){const L=[],P=K.length;for(let N=0,U;P>N;N++)U=K[N],U=null==U?"":U.toString(),U=U.replace(/"/g,'""'),0<=U.search(/("|,|\n)/g)&&(U='"'+U+'"'),L.push(U);return L.join(",")}d("Unexpected row",K);return null},D=[];if(y){var F=B(y);F&&D.push(F)}if(p)if(b(p)){F=p.length;for(let K=
0;F>K;K++){const L=B(p[K]);L&&D.push(L)}}else d("Rows were not an array");return l.encodeCSVDataURI(D.join("\r\n"),y)},encodeCSVDataURI:(p,y)=>"data:text/csv;charset=utf-8"+(null==y?"":";header="+(y?"present":"absent"))+","+encodeURIComponent(p),dataURIToBlob:p=>{var y=p.indexOf(",");const B=p.slice(0,y).split(":")[1].split(";")[0];p=p.slice(y+1);if(B.startsWith("text/"))p=decodeURIComponent(p);else{p=atob(p);y=p.length;const D=new Uint8Array(y);for(;y;)D[--y]=p.charCodeAt(y);p=D}return new Blob([p],
