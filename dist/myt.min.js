...
(t=>{class x extends Error{constructor(l,v,...A){super(...A);this.name="FetchError";this.status=l;this.url=v}}let w=0,u;const m=console.warn,d=Math,{abs:c,min:h,max:k,pow:a}=d,e=Array.isArray,f=document,g=f.head,b=f.createElement.bind(f),q={},n={"":!0},p=f.fonts,r=l=>{var v=l.family.split('"').join("");for(const A of[v,v+" "+l.weight])if(!n[A])for(n[A]=!0,v=q[A]??[];v.length;)(l=v.pop())&&!l.destroyed&&l.isVisible()&&l.sizeViewToDom()},z={},E=l=>{const v={};return(...A)=>{const B=JSON.stringify(A);
return B in v?v[B]:v[B]=l(...A)}},G=/\{\{plural:\$(.*?)\|(.*?)\|(.*?)\}\}/g,L=/\$(\d+)/g,J=/(,|\r?\n|\r|^)(?:"((?:\\.|""|[^\\"])*)"|([^,"\r\n]*))/gi,N=/[\\"](.)/g,y=t.myt={version:2.02306021558E7,generateGuid:()=>++w,hash:l=>l.split("").reduce((v,A)=>{v=(v<<5)-v+A.charCodeAt(0);return v&v},0),addEventListener:(l,v,A,B,D)=>{l.addEventListener(v,A,{capture:B||!1,passive:D||!1})},removeEventListener:(l,v,A,B)=>{l.removeEventListener(v,A,B||!1)},resolveName:(l,v)=>{if(l&&0!==l.length){v=v??global;var A=
e(l)?l:l.split("."),B=A.length;for(let D=0;D<B;D++)if(v=v[A[D]],null==v){m("resolveName failed for",l,"at part",D,A[D]);return}return v}},resolveClassname:l=>{"string"===typeof l&&(l=y.resolveName(l));return l&&"function"===typeof l.isA&&l.isA(JS.Class)?l:null},getExtension:l=>l?(l=l.split("."),1<l.length?l.pop():null):null,loadScript:function(l,v,A){const B=this._loadedScripts??(this._loadedScripts={});if(B[l])return m("script already loaded for src",l),null;B[l]=!0;const D=b("script");D.type="text/javascript";
D.async=!1;if(v){let H=!1;D.onload=D.onreadystatechange=()=>{H||D.readyState&&"complete"!==D.readyState||(H=!0,D.onload=D.onreadystatechange=null,v())}}D.src=l+(A?"":(l.includes("?")?"&":"?")+"cacheBust="+Date.now());g.appendChild(D);return D},dumpStack:(l,v)=>{let A;"string"===typeof l&&(A=l,l=null);y.global.error.notify(v||"error",null,A,l)},getRandom:l=>{const v=d.random();return l?k(0,h(l(v),.9999999999)):v},getRandomArbitrary:(l,v,A)=>{const B=h(l,v);return y.getRandom(A)*(k(l,v)-B)+B},getRandomInt:(l,
...
"body")[0<v?v:0],loadFontFaces:(l,v)=>{const A=[];for(const B of l)l=new FontFace(B.family,"url("+B.url+")",B.options),A.push(l.load());Promise.all(A).then(B=>{for(const D of B)p.add(D),r(D);v?.(B)})},loadFontFace:(l,v,A={},B)=>{l=new FontFace(l,"url("+v+")",A);l.loaded.then(D=>{p.add(D);r(D);B?.(D)});l.load()},registerForFontNotification:(l,v)=>{n[v]?l&&!l.destroyed&&l.isVisible()&&l.sizeViewToDom():(q[v]??(q[v]=[])).push(l)},createBaseFontCSSRule:l=>{y.addCSSRule(y.createStylesheet(),"body, input",
"font-family:"+l,0)},loadCSSFonts:l=>{l?.forEach(y.createStylesheetLink)},createStylesheetLink:l=>{const v=b("link");v.rel="stylesheet";l&&(v.href=l);g.appendChild(v);return v},createStylesheet:()=>{const l=b("style");g.appendChild(l);return l.sheet},addCSSRule:(l,v,A,B)=>{l.insertRule(v+"{"+A+"}",B)},removeCSSRules:l=>{let v=l.cssRules.length;for(;v;)l.deleteRule(--v)},createInputPlaceholderCSSRule:(l,v,A,B=1)=>{let D=l.__sheet;D?y.removeCSSRules(D):D=l.__sheet=y.createStylesheet();l=l.getODE();
l=l.id||(l.id="id"+ ++w);const H=[];v&&H.push("color:"+v);A&&H.push("font-family:"+A);B&&H.push("opacity:"+B);y.addCSSRule(D,"#"+l+"::placeholder",H.join("; "),0)},isNotSorted:(l,v)=>{const A=l.length;for(let B=1;B<A;B++)if(0<v(l[B-1],l[B]))return!0},chainSortFunc:(...l)=>(v,A)=>{for(const B of l){const D=B(v,A);if(0!==D)return D}return 0},getAlphaObjSortFunc:E((l,v,A,B)=>{const D=v?1:-1;return(H,K)=>{H=H[l];K=K[l];B?("string"!==typeof H&&(H=""+H),"string"!==typeof K&&(K=""+K)):(H=H||"",K=K||"");
A&&(H=H.toLowerCase(),K=K.toLowerCase());return H.localeCompare(K)*D}}),getNumericObjSortFunc:E((l,v)=>{const A=v?1:-1;return(B,D)=>{B=B[l]||0;D=D[l]||0;return(B-D)*A}}),csvStringToArray:(l,v)=>{if(!l)return[];const A=[[]];let B;for(;B=J.exec(l);)B[1].length&&","!==B[1]&&A.push([]),A[A.length-1].push(B[2]?B[2].replace(N,"$1"):B[3]);if(v){const D=A.shift();return A.map(H=>{let K=0;return D.reduce((Q,S)=>{Q[S]=H[K++];return Q},{})})}return A},prepareCSVDataURI:(l,v)=>{const A=H=>{if(H&&e(H)){const K=
[],Q=H.length;for(let S=0,M;Q>S;S++)M=H[S],M=null==M?"":M.toString(),M=M.replace(/"/g,'""'),0<=M.search(/("|,|\n)/g)&&(M='"'+M+'"'),K.push(M);return K.join(",")}m("Unexpected row",H);return null},B=[];if(v){var D=A(v);D&&B.push(D)}if(l)if(e(l)){D=l.length;for(let H=0;D>H;H++){const K=A(l[H]);K&&B.push(K)}}else m("Rows were not an array");return y.encodeCSVDataURI(B.join("\r\n"),v)},encodeCSVDataURI:(l,v)=>"data:text/csv;charset=utf-8"+(null==v?"":";header="+(v?"present":"absent"))+","+encodeURIComponent(l),
dataURIToBlob:l=>{var v=l.indexOf(",");const A=l.slice(0,v).split(":")[1].split(";")[0];l=l.slice(v+1);if(A.startsWith("text/"))l=decodeURIComponent(l);else{l=atob(l);v=l.length;const B=new Uint8Array(v);for(;v;)B[--v]=l.charCodeAt(v);l=B}return new Blob([l],{type:A})},formatAsPercentage:(l,v=2)=>{switch(typeof l){case "number":return v=h(16,k(0,v)),l=d.round(k(0,h(1,l))*a(10,2+v))/a(10,v),(0===l%1?l:l.toFixed(v))+"%";case "string":return l;default:return m("formatAsPercentage: expects a number"),
l}},memoize:E,debounce:(l,v,A)=>{const B="__DBTO_"+ ++w;return function(){const D=this,H=D[B],K=arguments,Q=A&&!H;clearTimeout(H);D[B]=setTimeout(function(){D[B]=null;A||l.apply(D,K)},v);Q&&l.apply(D,K)}},createFixedThresholdCounter:(l,v,A,B)=>{var D=y.AccessorSupport.generateName;const H="function"===typeof l||l instanceof JS.Module,K={};B=B||D("counter",A);const Q=D(B,"increment");D=D(B,"decrement");if(void 0!==(H?l.instanceMethod(Q):l[Q]))return m("Increment: Abort clobber",Q,l),!1;if(void 0!==
(H?l.instanceMethod(D):l[D]))return m("Decrement: Abort clobber",D,l),!1;K[Q]=function(){const S=this[B]+1;this[B]=S;this.fireEvent(B,S);S===v&&this.set(A,!0)};K[D]=function(){const S=this[B];if(0!==S){var M=S-1;this[B]=M;this.fireEvent(B,M);S===v&&this.set(A,!1)}};l[H?"include":"extend"](K);return!0},makeURLSearchParams:(l={})=>{const v=new URLSearchParams;for(const A in l){const B=l[A];void 0!==B&&v.set(A,B)}return v},doFetch:(l,v,A,B,D,H)=>fetch(l,v).then(K=>{if(!K.ok)throw new x(K.status,l,K.statusText);
return K}).then(K=>A?K.text():K.json()).then(K=>{if(B)try{if(A)B(K);else{if(!1===K.success)throw new x(200,l,K.message);B(K.data)}}catch(Q){throw K=new x(200,l,Q.message),K.stack=Q.stack,K;}}).catch(K=>{if(D){if("FetchError"!==K.name){const Q=new x(0,l,K.message);Q.stack=K.stack;K=Q}D(K)}}).finally(()=>{H?.()}),I18N:{setLocale:l=>{u=l},getLocale:()=>u,addDictionary:(l,v)=>{z[v]=Object.assign(z[v]??{},l)},setDictionary:(l,v)=>{z[v]=l??{}},get:(l,...v)=>{var A=u??=(navigator.language??"en").split("-")[0].toLowerCase();
A=(z[A]??z.en??{})[l];return null!=A?0<v.length?A.replaceAll(G,(B,D,H,K)=>H&&K?1==v[parseInt(D)]?H:K:"").replaceAll(L,(B,D)=>v[D]):A:l}}};p.onloadingdone=l=>{for(const v of l.fontfaces)r(v)}})(global);
...
a.columnGap;const y=G?b:g,l=a.isWrapNotAllowed();g=a.justifyContent;b=a.alignItems;var n=l?"stretch":a.alignContent;f.endsWith("Reverse")&&e.reverse();let v=0,A=0,B=0,D=0,H={items:[],shrinkCount:0,shrinkBasis:0,growCount:0,extraSize:0,crossPos:B,crossSize:0};f=[H];for(let K=0;K<E;){const Q=e[K++],S=G?Q.getFlexBasisWidth():Q.getFlexBasisHeight(),M=Q.getShrink(),V=Q.getGrow();!l&&0<H.items.length&&v+(0===D?0:J)+S>N&&(H.extraSize=N-v,B+=H.crossSize+q,A=w(A,v),D=v=0,H={items:[],shrinkCount:0,shrinkBasis:0,
...
this.getSubviews();for(let n=q.length;0<n;){const p=q[--n];b&&!b(p)||p.destroy()}},hasLayout:function(b){return this.getLayouts().includes(b)},getLayoutIndex:function(b){return this.getLayouts().indexOf(b)},layoutAdded:b=>{},layoutRemoved:b=>{},isBehind:function(b,q){return c(this,b,!1,q)},isInFrontOf:function(b,q){return c(this,b,!0,q)},bringToFront:function(){this.parent.bringSubviewToFront(this)},sendToBack:function(){this.parent.sendSubviewToBack(this)},sendBehind:function(b,q){this.parent.sendSubviewBehind(this,
b,q)},sendInFrontOf:function(b,q){this.parent.sendSubviewInFrontOf(this,b,q)},doSubviewsReorderedInDom:b=>{},bringSubviewToFront:function(b){const q=this;if(b?.parent===q){const n=q.getIDE(),p=b.getODE();p!==n.lastElementChild&&m(b,()=>{n.appendChild(p);q.doSubviewsReorderedInDom(b)})}},sendSubviewToBack:function(b){const q=this;if(b?.parent===q){const n=q.getIDE(),p=n.firstElementChild,r=b.getODE();r!==p&&m(b,()=>{n.insertBefore(r,p);q.doSubviewsReorderedInDom(b)})}},sendSubviewBehind:function(b,
q,n){n?.moveSubviewBefore(b,q);const p=this;if(b?.parent===p&&q?.parent===p){const r=b.getODE(),z=q.getODE();if(r!==z.previousElementSibling){const E=p.getIDE();m(b,()=>{E.insertBefore(r,z);p.doSubviewsReorderedInDom(b)})}}n?.update()},sendSubviewInFrontOf:function(b,q,n){n?.moveSubviewAfter(b,q);const p=this;if(b?.parent===p&&q?.parent===p){const r=b.getODE(),z=q.getODE().nextElementSibling;if(r!==z){const E=p.getIDE();m(b,()=>{E.insertBefore(r,z);p.doSubviewsReorderedInDom(b)})}}n?.update()},sortSubviews:function(b){const q=
this,n=q.getSubviews();t.isNotSorted(n,b)&&(n.sort(b),d(q,()=>{const p=document.createDocumentFragment(),r=n.length;for(let z=0;r>z;)p.appendChild(n[z++].getODE());q.getIDE().appendChild(p)},!0),q.doSubviewsReorderedInDom(null))},containsPoint:function(b,q,n){const p=this.getODE();if(!p)return!1;const {x:r,y:z}=w.getRelativePosition(p,n);return u(b,q,r,z,this.width,this.height)},isPointVisible:function(b,q){const {x:n,y:p}=this.getPagePosition(!0);h(this);return k(this,b-n,q-p)},intersectsWithSibling:function(b){return b.parent===
...
n.reverse?n.repeat-1:0;n.__prog=n.reverse?n.duration:0},q=(n,p)=>{const {running:r,paused:z,reverse:E,duration:G,repeat:L,__prog:J,easingFunction:N,callback:y}=n;if(r&&!z){E&&(p*=-1);p=J+p;let H=0;p>G?(H=p-G,p=G,++n.__loopCnt===L&&(H=0)):0>p&&(H=-p,p=0,0>--n.__loopCnt&&0<L&&(H=0));var l=n.target??n.parent;if(l){const {relative:K,attribute:Q,__isColorAnim:S,__loopCnt:M,to:V}=n;var v=x(0,p/G),A=x(0,J/G);null==n.from&&(n.__tmpFrom=!0,n.from=K?S?"#000":0:l.get(Q));A=N(v)-(K?N(A):0);var B=n.from,D=K?l.get(Q):
B;v=l.set;if(S){B=e(B);const ea=e(V);D=K?e(D):B;D.setRed(D.red+(ea.red-B.red)*A);D.setGreen(D.green+(ea.green-B.green)*A);D.setBlue(D.blue+(ea.blue-B.blue)*A);A=D.getHtmlHexString()}else A=D+(V-B)*A;v.call(l,Q,A);!E&&M===L||E&&0>M&&0<L?(n.setRunning(!1),y?.call(n,!0)):0<H?(n.fireEvent("repeat",M),n.__prog=E?G:0,q(n,H)):n.__prog=p}else console.log("No target for animator",n),n.setRunning(!1),y?.call(n,!1)}};t.Animator=new JS.Class("Animator",t.Node,{include:[t.Reusable],extend:{easings:f},initNode:function(n,
...
0;O.value=t.formatAsPercentage(P/255,0);this.callSuper(O,!0);k=P;x._isReady&&(w=!1,x.updateUI())}}]),C+=23);p=new t.InputText(x,{x:236,y:C,width:105,height:25,roundedCorners:3,textColor:"#333",border:ma,maxLength:11,fontFamily:"monospace"});x.attachToDom(p,"_submitInput","blur");x.attachToDom(p,"_handleKeyDown","keydown");p.getIDS().paddingLeft="6px";C=new T(x,{x:170,y:C,width:60,height:23,border:ma,domClass:"mytCheckerboardPattern"});new T(C,{width:30,height:23,focusIndicator:!1,bgColor:I},[wa,{doActivated:()=>
...
16))}else F&&7<C.length&&(C=C.slice(0,7));F?(C=qa.makeColorFromHexString(C).getHSV(),d=C.h/360,c=C.s,h=C.v,w=!1):w=!0;x.updateUI()},getColor:()=>{if(w)return"transparent";let C="";u&&(C=k.toString(16),2>C.length&&(C="0"+C));return qa.makeColorFromHSV(360*d,100*c,100*h).getHtmlHexString()+C},updateUI:()=>{const C=!w;n.setVisible(C);b.setVisible(C);C&&(b.setX(ia(0,ha(1,c)*(g.width+2))-5),b.setY(ia(0,ha(1,1-h)*(g.height+2))-5),n.setY(d*(q.height-2)-1));g.setBgColor(qa.makeColorFromHSV(360*d,100,100).getHtmlHexString());
m=x.getColor();p.setValue(m);r.setBgColor(m);x.redrawPalette()},redrawPalette:t.debounce(()=>{f.destroyAllSubviews();const C=new Set;e.push(...a);for(let F of e)if(u||7===F.length)7===F.length&&(F+="ff"),C.has(F)||(new Ma(f,{bgColor:F}),C.add(F))},50)}),ya=C=>{C=C.split(":");return[ia(0,ha(23,parseInt(C[0]))),ia(0,ha(60,parseInt(C[1])))]},Ja=new ea("SelectableBtn",na,{include:[t.Selectable],setSelected:function(C){this.callSuper(C);this.updateUI()},updateUI:function(){this.callSuper();this.isSelected()?
...
X===ka&&(M.select(Y),Y.focus()),U&&X===W&&Y.setIsToday(!0),X=new Date(R.getTime()),X.setHours(23),X.setMinutes(59),X.setSeconds(59),(null!=l&&l>X.getTime()||null!=v&&v<R.getTime())&&Y.setDisabled(!0));Y.updateUI()}}if(!L){F=60*B[0]+B[1];W=V;W.deselectAll();W.destroyAllSubviews();let [ja,fa]=A;for(R=new Date(da);60*ja+fa<F;)da=new Oa(V,{text:("0"+ja).slice(-2)+":"+("0"+fa).slice(-2)}),ja===P&&fa===C&&V.select(da),R.setHours(ja),R.setMinutes(fa),W=R.getTime(),(null!=l&&l>W||null!=v&&v<W)&&da.setDisabled(!0),
...
(y=[0,1,2,3,4,5,6]);l=Date.parse(I.minDate);l=isNaN(l)?null:l;v=Date.parse(I.maxDate);v=isNaN(v)?null:v;A=ya(I.minTime);B=ya(I.maxTime);C=new T(E,{visible:!J});I.showTodayButton&&new na(C,{width:24,text:oa(["home"]),tooltip:G.today},[{doActivated:()=>{pa(new Date)}}]);K=new na(C,{width:24,x:28,text:oa(["chevron-left"]),tooltip:G.prevMonth},[{doActivated:()=>{const O=(new Date(H.getFullYear(),H.getMonth(),0)).getDate();O<H.getDate()&&H.setDate(O);H.setMonth(H.getMonth()-1);pa(H);K.focus()}}]);Q=new la(C,
...
l;){x>=y&&(B=x=0);D=Q[x];var H=D.hdr;D.full?++B:(K?(H=m(v?H.flex:1,l),D.amt+H>D.limit&&(H=D.limit-D.amt,D.full=!0)):(H=d(v?-H.flex:-1,l),D.amt+H<D.limit&&(H=D.limit-D.amt,D.full=!0)),D.amt+=H,l-=H);if(B===y)break;++x}for(const S of Q)S.hdr.setValue(S.hdr.value+S.amt);A?.(l)}}},f=y=>(y=y.gridController?.getPrevHdr(y))?y.maxValue-y.value+f(y):0,g=y=>(y=y.gridController?.getNextHdr(y))?y.maxValue-y.value+g(y):0,b=y=>(y=y.gridController?.getPrevHdr(y))?y.minValue-y.value+b(y):0,q=y=>(y=y.gridController?.getNextHdr(y))?
...
[t.SizeToParent,t.Draggable,{requestDragPosition:function(D,H){H=D-this.x;if(B.fitToWidth&&(0<H?H=m(H,-q(v),v.maxValue-v.value+f(v)):0>H&&(H=d(H,-g(v),v.minValue-v.value+b(v))),0===H))return;D=v.value+H;v.resizable&&v.setValue(D);D-=v.value;H=D-H;let K=0;0>D?K=n(v,D):0<D&&(K=p(v,D));this.dragInitX+=K;H-=K;B.fitToWidth&&(0>H?r(v,H):0<H&&z(v,H))}}]);B&&(B.notifyAddHdr(v),B.notifyHdrXChange(v),B.notifyHdrVisibilityChange(v));v.setWidth(v.value);v.resizer.setVisible(!(v.last&&v.gridController.fitToWidth))},
...
k,a,e,f,g,b,q=60){const n=t.Color;for(var p of b)"string"===typeof p.color&&(p.color=n.makeColorFromHexString(p.color));1>q&&(x("Invalid segements",q),q=60);q=w/q;p=0;for(var r=b.length-1;p<r;){var z=b[p++];const {angle:A,color:B}=z,{angle:D,color:H}=b[p];var E=Math.round((D-A)/q),G=B.getDiffFrom(H);z.colorDelta={red:G.red/E,green:G.green/E,blue:G.blue/E}}a=new t.Path([h+a,k,h+e,k]);a.rotateAroundOrigin(f,h,k);e=a.vectors;let [L,J,N,y]=e,l=0,v=w/360;for(p=0;g>f;){E=L;G=J;r=N;z=y;f+=q;f>g&&(q+=g-f,
